{
  ...
  # libumem package from the flake input
  libumem = libumemPkgs.default;
...
#        pkgs-unstable.gcc15
  ...
  gdbConfig = pkgs.writeText "gdbinit-postgres" ''
    ...
    # --- libumem GDB extensions ---
    # Source the libumem GDB macros if available
    python
    import os
    umem_gdb = os.path.join("${libumem}", "share", "libumem", "gdb-macros.py")
    if os.path.exists(umem_gdb):
        gdb.execute("source " + umem_gdb)
    else:
        # Try the tools directory
        umem_gdb = os.path.join("${libumem}", "tools", "umem_gdb.py")
        if os.path.exists(umem_gdb):
            gdb.execute("source " + umem_gdb)
    end
    ...
    printf "libumem commands:  inspect-umem-cache, show-umem-audit, find-umem-leaks\n"
  '';
  ...
  devShell = pkgs.mkShell {
    ...
    in ''
      ...
      # libumem configuration
      export LIBUMEM_HOME="${libumem}"
      ...
      ulimit -c unlimited
      if [ "$(cat /proc/sys/kernel/core_pattern 2>/dev/null)" != "core.%p" ]; then
        echo "  Tip: Set core dump pattern for debugging:"
        echo "    sudo sysctl -w kernel.core_pattern=core.%p"
      fi
      ...
      echo "  Compiler: $CC"
      echo "  libc: glibc (patched, no -O0 fortify warning)"
      echo "  LLVM: $(llvm-config --version 2>/dev/null || echo 'not available')"
      echo "  libumem: $LIBUMEM_HOME"
