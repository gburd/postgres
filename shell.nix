{
  ...
  # `-O0 -Werror`, which turns that cpp warning into a hard error and
  # breaks reconfigure under our default CFLAGS.  The patch simply drops
  # the warning.  It is scoped to this dev shell only and never leaks
  # into system glibc or release builds.
  ...
  # Use LLVM for modern PostgreSQL development
  ...
  config = {
    ...
    pgBuildDirValgrind = "$PWD/build-valgrind";
    pgBuildDirAsan = "$PWD/build-asan";
  ...
  getPostgreSQLDeps = muslLibs:
    with pkgs;
      [
        ...
        binutils
        gnumake
        mold                    # fast linker, big wins on large postgres links
        ...
        linuxPackages.perf
        flamegraph
        bpftrace                # kernel-level tracing (probes, uprobes)
        rr                      # record-and-replay deterministic debugger
        htop
        iotop
        sysstat
        ccache
  ...
  # Shared shellHook fragments.  Each devShell prepends its own compiler/CFLAGS
  # block, then appends the common tail via ${commonHookTail variant}.
  commonHookHead = icon: ''
    ...
    # Loosen a few rules so ccache hits across rebuilds with touched headers.
  ...
  # Tail shared by every devShell: PG env vars, GDB, tool PATH, per-process
  # setup and alias load.  Kernel core_pattern is NOT touched here --
  # run 'pg-enable-cores' explicitly if you need per-PID cores in CWD.
  commonHookTail = label: ''
    # PostgreSQL environment
    export PG_SOURCE_DIR="${config.pgSourceDir}"
    export PG_BUILD_DIR="${config.pgBuildDir}"
    export PG_BUILD_DIR_VALGRIND="${config.pgBuildDirValgrind}"
    export PG_BUILD_DIR_ASAN="${config.pgBuildDirAsan}"
    ...
    # Per-process core dump size limit.  Kernel core_pattern is NOT
    # touched here -- run 'pg-enable-cores' explicitly when you need
    # per-PID cores in CWD.
    ulimit -c unlimited
    ...
    echo ""
    echo "PostgreSQL Development Environment Ready (${label})"
  ...
  muslDevShell = pkgs.mkShell {
    ...
    shellHook =
      (commonHookHead "f121")
      + ''
        # Cross-compilation to musl with GCC
        export CC="${pkgs.gcc}/bin/gcc"
        export CXX="${pkgs.gcc}/bin/g++"
        ...
        export CFLAGS="-ggdb -Og -fno-omit-frame-pointer -D_FORTIFY_SOURCE=1 -I${pkgs.pkgsMusl.stdenv.cc.libc}/include"
        export CXXFLAGS="-ggdb -Og -fno-omit-frame-pointer -D_FORTIFY_SOURCE=1 -I${pkgs.pkgsMusl.stdenv.cc.libc}/include"
  ...
  clangMuslDevShell = pkgs.mkShell {
    ...
    shellHook =
      ...
      + ''
        ...
        export PKG_CONFIG_PATH="${pkgs.pkgsMusl.openssl.dev}/lib/pkgconfig:${pkgs.pkgsMusl.zlib.dev}/lib/pkgconfig:${pkgs.pkgsMusl.icu.dev}/lib/pkgconfig"
        export CFLAGS="--target=x86_64-linux-musl -ggdb -Og -fno-omit-frame-pointer -D_FORTIFY_SOURCE=1 -I${pkgs.pkgsMusl.stdenv.cc.libc}/include"
        export CXXFLAGS="--target=x86_64-linux-musl -ggdb -Og -fno-omit-frame-pointer -D_FORTIFY_SOURCE=1 -I${pkgs.pkgsMusl.stdenv.cc.libc}/include"
