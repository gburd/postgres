...
**Impact:** V4 uses `seq_cst` for the atomic read/write, `relaxed` for the
explicitly-unlocked variants.
- Postgres 18: No ordering guarantees (plain volatile; compiler/CPU may reorder)
- V3: `relaxed` (atomic but CPU-reorderable)
- V4: `seq_cst` for `pg_atomic_read/write_u32/64`; `relaxed` for
  `pg_atomic_unlocked_{read,write}_*` (whose contract is "no guarantees").

**Correctness:** `relaxed` was found INSUFFICIENT on weak-memory hardware.
On RISC-V, both `relaxed` and `acquire/release` reads/writes let a concurrent
reader in parallel hash join miss a tuple (join_hash regression, reproduced and
bisected; 10/10 pass after switching to seq_cst). The traditional plain-volatile
load happens to be ordered by the surrounding PostgreSQL barriers in a way that
a C11 relaxed atomic is not, and only `seq_cst` (which participates in the
single total order over all seq_cst ops) reliably reproduces that behavior.
The separate `_membarrier` variants also use seq_cst.

**Performance:** On x86 (TSO) a seq_cst load/store is still a plain mov (the
fence is implicit in TSO), so no cost there. On ARM/RISC-V it adds fence
instructions on these paths; this is the price of correctness and can be
revisited with per-call-site relaxation once each hot-path caller is audited
for the exact ordering it needs.
...
|-----------|-------------------|----------------------|------------|------------|
| `pg_memory_barrier()` | `lock; addl` inline asm | `__atomic_thread_fence(seq_cst)` | `atomic_thread_fence(seq_cst)` | `signal_fence + thread_fence(seq_cst)` |
| `pg_read_barrier()` | compiler barrier only | `__atomic_thread_fence(acquire)` | `atomic_thread_fence(acquire)` | `signal_fence + thread_fence(acquire)` |
| `pg_write_barrier()` | compiler barrier only | `__atomic_thread_fence(release)` | `atomic_thread_fence(release)` | `signal_fence + thread_fence(release)` |

**V4 note:** each barrier pairs a compiler barrier (`atomic_signal_fence`) with
the thread fence, because a bare `atomic_thread_fence` only orders *atomic*
accesses and lets the compiler reorder plain loads/stores across it -- whereas
PostgreSQL's barrier contract must order non-atomic accesses too. This matches
the traditional generic-gcc.h barriers (compiler barrier + `__atomic_thread_fence`).
...
| stdatomic.h | `typedef _Atomic(uint32) pg_atomic_flag;` | 4 bytes | 4 bytes |
...
|----------------|-----------------|-------------------|
| Postgres 18 | YES Yes | NO No (uses C-specific constructs) |
| stdatomic.h (ours) | YES Yes (`<stdatomic.h>`) | YES Yes (`<atomic>` for C++11-C++20) |
...
|----------------|------------------|
| Postgres 18 | NO No (`#error` directive) |
| stdatomic.h (ours) | YES Yes (when USE_STDATOMIC_H defined) |
...
- YES ARM64 Windows
- YES ARM32 (ARMv7)
- CAUTION: PowerPC (critical - hand-optimized assembly replaced)
- YES RISC-V (if available)
...
**Completed:**
- YES Flag polarity difference documented in stdatomic_impl.h
- YES Spin delay choices documented in spin_delay.h
- YES Memory ordering documented in atomics.h
- YES C++ compatibility documented in stdatomic_impl.h
