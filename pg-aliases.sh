...
# Database management — with LD_PRELOAD for libumem and core dumps
alias pg-init='rm -rf "$PG_DATA_DIR" && \
  LD_PRELOAD="${LIBUMEM_HOME:+$LIBUMEM_HOME/lib/libumem.so}" \
  "$PG_INSTALL_DIR/bin/initdb" --debug --no-clean "$PG_DATA_DIR"'

alias pg-start='ulimit -c unlimited && \
  LD_PRELOAD="${LIBUMEM_HOME:+$LIBUMEM_HOME/lib/libumem.so}" \
  "$PG_INSTALL_DIR/bin/postgres" -D "$PG_DATA_DIR" -k "$PG_DATA_DIR"'
...
# Debugging — loads both the Nix gdbConfig and the project .gdbinit
...
# Spell checking for docs and comments
pg-spell() {
	local since=${1:-HEAD}
	...
	for f in $files; do
		...
		case "$f" in
			*.c|*.h)
				# Extract comments only, check spelling
...
# libumem aliases
alias pg-umem-start='ulimit -c unlimited && \
  UMEM_DEBUG=default \
  LD_PRELOAD="${LIBUMEM_HOME:+$LIBUMEM_HOME/lib/libumem.so}" \
  "$PG_INSTALL_DIR/bin/postgres" -D "$PG_DATA_DIR" -k "$PG_DATA_DIR"'

alias pg-umem-check='ulimit -c unlimited && \
  UMEM_DEBUG=guards \
  LD_PRELOAD="${LIBUMEM_HOME:+$LIBUMEM_HOME/lib/libumem.so}" \
  "$PG_INSTALL_DIR/bin/postgres" -D "$PG_DATA_DIR" -k "$PG_DATA_DIR"'

pg-umem-info() {
	echo "=== libumem Configuration ==="
	echo "  LIBUMEM_HOME: ${LIBUMEM_HOME:-<not set>}"
	echo "  UMEM_DEBUG:   ${UMEM_DEBUG:-<not set>}"
	echo "  UMEM_OPTIONS: ${UMEM_OPTIONS:-<not set>}"
	echo "  UMEM_LOGGING: ${UMEM_LOGGING:-<not set>}"
	echo "  LD_PRELOAD:   ${LD_PRELOAD:-<not set>}"
	if [ -n "$LIBUMEM_HOME" ] && [ -f "$LIBUMEM_HOME/lib/libumem.so" ]; then
		echo "  libumem.so:   found"
	else
		echo "  libumem.so:   NOT found"
	fi
}
...
alias pg-info='
  ...
  echo "Compiler: $CC"
  echo "libumem: ${LIBUMEM_HOME:-<not set>}"
  echo ""
  ...
  echo "  libumem: pg-umem-start, pg-umem-check, pg-umem-info"
  echo "=========================================="'
