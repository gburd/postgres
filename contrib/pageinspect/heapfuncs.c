...
heap_page_items(PG_FUNCTION_ARGS)
{
	...
	if (fctx->call_cntr < fctx->max_calls)
	{
		 ...
		 *
		 * Test ItemIdIsNormal rather than ItemIdHasStorage: a HOT-indexed data
		 * redirect is an LP_REDIRECT that carries a bitmap blob (lp_len > 0),
		 * so it has storage but is not a heap tuple and must not be decoded as
		 * a tuple header.
		 */
