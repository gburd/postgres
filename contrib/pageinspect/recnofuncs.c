...
#define RECNO_PAGE_STATS_COLS 13
		...
		(BLCKSZ - phdr->pd_special) >= sizeof(RecnoPageOpaqueData))
	{
		...
		values[9] = Int64GetDatum((int64) RecnoPageGetCommitTs(opaque));	/* pd_commit_ts */
		values[10] = UInt16GetDatum((uint16) PageGetFreeSpace(page));	/* pd_free_space
																		 * (computed) */
		values[11] = Int32GetDatum((int32) (RecnoPageGetFlags(opaque) >> RECNO_PAGE_FLAG_SHIFT));	/* pd_flags */
	}
	else
	{
		nulls[9] = true;
