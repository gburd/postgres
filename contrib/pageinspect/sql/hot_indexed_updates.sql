...
--
-- Fixed without marking any index entry: the fresh entry is stored as an
-- ordinary exact TID, and heap_update sets a per-block signal,
-- VISIBILITYMAP_LOCATOR_SPLIT (visibilitymapdefs.h), on the block instead.
-- BitmapAnd reads that bit lock-free through the table AM's bucket_may_disagree
-- hook (tidbitmap.c) and, for a flagged block, unions the two indexes' offsets
-- and forces a recheck rather than intersecting them; the existing heap-side
-- crossed-attribute recheck then makes the final call.  BitmapOr already
-- unioned, so it is unaffected.  Because the signal lives on the heap block and
-- not in any index entry, NO index access method needs to know about
-- HOT-indexed chains: btree, hash, GIN, GiST, SP-GiST, contrib/bloom, and any
-- out-of-tree AM are all correct with no AM-specific code.  One table per non-btree access method
...
-- Both index entries store ordinary, exact TIDs (no per-entry marker): the
-- changed index's fresh entry names the new heap-only tuple, the unchanged
-- index's entry names the chain root.  The count above proves BitmapAnd still
-- returns the row despite the two entries naming different offsets on the one
-- block; the block carries VISIBILITYMAP_LOCATOR_SPLIT so BitmapAnd unions
-- rather than intersects it (see amlocator.h).  amcheck's heapallindexed
-- verification therefore needs no marker-stripping: it fingerprints the plain
-- stored TIDs directly.
--
...
-- question and out of scope for this commit.)
...
--
-- A HOT-indexed (SIU) fresh entry is inserted as an ordinary exact heap TID
-- (no marker).  When many such entries share one key, deduplication copies
-- those TIDs verbatim into a posting list's heap-TID array.  heapallindexed
-- fingerprints each posting element against the heap TID from the heap scan;
-- because the stored TIDs are plain, no per-element stripping is needed and the
-- check must simply pass.  All rows are updated to a single key value to force
-- one large posting list of live TIDs; fillfactor=10 keeps the updates HOT.
-- ---------------------------------------------------------------------------
