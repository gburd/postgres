	...
	while (next_start_ptr != ntids)
	{
		...
		for (i = curr_start_ptr; i < next_start_ptr; i++)
		{
													  ...
													  (HeapTupleHeader) PageGetItem(page, itemid)))
			{
				 ...
				 * the heap: HEAP_FORCE_FREEZE would clear HEAP_XMIN_INVALID and
				 * surface a natts==0 phantom row, while HEAP_FORCE_KILL would
				 * drop a chain hop that stale index entries still depend on.
				 */
				ereport(NOTICE,
