...
{
	/*
	 * Scale partitions with pool size:
	 *   < 256 buffers:   1 partition
	 *   256-1023:        2 partitions
	 *   1024-4095:       4 partitions
	 *   4096-16383:      8 partitions
	 *   >= 16384:       16 partitions
	 */
	if (nbuffers < 256)
		return 1;
...
OsicOnHit(void *strategy_data, int buf_id, BufferTag *tag)
{
	...
	/*
	 * Clear the cool flag.  If it was already hot (0), this is a no-op.
	 * No spinlock needed -- just an atomic AND.
	 */
 ...
 *
 * Mark the buffer as hot (clear cool flag) and decrement inflight.
 */
static void
			 ...
			 bool vacuum_hint)
{
	...
	/* New page starts hot */
	pg_atomic_write_u32(&states[local_id].flags, 0);
			  ...
			  uint64 *buf_state, bool *from_ring)
{
	...
	*from_ring = false;
	...
	/*
	 * Choose a starting partition.  Use MyProcNumber to distribute
	 * backends across partitions.  Cast to unsigned to handle the case
	 * where MyProcNumber is INVALID_PROC_NUMBER (-1).
	 */
	start_part = ((unsigned int) MyProcNumber) % npartitions;

	/*
	 * Pass 1: Scan for free (untagged) buffers only -- no cooling.
	 * This prevents unnecessary cooling of working set pages when free
	 * buffers exist scattered among tagged pages.
	 */
	...
	/*
	 * Pass 2: Standard clock sweep (cool-and-evict).
	 * Only reached when no free buffers exist.
	 * We allow up to 3 full sweeps of all partitions before giving up.
	 */
	for (attempts = 0; attempts < 3 * npartitions; attempts++)
	{
		...
		/*
		 * Sweep within this partition.  Allow up to 2 * nbuffers sweeps
		 * (one full cycle to cool, one to evict).
		 */
		for (sweep_count = 0; sweep_count < 2 * part->nbuffers; sweep_count++)
		{
			...
			{
				/*
				 * Buffer is hot -> mark it cool (stage 1).
				 * Use atomic OR -- no need to hold spinlock for this.
				 */
			...
			/*
			 * Buffer is cool and unpinned -> evict it (stage 2).
			 * Try to pin it.
			 */
			{
				uint64		old_state = state;
...
_PG_init(void)
{
	RegisterDefaultPoolAlgorithm(BP_ALGO_OSIC, &osic_pool_routine);
}
