 ...
 */
static BufferDesc *
OsicGetVictim(void *strategy_data, uint64 *buf_state)
{
	OsicControl *ctl = (OsicControl *) strategy_data;
...
static const BufferPoolRoutine osic_pool_routine = {
	...
	.scan_resistant = false,	/* OSIC: insert-time admission helps but no
								 * ghost-list memory; not provably scan-
								 * resistant in the ARC/LIRS sense */
};
