...
/*
 * LruOnNewTag -- add the newly loaded buffer at MRU position.
 */
static void
			 ...
			 uint64 *buf_state, bool *from_ring)
{
	...
	*from_ring = false;
	...
	/*
	 * Try free (untracked) buffers first.  During warmup, not all buffers
	 * are on the list yet.
	 */
...
static const BufferPoolRoutine lru_pool_routine = {
	...
	.hint_vacuum = NULL,		/* LRU doesn't special-case VACUUM */
...
_PG_init(void)
{
	RegisterDefaultPoolAlgorithm(BP_ALGO_LRU, &lru_pool_routine);
}
