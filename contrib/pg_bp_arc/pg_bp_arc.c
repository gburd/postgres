...
typedef struct ArcControl
{
	...
	/*
	 * Statistics (atomics so they can be read without lock).
	 * These count operations since last reset.
	 */
...
arc_alloc_cdb(ArcControl *ctl, ArcCDB *cdb_arr)
{
	...
	/*
	 * No free CDBs.  Recycle from ghost lists.
	 * Prefer B1 if T1+B1 >= c (the total cache size), else B2.
	 */
	...
	{
		/* This shouldn't happen if sizing is correct */
		ereport(ERROR,
...
ArcOnMiss(void *strategy_data, BufferTag *tag)
{
	...
	if (ghost_idx >= 0)
	{
		...
		{
			/*
			 * B1 hit: T1 cache is probably too small.
		...
		{
			/*
			 * B2 hit: T2 cache is probably too small.
			...
			bool vacuum_hint)
{
	...
	if (state->ghost_cdb >= 0)
	{
		 ...
		 * right tag.  Remove it from the ghost list and hash, then promote
		 * to T2 MRU (since it's been accessed before).
		 */
...
complete_miss:
		...
		/*
		 * VACUUM optimization: insert at LRU of T1 so vacuum-loaded pages
			 ...
			 uint64 *buf_state, bool *from_ring)
{
	...
	*from_ring = false;
	...
	/*
	 * Try free/untracked buffers first.  During cold start or warm-up,
	 * T1 + T2 < nbuffers means there are free buffers available.
	 * Using them avoids unnecessary evictions from the ARC lists.
	 */
...
static const BufferPoolRoutine arc_pool_routine = {
	.type = T_Invalid,			/* vtable is never node-walked; T_Invalid is fine */
	.on_hit = ArcOnHit,
	.on_miss = ArcOnMiss,
	.on_evict = ArcOnEvict,
...
pg_bp_arc_size_recommendation(PG_FUNCTION_ARGS)
{
	...
	for (int i = 0; i < NBufferPools; i++)
	{
		...
		/*
		 * Simple heuristic: if ghost_pressure > 0.1, the pool is probably
		 * too small.  Recommend increasing by the ghost pressure ratio,
		 * capped at 2x current size.
		 */
...
_PG_init(void)
{
	RegisterDefaultPoolAlgorithm(BP_ALGO_ARC, &arc_pool_routine);
}
