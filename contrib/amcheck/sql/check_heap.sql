...
-- HOT-indexed tombstones (Tepid):
--
-- A HOT-indexed UPDATE plants two distinct on-page artifacts that classic
-- HOT did not produce: an "adjacent" tombstone carrying the modified
-- indexed-attrs bitmap next to the live HOT-indexed tuple, and a "bridge"
-- tombstone left behind by pruneheap when a mid-chain HOT-indexed LP cannot
-- yet be reclaimed (its btree entries may still be stale).  Both items are
-- LP_NORMAL with HeapTupleHeaderGetNatts == 0 and HEAP_INDEXED_UPDATED set,
-- and verify_heapam validates their on-disk format explicitly.  This
-- scenario exercises both kinds and asserts that verify_heapam reports zero
-- corruption against legitimate HOT-indexed activity.
...
-- successful HOT-indexed update emits one adjacent tombstone next to the
-- new live tuple.
UPDATE hot_indexed_check SET c1 = c1 + 1000;
-- Multi-step UPDATEs: drive several successive HOT-indexed updates against
-- the same rows so prune sees a chain of dead intermediates and converts
-- the dead-with-stale-btree LPs into bridge tombstones.  An explicit VACUUM
-- runs the prune path and exercises bridge creation.
