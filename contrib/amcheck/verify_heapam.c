	...
	while ((ctx.buffer = read_stream_next_buffer(stream, NULL)) != InvalidBuffer)
	{
			 ...
			 ctx.offnum = OffsetNumberNext(ctx.offnum))
		{
			...
			if (ItemIdIsRedirected(ctx.itemid))
			{
				...
				/*
				 * A HOT-indexed data redirect stores a bitmap blob in the
				 * dead root's freed bytes (lp_len > 0).  Bounds- and
				 * consistency-check that blob before
				 * HotIndexedRedirectGetTarget() dereferences it, so a corrupt
				 * line pointer cannot drive amcheck itself out of bounds.
				 */
				if (HotIndexedRedirectIsData(ctx.itemid))
				{
						...
						(uint32) roff + rlen > BLCKSZ)
					{
						report_corruption(&ctx,
										  psprintf("HOT-indexed data redirect has invalid storage (offset %u, length %u)",
												   roff, rlen));
						continue;
					}
					rd = (HotIndexedRedirectData *) (ctx.page + roff);
				...
				rdoffnum = HotIndexedRedirectGetTarget(ctx.page, ctx.itemid);
...
check_hot_indexed_tombstone(HeapCheckContext *ctx, OffsetNumber maxoff)
{
	...
	/*
	 * A page that carries a tombstone (adjacent or bridge) must never be
	 * marked all-visible: tombstones are invisible (HEAP_XMIN_INVALID), and
	 * the all-visible seqscan fast path skips per-tuple visibility checks and
	 * would surface the tombstone's payload bytes as a phantom live row.  The
	 * prune/vacuum paths enforce this by refusing to set PD_ALL_VISIBLE while
	 * a tombstone is present; verify the on-disk result here.
	 */
	if (PageIsAllVisible(ctx->page))
		report_corruption(ctx,
						  pstrdup("HOT-indexed tombstone found on an all-visible page"));
	...
	{
		/*
		 * Bridge tombstone: forwards the HOT chain to a same-page offset.
		 * We validate that the forward link names this page and an offset
		 * within the line-pointer array (other than itself).  We do NOT
		 * require that offset to currently be LP_NORMAL: a bridge may
		 * legitimately forward to a chain member that a later prune has
		 * already turned into LP_DEAD/LP_UNUSED (a chain walker simply stops
		 * there), so an LP_NORMAL requirement would raise false positives.
		 */
		if (tblk != ctx->blkno)
			report_corruption(ctx,
	...
	else
	{
		...
		/* Payload sanity: t_nbytes covers the relation's attribute count. */
