	...
	while ((ctx.buffer = read_stream_next_buffer(stream, NULL)) != InvalidBuffer)
	{
			 ...
			 ctx.offnum = OffsetNumberNext(ctx.offnum))
		{
			...
			if (HeapTupleHeaderIsHotIndexedTombstone(ctx.tuphdr))
			{
				 ...
				 * forward link as a successor so chain validation can see
				 * the connection.  Adjacent tombstones have
				 * t_ctid.blkno == InvalidBlockNumber and so are not part of
				 * any chain.
				 */
...
check_hot_indexed_tombstone(HeapCheckContext *ctx, OffsetNumber maxoff)
{
	...
	if (HeapTupleHeaderIsHotIndexedBridge(tuphdr))
	{
		 ...
		 * The forward target must be a real LP on this page.
		 */
		if (tblk != ctx->blkno)
			report_corruption(ctx,
	...
	else
	{
		...
		/*
		 * Adjacent tombstones carry a per-relation-natts bitmap.  If the
		 * page outlived a relation-altering operation that changed the
		 * attribute count, the LP length will not match; flag that.
		 */
