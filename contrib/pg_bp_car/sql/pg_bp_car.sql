...
-- Verify lookup/miss counters work (misses may be 0 if all evicted pages
-- land in ghost lists, which is correct CAR behavior for this workload)
SELECT lookups > 0 AS has_lookups,
       misses >= 0 AS misses_valid
FROM pg_stat_car WHERE name = 'car_adapt_pool';
