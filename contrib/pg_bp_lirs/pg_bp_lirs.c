...
lirs_demote_bottom_lir(LirsControl *ctl, LirsCDB *cdb_arr)
{
	...
	/*
	 * Find the bottom-most LIR entry.  After a prune, the stack bottom
	 * should be LIR, but be defensive and skip non-LIR entries.
	 */
...
LirsOnHit(void *strategy_data, int buf_id, BufferTag *tag)
{
	...
	else if (cdb_arr[cdb_idx].status == LIRS_STATUS_HIR)
	{
		...
		if (cdb_arr[cdb_idx].in_stack)
		{
			 ...
			 *
			 * Per LIRS algorithm: remove from Q, move to stack top,
			 * set LIR, then demote bottom-of-stack LIR to HIR.
			 */
		...
		{
			/*
			 * HIR not in stack: move to Q tail, push onto stack top.
			 * Stays as HIR (no promotion without stack membership).
			 */
			lirs_q_remove(ctl, cdb_arr, cdb_idx);
...
LirsOnEvict(void *strategy_data, int buf_id, BufferTag *old_tag pg_attribute_unused())
{
	...
	{
		/*
		 * LIR eviction (unusual -- normally only HIR evicted).
		 * Convert to ghost and keep in stack for future tracking.
		 */
		ctl->lir_count--;
		cdb_arr[cdb_idx].status = LIRS_STATUS_GHOST;
		cdb_arr[cdb_idx].buf_id = -1;
...
complete_miss:
		...
		{
			/*
			 * Cache warm-up: insert as LIR directly.
			 * During initial fill, all pages become LIR until we reach
			 * lir_capacity.
			 */
			cdb_arr[cdb_idx].status = LIRS_STATUS_LIR;
			ctl->lir_count++;
...
_PG_init(void)
{
	RegisterDefaultPoolAlgorithm(BP_ALGO_LIRS, &lirs_pool_routine);
}
