...
policy. AIO workers, the syslogger, archiver, WAL receiver, WAL summarizer,
background writer, checkpointer, and WAL writer bypass it; background writer,
checkpointer, WAL writer, archiver, WAL receiver, and WAL summarizer are
worker-specific narrowings for auxiliary classes whose common startup does not
run database/session bootstrap before entering the worker loop, with archiver,
WAL receiver, and WAL summarizer additionally validated through their
wakeup/progress, streaming, and clean shutdown paths. A broader attempted
bypass for additional non-session
