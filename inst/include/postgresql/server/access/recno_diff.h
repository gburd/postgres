...
/*
 * A single diff segment: a byte-level splice that turns the new tuple back
 * into the old one.  At reconstruction the segment says "at new-tuple byte
 * offset, drop del_len bytes and insert the ins_len old bytes that follow."
 *
 * A same-length change (the common in-place update) is the degenerate splice
 * where del_len == ins_len: del_len new bytes are replaced by an equal run of
 * old bytes, leaving the total length unchanged.  When a varlen attribute
 * grows or shrinks, del_len != ins_len and the reconstructed tuple ends up at
 * old_total_len (see RecnoDiffRecord).
 */
typedef struct RecnoDiffSegment
{
	uint16		offset;			/* Byte offset within the NEW tuple data */
	uint16		del_len;		/* Bytes removed from the new tuple here */
	uint16		ins_len;		/* Old bytes inserted here (== count of
								 * old_bytes that follow) */
	/* old_bytes[ins_len] follow immediately (variable length) */
} RecnoDiffSegment;

#define SizeOfRecnoDiffSegment	(offsetof(RecnoDiffSegment, ins_len) + sizeof(uint16))

/*
 * RecnoDiffRecord: a compact representation of the difference between
 * two tuple versions. Contains an array of splice segments.
 *
 * old_total_len records the length of the reconstructed (old) tuple so the
 * apply side can size its output buffer and the page-restore can resize the
 * slot, even when the splice changes the total length.
 */
...
#define SizeOfRecnoDiffRecord	(offsetof(RecnoDiffRecord, old_total_len) + sizeof(uint16))
 ...
 *
 * Handles both equal-length updates and length-changing (varlen) updates:
 * the result is a sequence of splice segments and an old_total_len, so
 * reconstruction yields a correctly-sized old tuple regardless of whether the
 * change grew or shrank the row.
 *
 * old_data/new_data: raw tuple data pointers (including the tuple header)
