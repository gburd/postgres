...
class Visualizer:
    ...
    def generate_speedup_chart(
        ...
        ax.set_xlabel("Speedup (Orvos / HEAP)")
        ax.set_title("Query Performance: Orvos vs HEAP")
    ...
    def generate_storage_chart(
        ...
        """Grouped bar chart comparing HEAP and Orvos storage sizes."""
        ...
        orvos_sizes = [sc.orvos_total_bytes / (1024 * 1024) for sc in storage_comps]

        fig, ax = plt.subplots(figsize=(max(8, len(labels) * 2), 6))
        x = range(len(labels))
        width = 0.35
        ax.bar([i - width / 2 for i in x], heap_sizes, width, label="HEAP", color="#3498db")
        ax.bar([i + width / 2 for i in x], orvos_sizes, width, label="Orvos", color="#2ecc71")

        ax.set_ylabel("Total Size (MB)")
        ax.set_title("Storage Comparison: HEAP vs Orvos")
        ...
        for i, sc in enumerate(storage_comps):
            ax.text(
                i, max(heap_sizes[i], orvos_sizes[i]) + 0.5,
    ...
    def generate_latency_heatmap(
        ...
        ax.set_title("Speedup Heatmap (green = Orvos faster)")

        for i in range(len(schemas)):
            for j in range(len(patterns)):
                ax.text(j, i, f"{data[i][j]:.2f}x", ha="center", va="center", fontsize=8)

        fig.colorbar(im, ax=ax, label="Speedup (Orvos/HEAP)")
        fig.tight_layout()
    ...
    def generate_compression_chart(
        ...
        orvos_widths = [col_data[c].get("orvos_avg_width", 0) for c in cols]
        ...
        ax.bar([i + width / 2 for i in x], orvos_widths, width, label="Orvos avg_width", color="#2ecc71")
    ...
    def export_csv(self, report: AnalysisReport) -> str:
        ...
        with open(timing_path, "w", newline="") as f:
            ...
            writer.writerow([
                ...
                "heap_median_s", "orvos_median_s", "speedup",
                "heap_p95_s", "orvos_p95_s",
                "heap_mean_s", "orvos_mean_s",
            ])
            for c in report.comparisons:
                writer.writerow([
                    c.schema_name, c.row_count, c.distribution, c.query_pattern,
                    f"{c.heap_timing.median:.6f}",
                    f"{c.orvos_timing.median:.6f}",
                    f"{c.speedup:.4f}",
                    f"{c.heap_timing.p95:.6f}",
                    f"{c.orvos_timing.p95:.6f}",
                    f"{c.heap_timing.mean:.6f}",
                    f"{c.orvos_timing.mean:.6f}",
                ])
        ...
        with open(storage_path, "w", newline="") as f:
            ...
            writer.writerow([
                ...
                "orvos_table_bytes", "orvos_index_bytes", "orvos_total_bytes",
            ...
            for sc in report.storage_comparisons:
                writer.writerow([
                    ...
                    sc.orvos_table_bytes, sc.orvos_index_bytes, sc.orvos_total_bytes,
        ...
        with open(col_path, "w", newline="") as f:
            ...
            writer.writerow([
                "config", "column", "type",
                "heap_avg_width", "orvos_avg_width", "width_ratio",
                "heap_n_distinct", "orvos_n_distinct",
            ])
    ...
    def _render_html(
        ...
        for c in report.comparisons:
            ...
            <tr>
                ...
                <td>{c.orvos_timing.median * 1000:.2f}</td>
        ...
        for sc in report.storage_comparisons:
            ...
            <tr>
                ...
                <td>{_human_bytes(sc.orvos_total_bytes)}</td>
...
<title>Orvos Benchmark Dashboard</title>
<style>
...
<body>
<h1>Orvos Benchmark Dashboard</h1>
...
<tr>
  ...
  <th>HEAP (ms)</th><th>Orvos (ms)</th><th>Speedup</th>
</tr>
</thead>
<tbody>
{timing_rows}
...
<tr>
  ...
  <th>HEAP Total</th><th>Orvos Total</th><th>Compression</th><th>Savings</th>
</tr>
</thead>
<tbody>
{storage_rows}
...
<footer style="text-align:center;color:#999;margin-top:30px;padding:20px;">
  Generated by Orvos Benchmark Suite
</footer>
</body>
</html>"""
