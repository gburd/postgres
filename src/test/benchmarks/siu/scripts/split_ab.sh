#!/usr/bin/env bash
# split_ab.sh -- A/B pgbench harness for the per-block VISIBILITYMAP_LOCATOR_SPLIT
# signal specifically.  Uses the same two variants build.sh already
# produced under $BENCH/{master,tepid}: "master" = the split bit REVERTED
# (split-off), "tepid" = the split bit present (split-on).  Both otherwise
...
start_pg() {
  local v=$1
  local datadir=$BENCH/_data_split_$v
  rm -rf "$datadir"
  mkdir -p "$datadir"
...
for i in $(seq 1 "$ITERATIONS"); do
  run_iter "$i" master   # split-off
  run_iter "$i" tepid    # split-on
done
