...
- `scripts/read_indexscan.sql` -- read-only btree index scans on a freshly
  reset `siu_table` (no stale entries); isolates the cost of the HOT-indexed
  read path requesting the index tuple on every btree scan (`xs_want_itup`),
  since that is the only master-vs-tepid difference on this cell.
