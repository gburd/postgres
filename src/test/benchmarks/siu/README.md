...
- `scripts/split_ab.sh` -- A/B harness that isolates the cost of the
  per-block VISIBILITYMAP_LOCATOR_SPLIT signal alone, rather than the
  selective-maintenance win.  It reuses the two variants `build.sh` produces,
  with the marker reverted in one of them, so the only difference measured is
  the lossy-page degradation a hinted entry causes in a bitmap scan.
...
- `scripts/bitmap_and_mixed.sql` -- forces a BitmapAnd across two indexes
  ...
  unions their shared block under BitmapAnd; used with `split_ab.sh` to price the
  recheck hint.
