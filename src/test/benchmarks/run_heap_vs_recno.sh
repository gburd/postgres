...
run_pgbench_test() {
    ...
    # Extract TPS (without initial connection time / excluding connections establishing)
    RESULT_TPS=$(echo "$output" | grep -iE "without initial connection time|excluding connections establishing" \
                 | sed 's/.*= *//' | sed 's/ .*//' || echo "")
