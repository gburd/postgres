...
class BenchmarkSuite:
    """Orchestrates the full Orvos benchmark suite."""
    ...
    async def setup(self):
        """Initialize database connections and verify Orvos availability."""
        logger.info("Initializing benchmark suite...")
        await self.db.initialize()

        # Check Orvos
        if not await self.db.check_orvos_available():
            raise RuntimeError(
                "Orvos table AM not found. Ensure PostgreSQL is built with Orvos support."
            )
        logger.info("Orvos table AM is available")
    ...
    async def run_single_benchmark(
        ...
        orvos_table = tables["orvos_table"]
        ...
        insert_sql_orvos = self.data_generator.generate_server_side_insert(
            schema, row_count, distribution, table_suffix="_orvos"
        )
        ...
        logger.info("Loading %d rows into %s...", row_count, orvos_table)
        t0 = time.perf_counter()
        await self.schema_builder.load_data(orvos_table, insert_sql_orvos)
        orvos_load_time = time.perf_counter() - t0
        logger.info("Orvos load: %.2fs", orvos_load_time)
        ...
        # 4. Run workloads
        heap_wr, orvos_wr = await self.workload_runner.run_workload(
            schema=schema,
            heap_table=heap_table,
            orvos_table=orvos_table,
            row_count=row_count,
        ...
        return heap_wr, orvos_wr, metrics
        ...
        for schema in self.config.schemas:
            for row_count in self.config.get_row_counts():
                for dist in self.config.distributions:
                    ...
                    try:
                        heap_wr, orvos_wr, metrics = await self.run_single_benchmark(
                            schema, row_count, dist
                        )
                        self._workload_pairs.append((heap_wr, orvos_wr))
