"""
CLI entry point for the Orvos benchmark suite.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Orvos Performance Benchmark Suite",
    ...
    parser.add_argument(
        "--distribution",
        choices=["random", "clustered", "low_cardinality", "all"],
        default="all",
...
def main():
    ...
    print("=" * 60)
    print("  Orvos Performance Benchmark Suite")
    print("=" * 60)
    ...
    if s.get("best_orvos_scenario"):
        best = s["best_orvos_scenario"]
        print()
        print(
            f"  Best Orvos scenario: {best['pattern']} on {best['schema']} "
            f"({best['distribution']}) = {best['speedup']:.2f}x"
        )
    if s.get("worst_orvos_scenario"):
        worst = s["worst_orvos_scenario"]
        print(
            f"  Worst Orvos scenario: {worst['pattern']} on {worst['schema']} "
