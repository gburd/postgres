...
for rowcount in 10000 100000; do
    ...
    run_sql "$RECNO_PORT" "RECNO" -c "DROP TABLE IF EXISTS bench_update; CREATE TABLE bench_update (id int PRIMARY KEY, counter int DEFAULT 0, data text) USING recno; INSERT INTO bench_update SELECT i, 0, md5(i::text) FROM generate_series(1,$rowcount) i;" >/dev/null
...
for rowcount in 10000 50000 100000; do
    ...
    run_sql "$RECNO_PORT" "RECNO" -c "DROP TABLE IF EXISTS bench_scan; CREATE TABLE bench_scan (id int, val int, data text) USING recno; INSERT INTO bench_scan SELECT i, i%1000, md5(i::text) FROM generate_series(1,$rowcount) i;" >/dev/null 2>&1 || { out "  ${rowcount} rows: RECNO insert failed (sLog OOM?), skipping"; continue; }
...
run_sql "$RECNO_PORT" "RECNO" -c "DROP TABLE IF EXISTS bench_idx; CREATE TABLE bench_idx (id int PRIMARY KEY, val int, data text) USING recno; INSERT INTO bench_idx SELECT i, i%10000, md5(i::text) FROM generate_series(1,$ROWCOUNT) i; CREATE INDEX bench_idx_val ON bench_idx(val);" >/dev/null

# Point lookup script
BENCH6_PK="$BENCH_BASE/bench6_pk.sql"
echo "SELECT * FROM bench_idx WHERE id = (random()*${ROWCOUNT})::int + 1;" > "$BENCH6_PK"

BENCH6_IDX="$BENCH_BASE/bench6_idx.sql"
echo "SELECT count(*) FROM bench_idx WHERE val = (random()*10000)::int;" > "$BENCH6_IDX"
...
for rowcount in 10000 100000; do
    ...
    run_sql "$RECNO_PORT" "RECNO" -c "DROP TABLE IF EXISTS bench_del; CREATE TABLE bench_del (id int, val text) USING recno; INSERT INTO bench_del SELECT i, md5(i::text) FROM generate_series(1,$rowcount) i;" >/dev/null
...
run_sql "$RECNO_PORT" "RECNO" -c "DROP TABLE IF EXISTS bench_bloat; CREATE TABLE bench_bloat (id int, counter int DEFAULT 0, data text) USING recno; INSERT INTO bench_bloat SELECT i, 0, md5(i::text) FROM generate_series(1,$ROWCOUNT) i;" >/dev/null
...
run_sql "$RECNO_PORT" "RECNO" -c "DROP TABLE IF EXISTS bench_mixed; CREATE TABLE bench_mixed (id int PRIMARY KEY, counter int DEFAULT 0, data text) USING recno; INSERT INTO bench_mixed SELECT i, 0, md5(i::text) FROM generate_series(1,$ROWCOUNT) i;" >/dev/null
...
WIDE_INSERT="INSERT INTO bench_wide SELECT i, i, i+1, i+2, i+3, i+4, i+5, i+6, i+7, i+8, i+9, md5(i::text), md5((i+1)::text), md5((i+2)::text), md5((i+3)::text), md5((i+4)::text) FROM generate_series(1,$ROWCOUNT) i"

run_sql "$HEAP_PORT" "HEAP" -c "$WIDE_CREATE USING heap; $WIDE_INSERT;" >/dev/null
run_sql "$RECNO_PORT" "RECNO" -c "$WIDE_CREATE USING recno; $WIDE_INSERT;" >/dev/null
