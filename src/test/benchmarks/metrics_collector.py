...
statistics from pg_statistic and Orvos internal catalogs.
"""
...
class StorageMetrics:
    ...
    row_count: int = 0
    dead_tuples: int = 0
    # Compression stats (Orvos-specific)
...
class BenchmarkMetrics:
    ...
    orvos_storage: Optional[StorageMetrics] = None
    ...
    def compression_ratio(self) -> float:
        """Overall storage compression ratio (heap_size / orvos_size)."""
        if self.heap_storage and self.orvos_storage:
            if self.orvos_storage.total_size_bytes > 0:
                return (
                    self.heap_storage.total_size_bytes
                    / self.orvos_storage.total_size_bytes
                )
        return 1.0
...
class MetricsCollector:
    ...
    async def collect_orvos_internals(
        self, table_name: str
    ) -> Dict[str, Any]:
        """Collect Orvos-specific internal statistics if available.

        Queries orvos_inspect functions for page-level compression data.
        """
        internals = {}
        try:
            ...
            exists = await self.db.fetchval(
                """
                SELECT EXISTS(
                    SELECT 1 FROM pg_proc WHERE proname = 'orvos_inspect'
                )
                """
            )
            if not exists:
                logger.debug("orvos_inspect function not found; skipping internals")
                return internals

            rows = await self.db.fetch(
                f"SELECT * FROM orvos_inspect('{table_name}'::regclass)"
            )
            if rows:
            ...
            logger.debug("Could not collect Orvos internals for %s: %s", table_name, e)
        return internals
    ...
    async def collect_all(
        ...
        metrics.orvos_storage = await self.collect_storage_metrics(
            orvos_table, "orvos"
        )

        # Compression stats from pg_statistic for both
        heap_comp = await self.collect_compression_stats(heap_table)
        orvos_comp = await self.collect_compression_stats(orvos_table)
        metrics.compression_stats = {
            "heap": heap_comp,
            "orvos": orvos_comp,
        }

        # Orvos internal page stats
        orvos_internals = await self.collect_orvos_internals(orvos_table)
        if orvos_internals:
            metrics.compression_stats["orvos_internals"] = orvos_internals
