...
class MetricsCollector:
    ...
    async def collect_pool_stats(self, pool_name: str) -> Dict[str, Any]:
        """Collect pg_stat_bufferpool metrics for a named pool."""
        try:
            row = await self.db.fetchrow(
            ...
            logger.debug("Could not collect pg_stat_bufferpool for %s: %s",
                         pool_name, e)
        return {}

    async def collect_arc_stats(self, pool_name: str) -> Dict[str, Any]:
        """Collect pg_stat_arc metrics for a named ARC pool."""
        try:
            row = await self.db.fetchrow(
            ...
            logger.debug("Could not collect pg_stat_arc for %s: %s",
                         pool_name, e)
        return {}

    async def collect_lirs_stats(self, pool_name: str) -> Dict[str, Any]:
        """Collect pg_stat_lirs metrics for a named LIRS pool."""
        try:
            row = await self.db.fetchrow(
            ...
            logger.debug("Could not collect pg_stat_lirs for %s: %s",
                         pool_name, e)
        return {}

    async def collect_buffercache_stats(
        self, pool_name: str
    ) -> Dict[str, Any]:
        """Collect aggregated pg_buffercache stats for a pool."""
        try:
            row = await self.db.fetchrow(
                """
                SELECT count(*) AS buffers,
                       ...
                       sum(pinning_backends) AS total_pins
                FROM pg_buffercache
                WHERE bufferpool = $1
                """,
                pool_name,
            ...
            logger.debug("Could not collect pg_buffercache for %s: %s",
                         pool_name, e)
        return {}

    async def collect_query_cache_stats(
        self, table_name: str
    ) -> Dict[str, Any]:
        """Collect per-query cache hit/read stats from pg_stat_statements."""
        try:
            rows = await self.db.fetch(
                """
                SELECT sum(shared_blks_hit) AS blks_hit,
            ...
            if rows and rows[0]:
                row = dict(rows[0])
                hit = row.get("blks_hit") or 0
                read = row.get("blks_read") or 0
                total = hit + read
                row["hit_ratio"] = hit / total if total > 0 else 0.0
                return row
        except Exception as e:
            logger.debug("Could not collect query cache stats for %s: %s",
                         table_name, e)
        return {}
