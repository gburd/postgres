...
class TproccBenchmark:
    ...
    def _build_pgbench_cmd(self, am: AccessMethod, clients: int,
                           script_paths: dict, duration: int,
                           logging_enabled: bool = True) -> List[str]:
        """Build the pgbench command line.

        Args:
            duration: pgbench -T seconds for THIS invocation. The measurement
                run passes config.duration; a throwaway warmup run passes
                config.warmup. Splitting warmup into its own discarded
                invocation means the measurement run's own
                "tps = ... (without initial connection time)" line is the
                true steady-state TPS -- no warmup dilution, no log slicing.
            logging_enabled: emit --log (per-txn latencies) only for the
                measured run; the warmup run does not need it.
        """
        ...
        # Logging for latency percentiles (measured run only)
        if logging_enabled:
    ...
    def _run_pgbench(self, am: AccessMethod, clients: int, rep: int,
        ...
        Runs in two phases: a throwaway warmup invocation (config.warmup
        seconds, output discarded) to fill shared_buffers, settle undo/sLog
        workers, and reach steady state; then the measured invocation
        (config.duration seconds) whose reported TPS is therefore already
        warmup-free. This replaces the old single-invocation approach where
        pgbench's overall TPS silently included the warmup window.
        """
        env = None
        ...
        if self.config.warmup > 0:
            warm_cmd = self._build_pgbench_cmd(
                am, clients, script_paths, self.config.warmup,
                logging_enabled=False)
            logger.info("  Warmup: am=%s clients=%d rep=%d (%ds, discarded)",
                        am.value, clients, rep, self.config.warmup)
            try:
                subprocess.run(warm_cmd, capture_output=True, text=True, env=env,
                               timeout=self.config.warmup + 120)
            except subprocess.TimeoutExpired:
                logger.warning("  Warmup timed out; proceeding to measured run")

        # --- Phase 2: measured run ---
        cmd = self._build_pgbench_cmd(
            am, clients, script_paths, self.config.duration,
            logging_enabled=True)

        logger.info("  Running: am=%s clients=%d rep=%d duration=%ds",
                    am.value, clients, rep, self.config.duration)
        ...
        result = subprocess.run(
            ...
            text=True,
            env=env,
            timeout=self.config.duration + 120,  # generous timeout
        )
        ...
        # TPS here is warmup-free: warmup ran as a separate discarded
        # invocation, so this measured run's "(without initial connection
        # time)" TPS is the steady-state number.
