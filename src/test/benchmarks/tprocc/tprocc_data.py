...
Large per-warehouse tables are populated in warehouse-range batches. Each
batch is a separate autocommit statement, which bounds the per-transaction
UNDO that RECNO accumulates. A single 30M-row INSERT overflows RECNO's
per-transaction UNDO buffer at MaxAllocSize (1GB) and aborts+rolls back; HEAP
is unaffected but batching is harmless for it. Batch of 10 warehouses caps the
largest table (order_line) at ~3M rows/statement, which is proven safe.
"""
...
# Warehouses per population batch. Caps order_line at ~10*300k = 3M rows/txn,
# well under RECNO's per-transaction UNDO overflow threshold (~1GB alloc guard).
BATCH_WH = 10


def _wh_batches(total_wh: int):
    """Yield (lo, hi) inclusive 1-based warehouse ranges of BATCH_WH each."""
    lo = 1
    while lo <= total_wh:
        hi = min(lo + BATCH_WH - 1, total_wh)
        yield lo, hi
        lo = hi + 1
...
def _random_str(length: int) -> str:
    ...
    reps = (length + 31) // 32
    parts = " || ".join("md5(random()::text)" for _ in range(reps))
...
FROM generate_series({gs_lo}, {gs_hi}) gs;
"""
        run_sql(sql, config)
    logger.info("  %s populated in %.1fs", tbl, time.time() - t0)
...
FROM {cust_tbl}
WHERE c_w_id BETWEEN {lo} AND {hi};
"""
        run_sql(sql, config)
    logger.info("  %s populated in %.1fs", tbl, time.time() - t0)
...
FROM {orders_tbl}
WHERE o_id > 2100 AND o_w_id BETWEEN {lo} AND {hi};
"""
        run_sql(sql, config)
...
WHERE o_w_id BETWEEN {lo} AND {hi};
"""
        run_sql(sql, config)
