...
def _delivery_script(am: AccessMethod, config: TproccConfig) -> str:
    ...
    The DELETE is wrapped in a CTE whose outer SELECT COALESCEs to 0 so that
    \\gset always sets :del_o_id even when SKIP LOCKED matches no row (district
    drained or all candidates locked). A pgbench \\if then skips the follow-up
    UPDATEs for the empty case; without this guard a zero-row \\gset leaves
    :del_o_id unset and the next statement aborts the client mid-run.
    """
    w = config.warehouses
    new_order = get_table_name("new_order", am)
    orders = get_table_name("orders", am)
...
-- COALESCE to 0 guarantees exactly one row so \\gset always sets :del_o_id.
WITH del AS (
...
SELECT COALESCE((SELECT no_o_id FROM del), 0) AS del_o_id;
\\gset
\\if :del_o_id <> 0
