...
class DatabaseManager:
    ...
    async def check_view_exists(self, view_name: str) -> bool:
        """Check whether a view or table exists in pg_class."""
        val = await self.fetchval(
    ...
    async def setup_bufferpool_extensions(self) -> dict:
        """Set up extensions and verify views for buffer pool benchmarks.

        Returns a dict of {feature: bool} indicating availability.
        """
        avail = {
            "pg_stat_statements": False,
            "pg_buffercache": False,
            "pg_stat_bufferpool": False,
            "pg_bp_arc": False,
            "pg_stat_arc": False,
            "pg_bp_car": False,
            "pg_stat_car": False,
        ...
        avail["pg_stat_statements"] = await self.ensure_extension(
            "pg_stat_statements"
        )
        avail["pg_buffercache"] = await self.ensure_extension("pg_buffercache")
        avail["pg_stat_bufferpool"] = await self.check_view_exists(
            "pg_stat_bufferpool"
        )

        # Load buffer replacement strategy extensions
        avail["pg_bp_arc"] = await self.ensure_extension("pg_bp_arc")
        if avail["pg_bp_arc"]:
            avail["pg_stat_arc"] = await self.check_view_exists("pg_stat_arc")

        avail["pg_bp_car"] = await self.ensure_extension("pg_bp_car")
        if avail["pg_bp_car"]:
            avail["pg_stat_car"] = await self.check_view_exists("pg_stat_car")

        avail["pg_bp_lirs"] = await self.ensure_extension("pg_bp_lirs")
        if avail["pg_bp_lirs"]:
            avail["pg_stat_lirs"] = await self.check_view_exists("pg_stat_lirs")

        avail["pg_bp_lru"] = await self.ensure_extension("pg_bp_lru")
        if avail["pg_bp_lru"]:
            avail["pg_stat_lru"] = await self.check_view_exists("pg_stat_lru")

        avail["pg_bp_osic"] = await self.ensure_extension("pg_bp_osic")
        if avail["pg_bp_osic"]:
            avail["pg_stat_osic"] = await self.check_view_exists("pg_stat_osic")

        logger.info("Buffer pool extension availability: %s", avail)
        return avail
