...
init_cluster() {
    ...
    # UNDO is always-on infrastructure; no GUC needed.
    # undo_on tables use RECNO AM via ALTER TABLE ... SET ACCESS METHOD recno.
    # undo_off tables use the default heap AM (UNDO code present but unused).
    # baseline: master branch (no UNDO code at all).
