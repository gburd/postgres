...
#
# RECNO's in-place UPDATE overwrites the tuple in the page, but the
# before-image (WS-PVS1/WS-PVS3) is preserved as a byte-diff in the
# per-relation UNDO fork before the overwrite.  A REPEATABLE READ/
# SERIALIZABLE reader that cannot see the update's committing xmin
# reconstructs the pre-update value from that fork
# (RecnoReconstructVisibleVersion) rather than losing the row.  This is
# NOT an architectural limitation anymore: s1 correctly sees the original
# x=50 AND y=50 below (see recno-before-image.spec / recno-repeatable-read.spec
# for the dedicated before-image-reconstruction coverage).
#
