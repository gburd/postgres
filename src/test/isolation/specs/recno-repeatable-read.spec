...
#
# RECNO's heap-shaped xmin/xmax MVCC must ensure that once a snapshot is
# established, only tuples whose inserter (t_xmin) is visible to the snapshot
# are seen, and tuples deleted/updated by transactions that committed after
# the snapshot are still visible in their pre-modification form.
