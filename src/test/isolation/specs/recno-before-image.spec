# Test version reconstruction for RECNO in-place updates.
#
# RECNO overwrites tuples in place during UPDATE.  For MVCC correctness,
# the pre-update tuple version is reconstructed on read by walking the
# tuple's UNDO-fork version chain (RecnoReconstructVisibleVersion) and
# served to concurrent readers whose snapshot predates the update commit.
#
# This test exercises:
# 1. A REPEATABLE READ reader with an older snapshot receives the
#    reconstructed prior version when reading a row updated and committed
#    by another transaction.
# 2. Multiple rows updated — prior versions served for all modified rows.
# 3. New transaction starting after commit sees on-page data directly.
# 4. READ COMMITTED sees new data after commit (no prior version served).
# 5. Chained updates within one transaction — walking the version chain
#    reconstructs the original pre-transaction state (the chain steps back
#    through each update to the version visible to the reader's snapshot).
...
# s3 starts after s2 commits, so it can see the updater's xmin;
