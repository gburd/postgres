...
-- Force a deterministic UNDO-fork discard state before measuring size:
-- RelUndoMaybeVacuum() is a per-backend, timer-throttled (5s) background
-- discard, NOT triggered per-statement.  Without this explicit VACUUM, the
-- total_size figures below (and in Test 6) race the wall-clock elapsed since
-- the last throttled discard in this backend and are not reproducible (see
-- HANDOFF § recno_performance flakiness).  table_size (main fork only) is
-- unaffected.  (Placed after CREATE INDEX in Test 4: an explicit VACUUM
-- issued before any index exists on this table, followed later by CREATE
-- INDEX + DELETE + VACUUM, corrupts the relation -- see the RECNO VACUUM
-- cross-page-defragmentation bug flagged in the test audit; do not move this
-- VACUUM earlier.)
