...
--
-- Test btree root splits with downlink page-fit validation.
-- Insert enough rows to force multiple levels of internal page splits,
-- exercising the nxbt_newroot() downlink overflow check.
--
CREATE TABLE t_noxu_btree_root_splits(id int, payload text) USING noxu;
INSERT INTO t_noxu_btree_root_splits
  SELECT i, repeat('x', 100) FROM generate_series(1, 50000) i;
SELECT COUNT(*) FROM t_noxu_btree_root_splits;
SELECT MIN(id), MAX(id) FROM t_noxu_btree_root_splits;
DROP TABLE t_noxu_btree_root_splits;
