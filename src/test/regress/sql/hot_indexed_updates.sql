...
--       tuples -- this covers the class of bugs where a stale btree
--       entry's key is still reachable via a looser scan key; the
--       canonical SIU recheck (indexqualorig re-eval) is insufficient
--       here because the original qual is looser than the leaf key
--
...
-- FIXME: want 1, today returns 2 (SIU false-dup bug)
SELECT count(*) FROM siu_range WHERE a = 1 AND b < 100 AND payload IS NOT NULL;
-- FIXME: want 1 row, today returns 2 (SIU false-dup bug)
...
SET enable_seqscan = off;
SET enable_bitmapscan = off;
-- IndexScan path on the secondary index.
-- FIXME: want 1, today returns 2 (SIU false-dup bug on secondary btree)
