...
--       bitmap-overlap test drops the stale arrival because the index's
...
-- the equality-lookup case the bitmap-overlap test handles.
...
--
-- The read-side bitmap-overlap test makes the IndexScan return the correct
...
-- The stale ('1','5') leaf is dropped by the bitmap-overlap test, so this
-- returns 1.
EXPLAIN (COSTS OFF)
...
-- IndexOnlyScan: the page holds a tombstone so it is never all-visible; IOS
-- performs the heap fetch and the bitmap-overlap test drops the stale ('1','5')
-- leaf, so count = 1.
...
-- A dead HOT-indexed tuple is first rewritten as a bridge (its stale leaf
-- may still exist at prune time) and its tombstone is retained; the index
-- cleanup pass then sweeps the stale leaf, and a second VACUUM reclaims the
-- bridge and its tombstone.  So the tombstone is gone after the second
-- VACUUM, not the first.
-- ---------------------------------------------------------------------------
...
-- the stale leaf; the second reclaims the bridge and the orphaned tombstone.
...
-- index-only scan must heap-fetch through the chain so the read-side
-- bitmap-overlap test can filter stale btree entries.
--
...
-- Two HOT-indexed updates leave a multi-hop chain.  (A single-hop chain
-- folds its lone tombstone into the data redirect at prune, leaving no
-- standalone tombstone; a multi-hop chain retains each surviving hop's
-- tombstone, which is what this test needs.)
...
-- ---------------------------------------------------------------------------
-- 17. CREATE INDEX and REINDEX over live HOT-indexed chains/tombstones
--
-- A freshly built or rebuilt index must reflect current values, never a
-- stale chain member: the build scans live tuples only (tombstones are
-- invisible) and points each HOT-indexed live tuple's entry at its own TID,
-- so the new entries have no hop after them and the bitmap-overlap test keeps
-- them.
-- ---------------------------------------------------------------------------
...
--
-- After all indexes are dropped, heap pages may still carry HOT-indexed bridge
-- tombstones left by earlier updates.  VACUUM of such a no-index relation must
-- not enroll those bridges for index cleanup (the indexed bridge-reclamation
-- path asserts nindexes > 0); it must complete without error and reads must
-- stay correct via transparent bridge following.
-- ---------------------------------------------------------------------------
...
-- Must not crash on the no-index bridge path; two passes exercise the
-- second-pass reclaim guard as well.
VACUUM hi_dropidx;
VACUUM hi_dropidx;
...
-- dangle.  Two passes also exercise the bridge second-pass reclaim.
