...
-- fully reclaimed after the second VACUUM, not the first.  autovacuum_enabled
-- is off: this test's assertion depends on exactly two VACUUMs, and a
-- concurrent autovacuum could steal one of our explicit VACUUMs' relation
-- lock (VACUUM silently skips a table it can't lock), shifting the count by
-- one and leaving the chain not-yet-reclaimed when we check.
-- ---------------------------------------------------------------------------
