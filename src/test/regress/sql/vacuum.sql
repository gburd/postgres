...
CREATE VIEW vac_option_tab_counts AS
  ...
  LEFT JOIN pg_class c ON s.relid = ANY(c.reltoastrelids)
...
SELECT t.relfilenode AS toast_filenode FROM pg_class c, pg_class t
  WHERE t.oid = ANY(c.reltoastrelids) AND c.relname = 'vac_option_tab' \gset
...
SELECT t.relfilenode = :toast_filenode AS is_same_toast_filenode
  ...
  WHERE t.oid = ANY(c.reltoastrelids) AND c.relname = 'vac_option_tab';
