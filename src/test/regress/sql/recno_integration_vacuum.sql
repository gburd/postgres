--
-- RECNO Integration Test: in-place UPDATE + VACUUM + FSM + row locking
--
-- This test validates the integration between:
-- 1. RECNO in-place UPDATE (RECNO's native equivalent of heap's HOT: an
--    UPDATE that does not change an indexed column reuses the tuple's slot
--    on the same page instead of creating a new version -- no HOT chains,
--    RECNO has no heap-only-tuple mechanism)
-- 2. VACUUM (dead-version reclamation and freezing)
-- 3. FSM (Free Space Map) management
-- 4. Concurrent row locking (FOR SHARE/FOR UPDATE via RecnoLockTuple,
--    which uses the shared MultiXact infrastructure for shared locks)
--
