...
CREATE OR REPLACE FUNCTION create_relfilenode_part(relname text, indname text)
  ...
  EXECUTE format('
    CREATE TABLE %I AS
      SELECT oid, relname, relfilenode, relkind, reltoastrelids
      FROM pg_class
      WHERE oid IN
...
SELECT oid, 'pg_toast_TABLE', relfilenode, relkind, reltoastrelids
FROM pg_class WHERE oid IN
	(SELECT unnest(reltoastrelids) FROM reindex_before WHERE reltoastrelids IS NOT NULL);
INSERT INTO reindex_before
SELECT oid, 'pg_toast_TABLE_index', relfilenode, relkind, reltoastrelids
FROM pg_class where oid in
