...
# value).  Under all three modes the bitmap-overlap test must filter the
...
for my $mode ('off', 'subset_only', 'always')
{
	...
	# live tuple at the chain root has payload=999.  The bitmap-overlap test
	# must filter the stale leaf, allowing this INSERT to succeed.
