...
# Set up primary with WAL consistency checking enabled
...
$primary->append_conf('postgresql.conf', 'wal_consistency_checking = RECNO');
$primary->start;
...
# No WAL consistency warnings should have been logged
my $log = $primary->logfile;
my $wal_errors = 0;
if (open(my $fh, '<', $log)) {
	while (<$fh>) {
...
is($wal_errors, 0, "No WAL consistency errors in server log");
