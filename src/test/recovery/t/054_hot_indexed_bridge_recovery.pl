...
# Force the page out so WAL is at least up-to-date for replay.
$node->safe_psql('postgres', q{CHECKPOINT});

# Crash-restart.  stop('immediate') is the standard "kill -9" simulation
# used elsewhere in src/test/recovery/.
$node->stop('immediate');
$node->start;
...
# through bridges must be dropped by xs_hot_indexed_recheck on equality.
