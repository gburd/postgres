...
FluxVMUpdateForDelete(Relation rel, Buffer buffer)
{
	...
	/*
	 * Clear BOTH bits.  ALL_FROZEN implies ALL_VISIBLE, so the frozen bit
	 * must never outlive the visible bit.  Leaving ALL_FROZEN set on a page
	 * with a freshly deleted tuple would cause VACUUM Phase I to skip the
	 * page (the all-frozen fast path), so the deleted tuple's storage and --
	 * critically -- its index entries would never be cleaned, eventually
	 * allowing TID recycling with stale index entries still present.
	 */
	FluxVMClear(rel, blkno, buffer, FLUX_VM_VALID_BITS);
}
