# FLUX on per-backend UNDO + always-in-place UPDATE — design notes

**Status: historical.** This file is the forward-looking plan ("Phase 8") for
moving FLUX onto the per-backend UNDO engine and making indexed-column UPDATE
stay in place.  That work is done, so the stage/gate language below describes how
it was sequenced, not what the tree looks like now.  Where the plan and the code
disagree, the code wins; two specifics worth flagging before you read on:

* The plan's stage 8d says the cleanup commit would delete `relundo*.c`,
  `atm.c`, `undo_xlog.c` and the `RM_RELUNDO_ID` / `RM_UNDO_ID` rmgrs.  Only the
  per-relation fork half happened.  `atm.c`, `undo_xlog.c`, `undolog.c`,
  `undoapply.c` and `RM_UNDO_ID` all still ship, because the cluster-wide
  UNDO-in-WAL store is retained for sparse cross-object consumers (FILEOPS,
  index UNDO).  `RM_RELUNDO_ID` and the fork are gone.
* FILEOPS was not collapsed to per-backend-only; it uses the cluster-wide store,
  which is the store it is the demonstrator for.

For what actually shipped, read the FLUX commit message and
`src/backend/access/flux/README`.
