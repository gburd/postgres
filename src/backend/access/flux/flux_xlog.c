					 ...
					 FluxLogicalImage *logical_imgs)
{
	 ...
	 * MAXALIGN(t_len) on the page, so the sum of SizeOfFluxMultiInsertTuple +
				...
				FluxLogicalImage *new_img)
{
	 ...
	 * savings exceed sizeof(xl_flux_prefix_suffix) (4 bytes), we log only
...
flux_xlog_insert_redo(XLogReaderState *record)
{
...

	{
		...
		if (action == BLK_NEEDS_REDO)
		{
			 ...
			 * doesn't set up FLUX opaque space. Initialize it here if
			 * needed.
			 */
			if (PageIsNew(page))
			{
			 ...
			 * Write.  FluxPageAddTuple sets the FLUX_PAGE_DEFRAG_NEEDED
			...
			if (ItemIdIsNormal(itemid) && ItemIdHasStorage(itemid))
			{
				...
				else
				{
					...
					if (use_prefix_suffix)
					{
						 ...
						 * (see flux_operations.c), so using existing_len here
					...
					else if (xlrec->new_tuple_len <= existing_len)
					{
						 ...
						 * into it (see flux_operations.c).  Redo must produce
...

	{
		...
		if (action == BLK_NEEDS_REDO)
		{
			...
			else
			{
				 ...
				 * (FluxOverflowRecordHeader + chunk data). Add it to the
...
flux_mask(char *page, BlockNumber blkno)
{
	 ...
	 * PageInit(page, BLCKSZ, 0).  FluxPageGetOpaque would read 8 bytes past
