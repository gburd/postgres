			...
			case RELUNDO_UPDATE:
				{
					...
					/*
					 * Layout written for a full UPDATE record is
					 * [RelUndoUpdatePayload][old tuple bytes]; the old tuple
					 * occupies the trailing tuple_len bytes of the payload
					 * region returned by RelUndoReadRecord.  For a byte-diff
					 * record (RELUNDO_INFO_PARTIAL_TUPLE) those trailing bytes
					 * are instead a RelUndoDiffRecord splice; reverse-apply it
					 * against the CURRENT candidate image to reconstruct the
					 * old tuple (the same self-describing splice the engine's
					 * RelUndoApplyDiffReverse uses for rollback).
					 */
					if (urec_hdr.info_flags & RELUNDO_INFO_PARTIAL_TUPLE)
					{
						const RelUndoDiffRecord *diff = (const RelUndoDiffRecord *)
							((const char *) payload + (payload_size - urec_hdr.tuple_len));
						Size		old_len = 0;

						next_image = (char *) palloc(diff->old_total_len);
						if (!FluxApplyDiffReverse(candidate, candidate_len,
												  diff, next_image, &old_len))
						{
							elog(WARNING,
								 "FLUX PVS: byte-diff reverse-apply failed at %llu",
								 (unsigned long long) verptr);
							pfree(next_image);
							next_image = NULL;
							break;
