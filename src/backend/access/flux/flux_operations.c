				   ...
				   LockTupleMode *lockmode, Bitmapset **modified_attrs)
{
	 ...
	 * live TID -- the RECNO A->B->A duplicate bug, since there is no RowID/gen
	 * to disambiguate and no stale-entry recheck.
	 *
	 * The correct, API-change-free design (this IS the design): when any
	 * indexed column changed (modified_attrs non-empty on input), store the
	 * new version at a NEW TID (DELETE old + INSERT new, zheap's non-in-place
	 * update).  The old tuple's index entries then die with its TID and are
	 * reclaimed by ordinary VACUUM, exactly like heap.  We set
	 * TableTupleUpdateAllIndexes so the executor inserts fresh entries in every
	 * index at the new TID.  Non-indexed-column UPDATEs fall through to the
	 * in-place path below (the zheap win: update without touching indexes).
	 */
	if (modified_attrs != NULL && !bms_is_empty(*modified_attrs))
	{
		TM_Result	del_res;
					...
					if (FluxTupleWriterTryLock(old_tuple_hdr, &expected))
					{
						...
						else
						{
							 ...
							 */
							{
								...
								/*
								 * Signal index maintenance.  FLUX updates in place (TID
								 ...
								 */
								/*
								 * FLUX: key-changing UPDATEs go out of place (top of
								 * flux_tuple_update); this in-place path runs only for
								 * non-indexed changes, so no index maintenance is needed.
								 */
...
force_shrink_retry:
	...
	/*
	 * Signal index maintenance.  FLUX updates in place (TID unchanged), so an
	 ...
	 */
	/*
	 * FLUX: key-changing UPDATEs are routed out of place at the top of
	 * flux_tuple_update, so this in-place path runs only for non-indexed
	 * changes and needs no index maintenance (*modified_attrs stays empty).
	 */
	{
		/*
