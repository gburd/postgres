 ...
 * the full old-tuple before-image in the UNDO fork instead of a compact
 * byte-diff.  Used to A/B-measure the byte-diff WAL-volume optimization.
 */
						...
						else
						{
							 ...
							 */
							{
								...
								if (RelUndoRecPtrIsValid(cas_undo_ptr))
								{
									...
									{
										/*
										 * Byte-diff before-image (BDB bam_repl style;
										 * same trim PG heap's log_heap_update does):
										 * carry only the changed middle of the OLD tuple
										 * in a self-describing RelUndoDiffRecord splice,
										 * reverse-applied against the current on-page
										 * bytes with no AM callback
										 * (RelUndoApplyDiffReverse for rollback,
										 * FluxApplyDiffReverse for PVS reads).  Cuts the
										 * dominant per-UPDATE WAL volume.  Falls back to
										 * the full old tuple when the diff is absent or
										 * not compact, or when flux_disable_delta_undo
										 * forces full images (A/B measurement).
										 */
