				  ...
				  uint32 options, BulkInsertState bistate)
{
	 ...
	 * from birth with no on-page growth.  Stamp InvalidRelUndoRecPtr here;
				  ...
				  bool wait, TM_FailureData *tmfd)
{
	...
	if (snapshot)
	{
		...
		if (!visible)
		{
			...
			/*
			 * Lock-free: SLogTupleGetDirtyXid reads the seqlock-guarded sLog
			 * flat hash with EBR.  No need to release buffer lock.
			 */
				  ...
				  LockTupleMode *lockmode, TU_UpdateIndexes *update_indexes)
{
	 ...
	 * carry plain heap-style TID entries.  If we kept the tuple in place
	 * while an indexed column changed, the executor would insert a new
	 * (newkey, TID) index entry while the old (oldkey, TID) entry still
	 * pointed at the same live TID -- the A->B->A duplicate bug, since there
	 * is no RowID/gen to disambiguate and no stale-entry recheck.
	 *
	 ...
	 * HeapDetermineColumnsInfo): when any indexed column changed, store the
	 * new version at a NEW TID (DELETE old + INSERT new, zheap's non-in-place
	 ...
	 * then die with its TID and are reclaimed by ordinary VACUUM, exactly
	 * like heap.  Non-indexed-column UPDATEs fall through to the in-place
	 * path below and set TU_None (the zheap win: update without touching
	 * indexes).  FLUX never uses TU_Summarizing: a key-changing update always
	 * moves the TID, so TU_All already covers summarizing indexes.
	 */
	if (flux_indexed_attr_changed(relation, otid, slot))
	{
		...
		/*
		 * Insert the new version.  flux_tuple_insert sets slot->tts_tid to
		 * the new location and TOASTs wide columns.  The old version remains
	...
	/*
	 * In-place UPDATE: no indexed column changed, so the row keeps its TID
	 * and no index maintenance is needed.  Default the out-param to TU_None
	 * here so every TM_Ok exit of the in-place path below reports "no index
	 * update" (the caller does not pre-initialize *update_indexes; it reads
	 * it only on TM_Ok).
	 */
	if (update_indexes != NULL)
					...
					if (FluxTupleWriterTryLock(old_tuple_hdr, &expected))
					{
						...
						else
						{
							...
							}
							{
								 ...
								 * memcpy.
								 *
								 * RelUndoReserve may extend the fork and
								 * ereport -- neither is safe inside the
								 * critical section.  Lock order: data buffer
								 * then UNDO buffer.
								 */
								{
									...
									/*
									 * Reserve worst-case UNDO size
									 * (full-tuple record) up front.  We
									 * commit to either delta or full-tuple
									 * AFTER stamping the urec_ptr into the
									 * new image and computing the final diff.
									 * Over-reservation by a few bytes is
									 * benign -- the unused tail of the
									 * reserved space is wasted on that page,
									 * no worse than a SHRINK update.
									 */
									 ...
									 * path.  The trailing field is part of
									 * the on-disk slot, so the diff scan and
									 * the page memcpy both see this final
									 * layout. Stamp the freshly reserved
									 * cas_undo_ptr (NEVER preserve the prior
									 * pointer): the new diff reconstructs
									 * this update's before-image, and the
									 * header's urec_prevundorec already
									 * chains to the prior diff -- the row
									 * always points at the head of the chain.
									 * If no UNDO fork exists (unlogged/temp),
									 * stamp InvalidRelUndoRecPtr to keep slot
									 * length stable; readers fall back to the
									 * on-page tuple when the head is invalid.
									 */
									...
									{
										/*
										 * No actual data change -- release
										 * and return OK.  Cancel the UNDO
									...
									/*
									 * FOLD variant: stage the UNDO
									 * before-image BEFORE the critical
									 * section so the combined WAL record can
									 * carry both the main-fork redo byte-diff
									 * and the undo before-image in one
									 * XLogInsert (halving per-UPDATE WAL
									 * insert count vs the standalone
									 * RM_RELUNDO record).
									 *
									 * RelUndoReserve() already returned the
									 * undo buffer exclusively locked+pinned,
									 * and (for a new page) left the metapage
									 * locked; both stay held through the crit
									 * section below, so one XLogInsert may
									 * legally register buffers from both
									 * forks.  RelUndoStage() writes+dirties
									 * the undo page here (no WAL); the crit
									 * section emits the fold record and
									 * PageSetLSNs both pages.
									 *
									 * The tid the payloads reference is fully
									 * known now (blkno/offnum), so set it
									 * before staging.
									 */
									ItemPointerSet(&slot->tts_tid, blkno, offnum);
									slot->tts_tableOid = RelationGetRelid(relation);
									if (RelUndoRecPtrIsValid(cas_undo_ptr))
									{
										 ...
										 * always: FLUX matches ZHeap (whole
										 * old tuple to UNDO, no
										 * byte-diff/delta).
										 * [header][update-payload][old
										 * tuple].
										 */
									...
									/*
									 * WAL log.  When we staged an UNDO
									 * before-image, emit the single combined
									 * fold record carrying both the main-fork
									 * redo diff and the undo bytes; it
									 * PageSetLSNs both pages (and the
									 * metapage on a new undo page). Otherwise
									 * (no UNDO fork) emit the plain redo
									 * record.
									 */
									...
									/*
									 * Release the staged UNDO buffers now
									 * that the fold record has logged them
									 * (mirrors the release RelUndoFinish
									 * would have done), and register the
									 * record for rollback discovery.
									 */
									if (cas_undo_staged_valid)
									{
									...
									/*
									 * Same reasoning for the per-relation
									 * UNDO fork: this in-place AM can
									 * correctly report near-zero dead tuples,
									 * so autovacuum may never launch and
									 * RelUndoVacuum() (VACUUM-only) may never
									 * run. RelUndoMaybeVacuum() is the
									 * throttled backstop.
									 */
	 ...
	 */
	{
				 ...
				 !is_insert_entry)
		{
			...
			if (wait)
			{
				 ...
				 * value; EPQ recomputes on top of it. - ABORT:
				 * ApplyPerRelUndo() has already restored the pre-update image
...
force_shrink_retry:
		{
			...
			/*
			 * WS-PVS1: stamp InvalidRelUndoRecPtr into t_verptr, mirroring
	 ...
	 * the atomic record below). RelUndoReserve and
	...
	else
	{
		 ...
		 * PageIndexTupleDelete because FLUX pages may contain LP_UNUSED items
		 * left by opportunistic defragmentation. PageIndexTupleDelete asserts
		 * all items are LP_NORMAL, which fails when LP_UNUSED items are
		 * present. FluxPageIndexTupleDelete skips LP_UNUSED items in the
						...
						void *recdata, uint32 len)
{
	 ...
	 * an aborted in-place UPDATE -- the per-tuple sLog records below only
	 * flip visibility flags.  We run in the finishing backend's own live
