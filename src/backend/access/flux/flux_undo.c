 ...
 *
 * FLUX writes one UNDO record per tuple UPDATE and DELETE via the
 * shared UNDO-in-WAL infrastructure.  INSERT emits no UNDO record --
 * an inserted tuple has no before-image and its rollback is simply its
 * inserter (t_xmin) never committing -- but the FLUX_UNDO_INSERT apply
 * case is retained to revert INSERT records produced by older servers.
 * Records carry rmid UNDO_RMID_FLUX and an info subtype
 * (FLUX_UNDO_INSERT / UPDATE / DELETE / DELTA_UPDATE); rollback is
 * driven by undoapply.c which dispatches to flux_undo_apply() based on
 * rmid.
 *
				   ...
				   const char *tuple_data, uint32 tuple_len)
{
	...
	/*
	 * During crash recovery, UNDO is reverse-applied by PerformUndoRecovery()
	 * BEFORE WAL insertion is enabled (LocalSetXLogInsertAllowed runs later in
	 * StartupXLOG).  Emitting a CLR here would call XLogInsert ->
	 * GetWALInsertionTimeLine and trip its RECOVERY_STATE_DONE assertion (and
	 * is unnecessary): the physical restore is made durable by the
	 * end-of-recovery checkpoint, and re-application is prevented by the page
	 * LSN just as in the WAL-logged path.  So in recovery, set the page LSN to
	 * the current replay position and skip the CLR record.
	 */
	if (!XLogInsertAllowed())
	{
		TimeLineID	replayTLI;

		PageSetLSN(BufferGetPage(buffer), GetXLogReplayRecPtr(&replayTLI));
		return;
	}
				...
				const char *payload, Size payload_len, UndoRecPtr urec_ptr)
{
	...
	{
		FluxTupleHeader *new_hdr_copy = NULL;
		Size		new_hdr_len = 0;
		char	   *delta_recon = NULL;

		/*
		 * A delta record carries only the changed middle of the old image.
		 * Reconstruct the FULL old image from the current on-page bytes FIRST,
		 * then fall through to the identical FLUX_UNDO_UPDATE handling below:
		 * the shared restore + index-undo path needs the full old tuple (for
		 * indexed-key reversal) and the CLR must carry the full restored image
		 * so its redo stays order-free.
		 */
		if (info == FLUX_UNDO_UPDATE_DELTA)
		{
			Page		pg = BufferGetPage(buffer);
			ItemId		lp = PageGetItemId(pg, offnum);
			const FluxUndoDelta *delta = (const FluxUndoDelta *) image_bytes;

			if (!(hdr.flags & FLUX_UNDO_FLAG_HAS_TUPLE) ||
				image_len < SizeOfFluxUndoDelta)
			{
				ereport(WARNING,
						(errmsg_internal("FLUX UNDO UPDATE_DELTA: truncated "
										 "delta (%zu bytes) at %llu",
										 image_len,
										 (unsigned long long) urec_ptr)));
			}
			...
			{
				ereport(DEBUG2,
						(errmsg_internal("FLUX UNDO UPDATE_DELTA: item (%u,%u) "
										 "not normal, skipping",
										 blkno, offnum)));
			}
			else
			{
				char	   *cur = (char *) PageGetItem(pg, lp);
				Size		cur_len = ItemIdGetLength(lp);

				delta_recon = (char *) palloc(delta->old_len);
				if (FluxUndoDeltaReconstruct(delta, image_len,
											 cur, cur_len, delta_recon))
				{
					/* Continue as a full UPDATE from the reconstructed image. */
					image_bytes = delta_recon;
					image_len = delta->old_len;
					info = FLUX_UNDO_UPDATE;
				}
				else
				{
					ereport(WARNING,
							(errmsg_internal("FLUX UNDO UPDATE_DELTA: reverse-diff "
											 "failed at %llu; leaving tuple in place",
											 (unsigned long long) urec_ptr)));
					pfree(delta_recon);
					delta_recon = NULL;
					info = 0;	/* fall through to no-op default */
				}
			}
		}
		...
		switch (info)
		{
			...
			case FLUX_UNDO_DELETE:
				...
				/*
				 * Phase 8c: for an in-place indexed-column UPDATE we must also
				 * reverse the index changes (kill the new (k_new,TID) entry,
				 * clear the old (k_old,TID) delete-mark).  Snapshot the current
				 * on-page (post-update, k_new) tuple BEFORE we overwrite it
				 * with the before-image; the actual index work runs after the
				 * table buffer lock is released (index maintenance must not run
				 * under a table content lock -- AB-BA with bottom-up deletion).
				 */
				if (info == FLUX_UNDO_UPDATE &&
					RelationSupportsDeleteMarking(rel) &&
					rel->rd_rel->relhasindex)
				{
					...
					if (ItemIdIsNormal(lp))
					{
						new_hdr_len = ItemIdGetLength(lp);
						new_hdr_copy = (FluxTupleHeader *) palloc(new_hdr_len);
						memcpy(new_hdr_copy, PageGetItem(pg, lp), new_hdr_len);
					}
				}
		...
		/*
		 * Now that the table buffer lock is released, reverse the secondary
		 * index changes of a rolled-back in-place indexed-column UPDATE.  The
		 * before-image (image_bytes) is k_old; new_hdr_copy is k_new.
		 */
		if (new_hdr_copy != NULL)
		{
			flux_inplace_index_undo(rel, &hdr.tid,
									new_hdr_copy, new_hdr_len,
									(FluxTupleHeader *) image_bytes,
									(Size) image_len);
			pfree(new_hdr_copy);
		}
	...
	/*
	 * The physical before-image has been restored (or the inserted tuple
	 * marked deleted).  Drop this (relation, tid)'s sLog op for the aborting
	 * xid: FLUX kept the op as SLOG_OP_ABORTED (set at XACT_EVENT_ABORT) so
	 * concurrent readers treated the row as still-live while the physical
	 * restore was pending; now the restore is done, so the marker can go.
	 * This drops the marker scoped to the exact tid just restored so it is race-free whether
	 * this apply runs inline at abort or in the background undo worker (a
	 * global sweep on the first record would drop markers for tids not yet
	 * restored).  During a SUBXACT (SAVEPOINT) apply we skip this: the marker
	 * removal is subxact-scoped and handled by SLogTupleRemoveBySubXid in the
	 * SUBXACT_EVENT_ABORT_SUB handler (removing by top xid here would drop
	 * markers sibling/parent subtransactions still need).
	 */
	if (!FluxPbuInSubxactApply())
		SLogTupleRemoveByXidSingle(reloid, &hdr.tid, xid);
			   ...
			   const char *payload, Size payload_len)
{
	...
	if (payload_len >= SizeOfFluxUndoPayloadHeader)
	{
		...
		if (info == FLUX_UNDO_UPDATE_DELTA &&
			payload_len >= SizeOfFluxUndoPayloadHeader + SizeOfFluxUndoDelta)
		{
			FluxUndoDelta delta;

			memcpy(&delta, payload + SizeOfFluxUndoPayloadHeader,
				   SizeOfFluxUndoDelta);
			appendStringInfo(buf,
							 " delta old_len=%u prefix=%u suffix=%u middle=%u",
							 delta.old_len, delta.prefixlen, delta.suffixlen,
							 delta.old_len - delta.prefixlen - delta.suffixlen);
		}
	}
	else
