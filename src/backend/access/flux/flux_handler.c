						...
						bool *call_again, bool *all_dead)
{
	...
	*call_again = false;
	 ...
	 * recheck.  FLUX updates in place keeping the same TID, so a changed
	 ...
	 * stale: a committed in-place UPDATE (FLUX_TUPLE_UPDATED set, not
...
visibility_done:
	...
	if (visible && (tuple_hdr->t_flags & FLUX_TUPLE_DELETED))
	{
		...
		tts->tts_tableOid = RelationGetRelid(rel);
		tts->tts_tid = *tid;
		if (orig_flags & FLUX_TUPLE_UPDATED)
			scan->base.xs_inplace_maybe_stale = true;
		/*
	...
	if (FluxTupleToSlotWithOverflow(tuple_hdr, tts, rel))
	{
		 ...
		 *
		 * We do NOT exclude FLUX_TUPLE_UNCOMMITTED: that bit is a lazy hint
							 ...
							 TableScanDesc scan)
{
	...
	while (table_scan_getnextslot(scan, ForwardScanDirection, slot))
	{
		 ...
		 * For a wider-than-TID identity (FLUX's (TID, gen)) the index AM
 ...
 */
static Bitmapset *
flux_modified_attrs(Relation rel, Bitmapset *attrs,
...
static const TableAmRoutine flux_methods = {
	...
	/*
	 * FLUX does NOT participate in the RowID/in-place index contract.  Its
	 * key-changing UPDATE is heap-like (new TID, old version to UNDO, old
	 * index entry dies and is VACUUMed); its non-key UPDATE is in place.  In
	 * both cases secondary indexes are plain heap-TID nbtree entries with
	 * standard maintenance, so the executor must treat FLUX exactly like heap:
	 * no xs_want_itup forcing, no xs_inplace_recheck.  Leaving this flag false
	 * selects the heap-TID index path (see indexam.c / execIndexing.c).
	 */
	...
	.modified_attrs = flux_modified_attrs,
	...
	/*
	 * FLUX deliberately provides NO index_row_key_type callback: it does not
	 * implement the RowID/gen index contract.  Secondary indexes hold plain
	 * heap-style TID entries and are maintained exactly like heap indexes.
	 * A key-changing UPDATE stores the new version out of place (new TID) so
	 * the old (key, TID) index entry becomes dead and is reclaimed by VACUUM,
	 * never duplicated -- see flux_tuple_update().
	 */
};
