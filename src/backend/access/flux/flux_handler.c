...
flux_scan_getnextslot(TableScanDesc sscan, ScanDirection direction, TupleTableSlot *slot)
{
	...
	for (;;)
	{
		...
		while (scan->rs_cindex < scan->rs_ntuples)
		{
			 ...
			 * the per-relation UNDO fork (WS-PVS2):
	...
	*call_again = false;
	...
	if (snapshot)
	{
		if (snapshot->snapshot_type == SNAPSHOT_DIRTY)
		{
			 ...
			 * transaction).  Use t_xmin for the fast path, falling back to
			 * sLog for speculative inserts.
			 */
...
reacquire:
	...
	/*
	 * Lock succeeded.  Set traversed for FIND_LAST_VERSION callers. FLUX uses
	 * in-place updates, so the current tuple IS the latest version — the
	 * update chain was trivially "followed."
	 */
	if (tmfd)
	{
	...
	if (slot && result == TM_Ok)
	{
		 ...
		 */
		{
			...
			/*
			 * Record the precise LockTupleMode alongside the coarse lock_op
			 * so a concurrent updater can apply the real heavyweight conflict
			 * matrix: FOR KEY SHARE (AccessShareLock) stays compatible with a
			 * NoKeyExclusive UPDATE, while FOR SHARE/FOR UPDATE correctly
			 * block it.  See SLogTupleGetWriteConflictXid.
			 */
			FluxEnsureSLogCallbacks();
							 ...
							 TupleTableSlot *slot)
{
	...
	for (; rscan->rs_cindex <= maxoffset; rscan->rs_cindex++)
	{
		...
		if (sample_it)
		{
			 ...
			 * branch above).  Probing the fork record must happen with no
			 * data page content lock held.  RelUndoReadRecordHeader reads
			 * only the record header (no payload allocation) and returns
			 * false once the record has been discarded by VACUUM, so the
			 * proxy resets after the fork is reclaimed.
			 */
