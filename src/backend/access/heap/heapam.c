			...
			uint32 options, BulkInsertState bistate)
{
	...
	if (RelationNeedsWAL(relation))
	{
		 ...
		 *
		 * Tepid: also require that the page's tuple area contains nothing
		 * other than this tuple.  Vacuum's lp_truncate_only second pass
		 * (PRUNE_VACUUM_CLEANUP) does not call PageRepairFragmentation, so
		 * a page can legitimately end up with one LP_UNUSED slot at offset
		 * 1 plus orphan tuple bytes left over from the previous lifetime.
		 * If heap_insert reuses that LP_UNUSED slot, primary's page keeps
		 * the orphan bytes while a standby replaying INSERT+INIT zeroes
		 * them.  Emitting INSERT+INIT in that case trips
		 * wal_consistency_checking.  Falling back to a regular INSERT
		 * (with the FPI on first touch after a checkpoint) keeps replay
		 * byte-identical without sacrificing crash safety.
		 */
...
/*
 * RelationComputeHotIndexedEligibility --
 *
 * Compute and cache the index cost metrics for HOT-indexed eligibility.
 * These metrics are:
 *   - expr_indexes: count of indexes with expression columns
 *   - regular_indexes: count of regular B-tree/hash/GiST/GIN/BRIN indexes
 *   - partial_indexes: count of indexes with WHERE clauses
 *
 * We exclude summarizing indexes since they don't participate in HOT chains.
 *
 * This is computed lazily (on first call) and cached in the relation's
 * rd_expr_indexes, rd_regular_indexes, rd_partial_indexes fields.
 */
static void
RelationComputeHotIndexedEligibility(Relation relation)
{
	List	   *indexoids;
	ListCell   *lc;
	int			expr_count = 0,
				reg_count = 0,
				part_count = 0;
	Bitmapset  *summ_attrs;

	if (relation->rd_expr_indexes >= 0)
		return;				/* already computed */

	/* Get the list of indexes on this relation (includes summarizing indexes) */
	indexoids = RelationGetIndexList(relation);

	/* Get summarizing attribute bitmap to filter them out */
	summ_attrs = RelationGetIndexAttrBitmap(relation,
	...
	foreach(lc, indexoids)
	{
		...
		idx_rel = try_relation_open(idx_oid, AccessShareLock);
		if (!idx_rel)
			continue;		/* index was dropped */

		/* Get indexed attributes for this index */
		idx_attrs = RelationGetIndexAttrBitmap(idx_rel,
											   INDEX_ATTR_BITMAP_INDEXED);

		/* Check if all attributes are summarizing */
		is_summarizing = bms_is_subset(idx_attrs, summ_attrs);
		bms_free(idx_attrs);

		if (is_summarizing)
		{
			relation_close(idx_rel, AccessShareLock);
			continue;		/* skip summarizing indexes */
		}

		/* Check for expression indexes (rd_indexprs != NIL) */
		has_expression = (idx_rel->rd_indexprs != NIL);

		/* Check for partial indexes (rd_indpred != NIL) */
		has_predicate = (idx_rel->rd_indpred != NIL);
	...
	/* Cache the counts */
	relation->rd_expr_indexes = expr_count;
	relation->rd_regular_indexes = reg_count;
	relation->rd_partial_indexes = part_count;
}
...
HeapUpdateHotAllowable(Relation relation, const Bitmapset *modified_idx_attrs)
{
	...
	/*
	 * Self-tuning HOT-indexed eligibility heuristic based on index cost and
	 * column coverage.  We compute and cache index cost metrics (expression,
	 * regular, partial) to determine whether HOT-indexed updates are worthwhile.
	 *
	 * The heuristic:
	 *   - Ultra-high index cost (5.0+): enables HOT-indexed even with 100% coverage
	 *     because skipping expensive indexes (like many expressions) justifies overhead
	 *   - High cost (3.0+): enables HOT-indexed with coverage <75%
	 *   - Moderate cost (2.0+): enables HOT-indexed with coverage <75%
	 *   - Low cost (<2.0): disables HOT-indexed (overhead not justified)
	 *
	 * Cost calculation: expr_indexes * 1.5 + regular_indexes * 1.0 + partial_indexes * 0.5
	 */
	{
		int			natts = RelationGetNumberOfAttributes(relation);
		int			indexed_col_count;
		double		coverage;
		double		effective_index_cost;
		Bitmapset  *indexed_attrs;

		/* Compute and cache index counts if not already done */
		RelationComputeHotIndexedEligibility(relation);

		/* Calculate effective index cost */
		effective_index_cost = (double)(relation->rd_expr_indexes * 15 +
							  relation->rd_regular_indexes * 10 +
							  relation->rd_partial_indexes * 5) / 10.0;

		/* Get indexed column coverage (percentage of all columns indexed) */
		indexed_attrs = RelationGetIndexAttrBitmap(relation,
									   INDEX_ATTR_BITMAP_INDEXED);
		indexed_col_count = bms_num_members(indexed_attrs);
		coverage = natts > 0 ? (double)indexed_col_count / natts : 0.0;
		bms_free(indexed_attrs);

		/*
		 * Decision tree:
		 * 1. Ultra-high cost (5.0+): always enable (expression indexes valuable)
		 * 2. High cost (3.0+): enable if coverage < 75%
		 * 3. Moderate cost (2.0+): enable if coverage < 75%
		 * 4. Low cost (<2.0): disable
		 *
		 * Disable conditions (inverted for the if statement):
		 *   - cost < 2.0: always disable
		 *   - cost >= 5.0: never disable
		 *   - cost >= 3.0 && cost < 5.0: disable if coverage >= 75%
		 *   - cost >= 2.0 && cost < 3.0: disable if coverage >= 75%
		 */
		if (effective_index_cost < 2.0 ||
			(effective_index_cost >= 2.0 && effective_index_cost < 5.0 && coverage >= 0.75))
		{
			result = HEAP_HOT_MODE_NO;
			goto out;
		}
				...
				Size tombstone_item_size)
{
	...
	/*
	 * If new tuple is the single and first tuple on page, replay can
	 * reinit the page from scratch.  Tepid: also require that the page's
	 * tuple area contains nothing other than this tuple.  See heap_insert
	 * for why this matters when vacuum has left orphan tuple bytes behind
	 * an LP_UNUSED slot.
	 */
