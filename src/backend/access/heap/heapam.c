...
HeapUpdateHotAllowable(Relation relation, const Bitmapset *modified_idx_attrs)
{
	 ...
	 *
	 * Non-btree indexes need no carve-out: the read-side recheck is
	 * bitmap-overlap on the index's referenced heap attributes
	 ...
	 * kind.
	 *
	 * 4) An UPDATE that modifies an attribute referenced by an expression
	 ...
	 *
	 * 5) Partitions.  Partitioned-table support is not implemented yet and is
	 * deferred deliberately; a partition falls back to a non-HOT update until
	 * it lands.
	 *
	 * 6) The logical-replication apply path, gated by the per-subscription
	 * hot_indexed_on_apply option (off / subset_only / always).  HOT-indexed
	 * updates of a replica-identity attribute leave stale index leaves; the
	 * apply worker's replica-identity lookups cope with them because
	 * RelationFindReplTupleByIndex drops stale leaves (see
	 * subscription/013_partition).  off disqualifies when the subscriber has
	 * any indexed attribute beyond its PK; subset_only (the default) requires
	 ...
	 * expression index (see header comment, case 4).  Updates that leave
	 * every expression-index attribute unchanged remain eligible.
	 */
				...
				Size tombstone_item_size)
{
	 ...
	 * matters when vacuum has left orphan tuple bytes behind an LP_UNUSED
	 * slot.
	 */
