...
HeapUpdateHotAllowable(Relation relation, const Bitmapset *modified_idx_attrs)
{
	 ...
	 * README.HOT-INDEXED "Catalog Enablement").  HEAP_HOT_MODE_NO triggers
	 * below are:
	 *
	 ...
	 *
	 * 3) Per-relation chain-length cap (see RelationGetHotIndexedChainMax),
	 * enforced in heap_update.
	 *
	 * Non-btree and expression indexes need no carve-out: the read-side
	 * recheck is bitmap-overlap on the index's referenced heap attributes
	 * (RelationGetIndexedAttrs), which is defined for every access method and
	 * needs no key evaluation, so a stale leaf is recognised for any index
	 * kind.
	 ...
	 * safe against stale chain entries.  Falling back to a non-HOT update here
	 * is exactly the pre-HOT-indexed behaviour for such catalog updates.
	 */
	if (IsCatalogRelation(relation))
	{
		result = HEAP_HOT_MODE_NO;
