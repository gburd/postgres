			...
			HeapUpdateIndexMode hot_mode)
{
	...
	bool		set_locator_split = false;	/* set VM LOCATOR_SPLIT on block */
	...
	/*
	 * A HOT-indexed (selective index) update plants a fresh index entry
	 * mid-chain and must mark this block VISIBILITYMAP_LOCATOR_SPLIT so
	 * BitmapAnd unions rather than intersects it (see amlocator.h).  Pin the
	 * block's VM page now, before locking the heap buffer, whether or not the
	 * page is all-visible, so the bit can be set in the critical section.
	 */
	if (!bms_is_empty(modified_idx_attrs) && vmbuffer == InvalidBuffer)
		visibilitymap_pin(relation, block, &vmbuffer);
...
l2:
	...
	else
	{
		...
		Buffer		vmbuffers[2] = {
			(clear_all_visible ||
			 (newbuf == buffer && !bms_is_empty(modified_idx_attrs))) ?
			vmbuffer : InvalidBuffer,
	...
	if (use_hot_update)
	{
		...
		if (hot_mode == HEAP_SELECTIVE_INDEX_UPDATE)
		{
			...
			/*
			 * Only a real indexed-column change plants a fresh index entry, and
			 * only then do two indexes disagree on this block.  A promoted
			 * empty-modified-attrs update (a HOT-indexed chain member touched
			 * on no indexed column; see above) inserts into no index, so it
			 * needs no split signal.  Gate on modified_idx_attrs, which the
			 * executor pinned the VM buffer for at entry.
			 */
			if (!bms_is_empty(modified_idx_attrs))
				set_locator_split = true;
		}
	}
	else
	{
	...
	if (set_locator_split)
	{
		Assert(newbuf == buffer);		/* HOT-indexed update stays on-page */
		Assert(BufferIsValid(vmbuffer));
		if (!visibilitymap_set_locator_split(block, vmbuffer))
			set_locator_split = false;	/* already set; nothing to WAL */
	}
	...
	if (RelationNeedsWAL(relation))
	{
		...
		recptr = log_heap_update(relation, buffer,
								 (vmbuffer_modified || set_locator_split) ?
								 vmbuffer : InvalidBuffer,
								 newbuf,
		...
		if (set_locator_split && !vmbuffer_modified && BufferIsValid(vmbuffer))
			PageSetLSN(BufferGetPage(vmbuffer), recptr);
		if (vmbuffer_modified)
			PageSetLSN(BufferGetPage(vmbuffer), recptr);
 ...
 * on the executor path go through the table AM's modified_attrs callback
 * (table_modified_attrs; heapam_modified_attrs for heap), which intentionally
 * uses a different, storage-agnostic comparator; see README.HOT-INDEXED
 * section 6.
 */
static bool
 ...
 *
 * This is the non-executor counterpart of ExecUpdateModifiedIdxAttrs (which
 * delegates the comparison to the table AM's modified_attrs callback,
 * heapam_modified_attrs for heap), and the two are meant to be *functionally*
 * equivalent (both answer "which indexed attributes changed value?").  They
 * intentionally differ in the Datum comparator they use: this heap-side path
 * uses heap_attr_equals (datumIsEqual), the executor path uses datum_image_eq
 * -- see README.HOT-INDEXED section 6 for why.
 *
				   ...
				   bool *update_all_indexes)
{
	...
	*update_all_indexes = false;
	...
	if (!heap_fetch(relation, SnapshotAny, &bslot->base.tupdata, &buffer, false))
	{
		...
		/* local_modified_idx_attrs not yet initialized */
						...
						bool inexact_ok)
{
	...
	if (unlikely(indexpagehoffnum > maxoff))
	{
		 ...
		 *
		 * Only relations whose locator may be inexact can produce such an
		 * entry.  Everywhere else this is still the corruption it always was,
		 * so keep reporting it: the check is a genuine safety net for
		 * btree/heap corruption during ordinary DML.
		 */
		if (inexact_ok)
			return;
	...
	if (unlikely(!ItemIdIsUsed(iid)))
	{
		 ...
		 *
		 * As above, only an inexact-capable relation can get here legitimately;
		 * elsewhere an index entry pointing at an unused line pointer is still
		 * corruption and is still reported.
		 */
		if (inexact_ok)
			return;
		...
		ereport(ERROR,
				(errcode(ERRCODE_INDEX_CORRUPTED),
				 errmsg_internal("heap tid from index tuple (%u,%u) points to unused heap line pointer at offset %u of block %u in index \"%s\"",
								 ItemPointerGetBlockNumber(htid),
								 indexpagehoffnum,
								 istatus->idxoffnum, delstate->iblknum,
								 RelationGetRelationName(delstate->irel))));
 ...
 */
/*
 * heap_bucket_may_disagree
 *		LocatorDesc.bucket_may_disagree for heap: does this block carry a
 *		selective-indexed row whose fresh index entry names a different line
 *		pointer than an unchanged index's root-pointing entry?
 *
 * Answered by a lock-free read of VISIBILITYMAP_LOCATOR_SPLIT, set during the
 * update that planted the fresh entry and cleared by prune once no such row
 * remains on the block.  See tidbitmap.c (the caller) and amlocator.h.
 */
bool
heap_bucket_may_disagree(Relation rel, uint64 bucket, Buffer *cache)
{
	return visibilitymap_get_locator_split(rel, (BlockNumber) bucket, cache);
}
...
{
	/*
	 * A relation whose AM can produce blocks where two indexes' entries for one
	 * row disagree on offset (a selective-indexed heap; it declares the
	 * bucket_may_disagree hook) may legitimately have an index entry that
	 * points at a line pointer another index would not, so we judge such an
	 * entry less strictly; see index_delete_check_htid().
	 */
	bool		inexact_ok =
		RelationGetLocatorDesc(rel)->bucket_may_disagree != NULL;
				...
				bool walLogical)
{
	...
	if (set_locator_split)
		xlrec.flags |= XLH_UPDATE_NEW_LOCATOR_SPLIT;
	if (prefixlen > 0)
