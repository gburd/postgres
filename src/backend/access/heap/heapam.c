 ...
 *
 * Cache the per-relation HOT-indexed eligibility metrics.  Called from the
 * UPDATE hot path, so it must not open or lock individual index relations:
 * the executor already holds them open, and the heap-write path does not.
 * Instead we lean on the relcache-cached non-summarizing indexed-attribute
 * bitmap (INDEX_ATTR_BITMAP_INDEXED), which is computed once per relcache
 * lifetime and refreshed on invalidation.
 *
 * The three cached counts (rd_expr_indexes, rd_regular_indexes,
 * rd_partial_indexes) used to drive a cost-vs-coverage heuristic in
 * HeapUpdateHotAllowable.  That heuristic was removed; the fields are
 * retained so future per-relation tuning can latch onto them without a
 * relcache layout change.  For now rd_regular_indexes records the count of
 * heap attributes referenced by any non-summarizing index, and the other
 * two are zero.
 */
static void
RelationComputeHotIndexedEligibility(Relation relation)
{
	...
	relation->rd_expr_indexes = 0;
	relation->rd_regular_indexes = bms_num_members(idx_attrs);
	relation->rd_partial_indexes = 0;
...
HeapUpdateHotAllowable(Relation relation, const Bitmapset *modified_idx_attrs)
{
	...
	/*
	 * Eligibility cache.  The cost-vs-coverage heuristic that previously
	 * gated HOT-indexed at this point was removed: it disabled HOT-indexed
	 * for narrow tables with one or two cheap indexes, which is the exact
	 * regime the design targets.  Per-relation tuning, when reintroduced,
	 * should latch onto the cached counts populated here.  The
	 * hot_indexed_update_threshold GUC remains as the explicit operator
	 * knob below.
	 */
