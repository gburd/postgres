							  ...
							  OffsetNumber forward_offnum)
{
	 ...
	 * HEAP_XMIN_INVALID|HEAP_XMAX_INVALID, for which HeapTupleHeaderIsHotUpdated
	 * always returns false, so that bit would be inert and misleading.  Chain
	 * walkers recognise and follow bridges via the explicit
	 * HeapTupleHeaderIsHotIndexedBridge predicate (see heap_hot_search_buffer),
	 * not via HEAP_HOT_UPDATED.
	 */
