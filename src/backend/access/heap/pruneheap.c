...
prune_handle_tombstones(PruneState *prstate)
{
	...
	for (int i = 0; i < prstate->ntombstones; i++)
	{
		...
		{
			/*
			 * HARDENING A: Validate chain consistency before recording tombstone.
			 * Catch orphaned tombstones whose target chain root is missing or
			 * corrupted. This prevents "failed to find target" count mismatches.
			 */
			...
			if (ItemIdIsNormal(target_itemid))
			{
				...
				if ((thdr->t_infomask2 & HEAP_INDEXED_UPDATED) != 0)
				{
					...
					Assert(root != InvalidOffsetNumber);
					Assert(ItemIdIsNormal(root_itemid));
				}
			}
