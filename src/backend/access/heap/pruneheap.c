						...
						OffsetNumber *data_redirects, int ndata_redirects)
{
	 ...
	 * indexed-attribute change made while it was stale.  The union is computed
	 * here by walking the on-page chain -- all members are still present, the
	 * now-unused reclaim runs afterwards -- starting at the root's successor
	 * (or, for a re-collapse, the prior redirect's target seeded with its
	 * existing bitmap) and OR-ing each crossed member's adjacent-tombstone
	 * bitmap through to first_live.  The blob is written over the dead root
	 * tuple's freed bytes (lp_off unchanged; PageRepairFragmentation relocates
	 * it like any stored item).
	 */
	...
	for (int i = 0; i < ndata_redirects; i++)
	{
		...
		/*
		 * Seed the union and pick the walk start.  A re-collapse (root already
		 * a data redirect) already accounts for the root -> old-target hops in
		 * its existing blob, so seed from it and extend from the old target.
		 */
