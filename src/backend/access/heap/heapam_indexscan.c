...
heapam_index_fetch_begin(Relation rel, uint32 flags)
{
	 ...
	 * by heapam_index_entry_needs_recheck via the recheck callback.
	 */
 ...
 * heapam_index_fetch_tuple recorded, privately in IndexFetchHeapData, whether
 * it crossed a HOT/SIU hop after the arriving entry's own tuple
 ...
 * crossed-attribute bitmap with the index's indexed attributes.  This must
 * happen here (not in heapam_index_fetch_tuple) because IndexFetchTableData
 * carries the heap relation, not the index, so index_fetch_tuple alone lacks
 * the index's attrs.  No key comparison is needed: any overlap means one of
 * this index's inputs changed after the entry's tuple, so its key is stale.
 */
bool
heapam_index_entry_needs_recheck(struct IndexFetchTableData *scan,
