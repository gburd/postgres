...
UndoShmemRequest_internal(void *arg)
{
	 ...
	 */
	{
			...
			!IsUnderPostmaster && IsPostmasterEnvironment)
		{
			...
			/*
			 * Per-backend UNDO engine background workers.  Gated by the
			 * pbu_undo_workers_enabled GUC (default off): no access method
			 * produces per-backend UNDO in this phase, so the workers stay
			 * dormant unless explicitly enabled.  Registered here (request_fn
			 * phase) for the same reason as UndoWorkerRegister(): a static
			 * background worker cannot be registered after
			 * BackgroundWorkerShmemInit().
			 */
			if (pbu_undo_workers_enabled)
			{
 ...
 *
 * 2. The UndoRmgrs[] dispatch table and each registered AM's undo hook
