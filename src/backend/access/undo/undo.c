 ...
 * structures (e.g. RECNO's sLog) is registered as its own sibling entry in
 * storage/subsystemlist.h, not sized or initialized here.  Likewise, the
 ...
 * (e.g. RECNO's sLog) are sized by their own PG_SHMEM_SUBSYSTEM entry and
 ...
 * AM-specific shared structure with the same "no found-guard" hazard as
 * RECNO's sLog handles its own EXEC_BACKEND re-attach via its own
 * PG_SHMEM_SUBSYSTEM entry (see storage/subsystemlist.h); this function is
 * only responsible for the generic UNDO sub-modules listed above.
 */
static void
