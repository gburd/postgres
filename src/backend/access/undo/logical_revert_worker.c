...
LogicalRevertWorkerMain(Datum main_arg)
{
	...
	while (!got_SIGTERM)
	{
		...
		if (ATMGetNextUnreverted(&xid, &entry_dboid, &last_batch_lsn))
		{
			...
			/*
			 * Test hook: let a recovery test freeze the worker here, after the
			 * ATM entry has been (re)discovered but before it is reverted and
			 * forgotten, so the test can observe the reconstructed ATM state.
			 */
			...
			PG_CATCH();
			{
				 ...
				 * never happen for an un-reverted abort.  The retention
				 * chain pins that WAL against recycling for exactly as long
				 * as the ATM entry survives:
				 *
				 ...
				 * that invariant was violated and UNDO WAL for an
				 * un-reverted abort is genuinely gone: the cluster's
				 * guaranteed-rollback claim is already broken.  Silently
				 * marking the entry reverted would fake a rollback that
				 * never happened and leave stale tuple state behind, so
				 * fail loudly instead -- the integrity guarantee is
				 * cluster-wide, hence PANIC rather than continuing.
				 *
