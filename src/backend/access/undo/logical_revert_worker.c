...
int			max_logical_revert_workers = 4;
...
LogicalRevertWorkerMain(Datum main_arg)
{
	...
	while (!got_SIGTERM)
	{
		 ...
		 * to-back ATM entries via the `continue` path below never reaches
		 * the WaitLatch sleep, and ALTER DATABASE ... SET TABLESPACE /
		 * CREATE DATABASE ... STRATEGY=FILE_COPY will hang waiting for
		 * this backend to accept PROCSIGNAL_BARRIER_SMGRRELEASE.
		 */
		...
		if (ATMGetNextUnreverted(&xid, &entry_dboid, &last_batch_lsn))
		{
			...
			PG_CATCH();
			{
				...
				/*
				 * If the WAL segment has been recycled (LSN behind redo
				 * pointer), the UNDO chain is permanently lost.  Mark the
				 * entry as reverted anyway to prevent infinite retry loops
				 * that would keep the worker busy and log-spamming forever.
				 *
				 * This is a data-loss scenario (the aborted transaction's
				 * effects are not fully reverted) but it's better than
				 * crashing the server repeatedly.  The tuples will have
				 * stale UNCOMMITTED/DELETED flags that are cleared lazily
				 * by the visibility check or VACUUM.
				 */
				{
					...
					if (XLogRecPtrIsValid(last_batch_lsn) &&
						last_batch_lsn < redo)
					{
						elog(WARNING, "logical revert worker: UNDO WAL for "
							 "xid %u at %X/%X has been recycled (redo at "
							 "%X/%X); marking as reverted (partial rollback)",
							 xid, LSN_FORMAT_ARGS(last_batch_lsn),
							 LSN_FORMAT_ARGS(redo));

						/*
						 * Force-forget the unrevertable entry. We're already
						 * in a failed transaction state from PG_CATCH, so
						 * abort it and start fresh to perform the cleanup.
						 */
