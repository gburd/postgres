...
relundo_get_metapage(Relation rel, int mode)
{
	...
	/*
	 * If the RELUNDO fork has no blocks (e.g., after crash recovery where
	 * the fork was created but the metapage wasn't written), create and
	...
	if (meta->magic != RELUNDO_METAPAGE_MAGIC)
	{
		 ...
		 * initialization WAL record wasn't replayed (e.g., the crash
		 * occurred between smgrcreate and the metapage write).
		 *
		 * Reinitialize the metapage so subsequent UNDO operations can
		 * proceed.  This is safe because an uninitialized metapage means
		 * no UNDO records were ever written, so there's nothing to lose.
		 */
		/*
	...
	if (meta->version != RELUNDO_METAPAGE_VERSION && meta->version != 1)
		ereport(ERROR,
				(errcode(ERRCODE_INDEX_CORRUPTED),
