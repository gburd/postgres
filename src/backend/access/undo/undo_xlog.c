 ...
 * should not know heap/recno WAL record formats.  This #include and the
 * RM_HEAP_ID/RM_RECNO_ID branches in UndoValidateBatchLSN() and
 ...
 * is an optional rmgr callback (e.g. rm_undo_batch_locate) that heap/recno
 * implement and core calls if present.  Deferred: adding an rmgr method
 ...
 * with a HAS_UNDO flag.  Returns false for RM_RECNO_ID or other unrecognized
 * record types.  Used by the inline UNDO path to avoid calling
...
UndoValidateBatchLSN(XLogRecPtr batch_lsn)
{
	...
	/* Any other rmid (including RM_RECNO_ID) is not a valid UNDO source */
	MemoryContextSwitchTo(old_ctx);
	return result;
}
