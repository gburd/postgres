...
UndoValidateBatchLSN(XLogRecPtr batch_lsn)
{
	...
	/* Allocate or reuse the cached reader */
	...
	if (rmid == RM_HEAP_ID)
	{
		...
		if (info == XLOG_HEAP_INSERT)
			return (((xl_heap_insert *) data)->flags & XLH_INSERT_HAS_UNDO) != 0;
		if (info == XLOG_HEAP_DELETE)
			return (((xl_heap_delete *) data)->flags & XLH_DELETE_HAS_UNDO) != 0;
		if (info == XLOG_HEAP_UPDATE || info == XLOG_HEAP_HOT_UPDATE)
			return (((xl_heap_update *) data)->flags & XLH_UPDATE_HAS_UNDO) != 0;
	}
