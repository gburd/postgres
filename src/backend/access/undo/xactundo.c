...
typedef struct XactUndoData
{
	 ...
	 * per-relation UNDO fork.  Allocated in CurTransactionContext, so the
	 * list pointer is reset to NULL at every transaction end.
	 */
...
ApplyPerRelUndo(void)
{
	 ...
	 *
	 * This ordering is mandatory for in-place MVCC: a second writer blocked
	 * in XactLockTableWait wakes the instant our XID leaves the proc array.
	 * If the before-image were restored asynchronously by the background
	 * worker (which runs only after lock release), the waiter would read our
	 * not-yet-reverted in-place value, write on top of it, commit, and then
	 * the worker would clobber the waiter's committed value with our stale
	 * before-image -- a lost update.  Restoring synchronously here closes
	 * that window: the page already holds the pre-abort image when the waiter
	 * wakes.
	 *
	...
	for (entry = XactUndo.relundo_list; entry != NULL; entry = entry->next)
	{
		...
		PG_CATCH();
		{
			 ...
			 * locks on the pages it had pinned into touched[]
			 * (RelUndoApplyChain releases them only on its normal CLR path).
			 * Release them here so a caught error cannot leak a held lock
			 * into the next iteration -- a subsequent entry that re-locks the
			 * same buffer would trip the BufferLockAcquire lockmode==UNLOCK
			 * assert.
			 */
...
AtAbort_XactUndo(void)
{
	...
	/*
	 * Roll back per-relation UNDO chains.  Queues each modified relation for
	 * the background per-relation UNDO worker; the aborting backend waits for
	 * completion in WaitForPendingRelUndo() after lock release.
	 */
	ApplyPerRelUndo();
