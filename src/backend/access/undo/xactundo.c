...
/*
 * Per-relation UNDO registration.  RECNO (and any other AM using the
 * per-relation UNDO fork) registers the head of its UNDO chain for each
 * relation it modified.  On abort, ApplyPerRelUndo() walks this list and
 * either queues synchronous worker rollback or records an ATM instant-abort
 * entry for asynchronous rollback.
 */
...
typedef struct XactUndoData
{
	 ...
	 * per-relation UNDO fork (RECNO).  Allocated in CurTransactionContext, so
...
AtAbort_XactUndo(void)
{
	...
	/*
	 * Roll back per-relation UNDO chains (RECNO).  Queues each modified
