 ...
 * core can clear AM transient tuple flags, reverse byte-diffs, and reclaim
 * retained before-images without compile-time knowledge of any AM's tuple
 * format.  They remain NULL when no in-place AM is registered.
 */
void		(*RelUndoClearTransientFlags_hook) (char *tuple_data) = NULL;
bool		(*RelUndoReverseDelta_hook) (const char *new_data, Size new_len,
										 const void *diff,
										 char *old_out, Size *old_len_out) = NULL;
...
RelUndoReserve(Relation rel, Size record_size, Buffer *undo_buffer)
{
	 ...
	 * exclude, so the non-atomic store on this path could straddle and clobber
	 * a concurrent CAS on the same hot head page, handing two reservers
	 * overlapping offsets and corrupting the resulting UNDO records.  Every
	 * remaining reserve path advances pd_lower only under the buffer lock, so
		...
		smgrnblocks(RelationGetSmgr(rel), RELUNDO_FORKNUM))
	{
		 ...
		 * this fork.  smgrtruncate broadcasts a smgr invalidation but does NOT
		 * touch any other backend's relundo_head_cache, so our cached
 ...
 * updates are in-place (RECNO) can correctly report near-zero dead tuples,
 ...
 *
 * This is the per-relation UNDO fork's equivalent of
 * SLogTupleMaybeCleanupRetained() (slog.c): a cheap, throttled backstop an
 * AM's DML path can call so fork discard runs on its own schedule,
 * decoupled from VACUUM ever being triggered.  Intended to be called from
 * RECNO's (or any AM's) DML hot path after releasing all buffer/tuple
 * locks -- RelUndoDiscard() takes the UNDO fork's own metapage lock, which
 * must never be acquired while holding a data page lock (that would convoy
 * every writer to a hot page behind the discard sweep).
 *
 * The cadence intentionally matches SLogTupleMaybeCleanupRetained(): a
 * per-backend static timestamp means each backend independently throttles
 * to once every 5 seconds, so concurrent backends do redundant but bounded
 * work rather than needing shared coordination.
 */
void
RelUndoMaybeVacuum(Relation rel)
{
	...
	/*
	 * Hot-path discard MUST NOT block: this runs inline from recno_tuple_update
	 * for every qualifying update.  Blocking on the per-relation UNDO metapage
	 * EXCLUSIVE lock here serializes ALL concurrent updates to the relation
	 * behind one lock (measured: the dominant cause of RECNO's TPROC-C
	 ...
	 * skipped -- some other backend (or a later call) reclaims the space; UNDO
	 * discard is space reclamation, never per-update correctness.
	 */
