...
flat_hash_apply_insert(SLogFlatHash * ht, const SLogFlatOp * op)
{
	 ...
	 */
	{
		 ...
		 * marker a concurrent reader or writer still needs.  Every marker that
		 * passes the horizon predicate is equally reclaimable, so we take the
		 * first one found.
		 *
		 ...
		 * reader's / in-progress writer's op could be freed (lost update / MVCC
		 * corruption).
		 *
