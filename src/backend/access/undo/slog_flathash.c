...
SLogFlatHashProbe(const SLogFlatHash * ht, const SLogTupleKey *key)
{
	 ...
	 * computation and bucket walk entirely.  This lookup sits on the RECNO
	 * per-tuple visibility/conflict hot path and the sLog (uncommitted-writer
	 * tracking) is empty for the vast majority of lookups in a committed-data
	 * OLTP workload (measured ~4%% of RECNO CPU under TPROC-C before this
	 * guard).  Callers read the hash under the partition seqlock, so a
	 * concurrent writer's insert (which bumps num_entries) is caught by the
	 * seqlock retry.
	 */
	if (ht->num_entries == 0)
		return NULL;
...
flat_hash_apply_insert(SLogFlatHash * ht, const SLogFlatOp * op)
{
	 ...
	 */
	{
		 ...
		 * oldest_idx == -1.  This is the fail-safe direction and is exactly what
		 * the caller relies on -- SLogTupleInsert passes InvalidTransactionId on
		 * the fast path (skipping the ProcArrayLock horizon scan) and only
		 * supplies a real, freshly-computed horizon on the full-array retry.  A
		 * missing horizon must never make markers look reclaimable, or a live
		 * reader's / in-progress writer's before-image could be freed (lost
		 * update / MVCC corruption).
		 *
		...
		for (i = 0; i < SLOG_MAX_TUPLE_OPS; i++)
		{
			...
			if (!TransactionIdIsValid(reclaim_xid_horizon) ||
				...
				continue;		/* invalid horizon disables reclaim (fail-safe);
								 * else still visible-relevant to an active
