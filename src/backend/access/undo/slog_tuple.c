...
static SLogAmDescriptor slog_am_desc = {
	.before_image_max = 0,		/* 0 = no AM registered / before-images disabled */
};
				...
				uint32 spec_token, LockTupleMode lock_mode)
{
	 ...
	 * yet committed in CLOG.  They are cleaned up by SLogTupleCleanupRetained(),
	 * driven by the UNDO background worker or, when it is disabled, by the
	 * access method calling SLogTupleMaybeCleanupRetained() outside any
	 * buffer-locked section.  This function never triggers that sweep itself
	 * (see note below).
	 *
						...
						SLogOpType op_type)
{
	 ...
	 * make the AM's visibility check trip on TransactionIdDidAbort()/IsInProgress()
...
SLogTupleCommitByXid(TransactionId xid)
{
	 ...
	 * chain (via the AM's version-reconstruction walk); the write-write
	 * conflict probe reads the head verptr on the on-page tuple and resolves
	 * it in the same fork.
	 *
...
SLogTupleRemoveBySubXid(TransactionId xid, TransactionId subxid)
{
	...
	for (tk = slog_tracked_keys; tk != NULL; tk = tk->next)
	{
		...
		/*
		 * Mark the backend-local tracked key as aborted so the AM's commit-time
		 * flag clearing skips it.  Without this, a local-only INSERT tracked key
		 * still reads as a live INSERT at top-level commit, and the AM's
		 * uncommitted-flag clearing would clear the tuple's UNCOMMITTED flag and
		 * stamp a commit marker -- resurrecting a tuple that the savepoint
...
SLogTupleMarkAborted(TransactionId xid)
{
	...
	for (part = 0; part < SLogNumPartitions; part++)
	{
		 ...
		 * counter at a boundary sees a valid partial state.
		 */
		ops_since_publish = 0;
...
SLogTupleRemoveByXidGlobal(TransactionId xid)
{
	 ...
	 * copy is stable while we scan it.  We apply REMOVE_XID inside one seqlock
	 * cycle.
	 */
	for (part = 0; part < SLogNumPartitions; part++)
	{
						  ...
						  RelFileLocator rlocator, char relpersistence)
{
	...
	for (tk = slog_tracked_keys; tk != NULL; tk = tk->next)
	{
		...
		/*
		 * Only the local per-backend before-image is stored -- consumed by the
		 * AM's intra-transaction savepoint rollback.  Cross-backend MVCC reads
		 * no longer need a shared before-image: WS-PVS3 walks the durable UNDO
		 * fork chain via the AM's version-reconstruction walk, and the
		 * write-write conflict probe resolves the head verptr in the same fork.
		 */
...
SLogTupleCleanupRetained(void)
{
	 ...
	 * runs and forward-path inserts) for the partition, so the single copy is
	 * stable to scan.  We apply the CLEANUP_RETAINED ops inside seqlock
	 * cycles.
	 */
	for (part = 0; part < SLogNumPartitions; part++)
	{
		...
		while ((bucket = SLogFlatHashScanNext(fp->hash, &scan)) != NULL)
		{
			...
			for (i = 0; i < SLOG_MAX_TUPLE_OPS; i++)
			{
				 ...
				 * xmin (xid authority -- see the function header).  This is the
				 * authoritative, self-healing gate: a below-horizon marker is
				 * reclaimable whether or not it was ever committed in CLOG, and
				 * an in-progress xid can never precede the horizon.
				 */
