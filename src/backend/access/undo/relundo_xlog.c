...
relundo_redo_init(XLogReaderState *record)
{
	 ...
	 * RelUndoInitRelation sets counter to 1 so that 0 is clearly
	 * "no counter / uninitialized".  Accept any small value as valid
	 * since the counter only increments from 1 and a freshly initialized
	 * metapage will have counter == 1.
	 */
	if (xlrec->counter > 1)
...
relundo_redo_insert(XLogReaderState *record)
{
	 ...
	 * inserting.  page_offset is page-absolute (MAXALIGN(SizeOfPageHeaderData)
	 * + contents offset) while new_pd_lower is relative to the page contents
	 * area, so compare them in the same coordinate system by adding the
	 * standard page-header size to the contents-relative pd_lower.
	 */
...
relundo_redo_truncate(XLogReaderState *record)
{
	 ...
	 * later in the WAL stream), and mirror smgr_redo's XLOG_SMGR_TRUNCATE arm:
	 * advance the minimum recovery point, then smgrtruncate (which drops the
	 * to-be-removed buffers via DropRelationBuffers).
	 */
