 ...
 *
 * RECNO is an in-place MVCC table access method: an UPDATE overwrites the
 * committed tuple bytes on the data page, and the only durable copy of the
 * prior committed version is the before-image stored in the relation's UNDO
 * fork.  WAL redo faithfully re-establishes the page state as it was at crash
 * time, including modifications made by transactions that never committed.
...
RelUndoRecoveryScanOneFork(RelFileLocator rloc)
{
	 ...
	 * 1, below the fork's block count) are valid chain links.  Block 0 is the
	 * metapage: a zeroed data page carries a prev_blkno of 0 -- not
	 * InvalidBlockNumber -- and BlockNumberIsValid(0) is true, so an unguarded
	 * walk would read the metapage as a data page, interpret its magic as the
	 * next block number, and fault.  Treat block 0 and any out-of-range block
	 * as the chain terminus.
	 *
	 * A corrupt prev_blkno could form a cycle.  Each chain has at most nblocks
	 * links, so cap the walk at nblocks iterations and fail loudly on overrun
	 * rather than spinning forever during recovery.
	 */
	for (int slot = 0; slot < RELUNDO_NUM_HEADS; slot++)
	{
