...
process_relundo_work_item(RelUndoWorkItem *item)
{
	 ...
	 * under high concurrency: the UNDO worker's exclusive lock request
	 * would queue behind active transactions, and all new transactions
	 * would queue behind the UNDO worker, causing a complete stall.
	 */
	PG_TRY();
	{
		 ...
		 */
		SLogTupleRemoveByXidGlobal(item->xid);
