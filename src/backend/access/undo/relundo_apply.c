...
RelUndoApplyChain(Relation rel, RelUndoRecPtr start_ptr)
{
	 ...
	 * every restored page plus the UNDO-fork page(s) (carrying the
	...
	while (RelUndoRecPtrIsValid(current_ptr))
	{
		 ...
		 * itself a single-page record (batch_blkno valid); a multi-page
		 * record leaves ntouched > 1 and ends the batch immediately.
		 */
		while (ntouched == 1 &&
			   ...
			   RelUndoRecPtrIsValid(current_ptr))
		{
			...
			/*
			 * The CLR will register one forced image per distinct fork page
			 * in the batch.  If folding this record would exceed the
			 * fork-page cap (and its fork page is not already in the batch),
			 * stop here.
			 */
		 ...
		 * release the data buffers.  For non-WAL relations there is nothing
		 * to log; just mark dirty and release.
		 */
		...
		case RELUNDO_UPDATE:
			{
				 ...
				 * updates the old and new tuple share the same slot, so
				 * marking it unused would destroy the just-restored old
				 * tuple.
				 */
				   ...
				   char *tuple_data, uint32 tuple_len)
{
	...
	if (ItemIdIsUsed(lp))
	{
		 ...
		 * whose DELETE is in place (it only flags the tuple deleted and
		 * leaves the full-length body on the page).  Restoring is an in-place
				   ...
				   char *tuple_data, uint32 tuple_len)
{
	...
	{
		/*
		 * Old tuple is larger than the new one.  Delete the current item and
		 * re-add the old tuple at the same offset.
		 */
		...
		if (restored_offset == InvalidOffsetNumber)
		{
			 ...
			 * enough free space since we just removed the (smaller) new
			 * tuple.
			 */
			restored_offset = PageAddItem(page, tuple_data,
				   ...
				   Buffer *touched, int ntouched)
{
	 ...
	 * consistent order (deadlock-free).  Data pages were already locked by
	 * the apply path before any fork page, so the global order is
	 * data-then-fork.
	 */
	for (i = 0; i < nptrs; i++)
	{
	...
	for (i = 0; i < nptrs; i++)
	{
		...
		rec_hdr->info_flags |= RELUNDO_INFO_CLR_APPLIED;
	}
	 ...
	 * before-image we just restored exists only in these (still dirty)
	 * buffers and in this CLR.  If we crash before the CLR is flushed, redo
	 * replays only the forward (aborted) page change and the rollback is
	 * silently lost.  Flushing here makes the compensation durable so crash
	 * redo reinstates the before-image from the forced full-page images.
	 * Batching amortizes this flush across every record folded into the CLR.
	 */
	XLogFlush(recptr);
