...
UndoRecordSetInsert(UndoRecordSet *uset)
{
	...
	/*
	 * Log the allocation in WAL for crash recovery.
	 * This ensures the UNDO log state can be reconstructed.
	 */
	XLogBeginInsert();
	...
	/*
	 * Update the record set's previous pointer chain.
	 * Each subsequent insertion will chain backward through this pointer.
	 */
	current_ptr = start_ptr;
	if (uset->nrecords > 1)
	{
		/*
		 * The last record in the set becomes the previous pointer
