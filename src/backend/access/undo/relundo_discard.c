...
RelUndoDiscard(Relation rel, TransactionId oldest_xmin, bool nowait)
{
	 ...
	 * is set (the inline hot-path caller, RelUndoMaybeVacuum), acquire the
	 * lock CONDITIONALLY: if another backend holds it, skip this discard
	 * entirely rather than block the update.  This keeps concurrent updates
	 * to the same relation from serializing behind the single per-relation
	 * UNDO metapage lock.  Space reclamation is best-effort; a skipped
	 * discard is retried on a later update.
	 */
	if (nowait)
	{
		/*
		 ...
		 * so block 0 is present and initialized.  If another backend holds
		 * the lock, skip this discard rather than block the update.
		 */
	 ...
	 * data block -- the contiguous physical suffix [1 ..
	 * system_alloc_watermark] -- so the fork can be physically truncated back
	 * to the metapage.  That is the only provably-safe truncate case, and we
	 * still hold the metapage exclusive lock (blocking any concurrent
	 * allocator).
	 */
	all_empty = true;
	for (int slot = 0; slot < RELUNDO_NUM_HEADS; slot++)
				   ...
				   TransactionId oldest_xmin)
{
	...
	/*
	 * Pass 1 (read-only): walk from head toward tail following prev_blkno. A
	 * page is discardable iff its max_xid precedes oldest_xmin.  The last
	 ...
	 *
	 * This relies on a precondition of the append-only fork: discardability
	 * is monotonic from tail (oldest) to head (newest).  Records are appended
	 * in commit order, so a page's max_xid never decreases as the chain
	 * advances head-ward; once a page is non-discardable (its max_xid reaches
	 * oldest_xmin) every newer page above it is non-discardable too.  Thus
	 * the discardable pages always form a single contiguous run at the tail,
	 * and keeping the closest-to-tail non-discardable page as the new tail
	 * never splices a still-live page onto the free list.
	 */
	...
	while (BlockNumberIsValid(current_blkno) && current_blkno != RELUNDO_METAPAGE_BLKNO)
	{
		 ...
		 * InvalidBlockNumber, never 0.  Guard against a 0 (or meta-block)
		 * link so a malformed/uninitialized chain head cannot self-deadlock
		 * by re-locking the metapage.
		 */
	 ...
	 * WAL-logged set of mutations.  Both the free list and the discardable
	 * run are threaded through the same durable prev_blkno fields (logged at
	 * insert time), so the run's internal links are left untouched and only
	 * its boundaries change:
	 *
	 * - the run's tail (old chain tail) gets prev_blkno = old_free_head,
	 * appending the prior free list after the run; - the new live tail (if
	 * any) gets prev_blkno = InvalidBlockNumber, detaching the live chain
	 * from the run; - the metapage's tail and free-list head are updated.
	 *
	...
	START_CRIT_SECTION();
	{
		 ...
		 * pd_lower at the empty value and track their real extent in the
		 * shadow RelUndoPageHeader.  REGBUF_STANDARD would elide the whole
		 * contents as a free "hole", so a restored FPI would return a zeroed
		 * page and lose the prev_blkno chain links.  Log the full page image
		 * (flag 0).
		 */
		 ...
		 * WAL record (WAL-before-data).  Otherwise a crash could leave a
		 * page's spliced prev_blkno link on disk without the matching WAL,
		 * corrupting the free-list / data-chain threading.
		 */
...
RelUndoTruncateEmptyChain(Relation rel, Buffer metabuf)
{
	...
	/*
	 * With the whole chain discarded, the free list holds every data block
	 * the metapage durably knows about -- the contiguous suffix [1 ..
	 * watermark]. The watermark is advanced in the same WAL-logged metapage
	 * mutation that threads a freshly extended block onto the chain, but
	 * ExtendBufferedRel grows the file before that record is flushed.  A
	 * crash in that window leaves orphaned tail blocks (watermark < nblocks -
	 * 1) that belong to no chain and no free list.  Truncating to the
	 * metapage drops them too, which is safe under the AccessExclusiveLock
	 * gate below since no reserver can hold a pin.  The watermark can never
	 * exceed the physical EOF, so assert only that bound.  It may also be
	 * invalid if the only blocks on disk are orphans from a torn extend whose
	 * metapage update never reached WAL.
	 */
