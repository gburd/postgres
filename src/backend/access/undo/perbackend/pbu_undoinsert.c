						   ...
						   UndoRecPtr xact_urp)
{
	...
	/*
	 * The log is gone (already discarded, or xact_urp is stale): nothing to
	 * update.  UndoLogGet() returns NULL rather than handing back an
	 * uninitialised UndoLogControl, so this must be checked before any
	 * dereference.
	 */
	if (log == NULL)
		return;
 ...
 *
 * Returns true if an entry was pushed onto xact_urec_info[] (so the caller
 * must follow up with UndoRecordUpdateTransInfo() to write it out), false if
 * there is nothing to update.  Callers MUST honour the false return: calling
 * UndoRecordUpdateTransInfo() anyway would dereference an xact_urec_info[]
 * slot we never filled in, which crashes on the stale/invalid log number
 * (seen as a segfault in LWLockAcquire(&log->discard_update_lock) when a temp
 * relation's undo is rolled back).
 */
bool
								...
								UndoRecPtr urecptr, int progress)
{
	...
	/*
	 * The log is gone (already discarded, or urecptr is stale): nothing to
	 * update.  Every other UndoLogGet() caller treats NULL this way.
	 */
	if (log == NULL)
		return false;

	/*
	 * Temporary undo logs are discarded on transaction end, so there is no
	 * transaction header worth updating.
	 */
	if (log->meta.persistence == UNDO_TEMP)
		return false;
...
UndoRecordUpdateTransInfo(int idx)
{
	...
	/*
	 * Nothing was prepared for this slot (the matching Prepare* call returned
	 * early, e.g. for a temp undo log), so there is nothing to write out.
	 * Guard here as well as at the call sites so a future caller that forgets
	 * to check gets a no-op instead of a wild pointer.
	 */
	if (idx >= xact_urec_info_idx ||
		!UndoRecPtrIsValid(xact_urec_info[idx].urecptr))
		return;
				  ...
				  XLogReaderState *xlog_record)
{
	...
	for (i = 0; i < buffer_idx; i++)
	{
		...
		if ((blk == undo_buffer[i].blk) &&
			(undo_buffer[i].logno == rnode.relNumber))
		{
			/* caller must hold exclusive lock on buffer */
			Assert(BufferIsLocal(undo_buffer[i].buf) ||
				   BufferIsLockedByMeInMode(undo_buffer[i].buf,
	...
	if (i == buffer_idx)
	{
		...
		{
			/*
			 * zheap's XLogReadBufferForRedoBlock() has no master equivalent
			 * (master's XLogReadBufferForRedo works by WAL block_id, but undo
			 * pages here are addressed by explicit (logno, blk), not by a
			 * registered block index).  Read the specific undo block with
			 * XLogReadBufferExtended() and derive the redo action the same way
			 * XLogReadBufferForRedoExtended() does, so undo redo is idempotent:
			 *
			 *   - invalid buffer               -> BLK_NOTFOUND (page discarded)
			 *   - page LSN already >= this WAL  -> BLK_DONE (already applied)
			 *   - otherwise                     -> BLK_NEEDS_REDO
			 *
			 * The BLK_DONE case is essential for crash-recovery correctness:
			 * without it a replayed undo insert would be applied twice to a
			 * page that a checkpoint (or an earlier replay) already advanced
			 * past this record's LSN.
			 */
			buffer = XLogReadBufferExtended(rnode, UndoLogForkNum, blk, rbm,
											InvalidBuffer);
			if (!BufferIsValid(buffer))
				action = BLK_NOTFOUND;
			else
			{
				/*
				 * XLogReadBufferExtended() returns the buffer PINNED but not
				 * locked (unlike XLogReadBufferForRedoExtended()).  The undo
				 * redo path (InsertPreparedUndo / UndoRecordUpdateTransInfo)
				 * writes into and MarkBufferDirty()s this buffer, which asserts
				 * an exclusive lock, so take it here -- matching the exclusive
				 * lock the non-recovery branch below acquires.
				 */
				if (rbm != RBM_ZERO_AND_LOCK)
					LockBuffer(buffer, BUFFER_LOCK_EXCLUSIVE);

				if (xlog_record != NULL &&
					xlog_record->EndRecPtr <= PageGetLSN(BufferGetPage(buffer)))
					action = BLK_DONE;
				else
		...
		else
		{
			...
			/* Lock the buffer (RBM_ZERO_AND_LOCK already locked it) */
			if (rbm != RBM_ZERO_AND_LOCK)
				LockBuffer(buffer, BUFFER_LOCK_EXCLUSIVE);
		}
		...
		else
		{
			...
			undo_buffer[buffer_idx].logno = rnode.relNumber;
				...
				RelFileNode *prevrec_rnode)
{
	...
	if (BufferIsValid(urec->uur_buffer))
	{
		...
		if ((UndoRecPtrGetBlockNum(urp) !=
			...
			(prevrec_rnode->relNumber != rnode->relNumber))
		{
...
UndoGetPrevRecordLen(UndoRecPtr urp, Buffer *input_buffer)
{
	...
	{
		/*
		 * Read the next prevlen byte from the current page while one is left
		 * above this page's block header; otherwise move to the previous page.
		 *
		 * The bound is tested BEFORE the decrement because page_offset is
		 * unsigned.  A page_offset of 0 is a legitimate input: it addresses the
		 * position just past the last data byte of the preceding page, which is
		 * what a caller gets by stepping an insert pointer back over the block
		 * header after a record ended flush against the page boundary.
		 * Decrementing first would wrap 0 around to SIZE_MAX, satisfy the bound
		 * and read a garbage length from outside the page.
		 */
	 ...
	 *
	 * page_span is how far urp sits into its page, i.e. how many bytes of the
	 * preceding record are addressable there.  For a page_offset of 0 -- urp
	 * addressing the position just past the preceding page's last data byte --
	 * that record had the whole preceding page's data area available, so the
	 * span to compare against is BLCKSZ.  Computing the span as page_offset -
	 * UndoLogBlockHeaderSize would wrap around on unsigned arithmetic and
	 * misreport a split record as unsplit.
	 */
	{
		UndoLogOffset page_span = UndoRecPtrGetPageOffset(urp);

		if (page_span == 0)
			page_span = BLCKSZ;

		if (page_span - UndoLogBlockHeaderSize < prev_rec_len)
...
/*
 * RegisterUndoLogBuffersWithData - Register the undo buffers and attach
 * caller-supplied bytes as block data on the FIRST of them.
 *
 * Used by producers whose redo handler re-runs the undo insert and therefore
 * needs the undo record's own description to travel with the buffers (see
 * flux_pbu.c).  Attaching the bytes to an undo block rather than to the main
 * data channel makes the region self-locating on the redo side: it is found via
 * XLogRecGetBlockData() on whichever block id the undo pages landed at, so the
 * producer's per-opcode main-data layout is unaffected.
 *
 * REGBUF_KEEP_DATA is essential: without it the block data is dropped whenever
 * the page also carries a full-page image (needs_data = !needs_backup in
 * XLogRecordAssemble), and redo would then be unable to replay the insert --
 * leaving the undo log's insert pointer un-advanced and every later record in
 * that log misplaced.
 *
 * Returns the number of undo buffers registered.
 */
int
RegisterUndoLogBuffersWithData(uint8 first_block_id,
							   const char *data, uint32 datalen)
{
	...
	for (idx = 0; idx < buffer_idx; idx++)
	{
		flags = REGBUF_KEEP_DATA;
		if (undo_buffer[idx].zero)
			flags |= REGBUF_WILL_INIT;
		XLogRegisterBuffer(first_block_id + idx, undo_buffer[idx].buf, flags);
	}

	if (buffer_idx > 0 && data != NULL && datalen > 0)
		XLogRegisterBufData(first_block_id, data, datalen);

	return buffer_idx;
}

/*
 * RegisterUndoLogBuffersForImage - Register the undo buffers forcing a
 * full-page image of each into the WAL record.
 *
 * Used by producers (e.g. FILEOPS) that do NOT re-run the undo-insert during
 * their own redo handler.  Forcing a full-page image makes the undo page
 * content self-contained in the producer's WAL record, so a plain
 * XLogReadBufferForRedo() in the producer's redo restores the exact undo-page
 * bytes -- no undo-insert replay logic is needed on the redo side.  Zeroed
 * (freshly initialized) pages are also forced-imaged (REGBUF_WILL_INIT alone
 * would omit the image and require redo-side reconstruction).
 */
void
RegisterUndoLogBuffersForImage(uint8 first_block_id)
{
	int			idx;

	/*
	 * Force a full-page image unconditionally.  REGBUF_WILL_INIT is
	 * deliberately NOT set even for freshly-zeroed pages: WILL_INIT means "no
	 * image, redo rebuilds the page" and is mutually exclusive with
	 * REGBUF_FORCE_IMAGE (see the assert in XLogRegisterBuffer).  We want the
	 * image so the producer's redo can restore the exact undo-page bytes
	 * without re-running the undo insert.
	 */
	for (idx = 0; idx < buffer_idx; idx++)
		XLogRegisterBuffer(first_block_id + idx, undo_buffer[idx].buf,
						   REGBUF_FORCE_IMAGE);
}
