...
DiscardWorkerMain(Datum main_arg)
{
	...
	/*
	 * Make it easy to identify our processes.  This must run AFTER
	 * BackgroundWorkerInitializeConnection(): SetConfigOption(PGC_S_SESSION)
	 * reads CurrentUserId (via SetSessionAuthorization bookkeeping), which is
	 * only valid once the connection has established the session user.  Doing
	 * it earlier trips Assert(OidIsValid(CurrentUserId)).
	 */
	...
	while (!got_SIGTERM)
	{
		...
		oldestXidHavingUndo = GetXidFromEpochXid(
												 pg_atomic_read_u64(UndoOldestXidHavingUndo_pbu));
