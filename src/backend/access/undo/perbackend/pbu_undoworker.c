...
/*
 * Gate for the per-backend UNDO background workers.  The per-backend and
 * per-relation UNDO engines coexist; no access method produces per-backend
 * UNDO yet, so the discard/apply workers default off and are only registered
 * when this is turned on (PGC_POSTMASTER).
 */
bool		pbu_undo_workers_enabled = false;
...
UndoWorkerLaunch(UndoRequestInfo urinfo)
{
	...
	snprintf(bgw.bgw_function_name, BGW_MAXLEN, "PbuUndoWorkerMain");
 ...
 */
void
UndoLauncherShmemRequest(void)
{
	/*
	 * Reserve the launcher shmem in the request phase so the framework sizes
	 * the segment for it and sets UndoApplyCtx before UndoLauncherShmemInit()
	 * runs (mirroring the sLog subsystem's pattern).
	 */
	ShmemRequestStruct(.name = "Undo Worker Launcher Data",
					   .size = UndoLauncherShmemSize(),
					   .ptr = (void **) &UndoApplyCtx);
}

void
UndoLauncherShmemInit(void)
{
	/* Framework has allocated the region and set UndoApplyCtx. */
 ...
 *		Make sure an undo worker launcher will run, registering one dynamically
 *		if the static registration was skipped.
 *
 * Called by the crash-recovery undo driver (PbuPerformUndoRecovery) when it has
 * queued rollback requests that it cannot apply itself.  Those requests are
 * drained by an undo apply worker, which only exists if the launcher runs; with
 * pbu_undo_workers_enabled off, UndoLauncherRegister() was never called at
 * postmaster start, so the queued requests would sit there and the loser
 * transactions would never be rolled back.
 *
 * A dynamic registration is legal from the startup process (which is a
 * postmaster child) and is honoured at the right time: bgw_start_time is
 * BgWorkerStart_RecoveryFinished, so the postmaster holds the worker back until
 * pmState reaches PM_RUN -- i.e. until the system is open for writes, which is
 * the earliest point at which undo apply may legally run (it needs relcache,
 * syscache and the index AMs).
 *
 * Idempotent, and a no-op when the static registration already happened, so
 * there is never more than one launcher.
 */
void
PbuEnsureUndoLauncher(void)
{
	static bool launcher_requested = false;
	BackgroundWorker bgw;

	if (launcher_requested || pbu_undo_workers_enabled)
		return;
	if (max_undo_workers == 0)
	{
		ereport(WARNING,
				(errmsg("per-backend UNDO: max_undo_workers is 0, so recovered "
						"rollback requests cannot be processed"),
				 errhint("Raise max_undo_workers and restart to complete the "
						 "rollback of transactions that crashed in flight.")));
		return;
	}
	if (!IsUnderPostmaster)
		return;					/* single-user mode: no worker is possible */
	...
	snprintf(bgw.bgw_library_name, BGW_MAXLEN, "postgres");
	snprintf(bgw.bgw_function_name, BGW_MAXLEN, "UndoLauncherMain");
	snprintf(bgw.bgw_name, BGW_MAXLEN, "undo worker launcher");
	snprintf(bgw.bgw_type, BGW_MAXLEN, "undo worker launcher");
	bgw.bgw_restart_time = 5;
	bgw.bgw_notify_pid = 0;
	bgw.bgw_main_arg = (Datum) 0;
	...
	if (!RegisterDynamicBackgroundWorker(&bgw, NULL))
		ereport(WARNING,
				(errmsg("per-backend UNDO: could not register an undo worker "
						"launcher to complete recovered rollback requests")));
	else
		ereport(LOG,
				(errmsg("per-backend UNDO: started an undo worker launcher to "
						"complete rollback requests recovered from the crash")));
...
/*
 * PbuUndoWorkerMain -- Main loop for the undo apply worker.
 */
pg_noreturn void
PbuUndoWorkerMain(Datum main_arg)
{
	UndoRequestInfo urinfo;
