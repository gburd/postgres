...
/*
 * Gate for the per-backend UNDO background workers.  FLUX and RECNO both
 * produce per-backend UNDO, but their rollback is applied synchronously by the
 * aborting backend, so the discard/apply workers remain opt-in
 * (PGC_POSTMASTER) rather than on by default.
 */
