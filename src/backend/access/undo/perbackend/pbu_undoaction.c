					 ...
					 UndoRecPtr to_urecptr, bool nopartial)
{
	...
	if (nopartial)
	{
		 ...
		 * transaction header.
		 *
		 * If nothing was prepared there is no transaction header to mark: the
		 * undo log is temporary (discarded at transaction end) or already
		 * gone.  In that case skip the update and its CLR entirely -- temp
		 * undo is not WAL-logged, and UndoRecordUpdateTransInfo() would be
		 * reading an xact_urec_info[] slot that was never filled in.
		 */
		if (PrepareUpdateUndoActionProgress(NULL, to_urecptr, 1))
		{
			START_CRIT_SECTION();
...
PbuAtAbort_ApplyUndo(void)
{
	...
	/*
	 * Release any buffer content locks this transaction still holds before
	 * applying UNDO.
	 *
	 * A statement that errors out mid-update can unwind while still holding a
	 * buffer content lock -- e.g. flux_tuple_update_inplace() calls
	 * CheckForSerializableConflictIn() under the tuple's exclusive buffer lock,
	 * and that ereports on an SSI failure.  Resource-owner cleanup only
	 * releases those locks later, in ResourceOwnerRelease(RESOURCE_RELEASE_
	 * LOCKS), which runs AFTER this abort-time UNDO application.  The UNDO
	 * appliers re-lock the very same buffer (flux_undo_apply ->
	 * BufferLockAcquire), so without this the backend deadlocks against its own
	 * held lock and the abort never completes (observed as a permanently hung
	 * backend after "could not serialize access ..." in the total-cash
	 * isolation test).
	 *
	 * This mirrors what the cluster-wide engine already does around inline UNDO
	 * application in xactundo.c.  We are in an error-recovery path, so dropping
	 * content locks here is safe -- no live statement can still be relying on
	 * them.
	 */
