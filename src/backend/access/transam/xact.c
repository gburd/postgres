...
/*
 * FILEOPS-style pending-structural-file-operation hooks, mirroring smgr's
 * own pending-deletes housekeeping ordering (see access/xact.h).  NULL when
 * no such subsystem is compiled in.
 */
void		(*FileOpsDoPendingOps_hook) (bool isCommit) = NULL;
void		(*FileOpsPostPrepare_hook) (void) = NULL;
void		(*FileOpsAtSubCommit_hook) (void) = NULL;
void		(*FileOpsAtSubAbort_hook) (void) = NULL;
 ...
 * (e.g. recno_undo_apply) need to open relations, which trips
...
PrepareTransaction(void)
{
	 ...
	 * UNDO -- cluster-wide (FILEOPS filesystem operations, via
	 * InsertUndoRecord) or per-relation (RECNO and other table access
	 * methods, via the per-relation UNDO fork).  Neither has a working
	 * ROLLBACK PREPARED apply path yet: per-relation UNDO's before-images
	 * live in backend-private, CurTransactionContext-allocated state
	 * (XactUndo.relundo_list) that is gone the moment this function
	 * returns; cluster-wide UNDO's chain-head LSN is durably saved in the
	 * 2PC state file (xl_xact_prepare.last_batch_lsn) but nothing on the
	 * COMMIT PREPARED / ROLLBACK PREPARED path or crash-recovery path ever
	 * reads it back or calls into the UNDO apply machinery (confirmed:
	 * twophase.c's RecordTransactionCommitPrepared/
	 * RecordTransactionAbortPrepared never call AtAbort_XactUndo() or
	 * ATMAddAborted(), the only two entry points that trigger UNDO
	 * application).  Silently proceeding would make ROLLBACK PREPARED a
	 * no-op for either kind of UNDO, corrupting data.  Reject early,
	 * before StartPrepare() writes any 2PC state.
	 */
	if (XactUndoHasUnrecoverableUndo())
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				 errmsg("cannot PREPARE a transaction that generated UNDO (e.g. modified a RECNO table, or performed a transactional filesystem operation)"),
...
AbortTransaction(void)
{
	...
	if (TopTransactionResourceOwner != NULL)
	{
		...
		/*
		 * Wait for any pending synchronous per-relation UNDO worker to finish.
		 * Done after lock release so the worker can acquire its own lock on the
		 * target relation, making per-relation rollback synchronous from the
