...
PrepareTransaction(void)
{
	 ...
	 * cluster-wide UNDO (FILEOPS, nbtree/hash) durably saves its chain-head
	 * LSN in xl_xact_prepare and pins its WAL while prepared, and per-relation
	 * UNDO (RECNO) is serialized by AtPrepare_Recno() (per-tuple sLog state
	 * plus RECNO_2PC_RELUNDO chain heads) and replayed by
	 * recno_twophase_postabort() on ROLLBACK PREPARED.  So
	 * XactUndoHasUnrecoverableUndo() returns false today; the call is kept as
	 * a single choke point where any future non-2PC-safe UNDO mechanism can
	 * re-assert a guard.
	 */
