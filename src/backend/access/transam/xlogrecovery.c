...
PerformWalRecovery(void)
{
	...
	if (record != NULL)
	{
		...
		/*
		 * Reverse-apply per-relation UNDO (RECNO in-place before-images) for
		 * loser transactions.  Like PerformUndoRecovery() above, this runs
		 * before WAL insertion is enabled, so it writes no WAL; durability is
		 * provided by the end-of-recovery checkpoint.
		 *
		 * Unlike the cluster-wide UNDO phase, this is driven by an end-of-redo
		 * scan of the UNDO forks on disk rather than redo-time tracking: a
		 * CHECKPOINT taken after an uncommitted in-place UPDATE advances the
		 * redo start past that UPDATE's WAL, so nothing is replayed for it even
		 * though its uncommitted value is durable.
		 *
		 * This block is reached only once redo has finished -- at crash
		 * recovery end or at standby promotion.  A streaming hot standby never
		 * arrives here.  At promotion StandbyMode is still set (it is cleared
		 * later in FinishWalRecovery), so gating on !StandbyMode would skip the
		 * scan for exactly the promoted-standby case that needs loser rollback.
		 * Run it unconditionally, matching PerformUndoRecovery() above.
		 */
