...
PerformWalRecovery(void)
{
	...
	if (record != NULL)
	{
		 ...
		 *
		 * This phase is only needed when enable_undo is on.  We check
		 * UndoRecoveryNeeded() to avoid overhead when no UNDO records
		 * were present in the WAL stream.
		 */
		if (enable_undo && UndoRecoveryNeeded())
		{
			ereport(LOG,
