...
StartupXLOG(void)
{
	...
	/*
	 * Rebuild the per-backend UNDO engine's UndoLogControl banks and freelists
	 * from the pg_undo/<redo> snapshot written by CheckPointUndoLogs() at the
	 * checkpoint we are starting recovery from.  This must precede WAL replay:
	 * redo of RM_UNDOLOG_ID records (META/ATTACH/EXTEND/DISCARD) advances the
	 * in-memory log metadata seeded here, and redo of registered undo pages
	 * reads/writes those logs' backing segments.  On a cluster that has never
	 * persisted per-backend undo (no snapshot file for this redo), this is a
	 * tolerated no-op (StartupUndoLogs treats a missing snapshot as "no active
	 * logs").
	 */
...
CheckPointGuts(XLogRecPtr checkPointRedo, int flags)
{
	 ...
	 * themselves are WAL-logged (attached to producer WAL records) and flushed
	 * by CheckPointBuffers() below; here we record the table of undo logs and
	 * their insert/discard pointers.  priorCheckPointRedo is the previous
	 * checkpoint's redo point (ControlFile->checkPointCopy.redo is not yet
	 * advanced to the new checkpoint at this stage of CreateCheckPoint), used
	 * to clean up the now-obsolete snapshot file.
	 */
...
KeepLogSeg(XLogRecPtr recptr, XLogSegNo *logSegNo)
{
	...
	/*
	 * If UNDO-in-WAL is active, retain WAL segments that contain UNDO records
	 * still needed for rollback of in-progress transactions.
	 *
