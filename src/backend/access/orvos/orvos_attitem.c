					  ...
					  bool has_nulls, int datasz)
{
	...
	if (has_nulls)
		itemsz += OVBT_ATTR_BITMAPLEN(num_elements);
	itemsz += datasz;
						   ...
						   ovtid *removetids)
{
	...
	for (i = 0; i < origitem->t_num_elements; i++)
	{
		...
		if (!this_isnull)
			this_datasz = ovbt_attr_datasize(attr->attlen, src);
		else
			this_datasz = 0;
...
ovbt_attr_explode_item(OVAttributeArrayItem * item)
{
	...
	/* nulls */
	if ((item->t_flags & OVBT_HAS_NULLS) != 0)
	{
