 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/orvos/orvos_attpage.c
 */
#include "postgres.h"
...
static void ovbt_attr_repack_replace(Relation rel, AttrNumber attno,
									 Buffer oldbuf, List *items);
static void ovbt_attr_add_items(Relation rel, AttrNumber attno, Buffer buf,
								List *newitems);
 ...
 */
void
ovbt_attr_begin_scan(Relation rel, TupleDesc tdesc, AttrNumber attno,
					 OVAttrTreeScan * scan)
{
	scan->rel = rel;
	scan->attno = attno;
	...
	scan->array_tids = MemoryContextAlloc(scan->context, sizeof(ovtid));
...
void
ovbt_attr_end_scan(OVAttrTreeScan * scan)
{
	if (!scan->active)
		return;
 ...
 *
 * This is normally not used directly. Use the ovbt_attr_fetch() wrapper,
 * instead.
 */
bool
ovbt_attr_scan_fetch_array(OVAttrTreeScan * scan, ovtid nexttid)
{
	if (!scan->active)
		return InvalidOVTid;
	...
	for (;;)
	{
		 ...
		 */
		buf = ovbt_find_and_lock_leaf_containing_tid(scan->rel, scan->attno,
		...
		for (off = FirstOffsetNumber; off <= maxoff; off++)
		{
			 ...
			 *
			 * NOTE: ovbt_attr_item_extract() always makes a copy of the data,
 ...
 */
void
ovbt_attr_multi_insert(Relation rel, AttrNumber attno,
					   Datum *datums, bool *isnulls, ovtid *tids, int nitems)
{
	...
	newitems = ovbt_attr_create_items(attr, datums, isnulls, tids, nitems);

	buf = ovbt_descend(rel, attno, insert_target_key, 0, false, InvalidBuffer, InvalidBuffer);
	 ...
	 * ovbt_attr_add_items() would not handle that correctly.
	 */

	/* recompress and possibly split the page */
	ovbt_attr_add_items(rel, attno, buf, newitems);

	/* ovbt_attr_add_items unlocked 'buf' */
	ReleaseBuffer(buf);
}
 ...
 */
void
ovbt_attr_remove(Relation rel, AttrNumber attno, IntegerSet *tids)
{
	Form_pg_attribute attr;
	Buffer		buf;
	Page		page;
	...
	if (!intset_iterate_next(tids, &nexttid))
		nexttid = InvalidOVTid;

	while (nexttid < MaxPlusOneOVTid)
	{
		buf = ovbt_descend(rel, attno, nexttid, 0, false, InvalidBuffer, InvalidBuffer);
		page = BufferGetPage(buf);
		...
		for (;;)
		{
			...
			item = (OVAttributeArrayItem *) PageGetItem(page, iid);
			off++;
			 ...
			 * the target tids.
			 */
			tids_arr = palloc((item->t_num_elements + 1) * sizeof(ovtid));
			num_to_remove = 0;
			...
			tids_arr[num_to_remove++] = MaxPlusOneOVTid;
			newitem = ovbt_attr_remove_from_item(attr, item, tids_arr);
			pfree(tids_arr);
			if (newitem)
		...
		if (newitems)
		{
			ovbt_attr_repack_replace(rel, attno, buf, newitems);
		}
		else
		{
			ov_split_stack *stack;

			stack = ovbt_unlink_page(rel, attno, buf, 0);
		...
		ReleaseBuffer(buf);		/* ovbt_apply_split_changes unlocked 'buf' */
 ...
 */
static void
ovbt_attr_add_items(Relation rel, AttrNumber attno, Buffer buf, List *newitems)
{
	Form_pg_attribute attr;
	...
	{
		ItemId		iid;
		OVAttributeArrayItem *lastitem;

		iid = PageGetItemId(page, maxoff);
		lastitem = (OVAttributeArrayItem *) PageGetItem(page, iid);
	 ...
	 */
	if (((OVAttributeArrayItem *) lfirst(nextnewlc))->t_firsttid >= last_existing_tid)
	{
		growth = 0;
		foreach(lc, newitems)
		{
			OVAttributeArrayItem *item = (OVAttributeArrayItem *) lfirst(lc);
		...
		if (growth <= PageGetExactFreeSpace(page))
		{
			...
			if (RelationNeedsWAL(rel))
				ovbt_wal_log_leaf_items(rel, attno, buf, startoff, false, newitems, NULL);
	...
	if (off <= maxoff)
	{
		...
		olditem = (OVAttributeArrayItem *) PageGetItem(page, iid);
		off++;
	}
	else
		olditem = NULL;
	...
	for (;;)
	{
		...
		{
			OVExplodedItem *left_newitem;
			OVExplodedItem *right_newitem;
			 ...
			 * NNNNNnnnn OOOOOOOOO
			 */
			ovbt_split_item(attr, (OVExplodedItem *) newitem, olditem->t_firsttid,
							&left_newitem, &right_newitem);
			items = lappend(items, left_newitem);
			newitem = (OVAttributeArrayItem *) right_newitem;
			continue;
		}
		...
		if (olditem->t_firsttid < newitem->t_firsttid)
		{
			OVExplodedItem *left_olditem;
			OVExplodedItem *right_olditem;
			 ...
			 * OOOOOoooo NNNNNNNNN
			 */
			ovbt_split_item(attr, (OVExplodedItem *) olditem, newitem->t_firsttid,
							&left_olditem, &right_olditem);
			items = lappend(items, left_olditem);
			olditem = (OVAttributeArrayItem *) right_olditem;
			continue;
		}
	 ...
	 * new data. ovbt_attr_repack_replace() takes care of storing them on the
	 * page, splitting the page if needed.
	 */
	ovbt_attr_repack_replace(rel, attno, buf, items);
...
typedef struct
{
	...
	AttrNumber	attno;
	ovtid		hikey;
}			ovbt_attr_repack_context;

static void
ovbt_attr_repack_newpage(ovbt_attr_repack_context * cxt, ovtid nexttid, int flags)
{
	Page		newpage;
 ...
 *
 * First, calls ovbt_attr_recompress_items(), which will try to combine
 ...
 *
 * The items may contain "exploded" items, as OVExplodedItem. They will
 ...
 */
static void
ovbt_attr_repack_replace(Relation rel, AttrNumber attno, Buffer oldbuf, List *items)
{
	Form_pg_attribute attr = TupleDescAttr(rel->rd_att, attno - 1);
	ListCell   *lc;
	ovbt_attr_repack_context cxt;
	 ...
	 * chunks.
	 */
	recompressed_items = ovbt_attr_recompress_items(attr, items);
	...
	ovbt_attr_repack_newpage(&cxt, oldopaque->ov_lokey, (oldopaque->ov_flags & OVBT_ROOT));
	...
	foreach(lc, recompressed_items)
	{
		...
		if (PageGetFreeSpace(cxt.currpage) < MAXALIGN(item->t_size))
			ovbt_attr_repack_newpage(&cxt, item->t_firsttid, 0);
	...
	if (cxt.stack_head->next)
	{
		...
		if ((oldopaque->ov_flags & OVBT_ROOT) != 0)
		{
			...
			cxt.stack_tail->next = ovbt_newroot(rel, attno, oldopaque->ov_level + 1, downlinks);
		...
		{
			cxt.stack_tail->next = ovbt_insert_downlinks(rel, attno,
	...
	ov_apply_split_changes(rel, cxt.stack_head, NULL);
}
