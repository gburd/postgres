/*
 * orvos_visibility.c
 *		Routines for MVCC in Orvos
 *
 * Portions Copyright (c) 1996-2026, PostgreSQL Global Development Group
 * Portions Copyright (c) 1994, Regents of the University of California
 *
 * IDENTIFICATION
 *	  src/backend/access/orvos/orvos_visibility.c
 */
#include "postgres.h"
...
static bool
ov_tuplelock_compatible(LockTupleMode mode, LockTupleMode newmode)
{
	switch (newmode)
	{
...
static bool
am_i_holding_lock(Relation rel, OVUndoRecPtr undo_ptr,
				  OVUndoRecPtr recent_oldest_undo)
{
	...
	for (;;)
	{
		...
		else if (undorec->type == OVUNDO_TYPE_UPDATE)
		{
			if (TransactionIdIsCurrentTransactionId(undorec->xid))
				return true;
		}
 ...
 * check if the tuple lock is compatible with the update. ov_SatisfiesUpdate
 ...
 */
TM_Result
ov_SatisfiesUpdate(Relation rel, Snapshot snapshot,
				   OVUndoRecPtr recent_oldest_undo,
				   ovtid item_tid, OVUndoRecPtr item_undoptr,
				   LockTupleMode mode,
				   bool *undo_record_needed, bool *this_xact_has_lock,
				   TM_FailureData *tmfd,
				   ovtid *next_tid, OVUndoSlotVisibility *visi_info)
{
...
retry_fetch:
	...
	{
		OVUndoRec_TupleLock *lock_undorec = (OVUndoRec_TupleLock *) undorec;
	...
	{
		OVUndoRec_Delete *deleterec = (OVUndoRec_Delete *) undorec;
	...
	else if (undorec->type == OVUNDO_TYPE_UPDATE)
	{
		...
		*next_tid = updaterec->newtid;
		...
		{
			*this_xact_has_lock = true;
			if (ov_tuplelock_compatible(old_lockmode, mode))
				return TM_Ok;
		...
		tmfd->ctid = ItemPointerFromOVTid(((OVUndoRec_Update *) undorec)->newtid);
		tmfd->xmax = undorec->xid;
		tmfd->cmax = InvalidCommandId;
		return TM_Updated;
	}
 ...
 */
static bool
ov_SatisfiesAny(OVTidTreeScan * scan, OVUndoRecPtr item_undoptr, OVUndoSlotVisibility *visi_info)
{
...
/*
 * helper function to ov_SatisfiesMVCC(), to check if the given XID
 ...
 */
static bool
ov_SatisfiesMVCC(OVTidTreeScan * scan, OVUndoRecPtr item_undoptr,
...
fetch_undo_record:
	...
	else if (undorec->type == OVUNDO_TYPE_TUPLE_LOCK)
	{
		...
		goto fetch_undo_record;
			 ...
			 undorec->type == OVUNDO_TYPE_UPDATE)
	{
		if (undorec->type == OVUNDO_TYPE_UPDATE)
		{
			OVUndoRec_Update *updaterec = (OVUndoRec_Update *) undorec;
 ...
 */
static bool
ov_SatisfiesSelf(OVTidTreeScan * scan, OVUndoRecPtr item_undoptr,
				 ovtid *next_tid, OVUndoSlotVisibility *visi_info)
{
	Relation	rel = scan->rel;
	OVUndoRec  *undorec;
 ...
 */
static bool
ov_SatisfiesDirty(OVTidTreeScan * scan, OVUndoRecPtr item_undoptr,
				  ovtid *next_tid, OVUndoSlotVisibility *visi_info)
{
	Relation	rel = scan->rel;
	Snapshot	snapshot = scan->snapshot;
...
fetch_undo_record:
	...
	if (OVUNDO_TYPE_IS_INSERT(undorec->type))
	{
		OVUndoRec_Insert *insertrec = (OVUndoRec_Insert *) undorec;
 ...
 */
static bool
ov_SatisfiesNonVacuumable(OVTidTreeScan * scan, OVUndoRecPtr item_undoptr,
						  OVUndoSlotVisibility *visi_info)
{
	Relation	rel = scan->rel;
...
fetch_undo_record:
			 ...
			 undorec->type == OVUNDO_TYPE_UPDATE)
	{
		...
		else if (TransactionIdDidCommit(undorec->xid))
		{
			...
			{
				visi_info->nonvacuumable_status = OVNV_RECENTLY_DEAD;
				return true;
			}
		...
		do
		{
			...
			if (prevptr.counter < scan->recent_oldest_undo.counter)
				return true;
			undorec = ovundo_fetch_record(rel, prevptr);
			if (!undorec)
			{
				scan->recent_oldest_undo = ovundo_get_oldest_undo_ptr(rel);
				if (undo_ptr.counter >= scan->recent_oldest_undo.counter)
		...
		Assert(OVUNDO_TYPE_IS_INSERT(undorec->type));
 ...
 */
bool
ov_SatisfiesVisibility(OVTidTreeScan * scan, OVUndoRecPtr item_undoptr,
					   TransactionId *obsoleting_xid, ovtid *next_tid,
					   OVUndoSlotVisibility *visi_info)
{
	OVUndoRecPtr undo_ptr;
	...
	*obsoleting_xid = InvalidTransactionId;
	...
	{
		case SNAPSHOT_MVCC:
			return ov_SatisfiesMVCC(scan, item_undoptr, obsoleting_xid, next_tid, visi_info);

		case SNAPSHOT_SELF:
			return ov_SatisfiesSelf(scan, item_undoptr, next_tid, visi_info);
