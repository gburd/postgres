...
/*
 * hash_undo_write_perbackend - Write one hash index-UNDO record into the
 * per-backend UNDO engine, chaining off the current transaction's per-backend
 * UNDO location.  PERBACKEND arm of the engine-selectable write path.
 *
 * Builds the SAME payload as the UNDO-in-WAL (design (a)) arm and tags it with
 * uur_rmid = UNDO_RMID_HASH / uur_type = subtype, so abort dispatch
 * (execute_undo_actions -> GetUndoRmgr -> rm_undo) reaches the SAME
 * hash_undo_apply() callback.
 *
 * This path is unreachable at runtime until a per-backend table AM drives it
 * (see the equivalent note in access/nbtree/nbtree_undo.c,
 * nbtree_undo_write_perbackend).  The record shape, rmid/subtype tagging and
 * chain routing are correct; the full PrepareUndoInsert/InsertPreparedUndo WAL
 * choreography and the pbu dispatch subtype reconciliation remain to be wired.
 */
static void
hash_undo_write_perbackend(Relation heapRel, uint8 subtype,
						   const char *hdr, Size hdr_len)
{
	/*
	 * See nbtree_undo_write_perbackend() in access/nbtree/nbtree_undo.c: the
	 * per-backend engine headers clash with the fork engine's undorecord.h
	 * included here, so the UnpackedUndoRecord construction +
	 * PrepareUndoInsert/InsertPreparedUndo drive is deferred to Phase 9/10
	 * (dormant until a per-backend table AM exists).  Contract for the
	 * Phase 9/10 shim:
	 *   uur_rmid = UNDO_RMID_HASH, uur_type = subtype,
	 *   uur_reloid = RelationGetRelid(heapRel), payload = hdr[0..hdr_len),
	 * then SetCurrentTransactionPbuUndoLocation(urp).
	 */
	(void) heapRel;
	(void) subtype;
	(void) hdr;
				  ...
				  OffsetNumber offset)
{
	...
	{
		hash_undo_write_perbackend(heapRel, HASH_UNDO_INSERT,
