 ...
 *
 * recno_pvs.c
 *	  RECNO per-relation versioned storage (PVS) read path
 *
 * In-place UPDATEs on a RECNO tuple stamp a trailing RelUndoRecPtr (verptr)
 ...
 *
 * RecnoReconstructVisibleVersion walks that chain on behalf of an MVCC
 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/recno/recno_pvs.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
							   ...
							   char **out_data, int *out_len)
{
	...
	for (;;)
	{
		...
		verptr = RecnoTupleGetVersionPtr(hdr, candidate_len);
		...
		switch (urec_hdr.urec_type)
		{
			case RELUNDO_DELTA_UPDATE:
				{
					const RelUndoDeltaUpdatePayload *du;
					const RecnoDiffRecord *diff;

					if (payload_size < SizeOfRelUndoDeltaUpdatePayload)
					{
						elog(WARNING,
					...
					if (!RecnoApplyDiffReverse(candidate, candidate_len,
											   diff, next_image, &next_len))
					{
						elog(WARNING,
							 "RECNO PVS: reverse-diff failed at %llu",
							 (unsigned long long) verptr);
						pfree(next_image);
						next_image = NULL;
						break;
			...
			case RELUNDO_UPDATE:
				{
					...
					{
						elog(WARNING,
							 "RECNO PVS: RELUNDO_UPDATE without tuple at %llu",
							 (unsigned long long) verptr);
						break;
					}
					...
					{
						elog(WARNING,
							 "RECNO PVS: RELUNDO_UPDATE payload (%zu) smaller "
					...
					/*
					 * Layout written by RelUndoFinish for a full UPDATE record
					 * is [RelUndoUpdatePayload][old tuple bytes]; the old
					 * tuple occupies the trailing tuple_len bytes of the
			...
			default:
				elog(DEBUG2,
					 "RECNO PVS: cannot step past urec_type %u at %llu",
