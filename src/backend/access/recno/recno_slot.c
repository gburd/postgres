...
tts_recno_getsysattr(TupleTableSlot *slot, int attnum, bool *isnull)
{
	...
	/*
	 * Return the tuple's real MVCC system columns.  RECNO uses heap-shaped
	 ...
	 * accessed via RecnoTupleGetXmax (0 == not deleted/updated).
	 */
	*isnull = false;
	...
	switch (attnum)
	{
		...
		case MaxTransactionIdAttributeNumber:	/* xmax */
			return TransactionIdGetDatum(RecnoTupleGetXmax(rslot->tuple));
