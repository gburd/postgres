...
tts_recno_deform(TupleTableSlot *slot, int natts)
{
	...
	for (; attnum < natts; attnum++)
	{
		...
		/*
		 * A dropped attribute reads as NULL but may still OCCUPY BYTES here:
		 * tuples written before ALTER TABLE ... DROP COLUMN stored a real
		 * value and the on-disk image is never rewritten.  Skip exactly the
		 * bytes the writer emitted.  Returning early without advancing
		 * data_ptr desynchronised every FOLLOWING attribute, which then
		 * decoded a varlena header out of the middle of another datum and
		 * SIGSEGV'd in detoast_attr() on the resulting bogus TOAST pointer
		 * (ADD COLUMN ... DEFAULT, then DROP COLUMN, then SELECT).
		 *
		 * ALTER TABLE preserves attlen/attalign for dropped columns precisely
		 * so old images stay walkable; a dropped varlena has attlen == -1 and
		 * its stored length comes from the datum header.
		 */
		if (att->attisdropped)
		{
...
tts_recno_getsysattr(TupleTableSlot *slot, int attnum, bool *isnull)
{
	...
	/*
	 * If no physical tuple is attached, we can still answer xmin/xmax from the
	 * slot's TID by reading the tuple header off the page.
	 *
	 * RecnoTupleToSlotWithOverflow() fills tts_values/tts_isnull and clears
	 * TTS_FLAG_EMPTY but does not attach a RecnoTuple, so a slot populated that
	 * way has rslot->tuple == NULL.  ExecOnConflictLockRow reads xmin off such
	 * a slot via slot_getsysattr(MinTransactionIdAttributeNumber) after a
	 * TM_Invisible lock result, to decide whether to raise
	 * "ON CONFLICT ... command cannot affect row a second time".  Erroring out
	 * here replaced that intended error with a spurious
	...
	if (!rslot->tuple)
	{
			...
			OidIsValid(slot->tts_tableOid))
		{
			...
			rel = try_relation_open(slot->tts_tableOid, AccessShareLock);
			if (rel != NULL)
			{
				...
				if (blkno < RelationGetNumberOfBlocks(rel))
				{
						...
						offnum <= PageGetMaxOffsetNumber(page))
					{
						...
						if (ItemIdIsNormal(iid))
						{
							...
							hdr = (RecnoTupleHeader *) PageGetItem(page, iid);
							if (attnum == MinTransactionIdAttributeNumber)
								result_xid = hdr->t_xmin;
							else if (hdr->t_flags & RECNO_TUPLE_DELETED)
								result_xid = hdr->t_xmin;
						}
					}
			...
			*isnull = false;
			return TransactionIdGetDatum(result_xid);
		}
