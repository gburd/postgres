 ...
 *
 * When the relation's `compression` reloption is set (auto/lz4/zstd),
 * variable-length
...
#define RECNO_FORM_STACK_ATTRS	16
	 ...
	 * Compression is configured per-relation via the `compression` reloption
	 * (off / auto / lz4 / zstd) -- there is no GUC.  A NULL rel (transient
	 * slot) or a relation with compression off skips this phase.
	 * RecnoCompressAttribute returns the original value unchanged if the
	 * value is too small or compression did not achieve a worthwhile ratio.
	 */
	if (RecnoRelCompressionEnabled(rel))
	{
		RecnoCompressionType rel_codec;
		int			rel_level;

		rel_codec = RecnoRelCompressionType(rel, &rel_level);
	 ...
	 * pointer.
	 *
	 * Overflow is gated on overflow_buffers (the caller's atomic-WAL sink),
	 * NOT on rel: a caller may pass a non-NULL rel to enable per-relation
	 * compression while still opting out of overflow (overflow_buffers ==
	 * NULL) on a fast path where the tuple is known to stay inline.
	 */
	if (rel != NULL && overflow_buffers != NULL)
	{
		for (i = 0; i < tupdesc->natts; i++)
		{
			Form_pg_attribute att = TupleDescAttr(tupdesc, i);
