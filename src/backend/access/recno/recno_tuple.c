							 ...
							 TupleTableSlot *slot, Relation rel)
{
	 ...
	 */
	{
		...
		/*
		 * Deform-bounds oracle (fix-6): a single tuple can never exceed one
		 * page, so bound each data_ptr advance to
		 * [tuple_header, tuple_header + BLCKSZ).  A wrong VARSIZE/alignment
		 * advance (e.g. a varlena decoded at an unaligned offset so its
		 * length word is read out of the preceding attribute's padding) walks
		 * data_ptr off the tuple -- and off the page -- and the next
		 * VARSIZE_ANY dereferences inaccessible memory (SIGSEGV).  The on-page
		 * RecnoTupleHeader carries no length; the observed ~0x60000-byte
		 * off-page walk trips here on the FIRST bad advance instead of
		 * crashing several attributes later on garbage.
		 */
		const char *deform_lo = (const char *) tuple_header;
		const char *deform_hi = (const char *) tuple_header + BLCKSZ;
#endif
...
#ifdef USE_ASSERT_CHECKING
					/* Oracle: the length word must lie inside the tuple. */
					Assert(data_ptr >= deform_lo && data_ptr < deform_hi);
#endif
					attr_len = VARSIZE_ANY(data_ptr);
#ifdef USE_ASSERT_CHECKING
					/* Oracle: the whole varlena must lie inside the tuple. */
					Assert(data_ptr + attr_len > data_ptr &&
						   data_ptr + attr_len <= deform_hi);
#endif
	...
	{
		/*
		 * Global aliasing invariant (fix-4 generalized to every slot site).
		 *
		 * RECNO does in-place MVCC UPDATEs: a committed/uncommitted updater
		 * overwrites live on-page tuple bytes under BUFFER_LOCK_SHARE_EXCLUSIVE,
		 * which does NOT exclude plain SHARE readers.  Every deform above stores
		 * inline (non-overflow) column datums as raw pointers into tuple_header.
		 * If tuple_header still points into a shared buffer page when the caller
		 * has released (or never held) the content lock, a concurrent updater
		 * can tear those multi-byte reads (torn numeric -> Assert "borrow == 0",
		 * torn varlena length -> zstd/alloc-size errors / SIGSEGV).  Every
		 * correct caller copies the on-page tuple into slot-private memory under
		 * the SHARE lock before calling here (RecnoSlotStoreTuple, the
		 * fetch_cxt copy, the EPQ copy), so NO inline datum may point into the
		 * shared buffer pool [BufferBlocks, BufferBlocks + NBuffers*BLCKSZ).
		 *
		 * This is the deterministic oracle used to hunt any remaining
		 * unlocked-buffer torn-read site: a regression that hands an on-page
		 * pointer here trips this assert on the first row rather than corrupting
		 * data under a race that only shows up on slow hardware.
		 */
		const char *pool_start = BufferBlocks;
		const char *pool_end = BufferBlocks + ((Size) NBuffers) * BLCKSZ;
		int			ai;
		int			checked = Min(tupdesc->natts, tuple_header->t_natts);

		for (ai = 0; ai < checked; ai++)
		{
			Form_pg_attribute att = TupleDescAttr(tupdesc, ai);
			const char *dp;

			if (slot->tts_isnull[ai] || att->attbyval || att->attisdropped)
				continue;
			dp = (const char *) DatumGetPointer(slot->tts_values[ai]);
			Assert(dp < pool_start || dp >= pool_end);
		}
	}
#endif
