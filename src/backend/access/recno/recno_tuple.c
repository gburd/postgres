...
#define RECNO_FORM_STACK_ATTRS	16
	...
	/*
	 * Phase 2: Calculate total space needed using (possibly compressed/overflowed)
	 * values
	 */
	...
	{
		/*
		 * A dropped attribute stores NO bytes (every storage loop below skips
		 * it), so it must be recorded as NULL in the bitmap.  Previously only
		 * isnull[i] was consulted, so a dropped column whose incoming Datum
		 * happened to be non-null was marked NOT NULL while contributing no
		 * data.  The reader then believed bytes were present for it and, since
		 * its dropped-attribute branch does not advance data_ptr either, every
		 * following attribute was decoded from the wrong offset -- yielding a
		 * garbage varlena/TOAST pointer and a SIGSEGV in detoast_attr().
		 * Reproduced by: ADD COLUMN ... DEFAULT (an implicit missing value),
		 * then DROP COLUMN, then SELECT.
		 */
		if (isnull[i] || TupleDescAttr(tupdesc, i)->attisdropped)
		{
			has_nulls = true;
	...
	for (i = 0; i < tupdesc->natts; i++)
	{
		...
		{
			/*
			 * Fixed-length attribute - never compressed.
			 * Must distinguish byval from by-reference fixed-length types
			 * (e.g., timetz is 12 bytes but passed by reference).
			 */
			if (att->attbyval)
...
RecnoDeformTuple(RecnoTuple tuple, TupleDesc tupdesc, Datum *values, bool *isnull)
{
		...
		for (i = 0; i < loop_natts; i++)
	{
		...
		/* Check null bitmap: bit=0 means NULL (bit cleared in RecnoFormTuple) */
		...
		if (att->attlen > 0)
		{
			/*
			 * Fixed-length attribute - never compressed.
			 * Use actual attbyval flag (e.g., timetz is 12 bytes but by-ref).
			 */
		...
		else if (att->attlen == -1)
		{
			...
			if (tuple_has_compressed)
			{
				 ...
				 * compressed attribute has VARHDRSZ + RecnoCompressionHeader
				 * + compressed payload. We identify it by checking the
				 * comp_type field in the header position.
				 */
	...
	/*
	 * Fill missing attributes with defaults for columns added by
	 * ALTER TABLE ADD COLUMN after this tuple was stored.
	 */
...
/*
 * RecnoPageDeleteTuple
 *
 * Mark a tuple as deleted by setting the RECNO_TUPLE_DELETED flag on its
 * header.  The tuple data remains on the page (tombstone) for UNDO support
 * and MVCC visibility of older snapshots.  The tuple will be physically
 * removed during VACUUM or defragmentation.
 *
 * Parameters:
 *   page      - the page containing the tuple (must be exclusively locked)
 *   offnum    - offset number of the tuple to delete
 *   commit_ts - commit timestamp to record on the deleted tuple
 */
void
RecnoPageDeleteTuple(Page page, OffsetNumber offnum, uint64 commit_ts)
{
	ItemId		itemid;
	...
	tuple->t_commit_ts = commit_ts;

	/* Update page header */
	phdr = RecnoPageGetOpaque(page);
	phdr->pd_commit_ts = Max(phdr->pd_commit_ts, commit_ts);
	phdr->pd_flags |= RECNO_PAGE_DEFRAG_NEEDED;
}
					 ...
					 uint64 old_commit_ts, uint64 new_commit_ts)
{
	...
	if (new_size <= old_size)
	{
		...
		/* Update page header */
		phdr = RecnoPageGetOpaque(page);
		phdr->pd_commit_ts = Max(phdr->pd_commit_ts, new_commit_ts);
		phdr->pd_free_space = PageGetFreeSpace(page);
							 ...
							 TupleTableSlot *slot, Relation rel)
{
	 ...
	 * Use the tuple's actual natts for bitmap_len and data_ptr calculation.
	 * After ALTER TABLE ADD COLUMN, old tuples may have fewer attributes
	 * than the current schema expects.
	 */
	{
		...
		for (i = 0; i < loop_natts; i++)
	{
		...
		/*
		 * A dropped attribute always reads as NULL, but it may still OCCUPY
		 * BYTES in this tuple: tuples written before the ALTER TABLE ... DROP
		 * COLUMN stored a real value, and the on-disk image is not rewritten.
		 * So we must consult the null bitmap and, when bytes are present, skip
		 * over exactly as many as the writer emitted.  Returning early without
		 * advancing data_ptr desynchronised every FOLLOWING attribute, which
		 * decoded a varlena header from the middle of another datum and
		 * SIGSEGV'd in detoast_attr() on the resulting garbage TOAST pointer.
		 * Reproduced by: ADD COLUMN ... DEFAULT, then DROP COLUMN, then SELECT.
		 *
		 * attlen is still valid for a dropped column (ALTER TABLE preserves
		 * attlen/attalign precisely so old images stay walkable), except that
		 * a dropped varlena has attlen == -1 and its stored length must be read
		 * from the datum header, exactly as the live-varlena path does.
		 */
		...
		else
		{
			...
			if (att->attlen == -1)
			{
				...
				/*
				 * Align exactly as RecnoFormTuple did on the write side: it
				 * aligns data_ptr with att_align_nominal() *before* every
				 * attribute (fixed or varlena) and then advances by the raw
				 * stored length with no post-alignment.  The reader must use
				 * the identical rule or it desynchronises and later reads a
				 * varlena header from the middle of some other datum, which
				 * trips VARSIZE_ANY / VARTAG_SIZE(0) (the "tuple does not fit"
				 * overflow path made this collision frequent enough to crash).
				 */
				data_ptr = (char *) att_align_nominal(data_ptr, att->attalign);
				 ...
				 * compression since the on-disk data is an overflow pointer,
				...
				if (tuple_has_overflow && RecnoIsOverflowPtr(data_ptr))
				{
					if (rel != NULL)
					{
						 ...
						 * the fetched value (not data_ptr) for a compression
						 * header and decompress if needed.
						 */
						if (tuple_has_compressed)
						{
				...
				{
					/*
					 * Check for compression header in this varlena attribute.
					 */
					Size		data_size = VARSIZE_ANY_EXHDR(data_ptr);
