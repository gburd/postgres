 ...
 * 10,000 entries simultaneously.  The per-backend multiplier is the
 * GUC `recno_slog_entries_per_backend` (default 2048).  If a single
 * bulk load overflows the table, the backend will fail with
 *
 *   ERROR:  out of shared memory
 *   HINT:   raise recno_slog_entries_per_backend, or split the work
 *           across smaller transactions
 *
 * A future spill-to-SLRU path (like pg_xact) would eliminate the
 * cliff entirely; until then, the GUC is the operator's lever.
 *
 * The minimum (128) is a safety floor for edge-case configurations.
 */
int			recno_slog_entries_per_backend = 2048;
...
typedef struct RecnoSLogOp
{
	...
	uint8			aborted_op_type;	/* op_type before it was overwritten
									 * with RECNO_SLOG_ABORTED; only
									 * meaningful when op_type is
									 * RECNO_SLOG_ABORTED.  Preserved (rather
									 * than inferred from page state) so
									 * RecnoSLogProcessAbortedEntries can tell
									 * a rolled-back INSERT (TID becomes dead)
									 * from a rolled-back UPDATE/DELETE (TID
									 * reverts to a prior committed row). */
...
/*
 * File-scope statics: lifetime and thread-safety overview
 * -------------------------------------------------------
 *
 * Shared across all backends (allocated in ShmemInitHash / ShmemAlloc at
 * postmaster startup, read/written under RecnoSLogLocks partitions):
 *
 *   RecnoSLogHash            Hash table: (tid, relid) -> op array.
 *   RecnoSLogLocks           RECNO_SLOG_PARTITIONS LWLocks, one per
 *                            hash partition.  SHARED for lookups,
 *                            EXCLUSIVE for insert/remove/update and
 *                            for op-array mutation.
 *
 * Per-backend (private to each PostgreSQL process, zero-initialised
 * at fork, reset on each transaction boundary):
 *
 *   recno_slog_callbacks_registered
 *                            True once this backend has called
 *                            RegisterXactCallback + RegisterSubXactCallback;
 *                            prevents duplicate registration across
 *                            reused backends.
 *   slog_local_list          Singly-linked list of entries this backend
 *                            is tracking within the current transaction.
 *                            Allocated in TopTransactionContext; freed
 *                            implicitly at transaction end.  Walked by
 *                            the commit/abort callbacks for O(n)
 *                            cleanup without scanning the whole shared
 *                            hash table.
 *
 * Fetched-snapshot scratch space for the visibility hot path
 * (recno_mvcc.c's SLOG_ENSURE_FETCHED()) is declared in recno_mvcc.c,
 * not here.
 */
				...
				uint32 spec_token)
{
	...
	if (entry == NULL)
		ereport(ERROR,
				(errcode(ERRCODE_OUT_OF_MEMORY),
				 errmsg("RECNO sLog is full: %d entries in use",
						RecnoSLogNumEntries()),
				 errdetail("A RECNO table with enable_undo=on writes one sLog "
						   "entry per live row operation; the entry retires when "
						   "the originating transaction commits or aborts."),
				 errhint("Split the transaction into smaller batches, raise "
						 "recno_slog_entries_per_backend (requires restart), "
						 "or turn off enable_undo for bulk-load tables.")));
	...
	for (i = 0; i < entry->nops; i++)
	{
			...
			TransactionIdEquals(entry->ops[i].xid, xid))
		{
			...
			entry->ops[i].aborted_op_type = (uint8) op_type;
				...
				RecnoSLogEntry *entries, int max_entries)
{
	...
	if (hentry != NULL)
	{
		for (i = 0; i < RECNO_SLOG_MAX_OPS && nfound < max_entries; i++)
		{
			...
			entries[nfound].aborted_op_type = hentry->ops[i].aborted_op_type;
			...
			if (hentry != NULL)
			{
				for (i = 0; i < RECNO_SLOG_MAX_OPS; i++)
				{
					if (hentry->ops[i].in_use &&
						TransactionIdEquals(hentry->ops[i].xid, xid))
					{
						if (hentry->ops[i].op_type != (uint8) RECNO_SLOG_ABORTED)
							hentry->ops[i].aborted_op_type = hentry->ops[i].op_type;
			...
			if (hentry != NULL)
			{
				...
				for (i = 0; i < RECNO_SLOG_MAX_OPS; i++)
				{
					if (!hentry->ops[i].in_use)
					{
						...
						hentry->ops[i].op_type = (uint8) RECNO_SLOG_ABORTED;
						/*
						 * Local-only tracking is used exclusively by plain
						 * recno_tuple_insert (see RecnoSLogTrackSubXact), so a
						 * synthesized ABORTED entry always describes a
						 * rolled-back INSERT.
						 */
						hentry->ops[i].aborted_op_type = (uint8) RECNO_SLOG_INSERT;
	...
	for (lentry = slog_local_list; lentry != NULL; lentry = lentry->next)
	{
		...
		if (hentry != NULL)
		{
			for (i = 0; i < RECNO_SLOG_MAX_OPS; i++)
			{
				...
				{
					/*
					 * Only a rolled-back INSERT leaves a TID that must become
					 * dead.  For a rolled-back UPDATE or DELETE the TID holds a
					 * PRE-EXISTING COMMITTED row which the subxact-abort UNDO
					 * apply has already restored in place; stamping DELETED
					 * here would destroy that committed row.
					 */
					if (hentry->ops[i].aborted_op_type != (uint8) RECNO_SLOG_INSERT)
						continue;
		 ...
		 */
		{
			...
			/*
			 * Use try_relation_open: VACUUM FULL / CLUSTER can drop the
			 * transient relation referenced by this sLog entry before our
			 * PRE_COMMIT runs (the rewrite's inserts write sLog entries
			 * tagged with the *transient* table's OID; finish_heap_swap
			 * then drops it).  For those entries the tuple we'd be trying
			 * to update is already on disk via the same transaction's
			 * final pages and the flag no longer matters -- skip quietly.
			 */
			rel = try_relation_open(lentry->key.relid, AccessShareLock);
			if (rel == NULL)
				continue;
...
/*
 * RecnoSLogClearUncommittedFlags
 *
 * Fix B: Proactively clear RECNO_TUPLE_UNCOMMITTED on all tuples modified
 * by the current transaction at PRE_COMMIT time.
 *
 * This eliminates the "lazy clearing" write amplification pattern where
 * multiple concurrent readers each dirty the same committed tuple to clear
 * the stale flag.  By clearing proactively at commit:
 *
 *   - Each tuple is dirtied ONCE (at commit) instead of once per reader.
 *   - Subsequent visibility checks skip the sLog lookup entirely (the
 *     flag is clear before any reader can see the committed data).
 *   - The UNDO worker no longer sees spurious sLog contention from
 *     readers lazily clearing flags on its watched tuples.
 *
 * Only INSERT and UPDATE operations set UNCOMMITTED.  DELETE operations
 * mark the tuple DELETED (already handled by RecnoSLogProcessAbortedEntries
 * for subtransaction rollbacks).  We skip ABORTED entries since those
 * tuples will be marked DELETED by RecnoSLogProcessAbortedEntries.
 *
 * This is called at XACT_EVENT_PRE_COMMIT, before RecnoSLogRemoveByXid(),
 * while relation_open() and ReadBuffer() are still valid.
 */
static void
RecnoSLogClearUncommittedFlags(TransactionId xid)
{
	RecnoSLogLocalEntry *lentry;
	...
	for (lentry = slog_local_list; lentry != NULL; lentry = lentry->next)
	{
		...
		/* Check what operation type this entry has */
		RecnoSLogLockPartition(&lentry->key, LW_SHARED);
		hentry = (RecnoSLogHashEntry *)
			hash_search(RecnoSLogHash, &lentry->key, HASH_FIND, NULL);
		...
		if (hentry != NULL)
		{
			for (i = 0; i < RECNO_SLOG_MAX_OPS; i++)
			{
				if (hentry->ops[i].in_use &&
					TransactionIdEquals(hentry->ops[i].xid, xid))
				{
					found_op_type = hentry->ops[i].op_type;
					break;
				}
			}
		}
		...
		{
			/*
			 * Caveat-2: a plain INSERT writes NO shared sLog entry, only this
			 * local track entry.  It is by construction a committed INSERT, so
			 * clear its UNCOMMITTED flag here at commit.  If we skipped it (the
			 * old "no hentry -> continue" behavior), the flag would stay set on
			 * every bulk-inserted row and the lazy read-time clear
			 * (RecnoTupleVisible) would try to write a hint bit through an
			 * unlocked buffer -- "buffer is not locked" under concurrent read.
			 */
			found_op_type = (uint8) RECNO_SLOG_INSERT;
		}
		RecnoSLogUnlockPartition(&lentry->key);

		/*
		 * Only clear UNCOMMITTED for committed INSERT/UPDATE operations.
		 * Skip ABORTED (handled by RecnoSLogProcessAbortedEntries) and
		 * DELETE (the tuple's visibility is based on timestamp, not flag).
		 */
		if (found_op_type != (uint8) RECNO_SLOG_INSERT &&
			found_op_type != (uint8) RECNO_SLOG_UPDATE)
			continue;

		/*
		 * Open the relation and clear UNCOMMITTED on the tuple.
		 * Use MarkBufferDirtyHint since this is a hint bit change --
		 * it will be re-derived correctly on crash recovery.
		 *
		 * try_relation_open is essential here: VACUUM FULL / CLUSTER on
		 * a RECNO table writes sLog entries tagged with the *transient*
		 * relation's OID; finish_heap_swap drops that relation before
		 * our PRE_COMMIT callback fires.  For such entries we have
		 * nothing to do -- the old on-disk image is about to be
		 * unlinked anyway -- so skip them silently rather than
		 * emitting a scary "could not open relation" error into the
		 * log.
		 */
		rel = try_relation_open(lentry->key.relid, AccessShareLock);
		if (rel == NULL)
		...
		PG_TRY();
		{
			...
			if (offnum <= PageGetMaxOffsetNumber(page))
			{
				...
				if (ItemIdIsNormal(itemid))
				{
					...
					/*
					 * Clear UNCOMMITTED flag (t_xid_hint removed from header).
					 */
					if (tuple_hdr->t_flags & RECNO_TUPLE_UNCOMMITTED)
					{
						tuple_hdr->t_flags &= ~RECNO_TUPLE_UNCOMMITTED;
		...
		{
			/*
			 * Don't let flag-clearing failure abort the commit.
			 * The lazy-clearing path in RecnoTupleVisible() will handle
			 * any tuples we couldn't clear here.
			 */
			EmitErrorReport();
			FlushErrorState();
		}
...
RecnoSLogXactCallback(XactEvent event, void *arg)
{
	switch (event)
	{
		...
		case XACT_EVENT_PRE_PREPARE:
			{
				/*
				 * Pre-commit cleanup: must happen before COMMIT because
				 * both functions call relation_open() / ReadBuffer(), which
				 * require IsTransactionState() == true.
				 *
				 * 1. Clear UNCOMMITTED flags on all committed tuples (Fix B).
				 *    This prevents write amplification where N readers each
				 *    dirty the same committed tuple to lazily clear the flag.
				 *
				 * 2. Process ABORTED subtransaction entries (mark DELETED).
				 *    Must run after (1) so ABORTED tuples aren't cleared.
				 */
				TransactionId xid = GetCurrentTransactionIdIfAny();

				if (TransactionIdIsValid(xid))
				{
					RecnoSLogClearUncommittedFlags(xid);	/* Fix B */
		...
		case XACT_EVENT_PARALLEL_ABORT:
			{
				...
				{
					/*
					 * Mark the sLog entries for this xid as ABORTED so RECNO
					 * MVCC visibility treats rows written by this xid as dead.
					 *
					 * Under UNDO-in-WAL physical undo of those rows (reclaiming
					 * their on-disk space) is deferred to the logical-revert
					 * worker, which walks the xid's UNDO chain from WAL and
					 * dispatches each record to the UNDO_RMID_RECNO apply
					 * callback (see recno_undo.c).  Attempting synchronous
					 * apply here is unsafe because the abort path runs in a
					 * BumpContext where pfree() and relation_close() are not
					 * supported.
					 */
						 ...
						 void *arg)
{
	...
	switch (event)
	{
		case SUBXACT_EVENT_ABORT_SUB:
			 ...
			 *
			 * FIRST physically restore the pre-savepoint tuple images by
			 * reverse-applying THIS subtransaction's per-backend UNDO records,
			 * THEN mark this sub-level's sLog entries ABORTED.
			 *
			 * The physical restore is what makes a rolled-back in-place UPDATE
			 * or DELETE recover its before-image.  RECNO is a stable-TID
			 * in-place AM: the aborted op overwrote the row in place, so there
			 * is no second version to fall back on and nothing else will ever
			 * put the committed row back.  Without this apply the row stayed
			 * modified/deleted and the subsequent ABORTED marking made
			 * RecnoSLogProcessAbortedEntries stamp it RECNO_TUPLE_DELETED at
			 * top-level COMMIT -- silently destroying a committed row.
			 * (heap never faces this: an aborted version is a distinct tuple
			 * that simply never becomes visible.)
			 *
			 * pbuUndoStartPtr/pbuUndoLatestPtr live on the per-subxact
			 * TransactionState and CurrentTransactionState is still the
			 * aborting subxact here, so start..latest bracket exactly this
			 * subxact's records (both Invalid if it wrote none).  Ordering
			 * matters: the restore must precede RecnoSLogRemoveBySubXid so the
			 * on-page images are correct when visibility later consults the
			 * (now ABORTED) markers.  We are in TRANS_ABORT, so present an
			 * in-progress state for the relation opens the apply performs.
			 * Mirrors the FLUX SUBXACT_EVENT_ABORT_SUB handler.
			 */
			{
				UndoRecPtr	sub_from = RecnoPbuGetCurrentUndoLatest();
				UndoRecPtr	sub_stop = RecnoPbuGetCurrentUndoStart();
				...
				if (UndoRecPtrIsValid(sub_from) && UndoRecPtrIsValid(sub_stop))
				{
					...
					RecnoPbuApplySubxactUndo(sub_from, sub_stop);
