...
RecnoVMClear(Relation rel, BlockNumber heapBlk, Buffer heapBuf, uint8 flags)
{
	...
	if (RelationNeedsWAL(rel))
	{
		 ...
		 * as in RecnoVMSet: the heap buffer may not be dirty.
		 */
		XLogRegisterBuffer(0, heapBuf, REGBUF_NO_IMAGE | REGBUF_NO_CHANGE);
		XLogRegisterBuffer(1, vmBuf, REGBUF_STANDARD);
...
/*
 * RecnoVMCheckCached - Check visibility map bits with caller-managed buffer cache
 *
 * Like RecnoVMCheck, but the caller provides pointers to a cached VM buffer
 ...
 */
bool
RecnoVMCheckCached(Relation rel, BlockNumber heapBlk, uint8 flags,
