 ...
 *
 * recno_vm.c
 *	  Visibility Map implementation for RECNO
 *
 * The Visibility Map (VM) tracks the visibility status of pages in a RECNO
 ...
 * (src/backend/access/heap/visibilitymap.c) but adapted for RECNO's
...
/* Bit manipulation - use RECNO-specific values that match PostgreSQL's VM bits */

/* Forward declaration */
static Buffer recno_vm_extend(Relation rel, BlockNumber vm_nblocks);
static Buffer recno_vm_readbuf(Relation rel, BlockNumber blkno, bool extend);

/*
 * RecnoVMInit - Initialize visibility map for a RECNO relation
 *
 * This is called when a RECNO table is created to ensure the VM fork exists.
 */
void
RecnoVMInit(Relation rel)
{
	/*
	 * Create the visibility map fork if it doesn't exist. This happens
	 * automatically when we first try to extend it via recno_vm_extend().
	 */
}

/*
 * RecnoVMSet - Set visibility map bits for a page
 *
 ...
 */
void
RecnoVMSet(Relation rel, BlockNumber heapBlk, Buffer heapBuf, uint8 flags)
{
	...
	/*
	 * Read or extend the visibility map buffer.  recno_vm_readbuf() will
	 * create the VM fork if it doesn't exist yet.
	 */
	vmBuf = recno_vm_readbuf(rel, mapBlock, true);
	...
	if (RelationNeedsWAL(rel))
	{
		...
		/*
		 * Register the heap buffer with REGBUF_NO_IMAGE.  We reference
		 * the heap page so that redo can update its LSN, but we do NOT
		 * need a full-page image of the heap page in this WAL record.
		 * The heap buffer may not be dirty (e.g., during VACUUM VM
		 * updates), so we must not let XLogInsert try to take an FPI
		 * of it -- that would trip the BufferIsDirty assertion.
		 */
		XLogRegisterBuffer(0, heapBuf, REGBUF_NO_IMAGE | REGBUF_NO_CHANGE);
		XLogRegisterBuffer(1, vmBuf, REGBUF_STANDARD);

		recptr = XLogInsert(RM_RECNO_ID, XLOG_RECNO_VM_SET);
...
/*
 * RecnoVMClear - Clear visibility map bits for a page
 *
 * Clears the specified bits for the given heap block. The heap buffer must
 * be exclusively locked.
 */
void
RecnoVMClear(Relation rel, BlockNumber heapBlk, Buffer heapBuf, uint8 flags)
{
	BlockNumber mapBlock;
	uint32		mapByte;
	uint8		mapOffset;
	 ...
	 * modification and WAL logging entirely.  This is the common case
	 * after the first modification to a page since the last VACUUM, and
	 * avoids significant WAL amplification on hot pages.
	 */
	...
	if (RelationNeedsWAL(rel))
	{
		...
		/*
		 * Register the heap buffer with REGBUF_NO_IMAGE for the same
		 * reason as in RecnoVMSet: the heap buffer may not be dirty.
		 */
		XLogRegisterBuffer(0, heapBuf, REGBUF_NO_IMAGE | REGBUF_NO_CHANGE);
		XLogRegisterBuffer(1, vmBuf, REGBUF_STANDARD);

		recptr = XLogInsert(RM_RECNO_ID, XLOG_RECNO_VM_CLEAR);
		PageSetLSN(page, recptr);
	}
...
/*
 * RecnoVMCheck - Check visibility map bits for a page
 *
 ...
 */
bool
RecnoVMCheck(Relation rel, BlockNumber heapBlk, uint8 flags)
{
	BlockNumber mapBlock;
	uint32		mapByte;
	uint8		mapOffset;
...
/*
 * RecnoVMPinBuffer - Pin the visibility map buffer for a heap block
 *
 ...
 */
void
RecnoVMPinBuffer(Relation rel, BlockNumber heapBlk, Buffer *vmbuf)
{
	BlockNumber mapBlock;
...
/*
 * RecnoVMExtend - Extend the visibility map to cover more heap blocks
 *
 * This is called when the heap relation is extended.
 */
void
RecnoVMExtend(Relation rel, BlockNumber nheapblocks)
{
	...
	if (newnblocks > 0)
	{
		...
		buf = recno_vm_extend(rel, newnblocks);
		ReleaseBuffer(buf);
	}
}

/*
 * RecnoVMTruncate - Truncate the visibility map
 *
 * This is called when the heap relation is truncated.
 */
void
RecnoVMTruncate(Relation rel, BlockNumber nheapblocks)
{
	BlockNumber newnblocks;
...
/*
 * RecnoVMGetPageSize - Get the size of a VM page
 */
Size
RecnoVMGetPageSize(void)
{
	return MAPSIZE;
}

/*
 * RecnoVMMapHeapToVM - Map a heap block number to VM block number
 */
BlockNumber
RecnoVMMapHeapToVM(BlockNumber heapBlk)
{
	return HEAPBLK_TO_MAPBLOCK(heapBlk);
}

/*
 * recno_vm_extend - Extend the VM fork to at least vm_nblocks.
 *
 ...
 */
static Buffer
recno_vm_extend(Relation rel, BlockNumber vm_nblocks)
{
	Buffer		buf;
...
/*
 * recno_vm_readbuf - Read or extend the VM to get the page for blkno.
 *
 ...
 */
static Buffer
recno_vm_readbuf(Relation rel, BlockNumber blkno, bool extend)
{
	Buffer		buf;
	...
	{
		if (extend)
			buf = recno_vm_extend(rel, blkno + 1);
		else
...
/*
 * RecnoVMUpdateForInsert - Update VM after inserting a tuple
 *
 ...
 */
void
RecnoVMUpdateForInsert(Relation rel, RecnoTupleHeader *tuple, Buffer buffer)
{
	BlockNumber blkno = BufferGetBlockNumber(buffer);

	/*
	 * Check if the new tuple affects the page's visibility status. In RECNO's
	 * timestamp-based MVCC, a tuple is visible to all if its commit timestamp
	 * is older than the oldest active transaction.
	 *
	 * TODO: In the future, we could check if the new tuple is already visible
...
/*
 * RecnoVMUpdateForUpdate - Update VM after updating a tuple
 *
 ...
 */
void
RecnoVMUpdateForUpdate(Relation rel, Buffer buffer)
{
	BlockNumber blkno = BufferGetBlockNumber(buffer);
...
/*
 * RecnoVMUpdateForDelete - Update VM after deleting a tuple
 *
 * Deletes clear the all-visible bit because the deleted tuple may still
 * be visible to some transactions.
 */
void
RecnoVMUpdateForDelete(Relation rel, Buffer buffer)
{
	BlockNumber blkno = BufferGetBlockNumber(buffer);
...
/*
 * RecnoVMVacuumPage - Update VM during VACUUM
 *
 * This is called by VACUUM after processing a page to set the appropriate
 * visibility map bits based on the page's contents.
 */
void
RecnoVMVacuumPage(Relation rel, Buffer buffer, bool all_visible, bool all_frozen)
{
	BlockNumber blkno = BufferGetBlockNumber(buffer);
	uint8		flags = 0;

	if (all_visible)
		flags |= RECNO_VM_ALL_VISIBLE;
	if (all_frozen)
		flags |= RECNO_VM_ALL_FROZEN;

	if (flags != 0)
		RecnoVMSet(rel, blkno, buffer, flags);
}
