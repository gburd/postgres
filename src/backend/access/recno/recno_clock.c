...
RecnoClockStartMonitor(void)
{
	...
	{
		ereport(DEBUG1,
				(errmsg("RECNO: failed to start clock monitor background worker")));
	}
}
...
RecnoClockMonitorMain(Datum main_arg)
{
	...
	ereport(FATAL,
			(errmsg("RECNO: shutting down due to excessive clock drift"),
...
RecnoClockShmemInit_cb(void *arg)
{
	 ...
	 * WARNING about failing to start pollutes logs during initdb and
