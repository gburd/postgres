 ...
 * an in-place UPDATE, from the executor-computed modified_idx_attrs bitmap.
 *
 ...
 * key must be inserted; the executor does this when we return TU_All.  The
 * stale (oldkey -> TID) entry is dropped at read time by the executor's
 * index recheck (ExecIndexInplaceEntryIsStale) against the single live tuple.
 *
 * Under the Tepid contract the executor already computed the set of modified
 * indexed attributes (ExecUpdateModifiedIdxAttrs) and passes it as
 * modified_idx_attrs, so we no longer re-discover the change by deforming the
 * old tuple.  This is heap-identical index-maintenance signalling:
 *   TU_None        -- no indexed column changed
 *   TU_Summarizing -- only summarizing (BRIN) indexed column(s) changed
 *   TU_All         -- a non-summarizing (regular) indexed column changed
 */
static TU_UpdateIndexes
recno_compute_index_update(Relation relation,
						   const Bitmapset *modified_idx_attrs)
{
	...
	/*
	 * Mirror heap: if every modified indexed attribute is used only by
	 * summarizing indexes we insert only into those; otherwise every index
	 * gets a fresh (newkey -> TID) entry.  HeapUpdateHotAllowable() returns
	 * whether a HOT-style (skip non-summarizing indexes) update is allowable
	 * and sets summarized_only accordingly.
	 */
	if (HeapUpdateHotAllowable(relation, modified_idx_attrs, &summarized_only))
		return summarized_only ? TU_Summarizing : TU_None;
				   ...
				   TU_UpdateIndexes *update_indexes)
{
	...
	/*
	 * Determine the row-lock strength from the executor-computed set of
	 * modified indexed attributes, exactly as heapam_tuple_update() does
	 * (weaker NoKeyExclusive when no key column changed, allowing more
	 * concurrency with FK checks).  Callers outside the executor may pass a
	 * NULL lockmode pointer.
	 *
	 * RECNO acquires its own heavyweight LOCKTAG_TUPLE lock internally with a
	 * fixed mode per operation (LockTupleNoKeyExclusive on the UPDATE path,
	 * LockTupleExclusive on the DELETE path).  The EPQ re-lock in
	 * nodeModifyTable feeds *lockmode straight into table_tuple_lock() ->
	 * recno_tuple_lock(), so *lockmode MUST match the mode the UPDATE path
	 * takes, or an EPQ locker and a concurrent updater request incompatible
	 * modes on the same tuple tag and deadlock (AccessExclusiveLock vs
	 * ExclusiveLock on the LOCKTAG_TUPLE).  RECNO's UPDATE path always uses
	 * LockTupleNoKeyExclusive, so report that.
	 */
	/*
	 * Report the row-lock strength for the executor's EPQ re-lock path
	 * (nodeModifyTable feeds *lockmode into table_tuple_lock() ->
	 * recno_tuple_lock() when it must re-fetch the latest version after a
	 * concurrent update).  Callers outside the executor may pass NULL.
	 *
	 * RECNO reports the SHARE-class LockTupleKeyShare rather than heap's
	 * key/no-key Exclusive choice.  RECNO does NOT rely on the LOCKTAG_TUPLE
	 * heavyweight lock for update serialization -- that is enforced by the
	 * sLog write-conflict machinery plus the buffer/t_writer probe and
	 * XactLockTableWait inside recno_tuple_update/recno_tuple_lock.  The
	 * LOCKTAG_TUPLE lock is only an advisory FIFO-fairness device.  An
	 * EXCLUSIVE-class EPQ re-lock, however, records an sLog LOCK_EXCL marker
	 * and takes LOCKTAG_TUPLE in an exclusive mode while sleeping in
	 * XactLockTableWait on a concurrent updater's xid; because RECNO's CAS
	 * fast path sets a tuple's writer WITHOUT taking LOCKTAG_TUPLE, that
	 * exclusive EPQ locker and the CAS updater's next update form an
	 * AccessExclusiveLock/ExclusiveLock cycle on the tuple tag under hot-row
	 * EPQ churn (observed as cascading "deadlock detected" + throughput
	 * collapse).  A SHARE-class re-lock is self-compatible, so it provides the
	 * required latest-version fetch and ordering without introducing that
	 * cycle; correctness (no lost update) is still guaranteed by RECNO's own
	 * conflict detection, as the recno isolation suite (row-locking,
	 * lost-update, concurrent-updates, cas-locker-starvation) verifies.
	 */
	if (lockmode)
	 ...
	 */
	{
		...
		/*
		 * EXCLUSIVE, not SHARE_EXCLUSIVE.  The same-size in-place overwrite
		 * below (memcpy(old_tuple_hdr, cas_full_image, ...)) is NON-atomic.
		 * SHARE_EXCLUSIVE is compatible with plain SHARE (bufmgr
		 * BufferLockAttempt: SHARE only conflicts with EXCLUSIVE), so a scan /
		 * index-fetch reader copying the tuple out under SHARE would run
		 * concurrently with this overwrite and capture a TORN image: a
		 * half-updated varlena length word then walks the reader's deform loop
		 * off the page (SIGSEGV in VARSIZE_ANY -- the field crash under
		 * "select max(d_id) from district").  EXCLUSIVE conflicts with SHARE,
		 * so readers wait for the (short) overwrite to finish and always see a
		 * complete tuple -- the same discipline heap_update uses.  Writers on a
		 * page already serialize (SHARE_EXCLUSIVE conflicts with itself), so
		 * this only adds reader-vs-writer exclusion during the byte copy, not
		 * new writer serialization; the update stays on the CAS fast path.
		 */
		LockBuffer(buffer, BUFFER_LOCK_EXCLUSIVE);
		page = BufferGetPage(buffer);
					...
					cas_target_size == ItemIdGetLength(itemid))
				{
					...
					if (RecnoTupleWriterTryLock(old_tuple_hdr, &expected))
					{
						...
						else
						{
							...
							bool		cas_torn_window = false;
							bool		cas_torn_wait = false;
#endif
...
#ifdef USE_INJECTION_POINTS
								/*
								 * Evaluate the torn-window injection point BEFORE the
								 * critical section: IS_INJECTION_POINT_ATTACHED may
								 * allocate, which is forbidden once CritSectionCount > 0.
								 */
								if (cas_target_size > 8)
								{
									cas_torn_window =
										IS_INJECTION_POINT_ATTACHED("recno-cas-torn-window");
									cas_torn_wait =
										IS_INJECTION_POINT_ATTACHED("recno-cas-torn-wait");
								}

								/*
								 * DETERMINISTIC torn-read window (test-only, test 011).
								 *
								 * The plain "recno-cas-torn-window" NOTICE point below only
								 * widens the tear with a blind pg_usleep, so a concurrent
								 * reader has to get lucky and copy THIS tuple during the
								 * pause -- it usually doesn't, so it can't prove the fix.
								 * The WAIT variant makes the race deterministic: scribble a
								 * garbage 0xEE tail into the LIVE on-page tuple, then BLOCK
								 * the writer on the injection point (injection_wait polls a
								 * shmem counter until injection_points_wakeup()) while still
								 * holding the buffer content lock, then restore the clean
								 * bytes.  A reader that copies the tuple out during the wait
								 * is GUARANTEED to see the torn image:
								 *   - WITHOUT the fix (SHARE_EXCLUSIVE): a SHARE reader is not
								 *     excluded, copies the torn tuple, and its deform walks
								 *     off the tuple -> deform-bounds Assert / SIGSEGV.
								 *   - WITH the fix (EXCLUSIVE): a SHARE reader is blocked out
								 *     for the whole window, waits for the wakeup+restore, and
								 *     only ever copies a clean tuple.
								 *
								 * This runs BEFORE START_CRIT_SECTION() because
								 * injection_wait() sleeps and runs CHECK_FOR_INTERRUPTS() --
								 * neither is legal inside a critical section.  The buffer
								 * content lock (the thing that actually excludes readers) is
								 * held the entire time; the crit section only guards the
								 * WAL+page-LSN atomicity, which the tear does not touch.
								 * cas_old_copy holds the pristine pre-update bytes, so after
								 * the wakeup we restore the on-page image and the crit
								 * section starts from the same clean state as always.  Zero
								 * cost when the point is not attached; compiled out without
								 * --enable-injection-points.
								 */
								if (cas_torn_wait)
								{
									Size		half = cas_target_size / 2;

									memset((char *) old_tuple_hdr + half, 0xEE,
										   cas_target_size - half);
									INJECTION_POINT("recno-cas-torn-wait", NULL);
									memcpy((char *) old_tuple_hdr, cas_old_copy,
										   cas_old_len);
								}
#endif
...
#ifdef USE_INJECTION_POINTS
								/*
								 * Torn-read race window (test-only).  With the
								 * "recno-cas-torn-window" point attached (regression test
								 * 011), split the in-place overwrite: scribble a garbage
								 * tail, pause, then lay down the real bytes, widening the
								 * window in which the on-page tuple is inconsistent.  A
								 * reader that copied the tuple out mid-window would
								 * capture a torn image (garbage varlena length -> deform
								 * walks off the page).  With the fix this buffer lock is
								 * EXCLUSIVE, so SHARE readers are blocked out for the
								 * whole window and never see the tear; without it
								 * (SHARE_EXCLUSIVE) they do.  memset/usleep/memcpy are
								 * crit-safe (no palloc/ereport).  Zero cost when the point
								 * is not attached; compiled out without
								 * --enable-injection-points.
								 */
								if (cas_torn_window)
								{
									...
									memset((char *) old_tuple_hdr + half, 0xEE,
										   cas_target_size - half);
									pg_usleep(200);
									memcpy((char *) old_tuple_hdr + half,
										   (char *) cas_full_image + half,
										   cas_target_size - half);
								}
#endif
...
force_shrink_retry:
	...
	/*
	 * Classify the index-update strategy from the executor-computed
	 * modified_idx_attrs.  RECNO updates in place (TID unchanged), so an index
	 * entry only needs refreshing when its key actually changed; the helper
	 * returns TU_None otherwise, avoiding a re-insert of an identical
	 * (key, TID) pair.  The stale (oldkey -> TID) entry left behind by a
	 * changed key is dropped by the executor's index recheck at read time.
	 */
	if (update_indexes)
						   ...
						   BufferAccessStrategy bstrategy)
{
	 ...
	 * (am_inplace_update_keeps_tid = true); its index integration and the
		 ...
		 src_blkno--)
	{
			 ...
			 offnum = OffsetNumberNext(offnum))
		{
			...
			/*
			 * Skip pages holding a tuple that is UNCOMMITTED (an in-flight
			 * INSERT/in-place UPDATE by a still-running transaction) or that
			 * carries a version pointer (UNDO-fork history head).  RECNO keeps
			 * a tuple's TID across in-place UPDATE (am_inplace_update_keeps_tid)
			 * and its UNDO records -- both the pending rollback record of an
			 * uncommitted updater and the retained before-image chain of a
			 * committed one -- reference that TID.  Cross-page relocation changes
			 * the TID and marks the old slot LP_UNUSED, orphaning those records:
			 * a subsequent abort's RelUndoApplyUpdate targets the vacated old
			 * offset and finds it non-normal.  (Directly observed: VACUUM Phase
			 * III moved an in-flight-updated district row, then the updater's
			 * inline UNDO apply hit the LP_UNUSED old TID and, running inside
			 * proc_exit, the elog(ERROR) was promoted to FATAL and crashed the
			 * cluster.)  Such a tuple is not relocatable this pass; a later pass,
			 * once the updater commits/aborts and the version pointer is
			 * discarded, can relocate it safely.
			 */
			if ((tuple_hdr->t_flags & RECNO_TUPLE_UNCOMMITTED) ||
