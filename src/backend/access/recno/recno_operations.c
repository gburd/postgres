...
RecnoPagePruneOpt(Relation relation, Buffer buffer)
{
	...
	int			ndead = 0;
	Size		minfree;
	OffsetNumber unused_offsets[MaxOffsetNumber];
	int			nunused;
	...
	/* Offsets retired below, recorded for redo (see RecnoXLogDefrag). */
	nunused = 0;
	...
	if (RelationNeedsWAL(relation))
	{
		 ...
		 *
		 * The retired offsets must be logged: redo has to retire exactly the
		 * same line pointers before it compacts, or it compacts a different
		 * set of items and the replayed page diverges from this one.
		 */
		recptr = RecnoXLogDefrag(relation, buffer, unused_offsets, nunused,
								 oldest_ts);
				   ...
				   uint32 options, BulkInsertState bistate)
{
	...
	/*
	 * Assign a top-level xid BEFORE the critical section.  On current master
	 * GetCurrentTransactionId() -> XactLockTableInsert() -> LockAcquire() may
	 * palloc, which trips the CritSectionCount assertion if called inside
	 * START_CRIT_SECTION().  We need the xid so the WAL record can be decoded
	 * for logical replication (ReorderBuffer groups by xid).
	 */
	...
	/*
	 * Stamp the inserter (sub)XID and command id on the tuple, heap/FLUX
	 * shaped.  These land in the WAL tuple image and let RecnoTupleVisible
	 * resolve an uncommitted/aborted plain INSERT directly from CLOG +
	 * snapshot, so a plain INSERT needs NO per-row shared sLog entry.  Using
	 * GetCurrentTransactionId() (the current subxid, not the top xid) makes
	 * savepoint rollback show up as an aborted xid in CLOG, so the row
	 * becomes invisible with no shared ABORTED marker.
	 */
	recno_tuple->t_data->t_xmin = GetCurrentTransactionId();
	...
	target_block = RecnoGetPageWithFreeSpace(relation,
											 RecnoGetInsertFreeSpace(relation,
																 tuple_size));
...
have_page:
	...
	/*
	 * SSI: check for rw-conflict in.  Our INSERT may conflict with a
	 * concurrent serializable transaction that holds a relation- or
	 * page-level predicate lock covering this page -- for instance a range
	 * scan whose result would have included this new row.  The tuple does not
	 * exist yet, so pass NULL tid and let only the coarser granularities be
	 * checked.  Must be outside the critical section: it can ereport.
	 */
	CheckForSerializableConflictIn(relation, NULL, BufferGetBlockNumber(buffer));

	/*
	 * Add the tuple to the page BEFORE the critical section so the target
	 * offset (hence TID) is known for the per-backend UNDO prepare, which may
	 * palloc / extend the undo log (forbidden inside a crit section).
	 */
	offnum = RecnoPageAddTuple(page, recno_tuple, tuple_size);
	if (offnum == InvalidOffsetNumber)
		elog(ERROR, "failed to add RECNO tuple to page");
	ItemPointerSet(tid, BufferGetBlockNumber(buffer), offnum);

	/*
	 * Per-backend UNDO: prepare an INSERT record before the crit section.  An
	 * INSERT record carries only its target TID (no before-image); on rollback
	 * recno_undo_apply() marks the inserted tuple RECNO_TUPLE_DELETED.
	 */
	if (RelationHasUndo(relation))
	{
		RecnoUndoPayloadHeader ins_undo_hdr;

		ins_undo_hdr.tid = *tid;
		ins_undo_hdr.tuple_len = 0;
		ins_undo_hdr.flags = 0;
		ins_undo_hdr.pad = 0;
		(void) RecnoPbuPrepareInsert(RECNO_UNDO_INSERT,
									 RelationGetRelid(relation), tid,
									 (const char *) &ins_undo_hdr,
	...
	/*
	 * Assign a top-level xid before logging.  RECNO's MVCC does not
	 * use xmin/xmax, so until now we never needed one here -- but WAL
	 * records without an attached xid cannot be decoded into a logical
	 * replication stream (ReorderBuffer groups changes by xid and no
	 * commit record is ever emitted for InvalidTransactionId).  The xid
	 * is now acquired BEFORE the critical section (see top of function).
	 */

	/*
	 * Write the per-backend INSERT UNDO record into its pinned undo buffers
	 * BEFORE the forward-op WAL record: RecnoXLogInsert folds the undo pages
	 * onto its record as full-page images, so the undo bytes must already be
	 * on the page.
	 */
	if (RelationHasUndo(relation) && RecnoPbuUndoPending())
		RecnoPbuInsertUndo();

	/* Log the insertion with all overflow buffers (and folded undo) atomically */
	if (RelationNeedsWAL(relation))
	{
		...
		if (RecnoPbuUndoPending())
			RecnoPbuFinishUndo(recptr);
	}
	else if (RecnoPbuUndoPending())
	{
		/* Unlogged/temp: no forward WAL, so no LSN to stamp on the undo page. */
		RecnoPbuFinishUndo(InvalidXLogRecPtr);
	}
	...
	/*
	 * The per-backend INSERT UNDO record was prepared before the critical
	 * section and written+folded onto the forward WAL record inside it
	 * (RecnoPbuInsertUndo / RecnoPbuFinishUndo); nothing further to do here.
	 */
	 ...
	 *
	 * We do NOT create a full shared sLog entry here by default (that
	 * caused "out of shared memory" during bulk inserts with 100K+ rows).
	 * Instead, we record (tid, xid, subxid) in the per-backend local list
	 * only.  If a savepoint is rolled back, RecnoSLogRemoveBySubXid will
	 * find the matching local entries and create a shared sLog ABORTED
	 * entry at that time.
	 *
	...
	/*
	 * Caveat-2 (bulk-INSERT OOM) fix: a plain INSERT writes NO per-row
	 * shared sLog entry.  Read visibility for an uncommitted, aborted, or
	 * savepoint-rolled-back plain INSERT is resolved from the tuple's
	 * t_xmin (stamped above) against CLOG + the reader's snapshot in
	 * RecnoTupleVisible -- exactly like heap/FLUX, which write no INSERT
	 * undo/sLog record and rely on "xmin never commits".  The shared sLog
	 * hash is capped (RECNO_SLOG_MAX_ENTRIES); one entry per inserted row
	 * exhausted it at ~400k rows/statement ("out of shared memory").
	 *
	 * Speculative inserts (ON CONFLICT) still register a full sLog entry in
	 * recno_tuple_insert_speculative(): the arbiter needs the token and the
	 * SNAPSHOT_DIRTY in-progress-xid probe.  The lightweight local subxact
	 * tracking above is retained as a cheap secondary signal; savepoint
	 * rollback is authoritative via CLOG on the subxid stamped in t_xmin.
	 */
			   ...
			   bool wait, TM_FailureData *tmfd)
{
	 ...
	 * creates an rw-antidependency that may complete a dangerous structure.
	 * Must precede the critical section: it can ereport.
	 */
	CheckForSerializableConflictIn(relation, tid, BufferGetBlockNumber(buffer));

	/*
	 * Per-backend UNDO: prepare a DELETE record (carrying the full before-image)
	 * BEFORE the crit section so rollback can restore the tuple.
	 */
	if (RelationHasUndo(relation))
	{
		...
		del_undo_hdr.pad = 0;
		(void) RecnoPbuPrepareInsert(RECNO_UNDO_DELETE,
									 RelationGetRelid(relation), tid,
	...
	/* Write the per-backend DELETE undo before the forward WAL record. */
	if (RelationHasUndo(relation) && RecnoPbuUndoPending())
		RecnoPbuInsertUndo();
	...
	/*
	 * The per-backend DELETE undo record was prepared before the crit section
	 * and written+folded onto the forward WAL record inside it.
	 */
...
/*
 * recno_inplace_index_maintenance
 *
 * Phase 10 delete-marking index maintenance for an in-place indexed-column
 * UPDATE (stable TID).  For each affected index whose key changed: insert the
 * new (k_new, TID) entry (or revive a delete-marked tombstone) so new-snapshot
 * readers reach the row under its new key, and DELETE-MARK the old (k_old, TID)
 * entry (index_delete_mark) so old-snapshot readers still reach the
 * before-image under the old key.  recno_tuple_update then reports TU_None so
 * the executor does no further index work.  Modeled on
 * flux_inplace_index_maintenance.
 *
 * old_hdr is the before-image tuple header (materialized to form old keys);
 * newslot is the post-update slot (already positioned at the stable TID).
 */
static void
recno_inplace_index_maintenance(Relation relation, ItemPointer otid,
								RecnoTupleHeader *old_hdr, Size old_len,
								TupleTableSlot *newslot)
{
	...
	RecnoTupleToSlotWithOverflow(old_hdr, oldslot, relation);
			   ...
			   LockTupleMode *lockmode, TU_UpdateIndexes *update_indexes)
{
	...
	bool		deferred_inplace_index_maint = false;
	RecnoDiffRecord *upd_undo_diff = NULL;
	UndoRecPtr	upd_undo_ptr = InvalidUndoRecPtr;
	RecnoInlineDiff upd_inline_diff_data;
	uint64		defrag_oldest_ts = 0;
	bool		upd_serialize_locked = false;
	...
	/*
	 * LOST-UPDATE SERIALIZATION: acquire the heavyweight per-tuple lock
	 * (LOCKTAG_TUPLE, NoKeyExclusive) on the row we are about to modify BEFORE
	 * reading the page and deciding the update verdict, and HOLD it until this
	 * transaction commits.  This is the same transaction-duration row lock
	 * SELECT ... FOR UPDATE / recno_tuple_lock take (proven to conserve on
	 * RECNO), and it gives a stable-TID in-place AM heap's lost-update safety:
	 *
	 ...
	 *     both apply V+1; the second BLOCKS here until the first has committed
	 *     and released the lock (in RECNO's XACT_EVENT_COMMIT callback, after
	 *     ProcArrayEndTransaction), then re-reads the committed new version.
	 *
	 *   - It makes the winner observable as invisible-to-our-snapshot.  RECNO
	 *     stamps an in-place UPDATE with the transaction START HLC, not the
	 *     commit HLC, so a winner that started before us can compare "visible"
	 *     to our snapshot's HLC even though it committed AFTER our snapshot --
	 *     the HLC check alone would let us clobber it.  Because the winner held
	 *     this lock while in progress, it was in our MVCC snapshot's
	 *     in-progress set when the snapshot was taken; the committer-xid probe
	 *     (RecnoTupleHasCommittedUpdateAfter) below then correctly classifies
	 *     it as invisible -> TM_Updated -> the executor re-reads the latest
	 *     version at this stable TID and re-runs the SET expression against it.
	 *     Heap gets the same guarantee from the persistent xmax it stamps at
	 *     update time; RECNO has none, so this tuple lock + version-chain probe
	 *     supply it.
	 *
	 * We take it ONLY on the PLAIN UPDATE entry (no pre-existing self sLog LOCK
	 * marker).  The ON CONFLICT DO UPDATE arbiter (ExecOnConflictUpdate ->
	 * recno_tuple_lock, flags == 0) and the EvalPlanQual re-fetch
	 * (recno_tuple_lock, FIND_LAST_VERSION) both already registered a
	 * RECNO_SLOG_LOCK_* marker for our xid on this tid and re-enter here;
	 * re-acquiring underneath the arbiter's own tuple lock would manufacture
	 * lock-order deadlocks and route the arbiter into the new TM_Updated path
	 * (EPQ-refetch into a NULL ri_oldTupleSlot).  Detect the self-held marker
	 * and SKIP the acquire, exactly as the arbiter-safe FLUX path does.
	 *
	...
	if (wait)
	{
		...
		{
			RecnoSLogEntry self_lock;
			int			self_nfound;

			self_nfound = RecnoSLogLookup(RelationGetRelid(relation), otid,
										  my_xid, &self_lock, 1);
			if (self_nfound > 0 &&
				(self_lock.op_type == RECNO_SLOG_LOCK_SHARE ||
				 self_lock.op_type == RECNO_SLOG_LOCK_EXCL))
		...
		if (!self_holds_lock)
		{
			...
			if (serialize_lock)
			{
				RecnoTrackSerializeLock(relation->rd_locator.dbOid,
										relation->rd_locator.relNumber,
										blkno, offnum);
				upd_serialize_locked = true;
			}
		}
	}
	...
	if (snapshot)
	{
		...
		else if (upd_serialize_locked)
		{
			/*
			 * LOST-UPDATE ENFORCEMENT: the on-page image compared "visible" to
			 * our snapshot, but we serialized behind a concurrent updater on
			 * this row (we acquired the serialization tuple lock and it was
			 * contended -- see the FIFO block at entry).  RECNO stamps an
			 * in-place UPDATE with the transaction START HLC, so a winner that
			 * started before us yet committed AFTER our snapshot passes the HLC
			 * visibility check and we would silently clobber it with our
			 * stale-based new tuple.  Probe the version-chain committer against
			 * the core MVCC snapshot (heap's xmax semantics): if the last
			 * committer of this row is invisible to our snapshot and committed
			 * and is not us, we are the loser -- return TM_Updated so the
			 * executor runs EvalPlanQual, re-fetches the latest version at this
			 * stable TID (FIND_LAST_VERSION), and re-runs the SET expression.
			 *
			 * If the committer is still in progress (PRE_COMMIT window), wait
			 * for it to reach CLOG, then re-probe.  If the chain shows no
			 * committer other than ourselves (the holder we blocked on aborted
			 * or held only a lock and made no change) there is nothing to lose:
			 * fall through and apply.
			 *
			 * EPQ termination: a prior EPQ cycle's table_tuple_lock registered
			 * a RECNO_SLOG_LOCK_* marker for our xid on this tid.  If we hold
			 * that marker we have already re-evaluated the quals against the
			 * latest version -- fall through and apply, so a hot row cannot
			 * livelock (heap re-evaluates once, then applies).
			 */
			TransactionId my_xid_epq = GetCurrentTransactionIdIfAny();
			bool		already_epqd = false;

			if (TransactionIdIsValid(my_xid_epq))
			{
				RecnoSLogEntry my_lock;
				int			my_nfound;

				my_nfound = RecnoSLogLookup(RelationGetRelid(relation), otid,
											my_xid_epq, &my_lock, 1);
				if (my_nfound > 0)
					already_epqd = true;
			}
			...
			if (!already_epqd)
			{
				...
				if (RecnoTupleHasCommittedUpdateAfter(relation, old_tuple_hdr,
													  snapshot, my_xid_epq,
													  &head_xid,
													  &head_inprogress))
				{
					/* Committed conflict: force EPQ. */
					if (tmfd)
					{
						tmfd->ctid = *otid;
						tmfd->xmax = head_xid;
				...
				else if (head_inprogress && TransactionIdIsValid(head_xid) &&
						 !TransactionIdIsCurrentTransactionId(head_xid))
				{
					/*
					 * PRE_COMMIT window: the committer cleared its markers but
					 * has not yet reached CLOG.  Wait for it, then re-probe;
					 * whatever it resolved to (committed value, or restored
					 * before-image on abort) drives the verdict.
					 */
					...
					if (TransactionIdDidCommit(wait_xid))
					{
						if (tmfd)
						{
							tmfd->ctid = *otid;
					...
					/* Aborted: its value never existed -- fall through. */
				}
			}
		}
	}
	...
	update_overflow_buffers.count = 0;
	{
		...
		bool		try_fast = (old_tuple_hdr->t_len <= RECNO_OVERFLOW_THRESHOLD &&
		...
		if (try_fast)
		{
			...
			/*
			 * If a grown varlena pushed the fast-path tuple past what any
			 * page can hold, it can never fit in place and the overflow
			 * spill it needs is disabled on this path.  Discard it and drop
			 * to the slow path, which re-forms with overflow enabled
			 * (RECNO_MAX_TUPLE_SIZE oversized attributes spill to overflow
			 * pages exactly as the INSERT path does).  This is what closes
			 * the "updated recno tuple does not fit on page / overflow
			 * during update not yet implemented" gap for narrow rows that
			 * grow on UPDATE (e.g. a short text column SET to a large value).
			 */
			if (new_tuple->t_len > RECNO_MAX_TUPLE_SIZE)
			{
				pfree(new_tuple);
				new_tuple = NULL;
				try_fast = false;
	...
	{
		/*
		 * Strategy 3: reclaim space on this page and retry.  The row must grow
		 * HERE: RECNO keeps a stable TID, so unlike heap it cannot migrate the
		 * row to a roomier page and leave a forwarding ctid behind.  If we
		 * cannot make the bytes appear on this page the UPDATE fails, and
		 * because the row's width is a property of the row (not of this
		 * statement) it would keep failing -- the row would be permanently
		 * un-updatable.  So try the reclaim unconditionally rather than only
		 * when RECNO_PAGE_DEFRAG_NEEDED happens to be set: that flag is a
		 * fragmentation HINT maintained by delete/update, and a page can hold
		 * prunable dead tuples with the hint clear (it is cleared by defrag and
		 * by REDO, and never set at all for a page whose dead rows were left by
		 * a transaction that aborted).  Gating on the hint made us report "too
		 * large for the page" while the page still had reclaimable space.
		 */
		Size		potential_free;
		OffsetNumber df_off;
		OffsetNumber df_maxoff = PageGetMaxOffsetNumber(page);
		OffsetNumber df_unused[MaxOffsetNumber];
		int			df_nunused = 0;

		/*
		 * Estimate how much space defragmentation could free by scanning for
		 * dead tuples.  This is a quick scan without actually defragmenting.
		 */
		...
		{
			/*
			 * Defragmentation should free enough space.  Do it now. We are
			 * already holding an exclusive lock on the buffer. First mark dead
			 * tuples as unused, then defragment.
			 *
			 * defrag_oldest_ts was pre-computed before acquiring the buffer
			 * lock to avoid shared-memory scans while holding page-level
			 * exclusive locks.
			 */
			START_CRIT_SECTION();
			{
				...
				for (prune_off = FirstOffsetNumber; prune_off <= prune_maxoff; prune_off++)
				{
						...
						prune_hdr->t_commit_ts < defrag_oldest_ts)
					{
						...
						df_unused[df_nunused++] = prune_off;
					}
				}
			}
			RecnoPageDefragment(page);
			...
			if (RelationNeedsWAL(relation))
			{
				...
				/* redo must retire the same offsets before compacting */
				df_lsn = RecnoXLogDefrag(relation, buffer, df_unused,
										 df_nunused, defrag_oldest_ts);
			...
			/*
			 * Re-fetch the item after defragmentation since line pointers may
			 * have been reorganized.  The offset number should still be valid
			 * for surviving tuples.
			 */
			itemid = PageGetItemId(page, offnum);
			old_tuple_hdr = (RecnoTupleHeader *) PageGetItem(page, itemid);
		}

		/* Check again whether the in-place update now fits */
		...
		else
		{
			...
			/*
			 * The new tuple was already re-formed with overflow enabled
			 * (oversized varlenas spilled to overflow pages, as the INSERT path
			 * does), yet even after reclaiming the page the residual inline
			 * image still does not fit.  The row cannot grow at this TID.
			 * Abort cleanly, without corruption.
			 *
			 * The INSERT path holds back RECNO_DEFAULT_FILLFACTOR worth of each
			 * page precisely so ordinary in-place growth (e.g. a numeric
			 * counter widening from 8 to 10 bytes when it crosses a base-10000
			 * digit-group boundary) never reaches here; a page can only be this
			 * tight if the reserve was overridden by a fillfactor reloption or
			 * the row grew by more than the whole reserve at once.
			 */
			ereport(ERROR,
					(errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
					 errmsg("updated recno tuple is too large for the page"),
					 errdetail("Row needs %zu bytes; %zu bytes are available at this TID after reclaiming the page.",
							   (Size) new_tuple_size,
							   (Size) (ItemIdGetLength(itemid) + PageGetFreeSpace(page))),
					 errhint("Lower the table's fillfactor to reserve more in-page room for updates, or move large columns to overflow storage.")));
		}
	}
	 ...
	 * handles UNDO recording for all in-place updates.  The deferred
	 * approach is safe because the WAL record includes both old and new
	 * tuple data, enabling crash recovery regardless of UNDO write order.
	 */
	...
	/*
	 * BUG 1 fix: skip the inline-diff fast path when the relation maintains a
	 * per-backend UNDO version chain.  The inline diff stores only ONE step of
	 * history in the tuple header (enough for ROLLBACK) and stamps no t_verptr,
	 * so an old snapshot could not walk back a multi-version chain to its
	 * before-image.  Routing every in-place UPDATE through the full-tuple UNDO
	 * record (below) gives a uniform, correct read-reconstruction chain.
	 */
	if (old_tuple_for_inplace_wal->t_len == new_tuple->t_len &&
		!RelationHasUndo(relation))
	{
	...
	if (!upd_use_inline_diff && RelationHasUndo(relation))
		upd_has_undo = true;

	/*
	 * Always set UNCOMMITTED so that visibility checks consult the sLog.
	 * Even though the tuple position hasn't moved (in-place update), the DATA
	 ...
	 *
	 * t_verptr and RecnoReconstructVisibleVersion let an old (REPEATABLE READ)
	 * snapshot that predates a committed in-place UPDATE reconstruct the
	 * before-image instead of seeing the row as absent.  The per-update UNDO
	 * record therefore carries the FULL old-tuple before-image
	 * (RECNO_UNDO_UPDATE) whenever a version chain is maintained: the compact
	 * byte-diff (DELTA_UPDATE) is stamped from a diff computed before t_verptr
	 * is written, so reverse-applying it would leave the NEW verptr in the
	 * reconstructed image and break the chain.  DELTA still serves rollback for
	 * non-versioned relations.  Full-tuple UNDO uses more space than a byte
	 * diff; including the 8-byte t_verptr region in the diff would let DELTA
	 * seed the read chain too.
	 */
	new_tuple->t_data->t_flags |= RECNO_TUPLE_UNCOMMITTED;

	/*
	 * Per-backend UNDO: prepare the UPDATE record BEFORE the crit section.
	 * Carry the full old-tuple before-image (RECNO_UNDO_UPDATE) so the read
	 * path can walk the version chain (see the note above).  The
	 * returned UndoRecPtr becomes this new version's t_verptr.  Inline-diff
	 * updates need no UNDO log record (the old bytes ride in the tuple
	 * header) and do not seed a read chain.
	 */
	/*
	 * SSI: check for rw-conflict in.  If a concurrent serializable
	 * transaction read this row (holds a SIREAD lock on its TID), our UPDATE
	 * creates an rw-antidependency.  RECNO updates in place, so the TID we
	 * report is the same one a reader locked -- which is exactly why this call
	 * is what makes write skew detectable on RECNO.  Must precede the critical
	 * section: it can ereport.
	 */
	...
	if (upd_has_undo)
	{
		...
		upd_undo_hdr.pad = 0;
		upd_undo_ptr = RecnoPbuPrepareInsert(RECNO_UNDO_UPDATE,
		...
		/*
		 * Stamp the version-chain head into the new on-page image.  The
		 * before-image copied into the UNDO record above still carries the
		 * OLD tuple's own t_verptr, so the chain links back one step further
		 * on each reverse-apply -- terminating at the original INSERT (whose
		 * t_verptr is invalid).
		 */
		new_tuple->t_data->t_verptr = upd_undo_ptr;
	}
	...
	if (new_tuple_size <= ItemIdGetLength(itemid))
	{
		 ...
		 * allocation.
		 *
		 * We deliberately do NOT shrink the ItemId length when the new tuple
		 * is smaller: keeping the original slot length lets a ROLLBACK restore
		 * the (larger) before-image in place (recno_undo.c refuses to grow a
		 * slot that a later update shrank).  The tuple's own t_len records the
		 * true payload length, so the trailing bytes are harmless padding that
		 * VACUUM reclaims.  Matches the in-place-MVCC contract that the on-page
		 * slot never shrinks under an updatable version chain.
		 */
	...
	/* Write the per-backend UPDATE undo before the forward WAL record. */
	if (upd_has_undo && RecnoPbuUndoPending())
		RecnoPbuInsertUndo();
	...
	if (RelationNeedsWAL(relation))
	{
		 ...
		 */
		upd_recptr = RecnoXLogUpdate(relation, buffer, offnum,
	 ...
	 * abort processing — no UNDO log record needed.
	 *
	 ...
	 * than 50% of tuple size), use RECNO_UNDO_DELTA_UPDATE to save UNDO space.
	 * Otherwise fall back to RECNO_UNDO_UPDATE with the full old tuple.
	 */
	if (upd_use_inline_diff)
	{
	...
	{
		/*
		 * The per-backend UPDATE undo record was prepared before the crit
		 * section and written+folded onto the forward WAL record inside it
		 * (RecnoPbuInsertUndo / RecnoPbuFinishUndo).  Free the diff payload.
		 */
		if (upd_undo_diff != NULL)
		{
			pfree(upd_undo_diff);
			upd_undo_diff = NULL;
		}
	...
	/*
	 * Fix 1C: Compute index update strategy before freeing old_tuple_for_inplace_wal.
	 * The old tuple data is needed to compare old vs new indexed column values.
	 * Must happen here — old_tuple_for_inplace_wal is freed immediately below.
	 */
	if (update_indexes)
	{
		...
		/*
		 * Phase 10 delete-marking: RECNO updates always IN PLACE (stable TID).
		 * If an indexed column changed (hot_changed), RECNO maintains the
		 * secondary indexes ITSELF here -- insert the new (k_new,TID) entry and
		 * delete-mark the old (k_old,TID) entry -- then reports TU_None so the
		 * executor does no delete+reinsert (which would duplicate the stable
		 * (key,TID) pair).  nbtree/hash suppress their own index UNDO for a
		 * delete-marking parent; rollback index cleanup is driven by RECNO's
		 * table UNDO.  Summarizing (BRIN) indexes are refreshed by the executor
		 * when only summarizing columns changed.
		 */
		if (hot_changed)
		{
			/*
			 * Do NOT run the index maintenance here: we still hold this data
			 * page's content lock, and index_insert -> _bt_check_unique on a
			 * UNIQUE index calls back into the table AM (recno_fetch_tid ->
			 * recno_index_fetch_tuple) to visibility-check the conflicting
			 * TID.  When that TID lives on the page we are updating -- the
			 * common case, since RECNO updates in place and the conflict is
			 * usually a sibling row on the same page -- the callback blocks
			 * acquiring a lock this very backend holds, and the statement
			 * self-deadlocks forever (no other backend can release it).  It
			 * hung both `INSERT ... ON CONFLICT (key) DO UPDATE` and a plain
			 * `UPDATE` that moves a row onto an existing unique value, and the
			 * expected unique violation could never be raised because the
			 * checker could not read the tuple.
			 *
			 * Defer it to after UnlockReleaseBuffer below.  The maintenance
			 * needs only the palloc'd before-image copy and the (already
			 * materialized) new slot, not the pinned page.  This mirrors the
			 * RecnoSLogInsert call, which is likewise deferred past the unlock
			 * to avoid a deadlock.
			 */
			deferred_inplace_index_maint = true;
	...
	for (upd_i = 0; upd_i < update_overflow_buffers.count; upd_i++)
	{
		...
		bool		is_main = (ovf_buf == buffer);
		bool		already_released = is_main;
		int			dup_j;
		...
		{
			/*
			 * The overflow spill reused the block we are updating: it was
			 * ReadBuffer'd a second time (into this overflow slot) on top of
			 * the pin `buffer` already holds, and shares the single buffer
			 * lock (the LockBuffer of the re-acquire was skipped when
			 * buffer_already_locked was detected).  Drop only that duplicate
			 * pin here; the shared lock and the primary pin stay for the
			 * final UnlockReleaseBuffer(buffer) below.  Without this the pin
			 * leaks ("resource was not closed: block 0 refcount=1").
			 */
			ReleaseBuffer(ovf_buf);
		}
	 ...
	 */
	{
		...
		/*
		 * Now that the data page's content lock is released, run the deferred
		 * Phase 10 delete-marking index maintenance.  _bt_check_unique's
		 * recno_index_fetch_tuple callback can now take the page lock itself.
		 */
		if (deferred_inplace_index_maint)
			recno_inplace_index_maintenance(relation, otid,
											old_tuple_for_inplace_wal->t_data,
											old_tuple_for_inplace_wal->t_len,
											slot);

		/* Free the before-image copy (needed by the maintenance above). */
	 ...
	 *
	 * Since RECNO always updates in-place, the TID never changes. B-tree
	 * and similar indexes store (key, TID) pairs and remain valid after an
	 * in-place update — the entries still point to the correct TID.
	 *
	 * We return TU_Summarizing, which tells the executor to:
	 *   - SKIP delete+reinsert for non-summarizing indexes (B-tree, hash, etc.)
	 ...
	 * would delete the b-tree entry and then try to reinsert the same
	 * (key, TID) pair — failing the unique constraint check.
	 *
	 * TODO: In future, compare old vs new indexed column values to return
	 * TU_None when no summarizing columns changed (optimization only).
	 */
	if (update_indexes)
	pfree(new_tuple);
				   ...
				   CommandId cid, uint32 options, BulkInsertState bistate)
{
	...
	while (ndone < ntuples)
	{
		...
		/* Find a page with space for at least one tuple (plus UPDATE reserve) */
		target_block = RecnoGetPageWithFreeSpace(relation,
												 RecnoGetInsertFreeSpace(relation,
		...
		/*
		 * SSI: check for rw-conflict in, once per page-batch.  The tuples do
		 * not exist yet, so only the relation- and page-level granularities
		 * can match, and those are the same for every tuple landing on this
		 * page -- one check covers the batch.  Must precede the critical
		 * section: it can ereport.
		 */
		CheckForSerializableConflictIn(relation, NULL, target_block);
		...
		/*
		 * Insert as many tuples as fit on this page, each still leaving the
		 * per-page in-place UPDATE reserve intact (RecnoGetInsertFreeSpace).
		 * Without the reserve here the batch path would pack pages to 100%%
		 * and re-create the un-updatable-row error that the reserve exists to
		 * prevent -- COPY-loaded tables are exactly where that hurts most.
		 */
		while (ndone < ntuples &&
			   !needs_single_insert[ndone] &&
			   PageGetFreeSpace(page) >=
			   RecnoGetInsertFreeSpace(relation, formed_tuples[ndone]->t_len))
		{
			OffsetNumber offnum;
						   ...
						   BufferAccessStrategy bstrategy, bool verbose)
{
		 ...
		 src_blkno--)
	{
			 ...
			 offnum = OffsetNumberNext(offnum))
		{
			 ...
			 * truncation.
			 *
			 * Ask for the relocated row PLUS the per-page in-place UPDATE
			 * reserve (RecnoGetInsertFreeSpace).  Relocating a row is an
			 * insert into the target page, so it has to respect the reserve
			 * for the same reason INSERT does: RECNO keeps a stable TID, so a
			 * later UPDATE that widens a row (e.g. a numeric counter crossing
			 * a base-10000 digit-group boundary) must find the extra bytes on
			 * the row's own page or fail permanently.  Compacting to 100%% here
			 * would hand back space-efficient pages full of rows that can
			 * never be updated again -- observed as VACUUM repacking a
			 * district table from 73 to 80 rows/page and turning a clean
			 * 2.2M-transaction hot-UPDATE run into an immediate storm of
			 * "updated recno tuple is too large for the page".
			 */
			dst_blkno = GetPageWithFreeSpace(rel,
											 RecnoGetInsertFreeSpace(rel, tuple_len));
					  ...
					  BufferAccessStrategy bstrategy)
{
	...
	for (blkno = 0; blkno < nblocks; blkno++)
	{
		...
		{
			OffsetNumber vac_unused[MaxOffsetNumber];
			int			vac_nunused;
			 ...
			 * already identified them; now set LP_UNUSED.  The offsets are
			 * collected so RecnoXLogDefrag can log them: redo must retire the
			 * same line pointers before it compacts, or the replayed page
			...
			if (RelationNeedsWAL(onerel))
			{
				...
				/* redo must retire the same offsets before compacting */
				recptr = RecnoXLogDefrag(onerel, buf, vac_unused, vac_nunused,
										 oldest_ts);
				PageSetLSN(page, recptr);
			}
	 ...
	 *
	 * Under UNDO-in-WAL, per-relation UNDO records live in the shared
	 * UNDO log and are discarded asynchronously by the undo discard worker
	 * once no active transaction needs them for visibility.  VACUUM has
	 * nothing RECNO-specific to do here beyond the phases above.
	 * -----------------------------------------------------------------------
	 */
