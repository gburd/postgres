					...
					cas_new_size == ItemIdGetLength(itemid))
				{
					...
					if (RecnoTupleWriterTryLock(old_tuple_hdr, &expected))
					{
						 ...
						 */
						{
							...
							if (RelationNeedsWAL(relation))
							{
								RecnoUndoPayloadHeader cas_undo_hdr;
								XactUndoContext cas_undo_ctx;

								cas_undo_hdr.tid = slot->tts_tid;
								cas_undo_hdr.tuple_len = cas_old_len;
								cas_undo_hdr.flags = RECNO_UNDO_FLAG_HAS_TUPLE;
								cas_undo_hdr.pad = 0;

								PrepareXactUndoDataParts(&cas_undo_ctx,
														 relation->rd_rel->relpersistence,
														 UNDO_RMID_RECNO,
														 RECNO_UNDO_UPDATE,
														 RelationGetRelid(relation),
														 (const char *) &cas_undo_hdr,
														 SizeOfRecnoUndoPayloadHeader,
														 cas_old_copy,
														 cas_old_len);
								InsertXactUndoData(&cas_undo_ctx);
								CleanupXactUndoInsertion(&cas_undo_ctx);
							}
