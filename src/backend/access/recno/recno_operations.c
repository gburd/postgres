				   ...
				   uint32 options, BulkInsertState bistate)
{
	...
	/*
	 * Assign a top-level xid now, BEFORE entering the critical section.
	 * RECNO's MVCC does not use xmin/xmax, so the AM itself never needs
	 * an xid -- but WAL records written without one cannot be decoded
	 * into a logical replication stream (ReorderBuffer groups changes by
	 * xid and never emits a commit record for InvalidTransactionId).
	 *
	 * This must happen outside the critical section because
	 * GetCurrentTransactionId() may allocate memory to grow the xid
	 * stack, which trips the CritSectionCount assertion in palloc.
	 */
...
have_page:
	 ...
	 */
	/*
	 * Per-tuple UNDO record.  Written via the shared UNDO-in-WAL API:
	 * if a write buffer is active for this relation, the record joins
	 * the batch; otherwise a one-shot XactUndoContext is used.  Apply
	 * at abort time is dispatched via UNDO_RMID_RECNO to recno_undo.c.
	 */
	...
	/*
	 * Write a full shared sLog entry for the INSERT.  This is what makes
	 * aborted-INSERT rollback correctly invisible: the abort callback
	 * transitions the entry to ABORTED and MVCC visibility
	 * (recno_mvcc.c's RECNO_TUPLE_UNCOMMITTED path) then hides the row
	 * from every snapshot.
	 *
	 * This was historically gated on RelationHasUndo(rel) on the theory
	 * that visibility of non-UNDO tables' aborts could be handled by the
	 * lighter-weight RecnoSLogTrackSubXact() path above, plus synchronous
	 * UNDO apply on abort.  Under UNDO-in-WAL the synchronous-apply path
	 * is unsafe (BumpContext), so visibility has to come from the sLog
	 * whether or not the relation opted into physical UNDO.  Gating this
	 * entry on enable_undo regressed recno_undo_redo and a dozen other
	 * regression tests.
	 *
	 * The bulk-INSERT shared-memory cost still needs a mitigation
	 * (per-backend buffering that batches into sLog at transaction
	 * boundaries); until that lands, very large single-transaction
	 * INSERTs should tune `shared_memory` / sLog partitions.  The
	 * lightweight subxact tracking above remains in place as a
	 * secondary fast path for savepoint rollback.
	 */
	...
	update_overflow_buffers.count = 0;
	{
			...
			!(old_tuple_hdr->t_flags & RECNO_TUPLE_HAS_OVERFLOW))
		{
			...
			new_tuple = RecnoFormTuple(RelationGetDescr(relation),
									   slot->tts_values,
									   slot->tts_isnull,
									   relation,	/* enable per-rel compression */
	...
	if (update_indexes)
	{
		...
		if (hot_attrs != NULL || sum_attrs != NULL)
		{
			...
			for (i = 1; i <= nattrs; i++)
			{
				...
				if (!bms_is_member(attidx, hot_attrs) &&
					!bms_is_member(attidx, sum_attrs))
					continue;
				...
				if (attr_changed)
				{
					if (bms_is_member(attidx, hot_attrs))
						hot_changed = true;
					if (bms_is_member(attidx, sum_attrs))
						sum_changed = true;
				}
				if (hot_changed)
					break;
