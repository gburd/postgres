				 ...
				 uint32 flags)
{
	...
	if (recno_use_hlc)
	{
		...
		/*
		 * READ COMMITTED: use current HLC (per-statement snapshot) so that
		 * before-image serving only applies to genuinely concurrent updates.
		 * REPEATABLE READ / SERIALIZABLE: use transaction-start HLC for a
		 * consistent point-in-time view across all statements.
		 */
		if (IsolationUsesXactSnapshot())
			hlc = RecnoGetTransactionHLC();
		else
			hlc = HLCNow(0);
	...
	for (;;)
	{
		...
		while (scan->rs_cindex < scan->rs_ntuples)
		{
				...
				scan->rs_snapshot_hlc != InvalidHLCTimestamp)
			{
				...
				{
					/*
					 * Serve the before-image instead of on-page data. bi_data
					 * is palloc'd; ownership transfers to the slot (freed
					 * when slot is cleared).
					 */
					RecnoTupleHeader *bi_tuple;

					bi_tuple = (RecnoTupleHeader *) bi_data;
		...
		LockBuffer(rscan->rs_cbuf, BUFFER_LOCK_SHARE);
		{
			...
			if (!tbmres.lossy)
			{
				...
				for (int j = 0; j < noffsets; j++)
				{
					...
					/* Check visibility (handles DELETED via sLog) */
					if (scan->rs_snapshot &&
	...
	if (offnum == InvalidOffsetNumber)
	{
		...
		if (offnum == InvalidOffsetNumber)
			elog(ERROR, "failed to add RECNO tuple to fresh page during "
				 "speculative insert (tuple_size=%u)", tuple_size);
	}
								   ...
								   MultiXactId *minmulti)
{
	...
	*minmulti = GetOldestMultiXactId();
	...
	/*
	 * Under UNDO-in-WAL, RECNO does not own a per-relation UNDO fork. UNDO
	 * records are written into the shared UNDO log via UndoBuffer* /
	 * PrepareXactUndoData, tagged with UNDO_RMID_RECNO. UNDO is always
	 * enabled; no on-disk fork initialisation is needed at CREATE TABLE /
	 * TRUNCATE time.
	 */
}
							 ...
							 double *allvisfrac)
{
	...
	*tuples = Max(tuple_count, 0);
	 ...
	 * each data block (N calls for N pages), which opened/closed the VM
	 * buffer N times even though all data blocks typically fit in 1-2 VM
	 * pages. For scale=10 pgbench_accounts (10,417 pages), this caused 10,417
	 * buffer pin/unpin operations during query planning, costing ~40ms per
	 * query.
	 *
	 * Replacement: read each VM page ONCE, count ALL_VISIBLE bits in bulk
	 * using pg_popcount, then scale to nblocks.  O(vmPages) instead of
	 * O(nblocks).
	 */
	{
		...
		if (vmBlocks > 0)
		{
			...
			/*
			 * HEAPBLOCKS_PER_PAGE = MAPSIZE * 4 — we count 2 bits per data
			 * block
			 */
...
#define RECNO_HEAPBLKS_PER_VM_PAGE (RECNO_VM_MAPSIZE * 4)
			...
			for (mapBlock = 0; mapBlock < vmBlocks; mapBlock++)
			{
				...
				bytesNeeded = (blksInPage + 3) / 4; /* 2 bits per block, 4
													 * blocks per byte */

				/*
				 * Count set ALL_VISIBLE bits (bit 0 of each 2-bit pair). Each
				 * byte holds 4 data blocks: bits [1:0], [3:2], [5:4], [7:6].
				 * Count bytes where each 2-bit group has bit 0 set. Shortcut:
				 * use pg_popcount on masked bytes.
				 */
							 ...
							 TupleTableSlot *slot)
{
	...
	for (;;)
	{
		...
		if (OffsetNumberIsValid(tupoffset))
		{
			 ...
			 * use the dual-mode visibility function which handles
			 * DELETED/UPDATED tuples via sLog consultation.
			 */
			if (tuple_hdr->t_flags & RECNO_TUPLE_SPECULATIVE)
				visible = false;
...
static const TableAmRoutine recno_methods = {
	 ...
	 * (UNDO_RMID_RECNO).  UNDO records written from recno_operations.c are
	 * dispatched at rollback time to the recno_undo_apply callback registered
	 * by RecnoUndoRmgrInit().
	 *
