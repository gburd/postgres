		...
		while (scan->rs_cindex < scan->rs_ntuples)
		{
												 ...
												 scan->rs_cbuf))
			{
												   ...
												   &bi_data, &bi_len))
				{
					...
					RecnoSlotSetRowID(slot, &slot->tts_tid, bi_tuple->t_gen);
					return true;
				}
			...
			ItemPointerSet(&slot->tts_tid, scan->rs_cblock, offnum);
			RecnoSlotSetRowID(slot, &slot->tts_tid, tuple_header->t_gen);
							 ...
							 uint64 *exact_pages)
{
	...
	for (;;)
	{
			...
			rscan->rs_cindex < rscan->rs_ntuples)
		{
			...
			/*
			 * RECNO updates in place keeping the same TID, so a changed indexed
			...
			slot->tts_tableOid = RelationGetRelid(scan->rs_rd);
			ItemPointerSet(&slot->tts_tid, rscan->rs_cblock, offnum);
			RecnoSlotSetRowID(slot, &slot->tts_tid, tuple_hdr->t_gen);
						...
						bool *call_again, bool *all_dead)
{
	 ...
	 * RECNO does in-place updates and never sets call_again, so each TID is
	...
	*call_again = false;
	 ...
	 * recheck.  RECNO updates in place keeping the same TID, so a changed
	 * indexed column leaves both (oldkey -> tid) and (newkey -> tid) in the
	 * index pointing at the one live tuple.  We set this flag below only when
	 * the tuple we return is a committed in-place UPDATE; for the
...
visibility_done:
	...
	if (!visible)
	{
														 ...
														 ItemIdGetLength(itemid))))
		{
											   ...
											   &bi_data, &bi_len))
			{
				...
				RecnoSlotSetRowID(tts, &tts->tts_tid, bi_tuple->t_gen);
				ReleaseBuffer(buffer);
	...
	{
		RecnoSlotStoreTuple(tts, tuple_hdr, ItemIdGetLength(itemid), buffer);
		tts->tts_tableOid = RelationGetRelid(rel);
		tts->tts_tid = *tid;
		RecnoSlotSetRowID(tts, &tts->tts_tid, tuple_hdr->t_gen);
	...
	/*
	 * Materialize while the page is still pinned: RecnoTupleToSlotWithOverflow
...
reacquire:
	...
	if (slot && result == TM_Ok)
	{
		...
		/*
		 * RecnoTupleToSlotWithOverflow leaves the slot virtual with
							 ...
							 TableScanDesc scan)
{
	...
	while (table_scan_getnextslot(scan, ForwardScanDirection, slot))
	{
		...
		/*
		 * Call the AM's callback with the tuple's own TID and full RowID.
		 * For a wider-than-TID identity (RECNO's (TID, gen)) the index AM
		 * needs the suffix so equal-key A->B->A versions stay distinct; the
		 * width-6 heap case passes rowid.len==0 and the callback ignores it.
		 */
 ...
 *
 * This mirrors heapam_modified_attrs (see heapam_handler.c): the input
 * 'attrs' bitmap is modified in place (bms_del_member may pfree it) and the
 * surviving set is returned; the caller must use only the returned pointer.
 * The per-attribute comparison is a value-identity test (datum_image_eq)
 * expressed against the executor's old/new TupleTableSlots.
 */
static Bitmapset *
recno_modified_attrs(Relation rel, Bitmapset *attrs,
					 TupleTableSlot *oldslot, TupleTableSlot *newslot)
{
	...
	while ((attidx = bms_next_member(attrs, attidx)) >= 0)
	{
		 ...
		 * other than tableOID; we cannot expect these to be consistent for a
		 * row that was updated in place, or even to be set correctly yet in
		 * the new slot.
		 */
		if (attrnum < 0)
		{
		...
		/*
		 * Compare with datum_image_eq: these are the logical, in-memory values
		 * of the old/new row in the executor's slots, before any storage path
		 * has run, so the image-identity test is the right notion of "did this
		 * attribute change?" and erring toward "changed" is the safe direction.
		 */
...
/*
 * ------------------------------------------------------------------------
 * RECNO RowID descriptor: index row identity is (stable TID, generation).
 *
 * RECNO keeps a tuple physically in place across every UPDATE, so its TID is
 * reused across row versions.  The 4-byte t_gen makes an A->B->A recurrence
 * of an indexed key value on that one physical TID produce DISTINCT index
 * RowIDs, so nbtree never holds two identical (key, TID) entries.  Width is
 * 10 = 6-byte ItemPointerData + 4-byte uint32 generation.
 * ------------------------------------------------------------------------
 */
static int32
recno_rowid_compare(const uint8 *a, const uint8 *b)
{
	int32		cmp;
	uint32		gena;
	uint32		genb;

	cmp = ItemPointerCompare((ItemPointer) a, (ItemPointer) b);
	if (cmp != 0)
		return cmp;

	memcpy(&gena, a + sizeof(ItemPointerData), sizeof(uint32));
	memcpy(&genb, b + sizeof(ItemPointerData), sizeof(uint32));
	if (gena < genb)
		return -1;
	if (gena > genb)
		return 1;
...
static const RowIDType RecnoRowIDType = {
	.width = sizeof(ItemPointerData) + sizeof(uint32),
	.cmp = recno_rowid_compare,
};

static const RowIDType *
recno_index_row_key_type(Relation rel)
{
	return &RecnoRowIDType;
}
...
static const TableAmRoutine recno_methods = {
	...
	.modified_attrs = recno_modified_attrs,
	...
	.index_row_key_type = recno_index_row_key_type,
};
							  ...
							  TupleTableSlot *slot)
{
	...
	for (; rscan->rs_cindex <= maxoffset; rscan->rs_cindex++)
	{
		...
		if (sample_it)
		{
			...
			RecnoSlotSetRowID(slot, &slot->tts_tid, tuple_copy->t_gen);
