				 ...
				 uint32 flags)
{
	...
	/*
	 * SSI: a sequential or sample scan reads the whole relation, so take one
	 * relation-level SIREAD lock instead of a lock per tuple.  This is what
	 * makes a concurrent INSERT conflict with a range we read but that did
	 * not exist yet (a phantom).  Bitmap and TID-range scans are deliberately
	 * excluded and take finer-grained locks on the tuples they actually
	 * touch, exactly as heapam.c's scan_begin does.
	 *
	 * SerializationNeededForRead() inside PredicateLockRelation filters out
	 * non-serializable transactions and non-MVCC snapshots, so this is a
	 * no-op outside SERIALIZABLE.
	 */
	if (snapshot != NULL && (flags & (SO_TYPE_SEQSCAN | SO_TYPE_SAMPLESCAN)))
...
recno_prepare_pagescan(RecnoScanDesc scan, Buffer buffer)
{
		 ...
		 offnum = OffsetNumberNext(offnum))
	{
		...
		{
			/*
			 * SSI: this tuple is not visible to us.  If a concurrent
			 * transaction wrote it we are reading around that write, which is
			 * an rw-conflict out (me --rw--> writer).  Report it while we
			 * still hold the page's share lock.
			 */
			RecnoCheckForSerializableConflictOut(false, scan->rs_base.rs_rd,
												tuple_header, buffer,
												scan->rs_base.rs_snapshot);

			/*
			 * BUG 1: the on-page (newest) image is not visible to our snapshot.
			 * If this row was updated in place and still has a version chain
			 * (t_verptr valid), an OLDER version may be visible -- keep it as a
			 * candidate so recno_scan_getnextslot can reconstruct and serve the
			 * before-image.  Only truly-invisible rows are dropped here.
			 */
			if (IsMVCCSnapshot(scan->rs_base.rs_snapshot) &&
				UndoRecPtrIsValid(tuple_header->t_verptr))
			{
		...
		/*
		 * SSI: take a SIREAD lock on the visible tuple so a concurrent writer
		 * that modifies it detects the rw-antidependency against us.  For a
		 * seq scan the relation-level lock taken in recno_scan_begin already
		 * covers this and PredicateLockTID becomes a no-op; for a bitmap scan
		 * (which shares this page loop but takes no relation lock) it is the
		 * lock that makes SSI work at all.
		 *
		 * Also report a conflict out: the tuple is visible to us, but a
		 * concurrent transaction may still be deleting or updating it.
		 */
		if (scan->rs_base.rs_snapshot)
		{
			ItemPointerData vis_tid;

			ItemPointerSet(&vis_tid, scan->rs_cblock, offnum);
			PredicateLockTID(scan->rs_base.rs_rd, &vis_tid,
							 scan->rs_base.rs_snapshot,
							 tuple_header->t_xmin);

			RecnoCheckForSerializableConflictOut(true, scan->rs_base.rs_rd,
												tuple_header, buffer,
												scan->rs_base.rs_snapshot);
		}
...
recno_scan_getnextslot(TableScanDesc sscan, ScanDirection direction, TupleTableSlot *slot)
{
	...
	for (;;)
	{
		...
		while (scan->rs_cindex < scan->rs_ntuples)
		{
			...
			/*
			 * BUG 1 (old-snapshot read): recno_prepare_pagescan kept two kinds
			 * of offsets -- ones visible on-page, and ones NOT visible on-page
			 * but carrying a version chain (t_verptr).  For the latter, walk the
			 * per-backend UNDO chain and serve the before-image the reader's
			 * snapshot can see (RecnoReconstructVisibleVersion, mirroring
			 * flux_pvs.c).  Re-check visibility here because the page is only
			 * pinned (not locked) now; reconstruction reads the UNDO log, not
			 * the page.
			 */
			if (scan->rs_base.rs_snapshot &&
												 ...
												 scan->rs_cbuf))
			{
				...
				if (RecnoReconstructVisibleVersion(scan->rs_base.rs_rd,
												   &item_tid,
												   ...
												   &bi_data, &bi_len))
				{
					RecnoSlotStoreMaterializedTuple(slot,
													(RecnoTupleHeader *) bi_data,
													(uint32) bi_len);
					ItemPointerSet(&slot->tts_tid, scan->rs_cblock, offnum);
					return true;
				}

				/* No visible older version: skip this row. */
				continue;
			}
		...
		for (offnum = scan->rs_coffset; offnum <= maxoff; offnum++)
		{
			...
			if (sscan->rs_snapshot &&
				!RecnoTupleVisibleToSnapshotDual(tuple_header,
												 ...
												 buffer))
			{
				/* SSI: we read around a concurrent writer. */
				RecnoCheckForSerializableConflictOut(false, sscan->rs_rd,
													tuple_header, buffer,
													sscan->rs_snapshot);
				continue;
			}
			...
			/*
			 * SSI: a TID-range scan takes no relation-level lock (see
			 * recno_scan_begin), so lock each visible tuple it returns and
			 * report any concurrent writer.
			 */
			if (sscan->rs_snapshot)
			{
				ItemPointerData tr_tid;

				ItemPointerSet(&tr_tid, block, offnum);
				PredicateLockTID(sscan->rs_rd, &tr_tid, sscan->rs_snapshot,
								 tuple_header->t_xmin);
				RecnoCheckForSerializableConflictOut(true, sscan->rs_rd,
													tuple_header, buffer,
													sscan->rs_snapshot);
			}
							 ...
							 uint64 *exact_pages)
{
	...
	for (;;)
	{
			...
			rscan->rs_cindex < rscan->rs_ntuples)
		{
			...
			/*
			 * BUG 1 (old-snapshot read via bitmap): if the on-page image is no
			 * longer visible because of a committed in-place UPDATE but a
			 * version chain exists, reconstruct and serve the before-image.
			 */
			if (scan->rs_snapshot &&
				IsMVCCSnapshot(scan->rs_snapshot) &&
				UndoRecPtrIsValid(tuple_hdr->t_verptr) &&
												 ...
												 rscan->rs_cbuf))
			{
				...
				if (RecnoReconstructVisibleVersion(scan->rs_rd, &item_tid,
		...
		LockBuffer(rscan->rs_cbuf, BUFFER_LOCK_SHARE);
		{
			...
			if (!tbmres.lossy)
			{
				...
				for (int j = 0; j < noffsets; j++)
				{
					...
					{
						/*
						 * BUG 1: keep an in-place-updated row that still has a
						 * version chain (t_verptr) as a candidate so the serve
						 * loop can reconstruct the before-image for an old
						 * snapshot; drop only truly-invisible rows.
						 */
						if (IsMVCCSnapshot(scan->rs_snapshot) &&
							UndoRecPtrIsValid(tuple_hdr->t_verptr))
...
static bool recno_index_getnext_slot(IndexScanDesc scan,
									 ScanDirection direction,
									 TupleTableSlot *slot);
static bool recno_index_fetch_tuple(IndexScanRecnoData *scan,
									ItemPointer tid,
									Snapshot snapshot,
									TupleTableSlot *tts,
...
/*
 * Prepare table-AM state for a RECNO index scan (slot-based interface, added
 * by upstream ddce1da5b1b).  Allocates RECNO's private IndexScanRecnoData into
 * scan->xs_table_opaque and resolves the getnext_slot implementation.
 *
 * RECNO enables in-place delete-marking (am_index_delete_marking = true), so a
 * plain index scan can reach a heap TID through an entry whose key no longer
 * matches the visible version.  recno_index_getnext_slot rechecks the entry
 * key against the visible tuple and skips on mismatch (Phase 8c invariant I2),
 * so we ask the index AM to keep the index tuple (xs_want_itup) for the whole
 * scan.  The planner suppresses genuine index-only scans on a delete-marking
 * table, so xs_want_itup here always means "plain scan that wants xs_itup for
 * the recheck", never a true index-only scan.
 */
static void
recno_index_scan_begin(IndexScanDesc scan, uint32 flags)
{
	IndexScanRecnoData *rscan = palloc0_object(IndexScanRecnoData);

	rscan->rel = scan->heapRelation;
	rscan->buffer = InvalidBuffer;
	rscan->all_dead = false;

	/*
	 * Ask the index AM to keep xs_itup for the delete-marking key recheck via
	 * xs_want_itup_recheck (a plain buffer-slot scan), NOT xs_want_itup (which
	 * is reserved for genuine index-only scans; see flux_index_scan_begin).
	 */
	if (RelationSupportsDeleteMarking(scan->heapRelation))
		scan->xs_want_itup_recheck = true;

	scan->xs_getnext_slot = recno_index_getnext_slot;
	scan->xs_table_opaque = rscan;
}

static void
recno_index_scan_reset(IndexScanDesc scan)
{
	IndexScanRecnoData *rscan = (IndexScanRecnoData *) scan->xs_table_opaque;
...
static void
recno_index_scan_end(IndexScanDesc scan)
{
	IndexScanRecnoData *rscan = (IndexScanRecnoData *) scan->xs_table_opaque;
 ...
 * point upstream ddce1da5b1b split out of the index-scan machinery.  RECNO has
 * no HOT chains, so this is a single fetch through the same visibility logic
 * the index scan uses.
 */
static bool
recno_fetch_tid(Relation rel, ItemPointer tid, Snapshot snapshot,
				bool *all_dead)
{
	IndexScanRecnoData rscan;
	TupleTableSlot *slot;
	...
	found = recno_index_fetch_tuple(&rscan, tid, snapshot, slot,
									&call_again, all_dead);
	if (BufferIsValid(rscan.buffer))
		ReleaseBuffer(rscan.buffer);
 ...
 * into `slot` for the scan's snapshot (Phase 8c invariant I2).  See the
 * matching helper in flux_handler.c for the rationale.  Returns true on
 * mismatch (caller must skip this entry).  Moved here from indexam.c
 * index_fetch_heap() when upstream ddce1da5b1b put the whole index-scan
 * heap-fetch loop under the table AM.
 */
static bool
recno_index_entry_key_mismatch(IndexScanDesc scan, TupleTableSlot *slot)
{
	IndexInfo  *ii;
	EState	   *estate;
...
/*
 * xs_getnext_slot callback for RECNO (slot-based table-AM index scan
 * interface).  Single entry point the executor reaches through
 * table_index_getnext_slot(): drives the index AM (amgettuple via
 * tableam_index_getnext_tid) and, for each TID, fetches and visibility-checks
 * the RECNO tuple into the caller's slot, applying the delete-marking key
 * recheck.  Mirrors heapam_index_getnext_slot's structure, minus HOT chains
 * (RECNO does in-place, stable-TID updates and never sets call_again).
 *
 * kill_prior_tuple is driven from the fetch's all_dead exactly as the old
 * indexam.c index_fetch_heap did (kill_prior_tuple = all_dead when not in
 * recovery), preserving RECNO's pre-existing scan-level all_dead semantics.
 */
static bool
recno_index_getnext_slot(IndexScanDesc scan, ScanDirection direction,
						 TupleTableSlot *slot)
{
	IndexScanRecnoData *rscan = (IndexScanRecnoData *) scan->xs_table_opaque;
	...
	for (;;)
	{
		...
		found = recno_index_fetch_tuple(rscan, &scan->xs_heaptid,
										scan->xs_snapshot, slot,
										&call_again, &all_dead);

		/*
		 * Mirror old indexam.c index_fetch_heap: set kill_prior_tuple from
		 * all_dead unless in recovery.  (All-dead entries can be reclaimed by
		 * the index AM; a found/visible tuple leaves all_dead false.)
		 */
		...
		if (scan->xs_itup != NULL &&
			...
			recno_index_entry_key_mismatch(scan, slot))
		{
			/*
			 * Stale entry does not describe the visible version.  Do NOT let
			 * it count toward kill_prior_tuple (an older snapshot legitimately
			 * needs it).  Fetch the next TID.
			 */
...
/*
 * Fetch the RECNO tuple at `tid` into `tts` after a visibility test according
 * to `snapshot`; returns true if a visible version was produced.  Called only
 * from recno_index_getnext_slot (the xs_getnext_slot callback) and from
 * recno_fetch_tid.
 *
 * *call_again is always left false (RECNO does in-place, stable-TID updates:
 * no HOT-style chain to keep walking).  *all_dead preserves RECNO's
 * pre-existing scan-level semantics: it is seeded from and updated in the
 * IndexScanRecnoData.all_dead field.
 */
static bool
recno_index_fetch_tuple(IndexScanRecnoData *scan,
						ItemPointer tid,
						Snapshot snapshot,
						TupleTableSlot *tts,
						bool *call_again, bool *all_dead)
	...
	*call_again = false;
	...
	if (snapshot)
	{
		if (snapshot->snapshot_type == SNAPSHOT_DIRTY)
		{
			...
			{
				/*
				 * Caveat-2: a plain INSERT stamps the inserter (sub)xid in
				 * t_xmin and writes no shared sLog entry, so SNAPSHOT_DIRTY
				 * uniqueness checks (_bt_check_unique) must resolve the
				 * inserter from t_xmin against CLOG here -- otherwise an
				 * aborted bulk INSERT's index tuples would be treated as live
				 * and re-inserting the same key would wrongly fail with a
				 * duplicate-key error.  Speculative inserts leave t_xmin
				 * invalid (0) and fall through to the sLog path below, which
				 * carries the speculative token.
				 */
				TransactionId hint_xid = tuple_hdr->t_xmin;
					...
					!TransactionIdIsCurrentTransactionId(hint_xid))
				{
					...
					{
						/*
						 * Inserter aborted or in-doubt (not in progress, not
						 * committed) -- the tuple is dead for a dirty snapshot.
						 */
						visible = false;
						goto visibility_done;
					}
...
visibility_done:
	...
	/*
	 * SSI: report the read to core predicate.c while we still hold the page's
	 * share lock.  A visible tuple gets a SIREAD lock so a concurrent writer
	 * detects the rw-antidependency against us; either way we may have read
	 * around (or been overtaken by) a concurrent writer, which is a conflict
	 * out.
	 */
	if (snapshot != NULL)
	{
		if (visible)
			PredicateLockTID(rel, tid, snapshot, tuple_hdr->t_xmin);

		RecnoCheckForSerializableConflictOut(visible, rel, tuple_hdr,
											buffer, snapshot);
	}
	...
	{
		/*
		 * BUG 1 (old-snapshot read via index): the on-page (newest) image is
		 * not visible to our snapshot, but if the row was updated in place and
		 * still has a version chain (t_verptr valid), an OLDER version may be
		 * visible.  Reconstruct and serve the before-image so a REPEATABLE READ
		 * reader whose snapshot predates the committed UPDATE still finds the
		 * row at its old indexed key.  Mirrors flux_handler.c's index-fetch
		 * before-image path.  RECNO updates in place for non-key columns; a key
		 * change delete-marks the old (key,TID) index entry, so the entry that
		 * led here has the same key as the before-image.
		 */
			...
			UndoRecPtrIsValid(tuple_hdr->t_verptr))
		{
			...
			LockBuffer(buffer, BUFFER_LOCK_UNLOCK);
			if (RecnoReconstructVisibleVersion(rel, tid,
											   (const char *) tuple_hdr,
											   tuple_hdr->t_len,
											   snapshot,
											   &bi_data, &bi_len))
			{
				RecnoSlotStoreMaterializedTuple(tts,
												(RecnoTupleHeader *) bi_data,
												(uint32) bi_len);
							  ...
							  TupleTableSlot *slot)
{
	...
	if (snapshot && snapshot != SnapshotAny)
	{
		...
		/*
		 * SSI: a row fetched by TID (trigger OLD images, RI checks, EPQ
		 * re-reads) is a read like any other.  Lock it if visible and report
		 * any concurrent writer, while the buffer is still share-locked.
		 */
		if (visible)
			PredicateLockTID(relation, tid, snapshot, tuple_hdr->t_xmin);
		RecnoCheckForSerializableConflictOut(visible, relation, tuple_hdr,
											buffer, snapshot);
		...
		{
			/*
			 * BUG 1 (old-snapshot fetch-by-TID): if the on-page image is not
			 * visible because the row was updated in place after our snapshot
			 * and it still has a version chain (t_verptr valid), reconstruct and
			 * serve the before-image.  Mirrors flux_handler.c's
			 * fetch-row-version path.  A DELETED on-page image has no live older
			 * in-place version to serve here, so only reconstruct for the
			 * not-visible (update superseded) case.
			 */
			if (!(tuple_hdr->t_flags & RECNO_TUPLE_DELETED) &&
				...
				UndoRecPtrIsValid(tuple_hdr->t_verptr))
			{
				...
				LockBuffer(buffer, BUFFER_LOCK_UNLOCK);
				if (RecnoReconstructVisibleVersion(relation, tid,
												   (const char *) tuple_hdr,
												   tuple_hdr->t_len,
												   snapshot,
												   &bi_data, &bi_len))
	...
	/*
	 * Materialize while the page is still pinned -- this is what makes
	 * "UPDATE ... RETURNING old.*" correct on RECNO.
	 *
	 * RecnoSlotStoreTuple leaves the slot pointing AT the on-page tuple bytes.
	 * This function's principal caller is the executor's "fetch the most recent
	 * version of old tuple" step (nodeModifyTable.c: ExecModifyTable's CMD_UPDATE
	 * case, ExecUpdate, ExecMerge), which hands that slot to the RETURNING
	 * projection as OLD and then performs an IN-PLACE update of that very tuple.
	 * A buffer pin does not protect the bytes from our own write, so without a
	 * copy the OLD slot's datums alias exactly the bytes the update overwrites,
	 * and every "RETURNING old.col" reports the NEW value (likewise whole-row
	 * "RETURNING old.*", and "MERGE ... RETURNING old").
	 *
	 * Heap gets this for free: its pre-update version is a physically distinct
	 * tuple that the update never touches.  An in-place AM has to copy instead.
	 *
	 * No UNDO-chain reconstruction is involved or needed here.  The executor
	 * fetches OLD strictly BEFORE the in-place write, so the on-page image at
	 * that moment still IS the old row; the defect was purely the slot's
	 * lifetime, not the availability of the before-image.  (Contrast the
	 * not-visible branch above, which reconstructs from UNDO because there the
	 * on-page image genuinely is a newer version.)
	 *
	 * Copying here also gives the slot the lifetime guarantee its other callers
	 * assume: the datums survive the buffer being unpinned or reused.
	 */
	...
	if (snapshot->snapshot_type == SNAPSHOT_DIRTY)
	{
		...
		if (tuple_hdr->t_flags & RECNO_TUPLE_UNCOMMITTED)
		{
			...
			else if (!TransactionIdIsValid(dirty_xid))
			{
				...
				/*
				 * Caveat-2: a plain INSERT writes no shared sLog entry;
				 * resolve the inserter recorded in t_xmin against CLOG /
				 * ProcArray (heap/FLUX shaped) so an aborted plain INSERT
				 * is reported invisible to SNAPSHOT_DIRTY.  Speculative
				 * inserts leave t_xmin invalid and use the sLog fallback.
				 */
				{
					TransactionId hint_xid = tuple_hdr->t_xmin;
							   ...
							   BulkInsertState bistate, uint32 specToken)
{
	...
	/*
	 * Speculative-insert rollback visibility fix: stamp the inserting
	 * (sub)XID and command id on the tuple, exactly like the plain INSERT
	 * path (recno_operations.c).  Previously the speculative path left t_xmin
	 * INVALID and relied solely on the shared sLog for rollback detection.
	 * That shared sLog lives in shared memory and is LOST on crash, so after
	 * crash recovery a rolled-back speculative insert had no rollback signal
	 * (t_xmin invalid + UNCOMMITTED -> RecnoTupleVisible's slog_nfound==0 path
	 * treated it as committed and VISIBLE).  Stamping t_xmin with
	 * GetCurrentTransactionId() lets RecnoTupleVisible resolve an
	 * aborted/rolled-back inserter directly from CLOG, matching plain INSERT.
	 * Using the current (sub)XID (not the top XID) also makes ROLLBACK TO
	 * SAVEPOINT show up as an aborted subxid in CLOG.
	 */
	tuple->t_data->t_xmin = GetCurrentTransactionId();
	tuple->t_data->t_cid = cid;

	/*
	 * Find a page with enough free space, extending the relation when the
	 * FSM is exhausted.  This mirrors the regular RECNO insert path in
	 * recno_operations.c (and heap's RelationGetBufferForTuple): under
	 * concurrency, a page returned by the FSM can be filled by another
	 * backend before we take its EXCLUSIVE lock, so we must LOOP and retry
	 * -- never give up after a bounded number of attempts.  A freshly
	 * extended empty page always fits a normal tuple (oversized attributes
	 * already went to overflow above), so this loop terminates.
	 *
	 * We also skip blocks already locked for overflow storage: if the FSM
	 * returns such a block for the main tuple we must not double-lock it.
	 */
	buf = InvalidBuffer;
	for (;;)
	{
		...
		/*
		 * If the FSM handed back a block we already hold locked for overflow
		 * storage, mark it full in the FSM and try again -- we need a
		 * different page for the main tuple.
		 */
		for (i = 0; i < overflow_buffers.count; i++)
		{
		...
		if (PageGetFreeSpace(page) >= tuple_size)
			break;				/* got a page with room -- proceed */

		/*
		 * Not enough room.  Try opportunistic pruning to reclaim space from
		 * dead tuples before falling back to the FSM.  RecnoPagePruneOpt()
		 * takes its own conditional lock, so release ours first.
		 */
		LockBuffer(buf, BUFFER_LOCK_UNLOCK);
		if (RecnoPagePruneOpt(relation, buf))
		{
			LockBuffer(buf, BUFFER_LOCK_EXCLUSIVE);
			page = BufferGetPage(buf);
			if (PageGetFreeSpace(page) >= tuple_size)
				break;			/* pruning freed enough -- proceed */
		}
		else
			LockBuffer(buf, BUFFER_LOCK_EXCLUSIVE);

		/* Still not enough: record actual free space and loop to retry. */
	...
	/*
	 * SSI: check for rw-conflict in.  A speculative insert (the INSERT ...  ON
	 * CONFLICT arbiter path) is still an insert and can collide with a
	 * concurrent serializable reader's relation- or page-level predicate lock.
	 * The row does not exist yet, so pass NULL tid.  Must precede the critical
	 * section: it can ereport.
	 */
	CheckForSerializableConflictIn(relation, NULL, BufferGetBlockNumber(buf));
	...
	if (RelationNeedsWAL(relation))
	{
		...
		/*
		 * Root cause of the crash-recovery "insert failed to add tuple"
		 * PANIC: the speculative-insert record must carry the tuple image in
		 * the MAIN data area, immediately after xl_recno_insert, exactly like
		 * the regular RecnoXLogInsert() path.  recno_xlog_insert_redo() reads
		 * the tuple as (xlrec + sizeof(xl_recno_insert)); logging it as block
		 * data (XLogRegisterBufData) instead left redo reading uninitialized
		 * main-data bytes as the tuple header, yielding a bogus t_len and a
		 * failing PageAddItem during ON CONFLICT crash recovery.
		 */
								 ...
								 uint32 specToken, bool succeeded)
{
	...
	if (succeeded)
	{
		 ...
		 *
		 * Rollback-visibility fix: do NOT clear RECNO_TUPLE_UNCOMMITTED here.
		 * Completing the speculative insertion only means the arbiter race is
		 * resolved for a fresh key -- the TOP transaction has NOT committed
		 * yet.  The flag must live exactly as long as a plain INSERT's flag:
		 * cleared at PRE_COMMIT by RecnoSLogClearUncommittedFlags on commit, or
		 * left set on ROLLBACK so RecnoSLogMarkAborted's ABORTED sLog entry (and
		 * t_xmin's aborted CLOG state) render the row invisible.  Clearing it
		 * here made a rolled-back successful speculative insert stay VISIBLE.
		 */
		START_CRIT_SECTION();
		...
		if (RelationNeedsWAL(relation))
		{
			...
			/*
			 * Tag this as a speculative *confirmation* so redo clears the
			 * speculative/uncommitted flags on the existing on-page tuple
			 * instead of trying to PageAddItem a second copy.  This record
			 * carries no tuple image; without the tag, redo would treat it
			 * as a plain insert and PANIC ("insert failed to add tuple").
			 */
			xlrec.flags = RECNO_WAL_SPEC_CONFIRM;
...
reacquire:
										...
										buf))
	{
		...
		/*
		 * Invisible because WE created this version in the current command.
		 *
		 * ON CONFLICT with two rows carrying the same arbiter key in ONE
		 * command lands here: the row was speculatively inserted a moment ago
		 * by this very command, so it is not visible to the command's snapshot.
		 * heap reports TM_Invisible, which lets ExecOnConflictLockRow raise
		 * "ON CONFLICT DO UPDATE command cannot affect row a second time".
		 *
		 * RECNO previously fell through to the "committed modification after
		 * our snapshot" branch and returned TM_Updated, telling ExecInsert to
		 * retry from scratch.  The retry re-inserted the same row and arrived
		 * here again -- an unbounded loop that grew the relation and memory
		 * without bound instead of raising the cardinality violation.  (Under
		 * REPEATABLE READ/SERIALIZABLE the same TM_Updated produced a bogus
		 * "could not serialize access due to concurrent update".)
		 *
		 * RecnoSLogGetDirtyXid deliberately skips our own xid, so it cannot
		 * report this case; check t_xmin directly.  Note the CID test: only a
		 * version created by the CURRENT command is reported invisible.  An
		 * earlier command in the same transaction produced a row this command
		 * legitimately sees and may lock, which is the ordinary
		 * update-a-row-I-inserted-earlier case and must keep working.
		 */
		if (TransactionIdIsValid(tuple_hdr->t_xmin) &&
			TransactionIdIsCurrentTransactionId(tuple_hdr->t_xmin))
		{
			RecnoSLogEntry self_entry;
			int			self_nfound;
			CommandId	tuple_cid = InvalidCommandId;

			self_nfound = RecnoSLogLookup(RelationGetRelid(relation), tid,
										  GetTopTransactionIdIfAny(),
										  &self_entry, 1);
			if (self_nfound > 0)
				tuple_cid = self_entry.cid;

			if (tuple_cid == InvalidCommandId || tuple_cid >= cid)
			{
				if (tmfd)
				{
					tmfd->ctid = *tid;
					tmfd->xmax = tuple_hdr->t_xmin;
					tmfd->cmax = tuple_cid;
					tmfd->traversed = false;
				}

				/*
				 * Populate the slot before returning TM_Invisible:
				 * ExecOnConflictLockRow reads xmin off it via
				 * slot_getsysattr(MinTransactionIdAttributeNumber) to decide
				 * whether to raise the cardinality violation.  Leaving the
				 * slot empty made that read fail with "cannot retrieve a
				 * system column in this context" instead of heap's error.
				 *
				 * Unlock (keeping the pin) first: RecnoTupleToSlotWithOverflow
				 * may fetch overflow data, which would try to re-lock this
				 * buffer if the overflow lives on the same page.  This mirrors
				 * the TM_Ok slot-population path below.
				 */
				if (slot)
				{
					ItemPointerData saved_tid = *tid;
					Oid			saved_reloid = RelationGetRelid(relation);
					...
					/*
					 * Restore the identity fields.  ExecClearTuple() (and a
					 * failed materialization) reset tts_tid to an invalid
					 * pointer and tts_tableOid to InvalidOid, which would
					 * leave tts_recno_getsysattr unable to resolve xmin from
					 * the page -- the very lookup ExecOnConflictLockRow makes
					 * after TM_Invisible to raise the cardinality violation.
					 */
					slot->tts_tid = saved_tid;
					slot->tts_tableOid = saved_reloid;
		...
		if (flags & TUPLE_LOCK_FLAG_FIND_LAST_VERSION)
		{
			if (tmfd)
			{
				...
				/*
				 * Do NOT set tmfd->traversed here.
				 *
				 * traversed means "I followed an update chain and ended up on
				 * a DIFFERENT tuple than you asked for", which is why
				 * heapam_handler.c sets it only after retargeting *tid to
				 * tmfd->ctid.  RECNO updates in place: the tuple at this TID
				 * already IS the latest version and we never moved, so there
				 * is nothing for the caller to re-evaluate.
				 *
				 * Claiming traversed made every caller run its concurrent-
				 * update recovery path spuriously.  GetTupleForTrigger() then
				 * called EvalPlanQual() and handed the result to
				 * ExecGetUpdateNewTuple(), which asserts
				 * ri_projectNewInfoValid and a non-empty slot -- neither holds
				 * on the ON CONFLICT DO UPDATE path, so a BEFORE UPDATE
				 * trigger on an upsert target SIGSEGV'd in
	 ...
	 * Lock succeeded.
	 *
	 * traversed must stay FALSE.  It does not mean "FIND_LAST_VERSION was
	 * requested"; it means "I followed an update chain and the tuple I locked
	 * is NOT the one you named", which is why heapam_handler.c sets it only
	 * after retargeting *tid to tmfd->ctid.  RECNO updates in place, so the
	 * tuple at this TID already IS the latest version and we never moved --
	 * the chain was not "trivially followed", there was no chain.
	 *
	 * Reporting traversed unconditionally made every FIND_LAST_VERSION caller
	 * take its concurrent-update recovery path on a perfectly ordinary lock.
	 * GetTupleForTrigger() then ran EvalPlanQual() and passed the result to
	 * ExecGetUpdateNewTuple(); on the ON CONFLICT DO UPDATE path that
	 * projection is not set up for an EPQ re-projection, so a BEFORE UPDATE
	 * trigger on an upsert target crashed in ExecGetUpdateNewTuple().
	 */
	if (tmfd)
	{
		tmfd->traversed = false;
								   ...
								   MultiXactId *minmulti)
{
	...
	*minmulti = GetOldestMultiXactId();
	...
	/*
	 * Under UNDO-in-WAL, RECNO does not own a UNDO log.
							 ...
							 TupleTableSlot *slot)
{
	...
	for (;;)
	{
		...
		if (OffsetNumberIsValid(tupoffset))
		{
			...
			/*
			 * SSI: report a conflict out for a tuple we could not see -- we
			 * read around a concurrent writer.  A sample scan already holds a
			 * relation-level SIREAD lock from recno_scan_begin, so visible
			 * tuples need no per-TID lock here.  Mirrors
			 * heapam_handler.c's sample-scan conflict-out call.
			 */
			if (scan->rs_snapshot)
				RecnoCheckForSerializableConflictOut(visible, scan->rs_rd,
													tuple_hdr, rscan->rs_cbuf,
													scan->rs_snapshot);
...
static const TableAmRoutine recno_methods = {
	...
	/*
	 * Phase 10: RECNO writes its DML UNDO to the PER-BACKEND engine
	 * (access/undo/perbackend) via recno_pbu.c, tagged uur_rmid =
	 * UNDO_RMID_RECNO.  On abort, PbuAtAbort_ApplyUndo() -> execute_undo_actions()
	 * dispatches each record to recno_undo_apply() (recno_undo.c).  Without
	 * am_undo_engine = UNDO_ENGINE_PERBACKEND GetTableAmRoutine() would reject
	 * the AM (the per-relation fork engine has been removed).
	 *
	 * am_index_delete_marking = true keeps an indexed-column UPDATE IN PLACE
	 * (stable TID): the new (k_new,TID) index entry is inserted and the old
	 * (k_old,TID) entry is delete-marked (index_delete_mark); nbtree/hash
	 * suppress their own index UNDO for a delete-marking parent because RECNO's
	 * table UNDO drives index cleanup on rollback.
	 */
	...
	.index_scan_begin = recno_index_scan_begin,
	.index_scan_reset = recno_index_scan_reset,
	.index_scan_end = recno_index_scan_end,
