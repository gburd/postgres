								   ...
								   MultiXactId *minmulti)
{
	...
	*minmulti = GetOldestMultiXactId();
	 ...
	 * smgrexists() guards in the DML paths will skip all UNDO record
	 * emission.
	 *
	 * We create the UNDO fork for both permanent and unlogged tables:
	 * - Permanent: UNDO is needed for crash recovery and WAL replay.
	 * - Unlogged: UNDO is needed for transaction rollback and MVCC
	 *   visibility while the table is in use.  Although unlogged tables
	 *   don't survive crashes, they still need UNDO for in-transaction
	 *   operations (e.g., UPDATE rollback, abort handling).
	 * - Temp tables: skip UNDO since they are session-private.
	 *
	 * IMPORTANT: RelUndoInitRelation() uses RelationGetSmgr(rel), which
	 * resolves to rel->rd_locator.  During initial CREATE TABLE, this is
	 * the correct (and only) locator.  During TRUNCATE, the caller
	 * (RelationSetNewRelfilenumber) passes a new locator via
	 * newrlocator, but rel->rd_locator still holds the old value at this
	 * point — it gets updated AFTER this function returns.  So
	 * RelUndoInitRelation(rel) would incorrectly target the old locator.
	 *
	 * To handle both cases, we temporarily swap rel->rd_locator to the
	 * new locator, call RelUndoInitRelation, then restore it.  The
	 * caller will overwrite rd_locator anyway after we return.
	 */
	if (persistence == RELPERSISTENCE_PERMANENT ||
		persistence == RELPERSISTENCE_UNLOGGED)
	{
		...
		RelationCloseSmgr(rel);	/* Force smgr re-open for new locator */
		RelUndoInitRelation(rel);
		RelationCloseSmgr(rel);	/* Clean up before restoring */
							 ...
							 double *allvisfrac)
{
	...
	*tuples = Max(tuple_count, 0);
	...
	/*
	 * Compute allvisfrac from the Visibility Map.  Count how many pages have
	 * the ALL_VISIBLE bit set and return the fraction.  This allows the
	 * planner to consider index-only scan paths.
	 */
	{
		...
		if (vmBlocks > 0)
		{
			for (mapBlock = 0; mapBlock < nblocks; mapBlock++)
			{
				if (RecnoVMCheck(rel, mapBlock, RECNO_VM_ALL_VISIBLE))
					visiblePages++;
			}
...
recno_tuple_from_slot(TupleTableSlot *slot)
{
	...
	tuple->t_data->t_cid = GetCurrentCommandId(false);
