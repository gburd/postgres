...
recno_prepare_pagescan(RecnoScanDesc scan, Buffer buffer)
{
	...
	if (PageIsAllVisible(page))
		...
		all_visible = RecnoVMCheckCached(scan->rs_base.rs_rd, scan->rs_cblock,
		 ...
		 offnum = OffsetNumberNext(offnum))
	{
		...
		if (RecnoIsOverflowRecordInline(tuple_header, ItemIdGetLength(itemid)))
			continue;
		 ...
		 * transaction state.
		 *
		 * NOTE: Do NOT skip RECNO_TUPLE_DELETED tuples here.  The DELETED
											 ...
											 buffer))
		{
			 ...
			 * visible -- keep it as a candidate so recno_scan_getnextslot can
			...
			if ((tuple_header->t_flags & RECNO_TUPLE_UPDATED) &&
				...
				RelUndoRecPtrIsValid(RecnoTupleGetVersionPtr(tuple_header,
			...
			if (IsolationIsSerializable())
			{
				RecnoCheckForSerializableConflictOut(scan->rs_base.rs_rd,
													 tuple_header,
													 buffer,
													 scan->rs_base.rs_snapshot);
			}
...
recno_scan_getnextslot(TableScanDesc sscan, ScanDirection direction, TupleTableSlot *slot)
{
	...
	for (;;)
	{
		 ...
		 * pattern.  RecnoSlotStoreTuple acquires its own pin so the slot data
		...
		while (scan->rs_cindex < scan->rs_ntuples)
		{
			...
			/*
			 * In-place MVCC torn-read guard: RECNO UPDATEs overwrite live
			 * on-page tuple bytes.  Page-mode collected the visible offsets
			 * under an earlier SHARE lock that was then released, so the bytes
			 * we are about to inspect (visibility flags, version pointer, and
			 * the image handed to reconstruct / copied into the slot) can be
			 * mutated by a concurrent in-place updater.  Re-take the buffer
			 * SHARE lock for the whole per-tuple read.  The same-size CAS
			 * updater takes BUFFER_LOCK_EXCLUSIVE across its non-atomic overwrite
			 * (see recno_tuple_update), which conflicts with SHARE, so every
			 * access below sees a stable image; the store copies the bytes out
			 * under this lock (RecnoSlotStoreTuple), and reconstruct reads the
			 * on-page image while it is pinned as its first step.  Lock order
			 * matches the update path (data buffer before UNDO fork buffer).
			 */
			LockBuffer(scan->rs_cbuf, BUFFER_LOCK_SHARE);

			/*
			 * Re-fetch the line pointer and tuple pointer UNDER the lock: the
			 * values computed above were read while unlocked and a concurrent
			 * growth UPDATE (delete + re-add on the same page) can have changed
			 * the item's offset/length, which would otherwise pair a stale
			 * header with a mismatched payload in the copy below.
			 */
			itemid = PageGetItemId(page, offnum);
			if (!ItemIdIsNormal(itemid))
			{
			 ...
			 * per-relation UNDO fork (WS-PVS2): RecnoReconstructVisibleVersion
												 ...
												 scan->rs_cbuf))
			{
				...
				if (RecnoReconstructVisibleVersion(
												   scan->rs_base.rs_rd,
												   &item_tid,
			...
			/* Normal: serve on-page data (copied out under the SHARE lock) */
							 ...
							 uint64 *exact_pages)
{
	...
	for (;;)
	{
			...
			rscan->rs_cindex < rscan->rs_ntuples)
		{
			...
			/*
			 * NOTE: Do NOT skip RECNO_TUPLE_DELETED here.  This tuple passed
			 ...
			 * CLOG + snapshot (RecnoTupleSatisfiesMVCC).
			 */
			if ((tuple_hdr->t_flags & RECNO_TUPLE_UPDATED) &&
				scan->rs_snapshot != NULL &&
				IsMVCCSnapshot(scan->rs_snapshot) &&
				RelUndoRecPtrIsValid(RecnoTupleGetVersionPtr(tuple_hdr,
															 ItemIdGetLength(itemid))) &&
...
recno_index_fetch_begin(Relation rel, uint32 flags)
{
	...
	scan->all_dead = false;
	scan->fetch_cxt = AllocSetContextCreate(CurrentMemoryContext,
										"RECNO index fetch",
...
recno_index_fetch_reset(IndexFetchTableData *scan)
{
	...
	if (rscan->fetch_cxt != NULL)
		MemoryContextReset(rscan->fetch_cxt);
}
...
recno_index_fetch_end(IndexFetchTableData *scan)
{
	...
	if (rscan->fetch_cxt != NULL)
	{
		MemoryContextDelete(rscan->fetch_cxt);
		rscan->fetch_cxt = NULL;
	}
						...
						bool *call_again, bool *all_dead)
{
	...
	*call_again = false;
	...
	/*
	 * all_dead is a PER-FETCH (per-TID) signal: it must be true iff the tuple
	 * at *this* tid is guaranteed dead to every backend (tableam.h contract).
	 * RECNO has no version chain to walk within one tid -- call_again is never
	 * set -- so each call resolves exactly one TID and there is nothing to
	 * accumulate across calls.  scan->all_dead therefore MUST be cleared here
	 * on entry; leaving it latched from a previous fetch of a genuinely-dead
	 * TID would wrongly report a subsequently-fetched LIVE tuple as all_dead,
	 * causing index_fetch_heap() to set kill_prior_tuple and nbtree
	 * (_bt_killitems) to LP_DEAD-and-remove that live row's correct
	 * (key -> tid) index entry.  amcheck then reports "heap tuple ... lacks
	 * matching index tuple", and a surviving stale entry pointing at a reused
	 * TID feeds IndexOnlyScan a bogus tuple that trips the deform-bounds
	 * oracle.
	 */
	scan->all_dead = false;
	if (all_dead)
	...
	/*
	 * Reclaim the previous fetch's slot-private tuple copy.  ExecClearTuple
	 * above already released the executor's reference to it, so resetting the
	 * scratch context here keeps at most one copy live per scan.
	 */
	MemoryContextReset(scan->fetch_cxt);
	...
	if (snapshot)
	{
		if (snapshot->snapshot_type == SNAPSHOT_DIRTY)
		{
			 ...
			 * behaviour.
			 *
			 * Uses t_xid_hint for INSERT visibility (fast path) and a single
			 ...
			 * transaction).  Use t_xid_hint for the fast path, falling back
			...
			if (tuple_hdr->t_flags & RECNO_TUPLE_DELETED)
			{
														  ...
														  slog_entries, SLOG_MAX_TUPLE_OPS);
				{
					...
					else if (slog_nfound == 0)
					{
						 ...
						 */
						Assert(!(tuple_hdr->t_flags & RECNO_TUPLE_DELETED) ||
			 ...
			 * RECNO_TUPLE_UPDATED flag means this tuple has been superseded
	...
	else
	{
		 ...
		 */
		visible = !(tuple_hdr->t_flags & RECNO_TUPLE_DELETED);
	}
visibility_done:
	...
	if (!visible)
	{
		 ...
		 * or future snapshot can still see it.  RecnoTupleDeadToAll does the
		 * heap-shaped XID-horizon check (t_xmax committed and < oldest_xmin);
		 * this replaced the HLC-era RecnoCanVacuumTimestamp, which compared the
		 ...
		 */
		if (RecnoTupleDeadToAll(tuple_hdr, RecnoGetOldestXminHorizon(rel)))
		{
			if (all_dead)
				*all_dead = true;
			scan->all_dead = true;
	 ...
	 *
	 * In-place MVCC torn-read guard: everything below (reconstruct, the
	 * RecnoSlotStore* calls, and RecnoTupleToSlotWithOverflow -- whose inline
	 * column datums alias the header) reads the tuple AFTER the content lock is
	 * dropped.  RECNO overwrites live tuple bytes in place on UPDATE, so a
	 * concurrent updater could tear those reads (garbage varlena lengths /
	 * compressed frames -> zstd errors, bad overflow lengths, SIGSEGV).  Copy
	 * the on-page tuple into slot-private memory now, while we still hold the
	 * SHARE lock, and point tuple_hdr at the copy.  The same-size CAS updater
	 * takes BUFFER_LOCK_EXCLUSIVE across its non-atomic overwrite (see
	 * recno_tuple_update), which conflicts with SHARE, so this copy never races
	 * a writer.  Visibility (which sets page hint bits via BufferSetHintBits16)
	 * already ran above on the on-page pointer under the lock, so the copy
	 * carries the up-to-date flags without needing the page again.
	 */
	{
		uint32		hdr_len = ItemIdGetLength(itemid);
		RecnoTupleHeader *hdr_copy;
		MemoryContext oldcxt;

		oldcxt = MemoryContextSwitchTo(scan->fetch_cxt);
		hdr_copy = (RecnoTupleHeader *) palloc(hdr_len);
		MemoryContextSwitchTo(oldcxt);
		memcpy(hdr_copy, tuple_hdr, hdr_len);
		tuple_hdr = hdr_copy;
	}
	 ...
	 * mirroring the sequential-scan path (recno_getnextslot) and the
	 * fetch-by-TID path (recno_tuple_fetch_row_version).  If the reader can
	 ...
	 * (RecnoTupleVisibleToSnapshotDual already returned true because the deleter
	 * xid is not visible to this snapshot).  RecnoTupleToSlotWithOverflow refuses
	 * a DELETED tuple outright (it can't tell visible-to-this-snapshot from
	 * physically-dead), so serve the on-page image via RecnoSlotStoreTuple --
	...
	{
		/*
		 * RecnoSlotStoreMaterializedTuple takes ownership and frees the tuple
		 * on slot clear, so it must own an independent allocation -- not the
		 * fetch_cxt copy, which is reset on the next fetch (double-free).
		 */
		uint32		del_len = ItemIdGetLength(itemid);
		RecnoTupleHeader *del_copy;
		MemoryContext oldcxt;

		oldcxt = MemoryContextSwitchTo(tts->tts_mcxt);
		del_copy = (RecnoTupleHeader *) palloc(del_len);
		MemoryContextSwitchTo(oldcxt);
		memcpy(del_copy, tuple_hdr, del_len);

		RecnoSlotStoreMaterializedTuple(tts, del_copy, del_len);
		tts->tts_tableOid = RelationGetRelid(rel);
		tts->tts_tid = *tid;
	...
	/*
	 * tuple_hdr is a slot-private copy (taken under the SHARE lock above), so
	 * RecnoTupleToSlotWithOverflow's inline-column datums alias private memory
	 * (scan->fetch_cxt), not the buffer.  Those datums stay valid until the
	 * next fetch resets fetch_cxt (recno_index_fetch_tuple entry) or the scan
	 * ends.  The buffer pin is no longer needed to keep the slot data valid;
	 * keep it here anyway so it is released via the standard
	 * release-on-next-fetch / reset path (scan->buffer bookkeeping).
	 */
	scan->buffer = buffer;
							  ...
							  TupleTableSlot *slot)
{
	...
	if (snapshot && snapshot != SnapshotAny)
	{
		 ...
		 * visible.  Do NOT additionally check RECNO_TUPLE_DELETED here —
	...
	if (snapshot->snapshot_type == SNAPSHOT_DIRTY)
	{
		...
		if (tuple_hdr->t_flags & RECNO_TUPLE_UNCOMMITTED)
		{
			...
			else if (!TransactionIdIsValid(dirty_xid))
			{
				...
				/*
				 * The t_xid_hint field was removed.  Use sLog to determine
				 * insert visibility for UNCOMMITTED tuples.
				 */
				{
							   ...
							   BulkInsertState bistate, uint32 specToken)
{
	...
	if (offnum == InvalidOffsetNumber)
	{
		...
		offnum = PageAddItem(page, tuple->t_data, tuple_size,
							 InvalidOffsetNumber, false, false);
		if (offnum == InvalidOffsetNumber)
			elog(ERROR, "failed to add RECNO tuple to fresh page during "
								 ...
								 uint32 specToken, bool succeeded)
{
	...
	if (succeeded)
	{
		 ...
		 *
		 * Dirty-read hole fix: this used to ALSO clear RECNO_TUPLE_UNCOMMITTED
		 ...
		 * see recno_tuple_insert_speculative), so leaving
		 * RECNO_TUPLE_UNCOMMITTED set here is safe and correct: the existing
		 * commit-time machinery (RecnoClearUncommittedFlags /
		 * recno_stamp_tuple_committed, driven by the tracked sLog key) clears
	...
	else
	{
		...
		tuple_hdr->t_flags |= RECNO_TUPLE_DELETED;
		RecnoTupleSetXmax(tuple_hdr, abort_xid);
...
reacquire:
										 ...
										 buf))
	{
		 ...
		 * follow the update chain and lock the latest version. In RECNO with
	...
	/*
	 * Lock succeeded.  Set traversed for FIND_LAST_VERSION callers. RECNO
	...
	{
		/*
		 * In-place MVCC torn-read guard for the EPQ tuple-lock path.
		 *
		 * RecnoTupleToSlotWithOverflow() stores the inline (non-overflow)
		 * column datums as raw pointers into the tuple header we hand it, and
		 * we must drop the buffer content lock before that call (overflow
		 * fetch may re-lock the same page).  If we deformed the on-page header
		 * directly, the resulting slot -- used as the EvalPlanQual input row --
		 * would alias a buffer that is unlocked here and unpinned just below
		 * (ReleaseBuffer).  RECNO does in-place UPDATEs (same-size CAS under
		 * BUFFER_LOCK_SHARE_EXCLUSIVE, which does NOT exclude readers, plus the
		 * delete+re-add growth path that relocates page data), so a concurrent
		 * updater would overwrite those bytes while the EPQ recheck reads them,
		 * tearing a multi-byte datum: a torn `numeric` trips
		 * Assert("borrow == 0") in numeric_sub, and torn varlena lengths give
		 * the zstd / alloc-size / SIGSEGV failures already seen on the scan
		 * paths (fix for the sequential-scan / index-fetch torn read).
		 *
		 * Copy the on-page tuple into slot-private memory now, while we still
		 * hold the buffer EXCLUSIVE lock (which excludes every in-place
		 * updater), and deform that copy.  The inline column datums then alias
		 * slot-owned memory instead of the buffer; overflow columns are fetched
		 * fresh via `relation` and are unaffected.  This mirrors the
		 * copy-under-lock done by RecnoSlotStoreTuple on the scan paths.
		 */
		uint32		hdr_len = ItemIdGetLength(itemid);
		RecnoTupleHeader *hdr_copy;
		MemoryContext oldcxt;
		...
		{
			/*
			 * Invariant: the EPQ input slot must not alias the buffer page.
			 * RecnoTupleToSlotWithOverflow deforms the slot-private copy taken
			 * above, so no inline column datum may point into the page we are
			 * about to unlock/unpin.  A regression that dropped the copy would
			 * leave a datum here pointing inside [page, page + BLCKSZ) and this
			 * assert would fire on the first concurrent-update EPQ recheck --
			 * exactly the torn-numeric (Assert "borrow == 0") hazard this guards.
			 */
			char	   *page_start = (char *) BufferGetPage(buf);
			int			ai;

			for (ai = 0; ai < slot->tts_nvalid; ai++)
			{
				char	   *dp = (char *) DatumGetPointer(slot->tts_values[ai]);

				Assert(slot->tts_isnull[ai] ||
					   dp < page_start || dp >= page_start + BLCKSZ);
			}
		}
#endif
							 ...
							 TableScanDesc scan)
{
	...
	while (table_scan_getnextslot(scan, ForwardScanDirection, slot))
	{
		...
		if (snapshot == SnapshotAny)
		{
			...
			if (tuple_hdr->t_flags & RECNO_TUPLE_DELETED)
			{
				if (RecnoTupleDeadToAll(tuple_hdr, RecnoGetOldestXminHorizon(tablerel)))
				{
 ...
 */
/*
 * Use table_block_relation_size() from tableam.c directly.  RECNO uses
							 ...
							 TupleTableSlot *slot)
{
	...
	for (;;)
	{
		...
		if (OffsetNumberIsValid(tupoffset))
		{
			...
			/*
			 * Determine visibility.  RECNO uses heap-shaped xmin/xmax MVCC via
			 * RecnoTupleVisibleToSnapshotDual, which handles DELETED/UPDATED
...
static const TableAmRoutine recno_methods = {
	...
	/*
	 * RECNO table DML records UNDO into the relation's own UNDO fork
	 * (RELUNDO_FORKNUM).  recno_operations.c reserves space via
	 ...
	 *
	 * The common-WAL UNDO path (UNDO_RMID_RECNO -> recno_undo.c, dispatched by
	 * undoapply.c) is the AM-agnostic mechanism shared with nbtree, hash, and
	 * FILEOPS.  Indexes on a RECNO relation piggyback their UNDO records onto
	 * the table's active UNDO context (see NbtreeUndoLogInsert), so an index
	 * insert rolls back through the same chain as the row change.  RECNO's own
	 * row DML does not emit UNDO_RMID_RECNO records.
	 *
	...
	/*
	 * RECNO updates rows in place, keeping the same physical TID.  This flag
	 * is REQUIRED (not merely an optimization) under the Tepid executor
	 * contract: it does NOT tell the executor to skip index maintenance --
	 * that is driven entirely by the TU_UpdateIndexes value RECNO returns from
	 * recno_tuple_update() (heap-identical: TU_All on a changed indexed key,
	 * TU_None otherwise).  Instead this flag enables the two mechanisms that
	 * make a stable-TID secondary index safe:
	 *
	 *   1. The read-time stale-entry recheck (indexam.c forces xs_want_itup;
	 *      nodeIndex[only]scan builds the IndexInfo and runs
	 *      ExecIndexInplaceEntryIsStale).  A changed indexed column leaves the
	 *      prior (oldkey -> TID) entry in place; the recheck reforms the live
	 *      key and drops the row when it reached the tuple through a now-stale
	 *      key, so every live row is returned exactly once via each legitimate
	 *      key and never via a stale key.  Dropping this flag would DISABLE the
	 *      recheck and reintroduce the stale-entry mis-fetch corruption class.
	 *
	 *   2. The dup-(key,TID) insert suppression in ExecInsertIndexTuples()
	 *      (index_entry_exists_for_tid probe): an indexed column oscillating
	 *      A -> B -> A would otherwise re-insert an already-present (key, TID)
	 *      pair, which nbtree forbids.
	 *
	 * RECNO's single stable TID (one live tuple per row, no mid-chain second
	 * TID) also makes it immune to the WARM/SIU bitmap-scan false-negative, so
	 * RECNO needs only this 0002 contract and none of the mid-chain SIU
	 * machinery.  See tableam.h.
	 */
	.am_inplace_update_keeps_tid = true,

	/*
	 * RECNO's in-place UPDATE overwrites the committed tuple bytes -- there
	 * is no old version left behind, so it creates zero dead tuples (see
	 * tableam.h).
	 */
	.am_inplace_update_no_dead_tuple = true,
	...
	for (; rscan->rs_cindex <= maxoffset; rscan->rs_cindex++)
	{
		...
		else
		{
			...
			*liverows += 1;
			 ...
			 * in-place RECNO UPDATE creates no genuine dead tuple, so the
			 ...
			 */
			proxy_verptr = RecnoTupleGetVersionPtr(tuple_hdr,
												   ItemIdGetLength(itemid));
		}
