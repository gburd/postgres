						 ...
						 RecnoOverflowBuffers *overflow_buffers)
{
	...
	ovp->ov_content_hash = hash_bytes((const unsigned char *) data_ptr, (int) data_len);
...
/*
 * Mark an overflow record's line pointer LP_UNUSED without collapsing the
 * page's line-pointer array.
 *
 * RECNO is a stable-TID access method: a live main tuple keeps the same
 * (block, offnum) for its whole lifetime because every UNIQUE/secondary
 * index entry points at that TID.  Overflow records share data pages with
 * indexed main tuples (spatial locality), so removing an overflow record
 * must NOT renumber the offsets of the main tuples that follow it.
 *
 * The old code used RecnoPageIndexTupleDelete(), which memmove()s the
 * line-pointer array down by one slot -- shifting every higher offset,
 * including live main tuples.  Their index entries then pointed one slot
 * too high, so amcheck heapallindexed reported "heap tuple (X,Y) lacks
 * matching index tuple" and index scans returned the adjacent row.
 *
 * Marking the slot LP_UNUSED leaves a hole (exactly like a deleted main
 * tuple); its ~2KB of storage is reclaimed lazily by the next
 * RecnoPageDefragment()/PageRepairFragmentation(), which compacts the data
 * area and truncates trailing unused line pointers but never renumbers a
 * surviving LP_NORMAL item.
 */
static void
RecnoOverflowMarkUnused(Page page, OffsetNumber offnum)
{
	ItemId		itemid = PageGetItemId(page, offnum);

	ItemIdSetUnused(itemid);
	RecnoPageSetFlag(RecnoPageGetOpaque(page), RECNO_PAGE_DEFRAG_NEEDED);
						 ...
						 OffsetNumber first_offset)
{
	...
	while (cur_block != InvalidBlockNumber)
	{
		...
		/*
		 * Remove the overflow record.  Mark its line pointer LP_UNUSED
		 * rather than compacting the array: this page may also hold live
		 * main tuples whose stable TIDs (and matching index entries) must
		 * not be renumbered.
		 */
		RecnoOverflowMarkUnused(page, cur_offset);
		MarkBufferDirty(buffer);
...
/*
 * Collect full overflow pointers from a tuple, tagged by attribute number.
 *
 * Like RecnoCollectOverflowPtrs, but instead of returning a flat list of
 * (block, offset) pairs it fills out_ptrs[attnum] with a by-value copy of the
 * on-page RecnoOverflowPtr and sets out_present[attnum] = true for every
 * attribute that carries an overflow pointer.  This lets an UPDATE caller
 * compare the old on-page content hash against the new value without fetching
 * the overflow chain.  natts must equal tupdesc->natts.
 */
void
RecnoCollectOverflowPtrsByAttr(RecnoTupleHeader *tuple_hdr, TupleDesc tupdesc,
							   RecnoOverflowPtr *out_ptrs, bool *out_present,
							   int natts)
{
	uint8	   *nulls_bitmap;
	char	   *data_ptr;
	...
	memset(out_present, 0, natts * sizeof(bool));
...
RecnoVacuumOverflowRecords(Relation rel)
{
	...
	for (blkno = 0; blkno < nblocks; blkno++)
	{
		...
		for (offnum = maxoff; offnum >= FirstOffsetNumber; offnum--)
		{
			...
			{
				/*
				 * Orphaned overflow record - free it.  Mark LP_UNUSED
				 * (do not compact the array) so live main tuples sharing
				 * this page keep their stable offsets/TIDs.
				 */
				RecnoOverflowMarkUnused(page, offnum);
				page_modified = true;
				orphans_removed++;
			}
		}
		...
		if (page_modified)
		{
			...
			/*
			 * WAL-log the cleanup as a full-page image (RecnoXLogDefrag), NOT
			 * RecnoXLogInitPage.  INIT_PAGE's redo zeroes the whole page
			 * (RBM_ZERO_AND_LOCK + RecnoInitPage), which would WIPE any live
			 * main tuples that share this page with the orphaned overflow
			 * records -- RECNO interleaves main and overflow tuples on the same
			 * page.  We only marked the orphan line pointers LP_UNUSED (the
			 * array is deliberately NOT compacted so live main tuples keep their
			 * stable TIDs), so redo must PRESERVE the page contents.  A later
			 * CAS_UPDATE_UNDO to a surviving main tuple on this page byte-diffs
			 * the tuple at its offset; replaying INIT_PAGE first left that offset
			 * unused and made recovery PANIC ("invalid item at offset").
			 * RecnoXLogDefrag registers the buffer REGBUF_FORCE_IMAGE so redo
			 * restores the exact post-cleanup page, live tuples intact.
			 */
			if (RelationNeedsWAL(rel))
				(void) RecnoXLogDefrag(rel, buffer, NULL, 0,
									   RecnoGetCommitTimestamp());
