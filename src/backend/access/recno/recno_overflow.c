...
RecnoVacuumOverflowRecords(Relation rel)
{
	...
	for (blkno = 0; blkno < nblocks; blkno++)
	{
		...
		/*
		 * Scan backwards so that RecnoPageIndexTupleDelete doesn't
		 * invalidate offsets we haven't checked yet.
		 */
...
/* ----------------------------------------------------------------
 * Legacy interface (kept for compatibility during transition)
 * ----------------------------------------------------------------
 */

/*
 * Legacy: Store overflow using old dedicated-page approach.
 * Delegates to new column-level overflow.
 */
RecnoOverflowRef *
RecnoStoreOverflow(Relation rel, Datum value, int attnum)
{
	RecnoOverflowRef *ref;
	Datum		overflow_datum;
	const RecnoOverflowPtr *ovp;

	/*
	 * Pass NULL for overflow_buffers - this legacy API uses immediate WAL
	 * logging instead of deferred atomic logging.
	 */
	overflow_datum = RecnoStoreOverflowColumn(rel, value, attnum,
											  recno_overflow_inline_prefix,
											  NULL);

	/* Extract the pointer info into a legacy ref */
	ovp = RecnoGetOverflowPtr(DatumGetPointer(overflow_datum));

	ref = (RecnoOverflowRef *) palloc0(sizeof(RecnoOverflowRef));
	ref->overflow_page = ovp->ov_first_block;
	ref->total_length = ovp->ov_total_length;
	ref->compression_info = 0;

	pfree(DatumGetPointer(overflow_datum));
	return ref;
}

/*
 * Legacy: Fetch overflow using old interface.
 */
Datum
RecnoFetchOverflow(Relation rel, RecnoOverflowRef *ref)
{
	/*
	 * The legacy interface stored overflow in a different format (dedicated
	 * pages with data in special space).  Since we're transitioning, this is
	 * not compatible with the new format.  For now, return an error if called
	 * with data stored in the old format.
	 */
	ereport(ERROR,
			(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
			 errmsg("legacy overflow fetch not supported in column-level overflow mode")));
	return (Datum) 0;
}

/*
 * Legacy: Delete overflow using old interface.
 */
void
RecnoDeleteOverflow(Relation rel, RecnoOverflowRef *ref)
{
	if (ref == NULL || ref->overflow_page == InvalidBlockNumber)
		return;

	/*
	 * Attempt to clean up using the new chain deletion.  This works if the
	 * overflow was stored via the new RecnoStoreOverflowColumn path, where
	 * overflow_page maps to the first block.  We use FirstOffsetNumber as a
	 * best guess since the legacy ref doesn't store the offset.
	 */
	RecnoDeleteOverflowChain(rel, ref->overflow_page, FirstOffsetNumber);
}
