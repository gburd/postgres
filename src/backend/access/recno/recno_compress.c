...
/*
 * Compression configuration is per-relation, via the `compression` and
 * `compression_level` reloptions -- there is intentionally NO GUC.  These
 * module-local defaults are used only when a relation has no rd_options
 * (e.g. transient slots with no backing relation).
 */
#define RECNO_DEFAULT_COMPRESSION_LEVEL		3

/*
 * RecnoRelCompressionType
 *		Map a relation's `compression` reloption to a RecnoCompressionType,
 *		or RECNO_COMP_NONE when compression is disabled / unavailable.
 *
 * `*level_out` receives the relation's compression_level reloption (or the
 * default when unset / no rd_options).
 */
RecnoCompressionType
RecnoRelCompressionType(Relation rel, int *level_out)
{
	StdRdOptions *opts;

	if (level_out)
		*level_out = RECNO_DEFAULT_COMPRESSION_LEVEL;

	if (rel == NULL || rel->rd_options == NULL)
		return RECNO_COMP_NONE;		/* no reloption -> off by default */

	opts = (StdRdOptions *) rel->rd_options;
	if (level_out)
		*level_out = opts->recno_compression_level;

	switch (opts->recno_compression)
	{
		case STDRD_OPTION_RECNO_COMPRESS_OFF:
			return RECNO_COMP_NONE;
		case STDRD_OPTION_RECNO_COMPRESS_LZ4:
			return RECNO_COMP_LZ4;
		case STDRD_OPTION_RECNO_COMPRESS_ZSTD:
			return RECNO_COMP_ZSTD;
		case STDRD_OPTION_RECNO_COMPRESS_AUTO:
			/* AM chooses: signal auto-select to RecnoCompressAttribute */
			return RECNO_COMP_NONE;
	}
	return RECNO_COMP_NONE;
}
...
/*
 * RecnoRelCompressionEnabled
 *		True when the relation requests any compression (auto or a named
 *		codec).  "off" or no-reloption returns false.
 */
bool
RecnoRelCompressionEnabled(Relation rel)
{
	StdRdOptions *opts;
	...
	if (rel == NULL || rel->rd_options == NULL)
		return false;
	opts = (StdRdOptions *) rel->rd_options;
	return opts->recno_compression != STDRD_OPTION_RECNO_COMPRESS_OFF;
}
 ...
 *
 * TODO: Move dictionary storage to a catalog table or shared UNDO-log
 * metadata so that dictionaries are visible to all backends and survive
					   ...
					   int comp_level)
{
	...
	switch (comp_type)
	{
		...
		case RECNO_COMP_ZSTD:
			comp_data = RecnoCompressZSTD(value, &comp_size, comp_level);
			is_success = (comp_data != (Datum) 0);
			break;
