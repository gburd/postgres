...
/*
 * Data-driven heuristic tuning (active on the default 'auto' path).
 *
 * RECNO compresses each attribute value as one standalone frame, so the only
 * signal available for tuning is the bytes of that single value.  We sample
 * them once, estimate Shannon entropy over a byte histogram, and use that to
 * (a) skip codecs entirely on already-dense data and (b) pick a ZSTD level
 * proportional to how compressible the value looks.  This replaces the old
 * fixed per-type codec + fixed level-3 choice with a per-value decision and
 * needs no GUC: a user who wants a fixed codec/level still sets the
 * recno_compression_algorithm / recno_compression_level GUCs as overrides.
 */
#define RECNO_ENTROPY_SAMPLE_BYTES	1024	/* bytes probed for entropy */
#define RECNO_ENTROPY_SKIP_BITS		7.6 /* >= this => treat as incompressible */

/*
 * Forward declarations
 */
static double RecnoEstimateEntropyBits(const unsigned char *data, Size len);
static int	RecnoHeuristicZstdLevel(double entropy_bits, Size value_size);
					   ...
					   RecnoCompressionType comp_type)
{
	...
	/*
	 * Pick the ZSTD level.  An explicit recno_compression_algorithm GUC means
	 * the user wants the GUC level too; the default 'auto' path derives a level
	 * from the value's estimated entropy so dense data is not over-worked and
	 * compressible data earns a stronger level.
	 */
	if (recno_compression_algorithm == RECNO_COMP_ALGO_AUTO &&
		comp_type == RECNO_COMP_ZSTD)
	{
		double		entropy;

		entropy = RecnoEstimateEntropyBits((const unsigned char *)
										   VARDATA_ANY(DatumGetPointer(value)),
										   orig_size);
		zstd_level = RecnoHeuristicZstdLevel(entropy, orig_size);
	}
	else
		zstd_level = recno_compression_level;
...
#endif
			dictid = RECNO_DICT_INVALID_ID;
			comp_data = RecnoCompressZSTD(value, &comp_size, zstd_level);
			is_success = (comp_data != (Datum) 0);
			break;
	...
	header->comp_level = (uint8) zstd_level;
...
/*
 * RecnoEstimateEntropyBits
 *
 * Estimate Shannon entropy (bits per byte, 0..8) over a prefix of the value.
 * Low entropy (lots of repetition / skew) predicts good compression; high
 * entropy (near 8 bits/byte) predicts incompressible data.  Sampling only the
 * first RECNO_ENTROPY_SAMPLE_BYTES keeps this O(1) per value regardless of
 * size; for the inline-overflow workloads RECNO targets the leading bytes are
 * representative enough to make a skip/level decision.
 */
static double
RecnoEstimateEntropyBits(const unsigned char *data, Size len)
{
	uint32		freq[256];
	Size		n = Min(len, (Size) RECNO_ENTROPY_SAMPLE_BYTES);
	double		entropy = 0.0;
...
/*
 * RecnoHeuristicZstdLevel
 *
 * Map estimated entropy and value size to a ZSTD level.  Compressible data
 * (low entropy) earns a higher level because the extra CPU buys real ratio;
 * dense data earns a low level because higher levels only burn cycles.  Larger
 * values also bias slightly higher since their amortized per-byte cost of a
 * stronger level is lower.  The range is clamped well below ZSTD's max (22):
 * the storage benchmark showed level 22 barely beat level 3 on these
 * single-value frames, so spending the CPU is not worth it.
 */
static int
RecnoHeuristicZstdLevel(double entropy_bits, Size value_size)
{
	int			level;

	if (entropy_bits <= 3.0)
		level = 9;
	else if (entropy_bits <= 5.0)
		level = 6;
	else if (entropy_bits <= 6.5)
		level = 3;
	else
		level = 1;

	/* Large, plausibly compressible values justify one extra notch. */
	if (value_size >= 8192 && entropy_bits <= 6.5)
		level++;
 ...
 *
 * Select the compression algorithm for a value.  The recno_compression_algorithm
 * GUC overrides everything: 'lz4'/'zstd' force that codec, 'none' is handled by
 * the caller.  On the default 'auto' path the decision is data-driven:
 *
 *   - Numeric fixed-width types use DELTA (compact varint) as before.
 *   - Variable-length / binary values are sampled for entropy.  Near-random
 *     data (entropy >= RECNO_ENTROPY_SKIP_BITS) returns RECNO_COMP_NONE so the
 *     caller skips the codec round-trip entirely instead of compressing,
 *     measuring no gain, and discarding the result.
 *   - Otherwise ZSTD is chosen; the actual level is derived later from the same
 *     entropy estimate in RecnoCompressAttribute.
 */
...
RecnoChooseCompressionType(Oid typid, Datum value, Size value_size)
{
	...
	/* An explicit codec from the GUC wins over the heuristic. */
	...
	/*
	 * Variable-length / binary value: probe its bytes.  If it looks
	 * incompressible, decline so the caller stores it raw.
	 */
	entropy = RecnoEstimateEntropyBits((const unsigned char *)
									   VARDATA_ANY(DatumGetPointer(value)),
									   value_size);
	if (entropy >= RECNO_ENTROPY_SKIP_BITS)
		return RECNO_COMP_NONE;
