...
#define SLOG_ENSURE_FETCHED() \
	...
	if (tuple->t_flags & RECNO_TUPLE_UNCOMMITTED)
	{
		 ...
		 */
		{
			...
			for (i = 0; i < slog_nfound; i++)
			{
					...
					TransactionIdEquals(e->xid, myxid))
				{
					...
					/*
					 * Subtransaction rollback of our own op.  For an
					 * UNCOMMITTED tuple this is the insert that created it;
					 * the rollback makes the tuple dead.  Remember this and
					 * decide after the loop (a later op on the same TID, if
					 * any, can still make it visible).
					 */
					if (e->op_type == RECNO_SLOG_ABORTED)
					{
						found_own_aborted = true;
						continue;
					}
			...
			/*
			 * Our own insert of this still-UNCOMMITTED tuple was rolled back
			 * by a subtransaction abort and no later op revived it: dead.
			 */
			if (found_own_aborted)
				goto not_visible;
		}
		 ...
		 * (Fix B makes this path rare by clearing the flag at commit time.)
		 */
clear_uncommitted:
...

		{
			...
			for (i = 0; i < slog_nfound; i++)
			{
					...
					TransactionIdEquals(e->xid, myxid))
				{
					...
					if (e->op_type == RECNO_SLOG_ABORTED)
					{
						found_own_aborted = true;	/* subtransaction rollback */
						continue;
					}
					if (tuple->t_flags & (RECNO_TUPLE_DELETED | RECNO_TUPLE_UPDATED))
						return false;
