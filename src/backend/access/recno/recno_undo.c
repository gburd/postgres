...
{
	/*
	 * RECNO's before-image size cap for the optional tuple sLog's
	 * backend-local savepoint-rollback scratch.  A tuple larger than this is
	 * not stashed; savepoint rollback of such a tuple would instead error.
	 * This is RECNO policy, supplied to the generic tuple sLog via its
	 * descriptor rather than hardcoded in the sLog.
	 */
	static const SLogAmDescriptor recno_slog_desc = {
		.before_image_max = 64 * 1024,
	};

	RegisterUndoRmgr(UNDO_RMID_RECNO, &recno_undo_rmgr);

	/* Opt in to tuple sLog tracking with RECNO's before-image policy. */
	SLogRegisterAmDescriptor(&recno_slog_desc);
