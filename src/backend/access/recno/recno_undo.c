					...
					const char *tuple_data, uint32 tuple_len)
{
	...
	{
		/*
		 * Unlogged / temp relations need no CLR: they do not survive
		 * a crash, so replay idempotency is irrelevant.
		 */
	...
	if (ItemIdGetLength(lp) < old_len)
	{
		ereport(DEBUG1,
				(errmsg_internal("RECNO UNDO: current slot at (%u, %u) is smaller "
								 "than before-image (%u < %u); skipping restore",
								 blkno, offnum,
				 ...
				 const char *payload, Size payload_len, UndoRecPtr urec_ptr)
{
	...
	/*
	 * Defer during crash recovery (syscache may not be initialised) or
	 * during an aborting transaction (BumpContext makes relation_close()
	 * and pfree() unsafe).  The logical-revert worker will re-drive the
	 * record from a clean memory context.
	 */
		...
		case RECNO_UNDO_DELTA_UPDATE:
			{
				 ...
				 * against the current (post-update) tuple to reconstruct
				 * the before-image, then restore.
				 */
				Page		page = BufferGetPage(buffer);
	...
	/*
	 * The physical before-image has been restored (INSERT undo marked the
	 * tuple deleted; UPDATE/DELETE undo restored the old image).  Now drop
	 * this (relation, tid)'s sLog op for the aborting xid: RECNO kept the op
	 * as RECNO_SLOG_ABORTED (set at XACT_EVENT_ABORT) so concurrent readers
	 * treated the row as still-live while the physical restore was pending.
	 * Now the restore is done, so the marker must go, otherwise the restored
	 * row stays hidden by the lingering ABORTED entry.  Scoped to the exact
	 * tid just restored so it is race-free.  During a SUBXACT (SAVEPOINT)
	 * apply the marker removal is handled by the subxact-scoped path, so skip
	 * it here.
	 */
	if (!RecnoPbuInSubxactApply())
		(void) RecnoSLogRemove(reloid, &hdr.tid, xid);
