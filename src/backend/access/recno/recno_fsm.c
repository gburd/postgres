 ...
 *	  (src/backend/storage/freespace).  It adds RECNO-specific relation
 ...
 *	  RecnoPageDefragment() in recno_tuple.c; the FSM only tracks free
 ...
 * a new page with ExtendBufferedRel(), initializes it with RecnoInitPage(),
...
RecnoGetPageWithFreeSpace(Relation rel, Size needed)
{
	...
	if (RelationNeedsWAL(rel))
	{
		...
		RecnoPageSetCommitTs(phdr, init_commit_ts);
