...
static void RecnoCompactPage(Page page);
...
/*
 * RecnoGetInsertFreeSpace
 *
 * Bytes an INSERT must find free on a page: the tuple itself plus its line
 * pointer plus the per-page in-place UPDATE reserve (see
 * RECNO_DEFAULT_FILLFACTOR in recno.h).  Every insert path routes its page
 * search through here so the reserve cannot be bypassed by one caller.
 *
 * UPDATE does NOT call this -- an in-place UPDATE is exactly who the reserve
 * is being held for, so it spends it.
 */
Size
RecnoGetInsertFreeSpace(Relation rel, Size tuple_size)
{
	Size		needed = tuple_size + sizeof(ItemIdData);
	Size		reserve = RelationGetTargetPageFreeSpace(rel,
													RECNO_DEFAULT_FILLFACTOR);
	Size		usable = BLCKSZ - RECNO_PAGE_OVERHEAD - SizeOfPageHeaderData;

	/*
	 * A row too wide to ever coexist with the reserve must still be storable,
	 * or the reserve would turn a working INSERT into an error.  Drop the
	 * reserve for such rows; they get a page (effectively) to themselves and
	 * simply have no growth room, which is the pre-existing behaviour.
	 */
	if (needed + reserve > usable)
		return needed;

	return needed + reserve;
}
...
RecnoDefragmentPage(Relation rel, BlockNumber blockno)
{
...

	{
		...
		/* Compact the page */
		RecnoCompactPage(page);

		MarkBufferDirty(buffer);

		/*
		 * Log the defragmentation.  This path retires no line pointers (see
		 * RecnoCompactPage), so there are no offsets for redo to replay and
		 * plain PageRepairFragmentation on both sides agrees.
		 */
		if (RelationNeedsWAL(rel))
		{
			XLogRecPtr	recptr = RecnoXLogDefrag(rel, buffer, NULL, 0,
												 defrag_ts);
...
/*
 * Compact a page, consolidating the free space left by tuple movement.
 *
 * This only closes gaps between live tuples; it does NOT reclaim deleted
 * tuples, because it does not retire their line pointers and RECNO deleted
 * tuples keep LP_NORMAL item pointers (deletion is tracked by
 * RECNO_TUPLE_DELETED in the tuple header).  Reclaiming them requires the
 * oldest-active-timestamp test done by the prune/vacuum paths in
 * recno_operations.c, which is where dead-tuple space is actually freed.
 *
 * Because no line pointers are retired here, redo reproduces this page by
 * running the same PageRepairFragmentation(), and XLOG_RECNO_DEFRAG needs to
 * carry no offsets for this path.
 */
static void
RecnoCompactPage(Page page)
{
