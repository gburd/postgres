				...
				RecnoOverflowBuffers * overflow_buffers)
{
	...
	int			i;
	int			recno_undo_block_id = 1;	/* next free WAL block id after main */
	 ...
	 * (before entering the critical section) to pre-allocate space for
	 * the main buffer plus all overflow buffers.
	 */
	XLogBeginInsert();
	...
	if (overflow_buffers != NULL)
	{
		...
		/*
		 * IMPORTANT: Overflow WAL headers must persist until XLogInsert()
		 * is called, because XLogRegisterBufData() stores pointers to the
		 * data, not copies.  Using a loop-local variable would cause all
		 * registered pointers to alias the same stack slot, so by the time
		 * XLogInsert() assembles the record every header would contain the
		 * values from the last loop iteration -- corrupting the WAL record
		 * and causing "corrupt overflow data" panics during redo.
		 */
		...
		for (i = 0; i < overflow_buffers->count; i++)
		{
			...
			/*
			 * First check if this overflow buffer is the SAME as the main buffer.
			 * This can happen when spatial locality places an overflow record on
			 * the same page as the main tuple. In this case, reuse block_id=0.
			 */
			...
			else
			{
				 ...
				 * buffers (spatial locality: multiple overflow records on same
				 * page). If so, reuse its block_id instead of registering again.
				 */
				...
				if (block_id < 0)
				{
					...
					/*
					 * Force a full-page image for overflow buffers.
					 * See RecnoXLogUpdate for the rationale.
					 */
			 ...
			 * This tells the redo handler where to place the record.
			 * Each header is stored in a dedicated array slot so the pointer
		...
		recno_undo_block_id = registered_block_id;
	}
	...
	/* Fold the per-backend undo pages onto this record as full-page images. */
	if (RecnoPbuUndoPending())
		RecnoPbuRegisterUndoBuffers((uint8) recno_undo_block_id);
	 ...
	 * may appear multiple times in overflow_buffers. PageSetLSN is
	 * idempotent (setting the same LSN multiple times is safe), so we can
	 * just iterate through all entries without checking for duplicates.
	 */
	if (overflow_buffers != NULL)
	{
		for (i = 0; i < overflow_buffers->count; i++)
				...
				Buffer new_buffer)
{
	...
	bool		is_cross_page = (BufferIsValid(new_buffer) &&
								new_buffer != buffer);
	/*
	 * Prefix/suffix header for delta-logged same-size in-place updates.
	 * Declared at function scope so the bytes registered via XLogRegisterData
	 * remain live until XLogInsert() assembles the record.  A block-scoped
	 * local here was a dangling pointer -- its stack slot was reused before
	 * XLogInsert copied it, so redo read a garbage prefix/suffix and PANICked
	 * once the data-page FPI was removed (caveat 3).
	 */
	...
	xlrec.old_tuple_len = old_tuple->t_len;
	 ...
	 *
	 * Caveat-3 (data-page FPI amplification) fix: force a full-page image
	 * ONLY for GROWING updates.  A growing update relocates the tuple via
	 * RecnoPageIndexTupleDelete + PageAddItem (page compaction), which the
	 * BLK_NEEDS_REDO delta path cannot reconstruct -- it PANICs "new tuple
	 * larger than existing slot without FPI" -- so those genuinely need the
	 * image.
	 *
	 * A same-size or shrinking same-page in-place UPDATE (RECNO's common
	 * case, e.g. TPC-C NEW_ORDER) is fully replayable from the logged
	 * prefix/suffix delta (or full new-tuple image) in recno_xlog_update_redo:
	 * the tuple stays at xlrec->offnum and the slot never grows.  Forcing an
	 * 8 KB image there was pure WAL amplification.  Cross-page updates already
	 * restore the destination page from its own FPI and mark the old tuple
	 * UPDATED in the delta path, so the source page needs no image unless the
	 * new tuple grew.
	 *
	 * wal_consistency_checking=all confirms the delta-replayed page is
	 ...
	 */
	{
			...
			for (i = 0; i < overflow_buffers->count; i++)
			{
				...
				else
				{
					...
					if (block_id < 0)
					{
						...
						/*
						 * Force a full-page image for overflow buffers.
						 * The redo handler for overflow pages reconstructs
						 * items using PageAddItem, but the page layout can
						 * differ from the primary when the page already
						 * contains items from prior operations (e.g.,
						 * free space fragmentation, item alignment).
						 * Using REGBUF_FORCE_IMAGE guarantees the page
						 * is restored exactly as the primary had it.
						 */
						XLogRegisterBuffer(block_id, ovb->buffer,
										   REGBUF_STANDARD | REGBUF_FORCE_IMAGE);
				...
				XLogRegisterBufData(block_id, ovb->record_data,
									ovb->record_len);
			}
		}
		...
		if (is_cross_page)
		{
			...
			XLogRegisterBuffer(next_block_id, new_buffer,
							   REGBUF_STANDARD | REGBUF_FORCE_IMAGE);
			next_block_id++;
		}
		recno_undo_block_id = next_block_id;
	}
	 ...
	 *   only the new tuple data.
	 * - Growing updates: REGBUF_FORCE_IMAGE is set above, so redo restores
	 *   the page from a full-page image and never enters BLK_NEEDS_REDO.
	 *
		...
		new_tuple->t_len > 0)
	{
		...
		/*
		 * Use compression only if the savings exceed the header overhead.
		 * The header is 4 bytes (two uint16s), so we need the prefix +
		 * suffix to save more than that.
		 */
				...
				RecnoTuple tuple, uint64 commit_ts)
{
	 ...
	 * the UNDO log.  The redo handler only needs the offset and commit_ts to
 ...
 *
 * unused_offsets/nunused must list exactly the line pointers the caller
 * retired with ItemIdSetUnused() before calling RecnoPageDefragment(), in
 * any order.  Redo retires the same set before compacting, which is what
 * makes the replayed page byte-identical to the primary's.  Callers that
 * retire nothing pass NULL/0.
 */
XLogRecPtr
RecnoXLogDefrag(Relation rel, Buffer buffer, OffsetNumber *unused_offsets,
				int nunused, uint64 commit_ts)
{
	...
	Assert(nunused >= 0 && nunused <= MaxOffsetNumber);
	Assert(nunused == 0 || unused_offsets != NULL);
	...
	if (nunused > 0)
		XLogRegisterData((char *) unused_offsets,
						 ...
						 const void *tuple_data, uint32 tuple_len)
{
	...
	XLogRegisterBuffer(0, dst_buf, REGBUF_STANDARD);
				   ...
				   const xl_recno_hlc_info * hlc_info)
{
	...
	Page		page = BufferGetPage(buffer);
	uint8		info = XLOG_RECNO_UPDATE_INPLACE;
	/* Function-scope so registered bytes outlive the if-block (see caveat 3). */
	xl_recno_prefix_suffix ps;
	...
	/* Force FPI for size-increasing updates (same rationale as RecnoXLogUpdate) */
	{
		uint8		buf_flags = REGBUF_STANDARD;
	...
	/*
	 * Prefix/suffix compression for same-size in-place updates.
	 * Same logic as RecnoXLogUpdate but simpler since there's no cross-page.
	 * ps is declared at function scope (see below) so the bytes registered
	 * with XLogRegisterData outlive this block until XLogInsert().
	 */
				   ...
				   const xl_recno_hlc_info * hlc_info)
{
	...
	XLogBeginInsert();
	XLogRegisterData((char *) &xlrec, sizeof(xl_recno_delete));
	/* Old tuple data is in UNDO log exclusively */
...
{
	RelFileLocator rlocator pg_attribute_unused();
	BlockNumber blkno;
	Buffer		buffer;
	Page		page;
...

	{
		...
		if (action == BLK_NEEDS_REDO)
		{
			...
			/*
			 * XLogInitBufferForRedo does standard PageInit for new
			 * pages, but doesn't set up RECNO opaque space.
			 * Initialize it here if needed.
			 */
			if (PageIsNew(page))
			{
			...
			/*
			 * Speculative confirmation.  This XLOG_RECNO_INSERT record
			 * carries no tuple image; it only clears the speculative and
			 * uncommitted flags on the already-present tuple and restores
			 * t_ctid to point at itself (mirroring
			 * recno_tuple_complete_speculative's succeeded path, and heap's
			 * XLOG_HEAP_CONFIRM).  Replaying it as an insert would PANIC
			 * with "insert failed to add tuple".
			 */
			if (xlrec->flags & RECNO_WAL_SPEC_CONFIRM)
			{
				...
				{
					RecnoTupleHeader *conf_hdr =
						(RecnoTupleHeader *) PageGetItem(page, itemid);

					/*
					 * Rollback-visibility fix: clear only SPECULATIVE, NOT
					 * RECNO_TUPLE_UNCOMMITTED -- mirror
					 * recno_tuple_complete_speculative's succeeded path so the
					 * redone page matches the running server.  UNCOMMITTED is
					 * cleared by commit, or stays set (with t_xmin's CLOG state)
					 * on a rolled-back xid so a recovered row is invisible.
					 */
					conf_hdr->t_flags &= ~RECNO_TUPLE_SPECULATIVE;
					ItemPointerSet(&conf_hdr->t_ctid, blkno, xlrec->offnum);
				}
			 ...
			 * This means overflow records get lower offsets (1, 2, 3...)
			 * and the main tuple gets a higher offset (4, ...).
			 *
			 * We MUST replay in the same order. If there are overflow
			 * records on block_id=0 (same page as main tuple due to
			 * spatial locality), replay them FIRST before the main tuple.
			 */
				...
				(xlrec->flags & RECNO_WAL_HAS_OVERFLOW_BLK0))
			{
				...
				/*
				 * Block 0 has overflow data. Parse and replay all
				 * overflow records on this block before the main tuple.
				 * Each overflow record has format:
				 * [xl_recno_overflow_write header][actual record data]
				 */
				while (ovf_remaining > sizeof(xl_recno_overflow_write))
				{
					...
					/*
					 * Place each overflow record at the EXACT offset the
					 * primary used (recorded in the WAL), with overwrite=true
					 * so redo is idempotent and immune to page-state
					 * divergence.  Using InvalidOffsetNumber (append) risked a
					 * "failed to add" PANIC when the page reconstructed during
					 * crash recovery did not match the primary's slot layout.
					 */
					ovf_offnum = PageAddItem(page, actual_data, actual_len,
											 ovf_xlrec->offnum, true, false);
					if (ovf_offnum == InvalidOffsetNumber)
						elog(PANIC, "RECNO INSERT redo: failed to add overflow record on block 0 at offnum %u",
							 ovf_xlrec->offnum);
			...
			/*
			 * Replay the main tuple at the EXACT offset the primary chose
			 * (xlrec->offnum), heap-style: overwrite=true makes redo
			 * idempotent, and placing at the recorded offset (rather than
			 * appending via InvalidOffsetNumber) keeps redo consistent with
			 * the primary even when the page's line-pointer layout during
			 * crash recovery differs from a fresh sequential build.  The
			 * append-based scheme could PANIC ("insert failed to add tuple")
			 * or silently diverge the TID<->offset mapping.
			 */
			if (PageGetMaxOffsetNumber(page) + 1 < xlrec->offnum)
				elog(PANIC, "RECNO redo: insert offnum %u beyond page max %u",
					 xlrec->offnum, PageGetMaxOffsetNumber(page));
			inserted_offnum = PageAddItem(page, tuple_hdr, tuple_hdr->t_len,
										  xlrec->offnum, true, false);
			if (inserted_offnum == InvalidOffsetNumber)
			{
				elog(PANIC, "RECNO redo: insert failed to add tuple at offnum %u (t_len=%u freespace=%zu)",
					 xlrec->offnum, tuple_hdr->t_len, PageGetFreeSpace(page));
			}

			/*
			 * Fix the tuple's t_ctid to point to itself at the
			 * correct location. During normal operation, this is set
			 * in recno_tuple_insert after we know the final TID.
			 * During redo, we must fix it here.
			 */
			{
				ItemId		itemid = PageGetItemId(page, inserted_offnum);
				RecnoTupleHeader *inserted_hdr = (RecnoTupleHeader *) PageGetItem(page, itemid);
			...
			/*
			 * Update page header.  CRITICAL: Must replicate the exact
			 * logic from RecnoPageAddTuple() so the page matches the
			 * Full Page Write.  RecnoPageAddTuple sets the
			 * RECNO_PAGE_DEFRAG_NEEDED flag based on fragmentation
			 * heuristics, so we must do the same here.
			 */
			phdr = RecnoPageGetOpaque(page);
			phdr->pd_commit_ts = Max(phdr->pd_commit_ts, xlrec->commit_ts);
			phdr->pd_free_space = PageGetFreeSpace(page);

			/*
			 * Mark page for defragmentation if fragmented. This
			 * matches the logic in RecnoPageAddTuple() at
			 * recno_tuple.c:513-517.
			 */
			if (PageGetFreeSpace(page) >= tuple_hdr->t_len * 2 &&
		...
		/*
		 * Process overflow buffers on separate pages (buffers 1..N).
		 * Each overflow buffer contains an overflow record that was
		 * registered with XLogRegisterBufData during WAL logging.
		 *
		 * Note: Overflow records on block_id=0 were already handled
		 * above before the main tuple to preserve insertion order.
		 */
		for (int ovf_idx = 1; ovf_idx < XLR_MAX_BLOCK_ID; ovf_idx++)
		{
			...
			/*
			 * Skip per-backend UNDO pages folded onto this record: they carry
			 * no full-page image (WILL_INIT) and are replayed by
			 * RecnoPbuRedoUndo, not as overflow pages.  Without this the
			 * WILL_INIT read below PANICs and undo bytes would be parsed as an
			 * overflow record.
			 */
			if (RecnoPbuBlockIsUndo(record, (uint8) ovf_idx))
				continue;

			ovf_action = XLogReadBufferForRedo(record, ovf_idx, &ovf_buffer);
			if (ovf_action == BLK_NEEDS_REDO)
			{
				...
				ovf_data = XLogRecGetBlockData(record, ovf_idx, &ovf_len);
				if (ovf_data != NULL && ovf_len > 0)
				{
					char	   *ovf_ptr = ovf_data;
					Size		ovf_remaining = ovf_len;
					 ...
					 * Multiple overflow records may be on the same page
					 * due to spatial locality optimization.
					 */
					while (ovf_remaining > sizeof(xl_recno_overflow_write))
					{
						xl_recno_overflow_write *ovf_xlrec = (xl_recno_overflow_write *) ovf_ptr;
						...
						/*
					 * Use the specific offset from WAL record.
					 * Overflow pointers reference these offsets.
					 * Use the specific offset from WAL record.
					 * Overflow pointers reference these offsets.
						 */
						/* overwrite=true: primary used InvalidOffsetNumber (slot reuse) */
						ovf_offnum = PageAddItem(ovf_page, actual_data, actual_len,
												 ovf_xlrec->offnum, true, false);
						if (ovf_offnum == InvalidOffsetNumber)
							elog(PANIC, "RECNO INSERT redo: failed to add overflow record on block %u",
								 BufferGetBlockNumber(ovf_buffer));
	...
	/*
	 * Re-run the per-backend UNDO insert folded onto this record, rebuilding
	 * the undo page content from the undo region rather than from a forced
	 * full-page image.  No-op if this INSERT carried no undo.
	 */
	RecnoPbuRedoUndo(record, 1);
}
...
recno_xlog_update_inplace_redo(XLogReaderState *record)
{
...

	{
		 ...
		 *
		 * Without prefix/suffix: [xl_recno_update][full new tuple data]
		 * With prefix/suffix:    [xl_recno_update][xl_recno_prefix_suffix][diff bytes]
		 *
		 * Old tuple data is in the UNDO log exclusively.
		 */
		...
		if (action == BLK_NEEDS_REDO)
		{
			...
			/*
			 * Process overflow records on block 0 BEFORE the main
			 * tuple, matching the original insertion order.  During
			 * normal operation, overflow records stored on the same
			 * page as the main tuple (spatial locality) get lower
			 * offsets.  We must replay them first so the main tuple
			 * ends up at the correct offset.
			 */
			...
			{
				elog(WARNING, "RECNO UPDATE REDO: ItemId at offnum=%u is not normal", xlrec->offnum);
			}

			/* Update page header */
			phdr = RecnoPageGetOpaque(page);
			phdr->pd_commit_ts = Max(phdr->pd_commit_ts, xlrec->new_commit_ts);
			phdr->pd_free_space = PageGetFreeSpace(page);
		...
		/*
		 * Process overflow buffers on separate pages (buffers 1..N)
		 * for UPDATE.  Multiple overflow records may share a single
		 * block due to spatial locality, so we loop through all
		 * records within each block's data (matching INSERT redo).
		 */
		for (int ovf_idx = 1; ovf_idx < XLR_MAX_BLOCK_ID; ovf_idx++)
		{
			Buffer		ovf_buffer;
			...
			/*
			 * Skip per-backend UNDO pages folded onto this record (see the
			 * INSERT redo loop): no full-page image, replayed by
			 * RecnoPbuRedoUndo.
			 */
			if (RecnoPbuBlockIsUndo(record, (uint8) ovf_idx))
				continue;
			...
			if (ovf_action == BLK_NEEDS_REDO)
			{
				...
				if (ovf_data != NULL && ovf_len > 0)
				{
					...
					/*
					 * Parse and replay all overflow records on
					 * this block.  Multiple overflow records may
					 * share a page due to spatial locality.
					 */
					while (ovf_remaining > sizeof(xl_recno_overflow_write))
					{
						...
						/*
						 * Use InvalidOffsetNumber to append
						 * sequentially, matching the original
						 * insertion order within this page.
						 */
						ovf_offnum = PageAddItem(ovf_page, actual_data, actual_len,
												 InvalidOffsetNumber, false, false);
						if (ovf_offnum == InvalidOffsetNumber)
	 ...
	 * Re-run the per-backend UNDO insert folded onto this record, rebuilding
	 * the undo page content from the undo region rather than from a forced
	 * full-page image.  No-op if this UPDATE carried no undo.
	 */
	RecnoPbuRedoUndo(record, 1);
}
...
recno_xlog_delete_redo(XLogReaderState *record)
{
...

	{
		...
		/*
		 * WAL record contains only the delete header (offset +
		 * commit_ts).  Old tuple data is stored exclusively in the
		 * UNDO log for transaction rollback and is not needed here.
		 */
	 ...
	 * Re-run the per-backend UNDO insert folded onto this record, rebuilding
	 * the undo page content from the undo region rather than from a forced
	 * full-page image.  No-op if this DELETE carried no undo.
	 */
	RecnoPbuRedoUndo(record, 1);
}
...
recno_xlog_defrag_redo(XLogReaderState *record)
{
...

	{
		...
		OffsetNumber *unused_offsets = (OffsetNumber *)
			((char *) xlrec + sizeof(xl_recno_defrag));
		XLogRedoAction action;
		...
		if (action == BLK_NEEDS_REDO)
		{
			...
			/*
			 * Retire the same line pointers the primary retired, BEFORE
			 * compacting.  RECNO marks deleted tuples with a flag in the
			 * tuple header and leaves the line pointer LP_NORMAL, so these
			 * items still have storage as far as PageRepairFragmentation is
			 * concerned; if we do not retire them first it preserves them and
			 * produces a different page than the primary did.  The offsets
			 * come from the WAL record rather than being recomputed, because
			 * reclaimability depends on the oldest active timestamp at the
			 * time of the original operation, which redo cannot reconstruct.
			 */
			for (int i = 0; i < xlrec->nunused; i++)
			{
				OffsetNumber offnum = unused_offsets[i];
				ItemId		itemid;

				if (offnum < FirstOffsetNumber ||
					offnum > PageGetMaxOffsetNumber(page))
					elog(PANIC, "recno defrag redo: offset %u out of range (max %u)",
						 offnum, PageGetMaxOffsetNumber(page));
...
recno_xlog_overflow_write_redo(XLogReaderState *record)
{
...

	{
		...
		if (action == BLK_NEEDS_REDO)
		{
			...
			{
				/*
				 * Link update: overwrite the existing overflow record
				 * header at the specified offset with updated chain
				 * pointers.
				 */
				ItemId		itemid;
			...
			{
				/*
				 * New overflow record: the logged data is the
				 * complete record (RecnoOverflowRecordHeader + chunk
				 * data). Add it to the page at the specified offset.
				 */
				OffsetNumber offnum;

				/*
				 * overwrite=true is REQUIRED here.  The primary added this
				 * item with PageAddItem(..., InvalidOffsetNumber, ...), which
				 * REUSES an existing LP_UNUSED slot in place when one exists.
				 * Replaying with overwrite=false and an explicit offset makes
				 * PageAddItemExtended set needshuffle and INSERT a new line
				 * pointer, shifting every later entry up by one -- the
				 * replayed page then has one more line pointer than the
				 * primary and different lp_off values throughout, which
				 * wal_consistency_checking reports as an inconsistent page.
				 */
...
recno_xlog_init_page_redo(XLogReaderState *record)
{
...

	{
		...
		if (action == BLK_NEEDS_REDO)
		{
			...
			phdr = RecnoPageGetOpaque(page);
			phdr->pd_commit_ts = xlrec->commit_ts;
			phdr->pd_flags = xlrec->flags;
...
recno_xlog_cross_page_defrag_redo(XLogReaderState *record)
{
...

	{
		...
		if (dst_action == BLK_NEEDS_REDO)
		{
			...
			/*
			 * overwrite=true is REQUIRED here.  The primary added this tuple
			 * with PageAddItem(..., InvalidOffsetNumber, ...), which REUSES an
			 * existing LP_UNUSED slot in place when one exists.  Replaying
			 * with overwrite=false and an explicit offset makes
			 * PageAddItemExtended set needshuffle and INSERT a new line
			 * pointer, shifting every later entry up by one -- the replayed
			 * page then has one more line pointer than the primary and
			 * different lp_off values throughout, which
			 * wal_consistency_checking reports as an inconsistent page.
			 */
			if (PageAddItem(page, tuple_data, xlrec->tuple_len,
							xlrec->dst_offnum, true, false)
				== InvalidOffsetNumber)
				elog(PANIC, "recno cross-page defrag failed to add tuple to target page");
...
{
	{
		...
		/*
		 * Block 0 is the heap buffer, registered with
		 * REGBUF_NO_CHANGE.  We don't need to redo it since the
		 * heap page is not modified by VM operations.
		 */
...
recno_mask(char *page, BlockNumber blkno)
{
	...
	/*
	 * Pages without a RECNO opaque special area (e.g. a page reinitialized
	 * by redo with PageInit(page, BLCKSZ, 0), or any non-RECNO page a
	 * full-page image happens to cover) must not be interpreted as RECNO
	 * data pages: RecnoPageGetOpaque() would read past the page end and the
	 * line-pointer loop below would mis-mask arbitrary bytes, corrupting the
	 * consistency-check scratch buffer (observed as an allocator assert /
	 * "pfree invalid pointer" later in redo).  This guard mirrors flux_mask
	 * and is what lets wal_consistency_checking run against RECNO at all.
	 */
	...
	/* Check page type before masking pd_flags */
	is_overflow = (phdr->pd_flags & RECNO_PAGE_OVERFLOW) != 0;

	/*
	 * Mask fields that can vary between primary and standby.
	 *
	 * pd_free_space: Recomputed during redo; may differ due to alignment.
	 * pd_flags: Heuristic flags (e.g., RECNO_PAGE_DEFRAG_NEEDED) may be
	 *           set by redo but not by the original operation, or vice versa.
	 * pd_commit_ts: Redo uses Max(existing, new) which can produce a
	 *               different value if the page was concurrently modified.
	 */
	phdr->pd_free_space = 0;
	phdr->pd_flags = 0;
	phdr->pd_commit_ts = 0;

	/*
	 * Mask the tuple-storage space that no live line pointer references.
	 *
	 * Unlike heap, RECNO does not clear a reclaimed tuple's bytes: the prune,
	 * vacuum and defrag paths retire the line pointer with ItemIdSetUnused()
	 * and leave the payload in place until some later
	 * PageRepairFragmentation() happens to overwrite it.  Which stale bytes
	 * survive above pd_upper therefore depends on the exact history of
	 * compactions a page went through, and the primary's history is not the
	 * standby's: the primary can prune opportunistically (RecnoPagePruneOpt,
	 * or the in-line defrag in recno_tuple_update) in between two WAL-logged
	 * operations, while redo arrives at the same live contents by replaying
	 * only the logged ones.  The result is a page whose header, line-pointer
	 * array and every live tuple match byte for byte and that differs only in
	 * dead space -- which mask_unused_space() does not reach, since it clears
	 * just the pd_lower..pd_upper hole.
	 *
	 * Only bytes actually covered by a live item are preserved, so genuine
	 * divergence inside a live tuple is still reported.  Alignment padding
	 * following an item whose length is not MAXALIGNed lies outside its
	 * lp_len and is therefore masked too, as heap_mask() also does.
	 */
	{
		int			region_start = ((PageHeader) recno_page)->pd_upper;
		int			region_end = ((PageHeader) recno_page)->pd_special;
		uint8		live[BLCKSZ / BITS_PER_BYTE] = {0};
		int			i;
		...
		for (offnum = FirstOffsetNumber; offnum <= maxoff; offnum++)
		{
			...
			start = ItemIdGetOffset(itemid);
			end = start + ItemIdGetLength(itemid);

			/* ignore anything not wholly inside the tuple region */
			if (start < region_start || end > region_end || end <= start)
				continue;

			for (i = start; i < end; i++)
				live[i / BITS_PER_BYTE] |= (1 << (i % BITS_PER_BYTE));
		}

		for (i = region_start; i < region_end; i++)
			if (!(live[i / BITS_PER_BYTE] & (1 << (i % BITS_PER_BYTE))))
				page[i] = MASK_MARKER;
	}

	/*
	 * Overflow pages contain RecnoOverflowRecordHeader items, not regular
	 * tuples.  Their contents are fully determined by the WAL data, so
	 * no per-item masking is needed.
	 */
	if (is_overflow)
		return;
	 ...
	 * faithfully reproduced by WAL redo.  The redo handlers only set
	 * the minimal fields needed for correctness (t_flags, t_commit_ts);
	 * transactional fields like infomask bits are set on the
	 * primary but not replayed.
	 */
	maxoff = PageGetMaxOffsetNumber(recno_page);
	for (offnum = FirstOffsetNumber; offnum <= maxoff; offnum++)
	{
		...
		tuple_hdr->t_flags = 0;		/* RECNO_TUPLE_UNCOMMITTED cleared lazily */
		tuple_hdr->t_commit_ts = 0;	/* redo folds via Max() */
