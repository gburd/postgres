 ...
 */
void
RecnoXLogPrepareLogicalImage(Relation rel, RecnoTuple rtup,
...
void
RecnoXLogReleaseLogicalImage(RecnoLogicalImage *img)
{
	if (img->data != NULL)
	{
		pfree(img->data);
 ...
 *		sets RECNO_WAL_LOGICAL_TUPLE in *flags.  No-op when img is NULL or
 *		empty (relation not logically logged).
 */
static void
RecnoXLogRegisterLogicalImage(RecnoLogicalImage *img, uint16 *flags)
{
				...
				bool force_page_image)
{
	...
	RecnoXLogRegisterLogicalImage(logical_img, &xlrec.flags);
 ...
 *
 * Unlike RecnoXLogInsert, this records EVERY tuple body in the batch rather
 ...
 *   [xl_recno_multi_insert header]
 *   ntuples * [xl_recno_multi_insert_tuple header][tuple t_data body]
 *   ntuples * [logical image bytes][uint32 len]   (only when logically logged)
 */
XLogRecPtr
RecnoXLogMultiInsert(Relation rel, Buffer buffer,
					 OffsetNumber *offnums, RecnoTuple *tuples,
					 int ntuples, uint64 commit_ts,
					 RecnoLogicalImage *logical_imgs)
{
	xl_recno_multi_insert xlrec;

	/*
	 * Serialize the whole per-tuple region into one scratch buffer and register
	 * it as a single rdata chunk.  XLogRegisterData() is limited to
	 * XLR_NORMAL_RDATAS (20) slots, and XLogEnsureRecordSpace() cannot grow that
	 * inside a critical section, so registering two chunks per tuple would
	 * overflow the slot array once a page packs more than a handful of rows.
	 * Mirrors heap_multi_insert()'s single-scratch-buffer pattern; the on-WAL
	 * bytes are identical to per-tuple registration because redo and logical
	 * decoding read the region as one contiguous XLogRecGetData() stream.
	 *
	 * The region is bounded by BLCKSZ: every tuple consumed sizeof(ItemIdData) +
	 * MAXALIGN(t_len) on the page, so the sum of SizeOfRecnoMultiInsertTuple +
	 * t_len across the batch is strictly less than one page.
	 */
	PGAlignedBlock scratch;
	char	   *scratchptr = scratch.data;
	XLogRecPtr	recptr;
	Page		page = BufferGetPage(buffer);
	uint8		info = XLOG_RECNO_MULTI_INSERT;
	int			i;
	...
	for (i = 0; i < ntuples; i++)
	{
		xl_recno_multi_insert_tuple *tuphdr;
		...
		tuphdr = (xl_recno_multi_insert_tuple *) scratchptr;

		tuphdr->offnum = offnums[i];
		tuphdr->datalen = (uint16) tuples[i]->t_len;
		scratchptr += SizeOfRecnoMultiInsertTuple;
	...
	/*
	 * Register the page WITHOUT forcing a full-page image.  Crash safety comes
	 * from logging every tuple body above, not from an FPI.
	 */
	XLogRegisterBuffer(0, buffer, REGBUF_STANDARD);

	XLogRegisterData((char *) &xlrec, SizeOfRecnoMultiInsert);
	XLogRegisterData(scratch.data, (uint32) (scratchptr - scratch.data));

	/*
	 * Append one logical-decoding image per tuple (when logically logged).  The
	 * caller pre-serialized all images into one contiguous blob before entering
	 * the critical section (logical_imgs[0].data), so a single registration
	 * covers the whole region and keeps the rdata slot count constant.
	 */
	if (logical_imgs != NULL && logical_imgs[0].data != NULL)
	{
		XLogRegisterData(logical_imgs[0].data, logical_imgs[0].len);
		xlrec.flags |= RECNO_WAL_LOGICAL_TUPLE;
	}
				...
				RecnoLogicalImage *new_img)
{
	 ...
	 * overwrites the slot with the new tuple data (or patches the prefix/suffix
	 * diff), so no full-page image is required.  An image is only needed when
	 * the new tuple is larger than the old one, because the write path then
	 * deletes and re-adds the line pointer (shifting page data), which redo
	 * cannot replay from the new tuple bytes alone.  The predicate
	 * new_tuple->t_len > old_tuple->t_len is a safe superset of that grow case:
	 * the on-page slot is always at least old_tuple->t_len, so any update that
	 * outgrows the slot also outgrows old_tuple->t_len and forces the image.
	 *
	 * For cross-page updates the redo handler marks the old tuple UPDATED
	 * directly (see RECNO_WAL_CROSS_PAGE handling in recno_redo) and the new
	 ...
	 */
	{
		...
		RecnoXLogRegisterLogicalImage(old_img, &tmpflag);
		RecnoXLogRegisterLogicalImage(new_img, &tmpflag);
		xlrec.flags |= tmpflag;
	}
...
/*
 * RecnoXLogCasUpdateUndo -- FOLD variant of RecnoXLogCasUpdate.
 *
 * Emits ONE combined WAL record carrying both the main-fork redo byte-diff
 * (block 0, identical to RecnoXLogCasUpdate) and the relundo-fork UNDO
 ...
 */
XLogRecPtr
RecnoXLogCasUpdateUndo(Relation rel, Buffer buffer, OffsetNumber offnum,
					   uint16 data_offset, uint16 data_len,
					   const char *new_data, uint64 new_commit_ts,
					   const struct RelUndoStageResult *undo)
{
	xl_recno_cas_update_undo xlrec;
	XLogRecPtr	recptr;
	Page		page = BufferGetPage(buffer);
	...
	XLogBeginInsert();
	XLogRegisterData((char *) &xlrec, SizeOfRecnoCasUpdateUndo);
	XLogRegisterData(new_data, data_len);
	 ...
	 * off xlrec.is_new_page, not a record-level info bit, because the RECNO
	 * rmgr consumes the whole info upper-nibble as the opcode); the staged
	 * block data prepends the RelUndoPageHeaderData in that case (RelUndoStage
	 * built wal_record_data accordingly).  An existing page registers the
	 * record bytes at page_offset with flag 0 to keep the FPI faithful
	 * (relundo data pages are non-standard).
	 */
	if (undo->is_new_page)
	...
	recptr = XLogInsert(RM_RECNO_ID, XLOG_RECNO_CAS_UPDATE_UNDO);
...
static void recno_xlog_multi_insert_redo(XLogReaderState *record);
...
recno_xlog_insert_redo(XLogReaderState *record)
{
	...
	insert_skip_tuple:
		 ...
		 * that).  Without this, the visibility check sees slog_nfound==0
...
/*
 * recno_xlog_multi_insert_redo
 *		REDO handler for XLOG_RECNO_MULTI_INSERT.
 *
 ...
 */
static void
recno_xlog_multi_insert_redo(XLogReaderState *record)
{
	RelFileLocator rlocator;
	BlockNumber blkno;
	Buffer		buffer;
	...
	{
		xl_recno_multi_insert *xlrec = (xl_recno_multi_insert *) XLogRecGetData(record);
		int			ntuples = xlrec->ntuples;
		char	   *cursor = (char *) xlrec + SizeOfRecnoMultiInsert;
		XLogRedoAction action;
		...
		if (action == BLK_RESTORED)
		{
			...
			for (i = 0; i < ntuples; i++)
			{
				xl_recno_multi_insert_tuple *thdr;

				c = (char *) SHORTALIGN(c);
				thdr = (xl_recno_multi_insert_tuple *) c;

				final_offnums[i] = thdr->offnum;
				c += SizeOfRecnoMultiInsertTuple + thdr->datalen;
			}
		}
		...
		if (action == BLK_NEEDS_REDO)
		{
			...
			for (i = 0; i < ntuples; i++)
			{
				...
				thdr = (xl_recno_multi_insert_tuple *) cursor;
				body = cursor + SizeOfRecnoMultiInsertTuple;
				...
				if (datalen == 0)
				{
					elog(WARNING, "RECNO MULTI_INSERT redo: zero-length tuple "
				 ...
				 * failure (the page may have been advanced past this record by
				 * a later checkpointed operation), exactly as the single
				...
				if (inserted_offnum == InvalidOffsetNumber)
				{
					elog(WARNING, "RECNO MULTI_INSERT redo: failed to add "
			 ...
			 * RecnoPageAddTuple().  Use the largest tuple body in the batch as
		...
		/*
		 * Capture the UNCOMMITTED flag from each replayed tuple while we still
		 * hold the buffer (true for both BLK_NEEDS_REDO and BLK_RESTORED).
		 */
		if (BufferIsValid(buffer))
		{
			Page		curpage = BufferGetPage(buffer);
			...
			if (ItemIdIsNormal(itemid) && ItemIdHasStorage(itemid))
			{
				...
				else
				{
					...
					if (use_prefix_suffix)
					{
						 ...
						 * must anchor on xlrec->new_tuple_len -- NOT the on-page
						 * slot length.  A prior non-shrinking shrink update can
						 * leave the slot larger than the tuple's logical length
						 * (see recno_operations.c), so using existing_len here
						 * would over-copy past the diff bytes and corrupt the
						 * tuple's suffix.
						 */
					...
					else if (xlrec->new_tuple_len <= existing_len)
					{
						 ...
						 * slot length so the undo before-image can be restored
						 * into it (see recno_operations.c).  Redo must produce
						 * the identical slot length or WAL consistency checking
						 * (wal_consistency_checking=recno) will diverge.
						 */
...
/*
 * recno_xlog_cas_update_undo_redo
 *		REDO handler for XLOG_RECNO_CAS_UPDATE_UNDO (FOLD variant).
 *
 * Replays two page changes from one record:
 *   block 0 - main-fork page: patch the tuple data bytes, set commit ts,
 *             clear t_writer (identical to recno_xlog_cas_update_redo).
 ...
 */
static void
recno_xlog_cas_update_undo_redo(XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_recno_cas_update_undo *xlrec =
		(xl_recno_cas_update_undo *) XLogRecGetData(record);
	char	   *new_data = ((char *) xlrec) + SizeOfRecnoCasUpdateUndo;
	Buffer		buffer;
	XLogRedoAction action;
	...
	if (XLogReadBufferForRedo(record, 0, &buffer) == BLK_NEEDS_REDO)
	{
		...
		itemid = PageGetItemId(page, xlrec->offnum);
		if (!ItemIdIsNormal(itemid) || !ItemIdHasStorage(itemid))
			elog(PANIC, "RECNO CAS_UPDATE_UNDO redo: invalid item at offset %u",
				 xlrec->offnum);
...
recno_redo(XLogReaderState *record)
{
	...
	switch (info)
	{
		...
		case XLOG_RECNO_CAS_UPDATE_UNDO:
			recno_xlog_cas_update_undo_redo(record);
			break;
		...
		case XLOG_RECNO_MULTI_INSERT:
			recno_xlog_multi_insert_redo(record);
			break;
