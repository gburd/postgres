				...
				Buffer new_buffer)
{
	 ...
	 * the UNDO fork (via RelUndoFinish) and is not needed during WAL replay:
	 *
	...
	/* Set LSN on destination page for cross-page updates */
	if (is_cross_page)
	{
		Page		new_page = BufferGetPage(new_buffer);

		PageSetLSN(new_page, recptr);
	}
		...
		case XLOG_RECNO_INSERT:
			{
				...
				if (action == BLK_NEEDS_REDO)
				{
						...
						(xlrec->flags & RECNO_WAL_HAS_OVERFLOW_BLK0))
					{
						...
						elog(LOG, "RECNO INSERT redo: block 0 has %zu bytes of data, "
							 "first 32 bytes: %02x %02x %02x %02x %02x %02x %02x %02x "
							 "%02x %02x %02x %02x %02x %02x %02x %02x "
							 "%02x %02x %02x %02x %02x %02x %02x %02x "
							 "%02x %02x %02x %02x %02x %02x %02x %02x",
							 ovf_len,
							 ovf_len > 0 ? (unsigned char)ovf_data[0] : 0,
							 ovf_len > 1 ? (unsigned char)ovf_data[1] : 0,
							 ovf_len > 2 ? (unsigned char)ovf_data[2] : 0,
							 ovf_len > 3 ? (unsigned char)ovf_data[3] : 0,
							 ovf_len > 4 ? (unsigned char)ovf_data[4] : 0,
							 ovf_len > 5 ? (unsigned char)ovf_data[5] : 0,
							 ovf_len > 6 ? (unsigned char)ovf_data[6] : 0,
							 ovf_len > 7 ? (unsigned char)ovf_data[7] : 0,
							 ovf_len > 8 ? (unsigned char)ovf_data[8] : 0,
							 ovf_len > 9 ? (unsigned char)ovf_data[9] : 0,
							 ovf_len > 10 ? (unsigned char)ovf_data[10] : 0,
							 ovf_len > 11 ? (unsigned char)ovf_data[11] : 0,
							 ovf_len > 12 ? (unsigned char)ovf_data[12] : 0,
							 ovf_len > 13 ? (unsigned char)ovf_data[13] : 0,
							 ovf_len > 14 ? (unsigned char)ovf_data[14] : 0,
							 ovf_len > 15 ? (unsigned char)ovf_data[15] : 0,
							 ovf_len > 16 ? (unsigned char)ovf_data[16] : 0,
							 ovf_len > 17 ? (unsigned char)ovf_data[17] : 0,
							 ovf_len > 18 ? (unsigned char)ovf_data[18] : 0,
							 ovf_len > 19 ? (unsigned char)ovf_data[19] : 0,
							 ovf_len > 20 ? (unsigned char)ovf_data[20] : 0,
							 ovf_len > 21 ? (unsigned char)ovf_data[21] : 0,
							 ovf_len > 22 ? (unsigned char)ovf_data[22] : 0,
							 ovf_len > 23 ? (unsigned char)ovf_data[23] : 0,
							 ovf_len > 24 ? (unsigned char)ovf_data[24] : 0,
							 ovf_len > 25 ? (unsigned char)ovf_data[25] : 0,
							 ovf_len > 26 ? (unsigned char)ovf_data[26] : 0,
							 ovf_len > 27 ? (unsigned char)ovf_data[27] : 0,
							 ovf_len > 28 ? (unsigned char)ovf_data[28] : 0,
							 ovf_len > 29 ? (unsigned char)ovf_data[29] : 0,
							 ovf_len > 30 ? (unsigned char)ovf_data[30] : 0,
							 ovf_len > 31 ? (unsigned char)ovf_data[31] : 0);
						...
						while (ovf_remaining > sizeof(xl_recno_overflow_write))
						{
							...
							/*
							 * Use InvalidOffsetNumber to let PageAddItem choose
							 * the next available offset. This ensures sequential
							 * offsets matching the original insertion order.
							 */
							ovf_offnum = PageAddItem(page, actual_data, actual_len,
													 InvalidOffsetNumber, false, false);
							if (ovf_offnum == InvalidOffsetNumber)
								elog(PANIC, "RECNO INSERT redo: failed to add overflow record on block 0");
					...
					{
						elog(PANIC, "RECNO redo: insert failed to add tuple");
					}
...
recno_mask(char *page, BlockNumber blkno)
{
	 ...
	 * transactional fields like t_cid and infomask bits are set on the
