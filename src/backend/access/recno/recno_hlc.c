...
HLCNow(HLCTimestamp msg_hlc)
{
	 ...
	 * computes the next value, and atomically swaps it in.  CAS failure
	 * means another backend advanced the clock concurrently — we simply
	 * retry with the updated value.  In the common case (different
	 * millisecond), CAS succeeds on the first attempt.  Under same-ms
	 * contention, convergence takes 1-3 retries because each retry sees
	 * the latest counter.
	 */
	for (;;)
	{
		...
		/*
		 * Handle logical counter overflow.  Extremely unlikely (65535
		 * events in the same millisecond), but we must be safe.
		 */
