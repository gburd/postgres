...
{
	toast_delete_datum(NULL, value, is_speculative);
}
...
create_detoast_iterator(struct varlena *attr)
{
	...
	if (VARATT_IS_EXTERNAL_ONDISK(attr))
	{
		...
		/*
		 * This is an externally stored datum --- initialize fetch datum
		 * iterator
		 */
...
create_toast_buffer(int32 size, bool compressed)
{
	...
	if (compressed)
	{
		...
		/*
		 * Note the constraint buf->position <= buf->limit may be broken at
		 * initialization. Make sure that the constraint is satisfied when
		 * consuming chars.
		 */
...
Datum
generic_iterator_create(Datum value)
{
...
void
generic_iterate_next(Datum detoast_iter, Datum buf)
{
