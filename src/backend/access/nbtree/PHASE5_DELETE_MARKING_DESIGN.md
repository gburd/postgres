# Delete-marking in nbtree indexes — design notes

**Status: historical.** This file is the forward-looking plan ("Phase 5") for
nbtree delete-marking.  The on-disk representation, the invariants I1-I5 and the
scan/write-path reasoning below are accurate and worth reading -- they are the
argument for why the feature is shaped the way it is.  Three plan-shaped details
did not ship as written:

* **I4 (index-only scans) shipped as option (b), not (a).**  The plan picked (a),
  "force recheck/visit for entries from a delete-marking-capable index, keep IOS
  available".  The code does (b): index-only scans are disabled for indexes on a
  delete-marking table, enforced in the planner
  (`plancat.c`, via `RelationSupportsDeleteMarking()`), not inside
  `btcanreturn()`.
* **The write path sets a status bit in `t_tid`, not `INDEX_AM_RESERVED_BIT`
  directly.**  `INDEX_AM_RESERVED_BIT` is what nbtree already calls
  `INDEX_ALT_TID_MASK`; delete-marking promotes the entry to alt-TID form and
  sets `BT_IS_DELETE_MARKED` (0x4000) in the offset-number status bits, as the
  representation section below correctly describes.  The one-line summary in the
  write-path section is the loose one.
* **Gating is a table-AM flag, not `RelationUndoEngine != NONE`.**  The shipped
  gate is `am_index_delete_marking` / `RelationSupportsDeleteMarking()`.

The "no table AM uses it until Phase 8/9/10" framing is also stale: FLUX and
RECNO both set `am_index_delete_marking` and drive it.  Hash delete-marking was
deferred as the plan proposed and is still not implemented.
