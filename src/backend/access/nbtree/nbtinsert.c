			   ...
			   bool split_only_page)
{
	...
	else
	{
		...
		/*
		 * Write nbtree UNDO record for the insertion.  This is done after
		 * the critical section (UNDO insertion involves I/O) but while we
		 * still hold the buffer lock.  The UNDO record enables cleanup of
		 * this index entry if the transaction aborts.
		 *
		 * Only write UNDO if the parent heap relation has UNDO enabled.
		 * The heaprel parameter is NULL during index builds and recovery.
		 */
		if (heaprel != NULL && RelationHasUndo(heaprel))
		{
