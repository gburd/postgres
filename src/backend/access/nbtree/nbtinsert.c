	...
	for (;;)
	{
		...
		if (offset <= maxoff)
		{
			...
			if (inposting || !ItemIdIsDead(curitemid))
			{
				 ...
				 * definite conflict; with HOT-indexed update that invariant no
				 * longer holds -- an old index entry for key K may chain-lead
				 * to a heap tuple whose actual index key is K'.  Such an entry
				 * is stale, not a conflict, and is filtered out below.
				 * chain_walk_attrs is computed lazily (once) and freed with
				 * chain_walk_slot at every exit.
				 */
													   ...
													   chain_walk_slot))
				{
					 ...
					 * tuple whose current key is K'.  hot_indexed_stale is set
					 * when the walk crossed a hop that changed one of this
					 * index's attributes (chain_walk_attrs) -- including a key
					 * that cycled away and back, which still registers as a
					 * change.  Such a leaf is stale, not a conflict: skip it;
					 * the fresh entry inserted for the current value is the
