...
/*
 * nbtree_undo_write_perbackend - Write one nbtree index-UNDO record into the
 * per-backend UNDO engine (access/undo/perbackend), chaining off the current
 * transaction's per-backend UNDO location.
 *
 * This is the PERBACKEND arm of the engine-selectable write path.  It builds
 * the SAME payload the UNDO-in-WAL (design (a)) arm builds (header struct +
 * optional tuple bytes)
 * and tags it with uur_rmid = UNDO_RMID_NBTREE and uur_type = the nbtree
 * subtype, so that on abort execute_undo_actions() -> GetUndoRmgr(uur_rmid) ->
 * rm_undo reaches the SAME nbtree_undo_apply() callback used by the
 * UNDO-in-WAL path.
 *
 * This path is not reached at runtime until a per-backend table AM exists (so
 * that RelationUndoEngine() returns PERBACKEND); it is compiled and kept
 * logically consistent with the UNDO-in-WAL payload so the first per-backend
 * AM only
 * has to flip its am_undo_engine.  Two pieces remain to be wired, and a real
 * per-backend AM is needed to exercise and validate them end to end:
 *   1. The full WAL-integrated PrepareUndoInsert/LogUndoMetaData/
 *      InsertPreparedUndo/UndoLogBuffersSetLSN/UnlockReleaseUndoBuffers
 *      choreography (the zheap undo-insert protocol).  There is no existing
 *      producer of per-backend UNDO in the tree to validate the exact WAL
 *      record ordering against, so replicating it blind here would be untested
 *      and unreachable.  Below we build and tag the UnpackedUndoRecord and
 *      route the chain pointer through SetCurrentTransactionPbuUndoLocation so
 *      the record shape and chaining are already correct.
 *   2. The pbu abort dispatch (execute_undo_actions_page) currently passes
 *      rec->uur_info (the UREC_INFO_* flag bits) as the callback's "info"
 *      argument, whereas the subtype we need lives in uur_type.  Reconciling
 *      that is a one-line change in the pbu engine (out of scope for Phase 3,
 *      which must not modify pbu internals); we store the subtype in uur_type
 *      here so the reconciliation is a pure dispatch fix.
 */
static void
nbtree_undo_write_perbackend(Relation heaprel, uint8 subtype,
							 const char *hdr, Size hdr_len,
							 const char *tup, Size tup_len)
{
	/*
	 * The per-backend engine's record/insert headers
	 * (access/perbackend/pbu_undorecord.h, pbu_undoinsert.h) cannot be
	 * included in this translation unit: they redefine UndoRecordHeader /
	 * UndoLogControl that the fork engine's access/undorecord.h (included
	 * above) already defines, and they still use the pre-RelFileLocator
	 * RelFileNode type.  The two UNDO engines are intentionally isolated.
	 *
	 * Because this path is unreached until a per-backend table AM exists (see
	 * the function-level note above), the actual UnpackedUndoRecord
	 * construction + PrepareUndoInsert/InsertPreparedUndo drive lives in a
	 * small pbu-side shim TU that CAN include the pbu headers, is called from
	 * here, and is exercised end to end.  Contract that shim must honor:
	 *   uur_rmid  = UNDO_RMID_NBTREE
	 *   uur_type  = subtype (this arg)
	 *   uur_reloid = RelationGetRelid(heaprel)
	 *   payload   = hdr[0..hdr_len) followed by tup[0..tup_len)
	 * then SetCurrentTransactionPbuUndoLocation(urp) to chain the record into
	 * the current xact's per-backend UNDO chain PbuAtAbort_ApplyUndo() walks.
	 */
	(void) heaprel;
	(void) subtype;
	(void) hdr;
	...
	elog(ERROR, "per-backend index UNDO is not wired until Phase 9/10");
}
					...
					bool isleaf)
{
	...
	if (isleaf)
	{
		...
		{
			nbtree_undo_write_perbackend(heaprel, NBTREE_UNDO_INSERT_LEAF,
										 (const char *) &hdr,
										 SizeOfNbtreeUndoInsertLeaf,
										 (const char *) itup, itemsz);
			return;
		}

		/*
		 * UNDO-in-WAL (design (a)): the cluster-wide UNDO-in-WAL write path.
	...
	else
	{
		...
		if (RelationUndoEngine(heaprel) == UNDO_ENGINE_PERBACKEND)
		{
			nbtree_undo_write_perbackend(heaprel, NBTREE_UNDO_INSERT_UPPER,
...
NbtreeUndoLogDedup(Relation rel, Relation heaprel, Buffer buf)
{
	...
	if (RelationUndoEngine(heaprel) == UNDO_ENGINE_PERBACKEND)
	{
		nbtree_undo_write_perbackend(heaprel, NBTREE_UNDO_DEDUP,
