...
index_fetch_heap(IndexScanDesc scan, TupleTableSlot *slot)
{
	...
	/*
	 * If the index entry that reached this tuple is stale for this index
	 * (a HOT-indexed hop changed one of the index's attributes between the
	 * entry and the live tuple), surface that on xs_hot_indexed_stale.
	 * Keeping it distinct from xs_recheck lets the executor drop a stale
	 * HOT-indexed leaf (the fresh entry returns the row via its own path)
