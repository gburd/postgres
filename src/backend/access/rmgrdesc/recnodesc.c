...
#define XLOG_RECNO_MULTI_INSERT		0xD0
#define XLOG_RECNO_CAS_UPDATE_UNDO	0xE0
...
/* Mirrors backend xl_recno_cas_update_undo (recno_xlog.h) */
typedef struct xl_recno_cas_update_undo_fe
{
	uint16		offnum;
	uint16		flags;
	uint16		data_offset;
...
typedef struct xl_recno_multi_insert_fe
{
	uint16		ntuples;
	uint16		flags;
	uint64		commit_ts;
}			xl_recno_multi_insert_fe;
...
recno_desc(StringInfo buf, XLogReaderState *record)
{
	...
	switch (info & XLOG_RECNO_OPMASK)
	{
		...
		case XLOG_RECNO_CAS_UPDATE_UNDO:
			{
				...
				{
					xl_recno_cas_update_undo_fe xlrec;

					memcpy(&xlrec, data, sizeof(xl_recno_cas_update_undo_fe));
		...
		case XLOG_RECNO_MULTI_INSERT:
			{
				if (datalen >= sizeof(xl_recno_multi_insert_fe))
				{
					xl_recno_multi_insert_fe xlrec;

					memcpy(&xlrec, data, sizeof(xl_recno_multi_insert_fe));
