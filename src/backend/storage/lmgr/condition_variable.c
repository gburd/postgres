							...
							uint32 wait_event_info)
{
	...
	while (true)
	{
		...
		/*
		 * Wait for an interrupt to be raised.  (If we're awakened for some
		 * other reason, the code below will cope anyway.)
		 */
		(void) WaitInterrupt(CheckForInterruptsMask | INTERRUPT_GENERAL,
							 wait_events, cur_timeout, wait_event_info);

		/* Reset the interrupt before examining the state of the wait list. */
		 ...
		 * raised by something other than ConditionVariableSignal; though we
...
ConditionVariableSignal(ConditionVariable *cv)
{
	...
	/* If we found someone sleeping, raise their interrupt to wake them up. */
	if (proc != NULL)
...
ConditionVariableBroadcast(ConditionVariable *cv)
{
	 ...
	 * extra "set" on our interrupt from the someone else's signal, which is
