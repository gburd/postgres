...
/*
 * Tiered backoff parameters for the publish wait loop.
 *
 * Phase 1: Spin with PAUSE instruction (cheap, ~10-40 cycles per iteration).
 * Phase 2: Yield CPU via pg_usleep(0) (equivalent to sched_yield).
 * Phase 3: Sleep via pg_usleep(1) (OS timer granularity, ~50-100us).
 */
#define LRLOCK_SPIN_LIMIT	100
#define LRLOCK_YIELD_LIMIT	10
...
struct LRLock
{
	...
	/* Writer mutex: only one writer at a time (LWLock for safe long holds) */
	LWLock		writer_lwlock;
...
retry:
	...
	if (!all_departed)
	{
		/* Report wait event once we start actually waiting */
		if (!reported_wait)
		{
			pgstat_report_wait_start(WAIT_EVENT_LRLOCK_PUBLISH);
			reported_wait = true;
		}
		...
		else if (yield_count < LRLOCK_YIELD_LIMIT)
		{
			yield_count++;
			pg_usleep(0);		/* yield CPU without sleeping */
		}
		else
		{
			pg_usleep(1);		/* true sleep (OS timer resolution) */
		}
		goto retry;
	}
		   ...
		   LRLockSyncFn sync_fn, int max_backends, const char *name)
{
	...
	/* Initialize writer LWLock */
	LWLockInitialize(&lock->writer_lwlock, LWTRANCHE_LRLOCK_WRITER);
}
...
LRLockWriteBegin(LRLock * lock)
{
	...
	LWLockAcquire(&lock->writer_lwlock, LW_EXCLUSIVE);
...
LRLockApplyOp(LRLock * lock, const void *operation, Size op_size)
{
	...
	/*
	 * If the oplog would overflow, auto-publish to flush it.  After publish,
	 * both copies are in sync and the oplog is empty -- ready for new ops.
	 * This eliminates runtime ShmemAlloc calls (the oplog never grows).
	 */
	if (lock->oplog_used + entry_size > lock->oplog_capacity)
	{
		if (entry_size > lock->oplog_capacity)
			elog(ERROR, "LRLock \"%s\": single operation (%zu bytes) exceeds oplog capacity (%zu bytes)",
				 lock->name, entry_size, lock->oplog_capacity);
		LRLockPublish(lock);
	}
...
{
	LWLockRelease(&lock->writer_lwlock);
}
