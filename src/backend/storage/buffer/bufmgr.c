 ...
 * assignment, cleared after BufferAlloc() returns.  NULL means use the
 * default pool (zero overhead -- single predicted-not-taken branch).
 */
				  ...
				  bool *foundPtr)
{
	...
	/* Set target buffer pool if relation specifies one */
...
retry:
	...
	/*
	 * Notify the pool's replacement algorithm that this buffer's content
	 * has been evicted.  This allows algorithms like ARC to move tracking
	 * entries to ghost lists and clear the buffer-to-CDB mapping.
	 */
	if (oldFlags & BM_TAG_VALID)
	{
...
BufferSync(int flags)
{
	 ...
	 * collect dirty buffers per-pool here; they will be written in a
	 * separate loop after the default pool's write phase.
	 *
					...
					int nforks, BlockNumber *firstDelBlock)
{
	...
	/*
	 * Also scan dynamic pool buffers.  Use EnsurePoolAttached during normal
	 * transaction processing so we don't miss buffers in unattached pools.
	 * Fall back to TryGetPoolAttached during abort cleanup to avoid DSM
	 * attachment when resource owners are being released.
	 */
...
DropRelationsAllBuffers(SMgrRelation *smgr_reln, int nlocators)
{
	...
	/*
	 * Also scan dynamic pool buffers.  Use EnsurePoolAttached during normal
	 * transaction processing; fall back to TryGetPoolAttached during abort
	 * cleanup when DSM attachment is unsafe.
	 */
	for (int p = 1; p < NBufferPools; p++)
	{
		BufferPoolDesc *pool = &BufferPoolDescs[p];
