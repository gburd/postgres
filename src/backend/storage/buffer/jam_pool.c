 ...
 */
static BufferDesc *
jam_get_victim(void *strategy_data, uint64 *buf_state)
{
	JamControl *ctl = (JamControl *) strategy_data;
	uint8	   *usage = JAM_USAGE(ctl);
...
static const BufferPoolRoutine jam_pool_routine = {
	...
	.shutdown = NULL,
	.scan_resistant = false,	/* TOAST-tuned aggressive eviction; a foreign
								 * sequential scan would still pollute the
								 * cache like clock-sweep */
};
