...
ClockSweepTick(void)
{
	VictimBufferState current,
				desired;
	uint32		victim;

	current.combined = pg_atomic_read_u64(&StrategyControl->u.combined);
	...
	{
		desired = current;
		desired.nextVictimBuffer += 1;
	} while (
			 !pg_atomic_compare_exchange_u64(&StrategyControl->u.combined,
											 &current.combined,
											 desired.combined));

	victim = current.nextVictimBuffer;

	/*
	 * nextVictimBuffer should be bounded by NBuffers, rather than modulo loop
	 * and subtract until we're within that range.
	 */
	while (unlikely(victim >= NBuffers))
		victim -= NBuffers;

	/*
	 * If we triggered the wrap-around then record another complete sweep of
	 * the hand through the "clock" of BufferDescriptors.
	 */
	if (victim == 0)
	{
		do
...
StrategyInitialize(bool init)
{
	...
	if (!found)
	{
		...
		/* Clear teh clock-sweep hand and related statistics */
