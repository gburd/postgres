...
clock_passes(uint64 hand)
{
 ...
 /*
  * Calculate the hand modulo the number of buffers in the pool (NBuffers).
  * The hand's value is a 64-bit counter that only increases, so the modulo is
  * the lower bits of the counter masked by NBuffersPow2Mask and adjusted if
  * the result is greater than or equal to NBuffers.
  */
static inline uint32
