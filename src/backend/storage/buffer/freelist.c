...
{
	slock_t		lock;
	pg_atomic_uint32 nextVictimBuffer;	/* monotonically increasing, mod nbuffers */
...
typedef struct BufferAccessStrategyData
{
	 ...
	 * private ring and routes through the shared RECYCLE pool instead.
	 * NULL when using the traditional per-backend ring buffer approach.
	 */
...
const BufferPoolRoutine clock_pool_routine = {
	...
	.on_hit = NULL,				/* clock-sweep uses usage_count, not explicit tracking */
	...
	for (;;)
	{
		...
		for (;;)
		{
			...
			if (BUF_STATE_GET_USAGECOUNT(local_buf_state) != 0)
			{
				 ...
				 * decrementing.  This gives each page exactly one sweep
				 * cycle to be re-accessed before eviction.
				 */
...
GetAccessStrategyWithSize(BufferAccessStrategyType btype, int ring_size_kb)
{
	 ...
	 * allocating a private ring buffer.  We still allocate a minimal
	 * strategy struct to carry the btype and recycle_pool pointer.
	 */
	recycle = GetBufferPoolByName("recycle");
