...
FileOpsCreate(const char *path, int flags, mode_t mode, bool register_delete)
{
	 ...
	 *
	 * Directory fsync is done via fsync_parent_path(), which is a no-op
	 * on Windows (not needed due to NTFS journal).
	 */
	if (enableFsync)
	{
		pg_fsync(fd);
		...
		if (pending->at_commit == isCommit)
		{
			switch (pending->type)
			{
				case PENDING_FILEOP_DELETE:
					/*
					 * Remove the file durably.  It is normal for the file
					 * to already be gone: smgrDoPendingDeletes runs before
					 * us and removes relation files via mdunlink, so by the
					 * time we get here the main-fork file usually no longer
...
fileops_redo(XLogReaderState *record)
{
	...
	switch (info)
	{
		case XLOG_FILEOPS_CREATE:
			{
				...
				if (fd < 0)
				{
					...
					/* Still failed after retry (or original error was not ENOENT) */
		...
		case XLOG_FILEOPS_DELETE:
			/*
			 * FILEOPS DELETE records log the *intent* to delete a file as
			 * a deferred (pending) operation -- they do NOT represent an
			 ...
			 *
			 * We must NOT delete the file here during WAL redo, because:
			 *  1. For delete-on-abort entries (at_commit=false): the file
			 *     was just created and the transaction may commit, so the
			 *     file must remain.
			 *  2. For delete-on-commit entries (at_commit=true): the file
			 *     should only be removed when the transaction's commit
			 *     record is replayed, not when this record is replayed.
			 *
			 * Performing the delete here would remove relation files on
			 * standbys immediately after creation, causing "No such file
			 * or directory" errors for subsequent WAL records that access
			 * the relation.
			 */
			break;
		...
		case XLOG_FILEOPS_MOVE:
			 ...
			 * Performing the rename here during WAL redo would be
			 * premature -- the transaction may not have committed yet
			 * in the WAL stream.  The rename will be effected when the
			 * transaction's commit record is replayed.
			 */
			break;
		...
		case XLOG_FILEOPS_TRUNCATE:
			{
				 ...
				 * file is truncated, there's no going back. This follows
				 * the same pattern as smgr_redo() for SMGR_TRUNCATE:
				 * doing this before truncation means that if the truncation
				 * fails, recovery cannot proceed past this point without
				 * fixing the underlying issue, but it prevents the
				 * WAL-first rule from being violated.
				 */
				XLogFlush(lsn);

				/*
				 * Open, truncate, and fsync for durability. This uses
				 * pg_fsync() which selects the platform-appropriate mechanism.
				 */
