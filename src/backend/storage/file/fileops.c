...
FileOpsCreate(const char *path, int flags, mode_t mode, bool register_delete)
{
	...
	if (XLogIsNeeded())
	{
		...
		/* Insert UNDO record for crash-safe rollback */
		{
			XactUndoContext undo_ctx;
			char		payload[sizeof(FileopsUndoCreate) + MAXPGPATH];
			FileopsUndoCreate *hdr = (FileopsUndoCreate *) payload;

			hdr->path_len = (uint16) pathlen;
			memcpy(payload + sizeof(FileopsUndoCreate), path, pathlen);
...
FileOpsTruncate(const char *path, off_t length)
{
	...
	if (XLogIsNeeded())
	{
		...
		/* Insert UNDO record to restore original length on rollback */
		{
			XactUndoContext undo_ctx;
		 ...
		 * ordering in RelationTruncate() (see catalog/storage.c).
		 */
...
FileOpsChmod(const char *path, mode_t mode)
{
	...
	/* Capture original mode for UNDO before chmod */
	if (stat(path, &st) == 0)
	...
	if (XLogIsNeeded())
	{
		...
		/* Insert UNDO record to restore original mode on rollback */
		{
			XactUndoContext undo_ctx;
...
FileOpsChown(const char *path, uid_t uid, gid_t gid)
{
	...
	if (XLogIsNeeded())
	{
		...
		/* Insert UNDO record to restore original ownership on rollback */
		{
			XactUndoContext undo_ctx;
...
FileOpsMkdir(const char *path, mode_t mode)
{
	...
	if (XLogIsNeeded())
	{
		...
		/* Insert UNDO record to rmdir on rollback */
		{
			XactUndoContext undo_ctx;
...
FileOpsSymlink(const char *target, const char *linkpath)
{
	...
	if (XLogIsNeeded())
	{
		...
		/* Insert UNDO record to unlink symlink on rollback */
		{
			XactUndoContext undo_ctx;
...
FileOpsLink(const char *oldpath, const char *newpath)
{
	...
	if (XLogIsNeeded())
	{
		...
		/* Insert UNDO record to unlink on rollback */
		{
			XactUndoContext undo_ctx;
				...
				const void *value, size_t len)
{
	...
	if (pg_setxattr(path, name, value, len) < 0)
	{
		...
		/*
		 * ENOTSUP/EOPNOTSUPP/EPERM/EACCES indicate the filesystem or security
		 * policy doesn't support extended attributes.  Return -1 to let
		 * callers handle gracefully (e.g., skip xattr tests).
		 */
		if (save_errno == ENOTSUP || save_errno == EOPNOTSUPP ||
			save_errno == EPERM || save_errno == EACCES)
		{
			errno = save_errno;
			return -1;
		}
	...
	if (XLogIsNeeded())
	{
		...
		/* Insert UNDO record to restore/remove original xattr on rollback */
		{
			XactUndoContext undo_ctx;
			Size		payload_len;
			char	   *payload;
...
FileOpsRemoveXattr(const char *path, const char *name)
{
	...
	if (XLogIsNeeded())
	{
		...
		/* Insert UNDO record to restore removed xattr on rollback */
		if (saved_value != NULL)
		{
			XactUndoContext undo_ctx;
			Size		payload_len;
