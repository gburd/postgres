...
IndexOnlyNext(IndexOnlyScanState *node)
{
	...
	while ((tid = index_getnext_tid(scandesc, direction)) != NULL)
	{
		...
		if (node->ioss_InplaceRecheckInfo != NULL)
		{
			 ...
			 * (am_inplace_update_keeps_tid), an indexed-column UPDATE leaves
		...
		if (live_itup != NULL)
		{
			 ...
			 * descriptor (xs_itupdesc), same as xs_itup.
			 */
			StoreIndexTuple(node, slot, live_itup, scandesc->xs_itupdesc);
...
ExecInitIndexOnlyScan(IndexOnlyScan *node, EState *estate, int eflags)
{
	...
	if (currentRelation->rd_tableam != NULL &&
		currentRelation->rd_tableam->am_inplace_update_keeps_tid)
