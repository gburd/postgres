...
lreplace:
	 ...
	 * attribute 0 in INDEX_ATTR_BITMAP_INDEXED, and the table AM's
	 * modified-attrs comparison (heapam_modified_attrs for heap) keeps
				   ...
				   HeapTuple oldtuple, TupleTableSlot *slot)
{
		...
		!bms_is_empty(updateCxt->modified_attrs))
	{
		...
		/*
		 * The AM adds the whole-row attribute (TableTupleUpdateAllIndexes) to
		 * modified_attrs when it stored an independent new version requiring a
		 * fresh entry in every index.  Its absence here means the update was
		 * heap-only (HOT): only summarizing indexes need a new entry, so
		 * restrict the insert to them.
		 */
		if (!bms_is_member(TableTupleUpdateAllIndexes,
						   updateCxt->modified_attrs))
...
#define MT_NRELS_HASH 64
	 ...
	 * INSERT, UPDATE, and DELETE operations regardless of estimated row count
	 * -- the UNDO write buffer overhead is negligible (one palloc of ~512
	 * bytes, reused for the entire transaction) and the benefit of batched
	 * UNDO recording applies to operations of any size.
	 *
	 * We pass the subplan's row estimate (input rows to be modified) as a
	 * hint; the AM may use it for buffer pre-sizing.
	 */
	{
