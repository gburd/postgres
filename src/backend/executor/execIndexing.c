 ...
 *
 * Populate each index's ii_IndexUnchanged hint ahead of ExecInsertIndexTuples.
 *
 * update_all_indexes means the table AM produced a new TID for this row, so
 * every index must receive a fresh entry regardless of which attributes
 * changed.  In that case we clear every ii_IndexUnchanged.
 *
					  ...
					  const Bitmapset *modified_idx_attrs)
{
	...
	if (update_all_indexes)
	{
		...
		{
			if (indexInfoArray[i] != NULL)
				indexInfoArray[i]->ii_IndexUnchanged = false;
		}
		return;
	}
	...
	for (int i = 0; i < numIndices; i++)
	{
		...
		/*
		 * Expression indexes follow the same bitmap-overlap rule as plain
		 * indexes here: RelationGetIndexedAttrs() includes the columns the
		 * indexed expression references, and an immutable expression's value
		 * can change only when one of those columns does.  An UPDATE that
		 * reaches this point and leaves them unchanged therefore leaves the
		 * expression's value -- and its index entry -- unchanged, so the
		 * index is safely skipped.  (An UPDATE that does touch an
		 * expression-index attribute never reaches the HOT-indexed path:
		 * HeapUpdateHotAllowable disqualifies it, pending expression-aware
		 * maintenance.)
		 */
