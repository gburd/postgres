					  ...
					  bool *specConflict)
{
	...
	for (i = 0; i < numIndices; i++)
	{
		 ...
		 * an exact one; see ItemPointerRecheckHintFlag in itemptr.h.
		 *
		 * The recheck-hint marker only means anything for a table AM whose
		 * locator declares it may hand out inexact entries; that is exactly
		 * the may_be_inexact property of the table's locator capability.  Gate
		 * on it rather than assuming heap semantics, so an AM whose locator is
		 * always exact never has the bit set (and its bitmap scans stay exact).
		 */
		if ((flags & EIIT_PARTIAL_UPDATE) && !indexInfo->ii_Summarizing &&
			GetLocatorCapability(heapRelation->rd_tableam->locator_capability)->may_be_inexact)
		{
			ItemPointerData siu_tid = *tupleid;

			/*
			 * ponytail: the recheck hint rides in the item pointer because
			 * today the BitmapAnd path (nodeBitmapIndexscan -> amgetbitmap ->
			 * tbm_add_tuples) hands raw TIDs to tidbitmap.c with no per-entry
			 * side channel.  When Peter's amgetbatch interface lands, move the
			 * inexact-locator signal to a per-item boolean carried in the
			 * table AM's batch (Option B) and stop overloading the offset bit;
			 * see BIT14_OPTIONS.md.
			 */
			ItemPointerSetRecheckHint(&siu_tid);
