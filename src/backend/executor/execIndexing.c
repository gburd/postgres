					  ...
					  const Bitmapset *modified_idx_attrs)
{
	...
	for (int i = 0; i < numIndices; i++)
	{
		...
		/*
		 * Expression indexes are not safe to skip under the HOT/SIU
		 * optimization yet.  A column-level "did this attr change?" answer
		 * cannot tell us whether the indexed expression value actually
		 * changed, and the expression-aware path that resolves this lands in
		 * a follow-up patch.  Until then, treat every expression index as
		 * always-changed so it receives a fresh aminsert on every update.
		 */
