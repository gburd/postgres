...
DecodeRecnoInsert(LogicalDecodingContext *ctx, XLogRecordBuffer *buf)
{
	...
	recno_tuple = (RecnoTupleHeader *) tupledata;
	datalen = recno_tuple->t_len;

	/*
	 * When RECNO_WAL_LOGICAL_TUPLE is set, the write path appended a
	 * heap-format image at the END of the main WAL data:
	 *		... [heap bytes] [uint32 heap_len]
	 * Read heap_len from the last 4 bytes, then back up heap_len more
	 * bytes to find the heap tuple payload.  This is immune to whatever
	 * HLC / compression data precedes it.
	 */
	...
	{
		/*
		 * Legacy path: no heap image in WAL (publisher compiled without
		 * or table not logically logged when the record was written).
		 * pgoutput will most likely reject this; initial-sync via COPY
		 * still works, so this is only a streaming-decoding concern.
		 */
		change->data.tp.newtuple =
...
DecodeRecnoUpdate(LogicalDecodingContext *ctx, XLogRecordBuffer *buf)
{
	...
	RecnoTupleHeader *new_tuple_hdr;
	RelFileLocator target_locator;
	xl_recno_hlc_info hlc_info;
	 ...
	 * xl_recno_update | old_tuple | new_tuple [| xl_recno_hlc_info] [| logical images]
	 */
	old_tuple_data = (char *) xlrec + sizeof(xl_recno_update);
	old_tuple_hdr = (RecnoTupleHeader *) old_tuple_data;
	...
	{
		/*
		 * Two heap-format images appended at the END of the record, each
		 * with trailing length:
		 *
		 *		... [old_heap bytes] [uint32 old_heap_len] [new_heap bytes] [uint32 new_heap_len]
		 *
		 * Walk backwards from end: read new_heap_len last, then
		 * old_heap_len before the new bytes + its length.
		 */
		Size		full_len = XLogRecGetDataLen(r);
		char	   *end = (char *) xlrec + full_len;
		uint32		old_heap_len;
	...
	else
	{
		...
		change->data.tp.oldtuple =
			ReorderBufferAllocTupleBuf(ctx->reorder, old_tuple_hdr->t_len);
		memcpy(change->data.tp.oldtuple->t_data, old_tuple_data,
			   old_tuple_hdr->t_len);
		change->data.tp.oldtuple->t_len = old_tuple_hdr->t_len;
		ItemPointerSetInvalid(&change->data.tp.oldtuple->t_self);
		change->data.tp.oldtuple->t_tableOid = InvalidOid;

		change->data.tp.newtuple =
			ReorderBufferAllocTupleBuf(ctx->reorder, new_tuple_hdr->t_len);
		memcpy(change->data.tp.newtuple->t_data, new_tuple_data,
			   new_tuple_hdr->t_len);
		change->data.tp.newtuple->t_len = new_tuple_hdr->t_len;
		ItemPointerSetInvalid(&change->data.tp.newtuple->t_self);
		change->data.tp.newtuple->t_tableOid = InvalidOid;
	}
...
DecodeRecnoDelete(LogicalDecodingContext *ctx, XLogRecordBuffer *buf)
{
	...
	tuple_hdr = (RecnoTupleHeader *) tupledata;

	if (xlrec->flags & RECNO_WAL_LOGICAL_TUPLE)
	{
		/*
		 * Heap image is the last region of the WAL record:
		 *		... [heap bytes] [uint32 heap_len]
		 */
		Size		full_len = XLogRecGetDataLen(r);
		char	   *end = (char *) xlrec + full_len;
	...
	else
	{
		/* Legacy path */
		change->data.tp.oldtuple =
			ReorderBufferAllocTupleBuf(ctx->reorder, tuple_hdr->t_len);
		memcpy(change->data.tp.oldtuple->t_data, tupledata,
			   tuple_hdr->t_len);
		change->data.tp.oldtuple->t_len = tuple_hdr->t_len;
		ItemPointerSetInvalid(&change->data.tp.oldtuple->t_self);
		change->data.tp.oldtuple->t_tableOid = InvalidOid;
	}
