...
DecodeRecnoInsert(LogicalDecodingContext *ctx, XLogRecordBuffer *buf)
{
	...
	/*
	 * Allocate tuple buffer for the reorderbuffer. For logical replication,
	 * we need to convert RECNO tuple format to heap-compatible format. For
	 * now, we store the RECNO tuple directly and let the output plugin handle
	...
	/*
	 * Copy the RECNO tuple data into the reorderbuffer. The tuple data
	 * includes the RecnoTupleHeader. Note: ReorderBufferTupleBuf is actually
	 * a HeapTupleData.
	 */
...
DecodeRecnoUpdate(LogicalDecodingContext *ctx, XLogRecordBuffer *buf)
{
	...
	/* Allocate and populate old tuple buffer */
	...
	/* Allocate and populate new tuple buffer */
	change->data.tp.newtuple =
