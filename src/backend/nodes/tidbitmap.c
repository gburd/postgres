			   ...
			   bool recheck)
{
	...
	for (int i = 0; i < ntids; i++)
	{
		 ...
		 * offset granularity here (see the function header).  The bit is set
		 * only by a table AM whose locator declares the may_be_inexact
		 * capability (see amlocator.h and execIndexing.c), so its presence
		 * means the entry may point somewhere other than the row a fetch will
		 * produce.  Test the raw TID -- before ItemPointerGetOffsetNumber
		 * strips the bit -- and if set, degrade the whole page to lossy and
		 * move on.
		 */
