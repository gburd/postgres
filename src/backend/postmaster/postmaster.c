...
PostmasterMain(int argc, char *argv[])
{
	 ...
	 * path.  Registration is gated behind the `enable_undo` cluster GUC
	 * so that clusters with UNDO turned off don't pay for an always-on
	 * bgworker that has nothing to do.  When `enable_undo = on`, the
	 * launcher scans pg_database once at startup and spawns a
