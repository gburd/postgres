...
backend_thread_run_worker(BackendThreadStart *thread_start)
{
	 ...
	 * catalog-startup smokes.  The archiver and WAL summarizer follow the
	 * auxiliary-process common startup path, publish their wakeup/progress
	 * state in shared memory, and keep their per-loop work state
	 * backend-local, so they can use the same narrow bypass class as the
	 * writer-style auxiliary workers.
	 */
