...
backend_thread_run_worker(BackendThreadStart *thread_start)
{
	 ...
	 * catalog-startup smokes.  The autovacuum launcher performs backend
	 * initialization but does not select a database or run user table work;
	 * autovacuum workers remain gated until their database bootstrap path is
	 * isolated.  The startup process, archiver, WAL receiver, and WAL
