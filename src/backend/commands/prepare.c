...
InitQueryHashTable(void)
{
	...
	MySessionData.prepared_queries = hash_create("Prepared Queries",
												 32,
												 &hash_ctl,
					   ...
					   bool from_sql)
{
	...
	if (!MySessionData.prepared_queries)
		InitQueryHashTable();

	/* Add entry to hash table */
	entry = (PreparedStatement *) hash_search(MySessionData.prepared_queries,
											  stmt_name,
											  HASH_ENTER,
											  &found);
...
DropPreparedStatement(const char *stmt_name, bool showError)
{
	...
	if (entry)
	{
		...
		hash_search(MySessionData.prepared_queries, entry->stmt_name, HASH_REMOVE, NULL);
	}
}
...
DropAllPreparedStatements(void)
{
	...
	/* walk over cache */
	hash_seq_init(&seq, MySessionData.prepared_queries);
...
pg_prepared_statement(PG_FUNCTION_ARGS)
{
	...
	if (MySessionData.prepared_queries)
	{
		...
		hash_seq_init(&hash_seq, MySessionData.prepared_queries);
