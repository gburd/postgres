...
BuildDescForRelation(const List *columns)
{
	...
	foreach(l, columns)
	{
		...
		if (entry->toaster)
			att->atttoaster = get_toaster_oid(entry->toaster, false);
...
MergeChildAttribute(List *inh_columns, int exist_attno, int newcol_attno, const ColumnDef *newdef)
{
	...
	if (inhdef->toaster == NULL)
		...
		ereport(ERROR,
				(errcode(ERRCODE_DATATYPE_MISMATCH),
				 errmsg("column \"%s\" has a toaster conflict",
						attributeName),
				 errdetail("%s versus %s",
						   inhdef->toaster,
						   newdef->toaster)));
						...
						const ColumnDef *newdef)
{
	...
	if (prevdef->toaster == NULL)
		prevdef->toaster = newdef->toaster;
			 ...
			 strcmp(prevdef->toaster, newdef->toaster) != 0)
		ereport(ERROR,
				(errcode(ERRCODE_DATATYPE_MISMATCH),
	...
	while (HeapTupleIsValid(depTup = systable_getnext(scan)))
	{
		...
		switch (foundObject.classId)
		{
			...
			case ToasterRelationId:
				...
				/*
				 * A toaster doesn't need rebuilding when column type changes.
				 */
				break;
