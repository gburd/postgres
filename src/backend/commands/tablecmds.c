...
BuildDescForRelation(const List *columns)
{
	...
	foreach(l, columns)
	{
		...
		/*
		 * att->atttoaster = GetAttributeToaster(att->atttypid,
		 * entry->toaster);
		 *
		 * if (OidIsValid(att->atttoaster)) validateToaster(att->atttoaster,
		 * att->atttypid, att->attstorage, att->attcompression, InvalidOid,
		 * false);
		 */
	}
...
MergeAttributes(List *columns, const List *supers, char relpersistence,
				bool is_partition, List **supconstr, Oid accessMethodId)
{
...
*/
	...
	/*
	 * If the table is source table of ATTACH PARTITION command, we did not
	 * modify anything about it that will change its toasting requirement, so
	 * no need to check.
	 */

	/*
	 * if (((tab->relkind == RELKIND_RELATION || tab->relkind ==
	 * RELKIND_PARTITIONED_TABLE) && tab->partition_constraint == NULL) ||
	 * tab->relkind == RELKIND_MATVIEW) AlterTableCreateToastTable(tab->relid,
	 * (Datum) 0, lockmode);
	 */
/*	} */
}
...
*/
	...
	/*
	 * If we're supposed to replace the reloptions list, we just pretend there
	 * were none before.
	 */
/*
			datum = (Datum) 0;
...
/*
 * resolve column toast specification to toast method.
 */
static Oid
GetAttributeToaster(Oid atttypid, const char *toaster)
{
	if (toaster == NULL ||
		strcmp(toaster, "default") == 0 ||
		!TypeIsToastable(atttypid))
		return DEFAULT_TOASTER_OID;

	return get_toaster_oid(toaster, false);
}
