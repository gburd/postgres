...
nextval_internal(Oid relid, bool check_permissions)
{
	...
	if (elm->last != elm->cached)	/* some numbers were cached */
	{
		...
		sequence_close(seqrel, NoLock);
		MySessionData.sequence_state.last_used_seq = elm;
		return elm->last;
	}
...
lastval(PG_FUNCTION_ARGS)
{
	...
	if (MySessionData.sequence_state.last_used_seq == NULL)
		ereport(ERROR,
				(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
				 errmsg("lastval is not yet defined in this session")));

	/* Someone may have dropped the sequence since the last nextval() */
	if (!SearchSysCacheExists1(RELOID, ObjectIdGetDatum(MySessionData.sequence_state.last_used_seq->relid)))
		ereport(ERROR,
				(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
				 errmsg("lastval is not yet defined in this session")));

	seqrel = lock_and_open_sequence(MySessionData.sequence_state.last_used_seq);

	/* nextval() must have already been called for this sequence */
	Assert(MySessionData.sequence_state.last_used_seq->last_valid);

	if (pg_class_aclcheck(MySessionData.sequence_state.last_used_seq->relid, GetUserId(),
						  ACL_SELECT | ACL_USAGE) != ACLCHECK_OK)
		ereport(ERROR,
				(errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
				 errmsg("permission denied for sequence %s",
	...
	result = MySessionData.sequence_state.last_used_seq->last;
	sequence_close(seqrel, NoLock);
...
create_seq_hashtable(void)
{
	...
	MySessionData.sequence_state.seqhashtab = hash_create("Sequence values", 16, &ctl,
...
init_sequence(Oid relid, SeqTable *p_elm, Relation *p_rel)
{
	...
	if (MySessionData.sequence_state.seqhashtab == NULL)
		create_seq_hashtable();

	elm = (SeqTable) hash_search(MySessionData.sequence_state.seqhashtab, &relid, HASH_ENTER, &found);
...
{
	if (MySessionData.sequence_state.seqhashtab)
	{
		hash_destroy(MySessionData.sequence_state.seqhashtab);
		MySessionData.sequence_state.seqhashtab = NULL;
	}

	MySessionData.sequence_state.last_used_seq = NULL;
}