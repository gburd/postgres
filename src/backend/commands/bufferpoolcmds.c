...
CreateBufferPool(CreateBufferPoolStmt *stmt)
{
	 ...
	 * buffer pool with DSM-backed memory.  Resolve the handler function
	 * to get the BufferPoolRoutine vtable.
	 */
	{
...
DropBufferPoolById(Oid bpoid)
{
	 ...
	 *
	 * We look up the pool name from the catalog, build the expected
	 * reloption string "buffer_pool=<name>", then scan pg_class for any
	 * tuple whose reloptions array contains a matching entry.
	 */
	{
		HeapTuple	bptup;
