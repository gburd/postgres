...
rule bslash_special matches /\\[;:]/ {
    ...
    if (matched[1] == ';')
        PSQL_STATE(user)->identifier_count = 0;
    LEX_SKIP();
}
...
rule ident matches /{identifier}/ {
    ...
    if (s->identifier_count == 0)
        memset(s->identifiers, 0, sizeof(s->identifiers));

    if (s->identifier_count < (int) sizeof(s->identifiers)) {
        char tag = 0;

        if (matched_len == 6 &&
            pg_strncasecmp(matched, "create", 6) == 0)
            tag = 'c';
        else if (matched_len == 8 &&
                 pg_strncasecmp(matched, "function", 8) == 0)
            tag = 'f';
        else if (matched_len == 9 &&
                 pg_strncasecmp(matched, "procedure", 9) == 0)
            tag = 'p';
        else if (matched_len == 2 &&
                 pg_strncasecmp(matched, "or", 2) == 0)
            tag = 'o';
        else if (matched_len == 7 &&
                 pg_strncasecmp(matched, "replace", 7) == 0)
            tag = 'r';
        else if (matched_len == 6 &&
                 pg_strncasecmp(matched, "schema", 6) == 0)
            tag = 's';
        s->identifiers[s->identifier_count] = tag;
    }
    s->identifier_count++;

    first = s->identifiers[0];
    if (first == 'c' &&
        (s->identifiers[1] == 'f' || s->identifiers[1] == 'p' ||
         (s->identifiers[1] == 'o' && s->identifiers[2] == 'r' &&
          (s->identifiers[3] == 'f' || s->identifiers[3] == 'p')) ||
         s->identifiers[1] == 's') &&
        s->paren_depth == 0) {
        if (matched_len == 5 && pg_strncasecmp(matched, "begin", 5) == 0)
            s->begin_depth++;
        else if (matched_len == 4 && pg_strncasecmp(matched, "case", 4) == 0) {
            if (s->begin_depth >= 1)
                s->begin_depth++;
        } else if (matched_len == 3 && pg_strncasecmp(matched, "end", 3) == 0) {
            if (s->begin_depth > 0)
                s->begin_depth--;
        }
    }
