...
/* FIFO of pre-scanned tokens. */
typedef struct PgcQueuedToken
{
	int			code;
	char	   *yytext;
	int			yyleng;
...
static PgcQueuedToken *fifo = NULL;
static int	fifo_cap = 0;
static int	fifo_head = 0;
static int	fifo_tail = 0;

static void
fifo_push(int code, const char *text, size_t len, YYSTYPE val)
{
	if (fifo_tail >= fifo_cap)
	{
		int			newcap = fifo_cap == 0 ? 64 : fifo_cap * 2;

		fifo = (PgcQueuedToken *) realloc(fifo, newcap * sizeof(PgcQueuedToken));
		if (fifo == NULL)
			mmfatal(OUT_OF_MEMORY, "out of memory");
		fifo_cap = newcap;
	}
	fifo[fifo_tail].code = code;
	fifo[fifo_tail].yytext = (char *) malloc(len + 1);
	if (fifo[fifo_tail].yytext == NULL)
		mmfatal(OUT_OF_MEMORY, "out of memory");
	memcpy(fifo[fifo_tail].yytext, text, len);
	fifo[fifo_tail].yytext[len] = '\0';
	fifo[fifo_tail].yyleng = (int) len;
	fifo[fifo_tail].val = val;
	fifo_tail++;
}

static bool
fifo_pop(int *code_out)
{
	if (fifo_head >= fifo_tail)
		return false;
	base_yytext = fifo[fifo_head].yytext;
	base_yyleng = fifo[fifo_head].yyleng;
	base_yylval = fifo[fifo_head].val;
	*code_out = fifo[fifo_head].code;
	fifo_head++;
	return true;
}
...
lex_init(void)
{
	...
	/* Reset the FIFO of queued tokens. */
	fifo_reset();
