 ...
 *
 * Memory ordering: sequentially consistent.  A weaker order (relaxed, or even
 * acquire) is NOT sufficient on architectures with a genuinely relaxed memory
 * model: on RISC-V, relaxed and acquire loads both allowed a concurrent reader
 * in parallel hash join to miss a tuple (join_hash regression, reproduced and
 * bisected).  Only seq_cst -- which participates in the single total order over
 * all seq_cst operations -- matches the ordering the traditional plain-volatile
 * implementation happens to provide in combination with PostgreSQL's barriers.
 */
