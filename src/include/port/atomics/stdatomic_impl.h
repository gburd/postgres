 ...
 * corrupt shared state.  Reject that at compile time.
 *
 * The ATOMIC_*_LOCK_FREE macros are 0 (never lock-free), 1 (sometimes / not
 * known at compile time), or 2 (always lock-free).  We require != 0: a value
 * of 0 is the fatal lock-table case.  We cannot require == 2 portably --
 * notably MSVC's <stdatomic.h> reports 1 for every width even though the
 * atomics are in fact lock-free on the hardware we target (atomic_is_lock_free
 * returns true at runtime).  Requiring >= 1 rejects the dangerous case while
 * accepting MSVC's conservative reporting.
 */
StaticAssertDecl(ATOMIC_CHAR_LOCK_FREE >= 1, "8-bit atomics may not be lock-free");
StaticAssertDecl(ATOMIC_SHORT_LOCK_FREE >= 1, "16-bit atomics may not be lock-free");
StaticAssertDecl(ATOMIC_INT_LOCK_FREE >= 1, "32-bit atomics may not be lock-free");
StaticAssertDecl(ATOMIC_LLONG_LOCK_FREE >= 1, "64-bit atomics may not be lock-free");
 ...
 *
 * Memory ordering: sequentially consistent, and this is load-bearing on
 * weakly-ordered hardware, not decoration.  The traditional implementation
 * reads through a volatile-qualified pointer; a volatile access cannot be
 * reordered by the compiler relative to the surrounding pg_read_barrier() /
 * spinlock, so those barriers actually constrain it.  A C11
 * memory_order_relaxed atomic load does NOT get that treatment -- the
 * compiler may hoist or sink it across the barrier -- and on RISC-V that let
 * a concurrent reader miss a tuple in a parallel hash join (join_hash
 * "extremely_skewed").  This was bisected on real hardware, 20 runs each,
 * with the compiler+thread-fence barriers already in place:
 *   traditional (volatile) read            : 20/20 pass
 *   relaxed atomic read                     :  2/20 pass
 *   relaxed atomic read + compiler barrier  :  4/20 pass
 *   seq_cst atomic read                     : 15/15 pass
 * Neither relaxed nor relaxed+signal_fence reproduces the volatile load's
 * effective ordering here; only seq_cst does, so that is what we use.
 *
 * Note that pg_atomic_write_u32_impl is deliberately relaxed (a plain store,
 * matching the traditional "no barrier" write contract and avoiding STLR on
 * ARM64).  A seq_cst read paired with a relaxed write does not by itself
 * establish a single total order across read and write; the read's seq_cst is
 * here specifically to restore the compiler-ordering property of the old
 * volatile read, which is what the failing consumer actually depended on.
 */
 ...
 * and, on ARM64, compiles to a plain STR rather than STLR -- avoiding the
 * store-release serialization that penalizes contended writes to hot shared
 * state (e.g. LWLock release).  The ordering requirement that the parallel
 * hash join exposed on RISC-V is carried by the seq_cst READ side, not the
 * write; see pg_atomic_read_u32_impl.
 */
static inline void
