/*-------------------------------------------------------------------------
 *
 * sparsemap_internal.h
 *	  Sparsemap chunk-level codec: types, constants, and operations.
 *
 * This header contains the internal chunk-level operations for the
 * sparsemap compressed bitmap implementation.  All functions are static
 * (inline where appropriate) and operate on raw chunk data (__sm_chunk_t)
 * without any sparsemap_t dependency.
 *
 * Copyright (c) 2024 Gregory Burd <greg@burd.me>
 * Portions Copyright (c) 1996-2026, PostgreSQL Global Development Group
 *
 * src/include/lib/sparsemap_internal.h
 *
 *-------------------------------------------------------------------------
 */
#ifndef SPARSEMAP_INTERNAL_H
#define SPARSEMAP_INTERNAL_H
 ...
 * Core types
 */
typedef uint64 __sm_bitvec_t;
typedef uint64 __sm_idx_t;
...
static inline size_t
__sm_chunk_calc_vector_size(const uint8 b)
{
  // clang-format off
  static int lookup[] = {
...
{
  size_t position = 0;
  register uint8 *p = (uint8 *)chunk->m_data;
...
static inline void
__sm_chunk_init(__sm_chunk_t *chunk, uint8 *data)
{
...
static inline int
__sm_chunk_clr_bit(const __sm_chunk_t *chunk, const uint64 idx, size_t *pos)
{
  __sm_bitvec_t w;
  const size_t bv = idx / SM_BITS_PER_VECTOR;
...
static inline int
__sm_chunk_set_bit(const __sm_chunk_t *chunk, const uint64 idx, size_t *pos)
{
  const size_t bv = idx / SM_BITS_PER_VECTOR;
  __sm_assert(bv < SM_FLAGS_PER_INDEX);
      ...
      for (int j = 0; j < SM_FLAGS_PER_INDEX_BYTE; j++) {
        ...
        case SM_PAYLOAD_MIXED:
          ...
          if (to >= SM_BITS_PER_VECTOR) {
            ...
            rank->pos += to + 1;
            const uint64 to_mask = (to == 63) ? UINT64_MAX : ((uint64)1 << (to + 1)) - 1;
            const uint64 from_mask = from == 0 ? UINT64_MAX : ~(UINT64_MAX >> (SM_BITS_PER_VECTOR - (from >= 64 ? 64 : from)));
...
static inline size_t
__sm_chunk_scan(const __sm_chunk_t *chunk, const __sm_idx_t start, void (*scanner)(uint64[], size_t, void *aux), size_t skip, void *aux)
{
  /* RLE fast path */
  if (__builtin_expect(__sm_chunk_is_rle(chunk), 0)) {
    const size_t length = __sm_chunk_rle_get_length(chunk);
    ...
    uint64 buffer[SM_BITS_PER_VECTOR];
