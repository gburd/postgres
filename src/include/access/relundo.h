...
typedef struct RelUndoPageHeaderData
{
	BlockNumber prev_blkno;		/* Previous page in chain (toward tail).
								 ...
								 * the chain (the tail). */
	TransactionId max_xid;		/* Largest urec_xid of any record on this
								 * page. InvalidTransactionId on an empty
								 * page. Used for discard eligibility checks. */
...
typedef struct RelUndoStageResult
{
	...
	Buffer		metabuf;		/* metapage buffer if is_new_page, else
								 * Invalid */
