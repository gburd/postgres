 ...
 * RelUndoReserve() buffer lock, cluster-wide (this is shared by every RECNO
 * table's writers, not scoped per relation).  Doubling to 16 halves that
 ...
 *
 * An in-place-update AM (RECNO) can correctly report near-zero dead tuples,
 ...
 * =============================================================================
 *
 * RECNO is an in-place MVCC table AM: an UPDATE overwrites the committed tuple
 * bytes, and the only durable copy of the prior committed version is the
 * before-image in the relation's UNDO fork.  If a transaction does an in-place
 * modification and the server crashes before it commits, redo re-establishes
 * the uncommitted page state, so a recovery-time driver must reverse-apply the
 * before-images of every incomplete (loser) transaction.
 *
 ...
 * format-agnostic: it stores and restores opaque tuple bytes and applies
 * byte-level diffs without knowing the in-place table AM's tuple layout.  The
 * three operations that do require AM-specific knowledge are reached through
 * these function pointers, which the owning table AM installs at subsystem
 * init.  When no in-place AM is registered the pointers stay NULL and the core
 * degrades gracefully (no transient-flag clearing, delta-reverse raises an
 * ERROR if a delta record is ever encountered).
 *
 ...
 *
 * RelUndoReverseDelta_hook: reconstruct the old tuple bytes from the current
 *   (new) tuple bytes plus an AM-private byte-diff record.  Returns false if
 *   the diff cannot be applied.
 *
 * RelUndoReverseDeltaOldLen_hook: report the length of the old tuple a diff
 *   record reconstructs to, so the caller can size the output buffer before
 *   invoking RelUndoReverseDelta_hook.  A length-changing (varlen) update
 *   reconstructs to a different length than the current on-page tuple.
 *
