...
typedef struct IndexFetchRecnoData
{
	...
	/*
	 * Per-fetch scratch context holding the slot-private copy of the on-page
	 * tuple taken under the buffer SHARE lock (RECNO does in-place UPDATEs, so
	 * the slot must not alias live buffer bytes; see recno_index_fetch_tuple).
	 * Reset at the top of each fetch so at most one copy is live at a time.
	 */
