...
typedef struct RecnoTupleHeader
{
	 ...
	 * this counter and the index entries change.  Occupies alignment padding
	 * that was previously unused (header stays MAXALIGN'd to 40 bytes: 8+8+4+4
	 * +4+6+2+2+1 = 39 -> 40), so there is no on-disk/WAL footprint change.
	 * Wraps at 2^32; VACUUM reclaims superseded old-gen index entries long
	 * before wraparound is a concern.
	 */
	 ...
	 * all in the common case -- see t_xid_hint above.)
	 */
...
/* Fixed size: 35 bytes raw (MAXALIGN'd to 40 bytes). */
