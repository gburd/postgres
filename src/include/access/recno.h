...
/*
 * RECNO special space structure - stored in page special space
 *
 * Reduced from the previous layout: pd_defrag_counter and
 * RECNO_PAGE_COMPRESSED flag have been removed.  Defragmentation is
 * now triggered by the RECNO_PAGE_DEFRAG_NEEDED flag alone.
 */
typedef struct RecnoPageOpaqueData
{
	uint64		pd_commit_ts;	/* Page-level commit timestamp */
	uint16		pd_free_space;	/* Cached free space */
...
/* Page flags */
#define RECNO_PAGE_OVERFLOW			0x0002
#define RECNO_PAGE_DEFRAG_NEEDED	0x0004
#define RECNO_PAGE_FULL				0x0008
...
{
	uint32		t_len;			/* 4B  Total tuple length */
	 ...
	 * (RecnoSLogEntry.cid) when RECNO_TUPLE_UNCOMMITTED is set.
	 * This saves 4 bytes per tuple (28B -> 24B header, HEAP parity).
	 * The sLog lookup is mandatory for uncommitted visibility anyway, so
	 * fetching the cid from there adds zero extra overhead.
	 *
	 * t_xid_hint also removed: the inserter XID is now obtained from the
	 * sLog entry (RecnoSLogEntry.xid) when RECNO_TUPLE_UNCOMMITTED is set.
	 */
...
/* Fixed size: 23 bytes -> MAXALIGN to 24 bytes (HEAP parity! t_cid and t_xid_hint removed) */
...
{
	uint64		current_commit_ts;	/* Current commit timestamp */
	uint64		oldest_active_ts;	/* Oldest active transaction timestamp */
	uint64		serializable_horizon;	/* Serializable isolation horizon */
	bool		anti_deps_tracking; /* Anti-dependency tracking enabled */
} RecnoMvccData;
...
/* Compression */
extern Datum RecnoCompressAttribute(Datum value, Oid typid, RecnoCompressionType comp_type, int comp_level);
extern RecnoCompressionType RecnoRelCompressionType(Relation rel, int *level_out);
extern bool RecnoRelCompressionEnabled(Relation rel);
...
extern bool RecnoCheckSerializableConflict(Relation rel, ItemPointer tid);
...
/* SSI (Serializable Snapshot Isolation) conflict tracking */
extern void RecnoRecordReadConflict(uint64 writer_start_ts,
									uint64 writer_commit_ts,
									bool writer_committed);
extern void RecnoRecordWriteConflict(uint64 reader_start_ts,
									 uint64 reader_commit_ts,
									 bool reader_committed,
									 bool reader_is_read_only);
extern void RecnoMarkTransactionWrite(void);
...
extern int	recno_slog_entries_per_backend;
...
extern Datum recno_tableam_handler(PG_FUNCTION_ARGS);

/* Compression statistics and management */
extern void RecnoGetCompressionStats(int *dict_entries, int *total_compressed,
...
/* GUC variables */
extern bool recno_enable_serializable;
extern int	recno_max_transactions;
