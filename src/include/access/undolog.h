...
/*
 * Legacy thresholds -- retained for compatibility with code that
 * references them (undostats.c, etc.).  No longer used for segment
 * rotation since UNDO data is now in WAL.
 */
#define UNDO_ROTATE_THRESHOLD_PCT		85
#define UNDO_PRESSURE_THRESHOLD_PCT		95
#define UNDO_CHECKPOINT_ROTATE_PCT		50
#define UNDO_BACKPRESSURE_MIN_US	   100
#define UNDO_BACKPRESSURE_MAX_US	100000
...
typedef struct UndoLogSharedData
{
	 ...
	 * segments containing data at or after this LSN must be retained.
	 * Updated by the UNDO discard worker as transactions complete and
	 * their UNDO records are no longer needed.
	 */
...
/* Note: undo_log_segment_size and max_undo_logs are compile-time constants
 * (UNDO_LOG_SEGMENT_SIZE and MAX_UNDO_LOGS in this header).  They are not
 * GUCs because they affect shared memory layout and require a server restart.
 * Reference those defines directly rather than adding runtime GUC indirection.
 */
