 ...
 * - Generic: Usable by any AM or subsystem (heap, nbtree, fileops, etc.)
 ...
 * The urec_info field carries RM-specific subtype/flags (e.g., the heap
 * RM uses it to distinguish INSERT vs DELETE vs UPDATE).
 *
 * Size: 48 bytes (optimized for alignment)
 */
typedef struct UndoRecordHeader
{
	uint8		urec_rmid;		/* UNDO RM ID (UNDO_RMID_HEAP, etc.) */
	...
	/*
	 * Payload length: size of the RM-specific opaque data that follows
	 * the header.  Interpretation is entirely RM-specific.
	 */
