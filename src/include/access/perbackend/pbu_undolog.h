 ...
 *-------------------------------------------------------------------------
 */
#ifndef PBU_UNDOLOG_H
#define PBU_UNDOLOG_H
...
#define UndoRecPtrAssignRelFileNode(rfn, urp)			\
	do													\
	{													\
		(rfn).spcOid = UndoRecPtrGetTablespace(urp);	\
		(rfn).dbOid = UndoLogDatabaseOid;				\
		(rfn).relNumber = UndoRecPtrGetRelNode(urp);	\
	} while (false);
...
typedef struct UndoLogControl
{
	...
	/*
	 * Identity of the executor this log is attached to, and the ownership key.
	 *
	 * procno is authoritative: it is the PGPROC slot of the attached executor,
	 * INVALID_PROC_NUMBER when unattached.  A slot index rather than a PID
	 * because that identifies one executor whether executors are processes or
	 * threads, and because it distributes evenly when used to spread sessions
	 * across logs (PIDs cluster).
	 *
	 * pid is retained for diagnostics only -- pg_stat views and debugging want
	 * a value an operator can correlate with the OS -- and must not be used to
	 * decide ownership.
	 */
	ProcNumber	procno;			/* INVALID_PROC_NUMBER for unattached */
	pid_t		pid;			/* diagnostic only; InvalidPid when unattached */
...
extern void UndoLogAdvance(UndoRecPtr insertion_point,
						   size_t size,
						   UndoPersistence persistence);
extern void PbuUndoLogDiscard(UndoRecPtr discard_point, TransactionId xid);
...
extern bool CaptureCurrentUndoLogMeta(xl_undolog_meta *xlrec);
extern void LogUndoMetaDataNow(xl_undolog_meta *xlrec);
