...
typedef struct IndexScanDescData
{
	 ...
	 * walked to reach the tuple crossed a HOT-indexed (HOT-indexed update) hop
	 * that changed an attribute this index covers, so the arriving leaf's key
	 * no longer matches the live tuple.  The executor drops such a tuple; the
	 * row is re-supplied by the fresh entry inserted for the new value.
