 ...
 * defined.
 *
 * ENGINE-AGNOSTIC: the tuple sLog is independent of which UNDO engine an AM
 * uses.  It keys purely on physical tuple identity (relid, tid) and holds no
 * reference to the per-relation-fork engine or the per-backend engine.  Any
 * in-place-MVCC AM may use it -- FLUX today, and the ZHEAP and RECNO AMs that
 * follow -- regardless of UNDO engine.  Cleanup is driven by the consumer AM
 * (or its UNDO worker) at commit/abort via the public SLogTupleRemoveByXid*
 * entry points; a per-backend AM drives the same removals from the
 * per-backend rollback path (execute_undo_actions) exactly as FLUX drives
 * them from its transaction hooks.
 *
...
typedef struct SLogAmDescriptor
{
	 ...
	 * stored; the AM falls back to its durable UNDO log for that version. 0
