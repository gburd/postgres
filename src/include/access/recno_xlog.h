...
typedef struct xl_recno_defrag
{
	uint16		ntuples;
	uint64		commit_ts;
	/* RecnoOffsetMapping[ntuples] follows as main data */
} xl_recno_defrag;
...
/* Page-defrag offset remapping entry (main data of XLOG_RECNO_DEFRAG). */
...
extern XLogRecPtr RecnoXLogDefrag(Relation rel, Buffer buffer,
								  RecnoOffsetMapping *mappings, int nmappings,
								  uint64 commit_ts);
