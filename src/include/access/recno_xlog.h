...
#define RECNO_WAL_HAS_OVERFLOW_BLK0	0x0004	/* Block 0 buf data has overflow records */
#define RECNO_WAL_PREFIX_SUFFIX	0x0008	/* Update uses prefix/suffix compression */
...
typedef struct xl_recno_update
{
	...
	uint8		dst_block_id;	/* Block ID of destination page for cross-page
								 * updates (only valid when RECNO_WAL_CROSS_PAGE
								 * is set in flags) */
	uint8		pad;			/* Padding for alignment */
