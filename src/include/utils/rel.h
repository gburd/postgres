...
typedef struct RelationData
{
	...
	/*
	 * Cached OPTIONAL index-deduplication posting codec for this index's table
	 * AM (RowIDType.posting).  NULL means no codec (heap, or a wide-RowID AM
	 * that supplies none -> no dedup).  Resolved together with rd_indexRowIdCmp
	 * / rd_indexRowIdWidth by RelationGetIndexRowIdCmp; reset on invalidation.
	 * Read by nbtree to decide dedup eligibility and to encode/decode/iterate/
	 * remove wide-RowID posting payloads.
	 */
	const RowIDPostingOps *rd_indexRowIdPosting;
