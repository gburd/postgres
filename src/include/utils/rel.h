...
typedef struct RelationData
{
	...
	/* HOT-indexed eligibility: counts of index types (non-summarizing, excluding
	 * primary key). Computed lazily on first access to HeapUpdateHotAllowable.
	 * Value of -1 means not yet computed. */
	int			rd_expr_indexes;    /* count of expression indexes */
	int			rd_regular_indexes; /* count of regular B-tree/hash/etc indexes */
	int			rd_partial_indexes; /* count of partial (WHERE clause) indexes */
