 ...
 * Latches with Interrupts" work, brought forward onto current master.  In
 * the process model it is a behaviour-neutral re-expression of the latch +
 * pending-flag machinery; in the (future) thread model the per-process
 * atomic bitmask reached through the session_local MyPendingInterrupts
 * pointer is what a shared-address-space backend needs in place of the
 * MyLatch / PGPROC.procLatch ownership dance.  See
 * docs/threading/INTERRUPTS_REDERIVATION.md for the full plan.
 *
 *
 ...
 * WaitInterruptOrSocket functions (except for bounded short periods, eg
...
typedef enum InterruptType
{
	 ...
	 *
	 * This is raised by the SIGINT signal handler, or can be sent directly by
	 * another backend e.g. with pg_cancel_backend(), or in response to a
	 ...
	 * is elapsed or not. TRUE if timeout occurred, FALSE otherwise.
	 */
