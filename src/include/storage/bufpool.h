...
typedef struct BufferPoolRoutine
{
	 ...
	 * state after pinning.
	 *
	 * Ring-buffer / RECYCLE-pool dispatch is handled by the framework's
	 * GetVictimWithStrategy() before calling this; algorithms see only the
	 * "normal eviction" path and do not need to consult any
	 * BufferAccessIntent or per-backend ring state.
	 */
	...
	/*
	 * scan_resistant -- true if this algorithm is provably scan-resistant in
	 * the cache-replacement sense: a sequential scan of pages larger than the
	 * pool cannot evict the entire frequency-tier working set. ARC, CAR,
	 * LIRS, and LIRS2/RCRD are scan-resistant by construction (T1/T2 or
	 * HIR/LIR partitioning absorbs scan traffic in the recency tier and
	 * protects the frequency tier).  Plain clock-sweep, LRU, OSIC, and JAM
	 * are not.
	 *
	 * When the active pool's algorithm is scan-resistant, the
	 * BufferAccessIntent dispatcher can skip the RECYCLE-pool detour for
	 * BUF_INTENT_BULKREAD strategies because the algorithm itself protects
	 * the cache from scan pollution.  BUF_INTENT_VACUUM still routes through
	 * RECYCLE regardless: VACUUM has different semantics (we want pages out
	 * of cache fast, not cached on a frequency-aware basis), and historically
	 * VACUUM has had its own ring-buffer policy. BUF_INTENT_BULKWRITE also
	 * still routes through RECYCLE: scan resistance is a read-side property;
	 * bulk writes benefit from RECYCLE regardless of the algorithm.
	 */
	bool		scan_resistant;
