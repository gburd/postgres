...
/*
 * Common victim-selection wrapper used by GetVictimBuffer() for both
 * the default pool and dynamic pools.  Handles BufferAccessIntent
 * ring + RECYCLE pre/post-dispatch in one place so every replacement
 * algorithm gets correct from_ring accounting without each one
 * needing to honour BufferAccessIntent itself.
 */
struct BufferDesc;
extern struct BufferDesc *GetPoolVictim(BufferPoolDesc *pool,
										void *strategy_data,
