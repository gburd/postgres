...
# ---------------- pre-flight: io_method=xtc actually starts+serves pooled ----
log "preflight io_method=xtc under pooled"
if start_pg threaded $CARRIERS; then
  iom=$("$PGBIN/psql" -h 127.0.0.1 -p $PORT -U postgres -tAc 'show io_method' postgres 2>/dev/null | tr -d ' ')
  q=$("$PGBIN/psql" -h 127.0.0.1 -p $PORT -U postgres -tAc 'select count(*) from warehouse' tpcc 2>/dev/null | tr -d ' ')
  say "PREFLIGHT io_method=$iom sample_query=${q:-ERR}"
  if [ "$iom" != "${IOM_THREADED:-xtc}" ]; then say "PREFLIGHT_IOM_MISMATCH want=${IOM_THREADED:-xtc} got=$iom"; fi
  stop_pg
else
  say "PREFLIGHT_START_FAIL io_method=${IOM_THREADED:-xtc} pooled would not start -- aborting"; exit 1
fi
...
for vu in $VU_LIST; do
  for rep in 1; do
    ...
    # threaded lane on BOTH io paths: sync (the OLTP-proven default) and xtc
    # (the fiber-native path + the new preadv2(RWF_NOWAIT) cached-read fast path).
    for iom in sync xtc; do
      export IOM_THREADED=$iom
      start_pg threaded $CARRIERS && run_cell "threaded-$iom" $CARRIERS $vu $rep || say "THREADED_CELL_SKIP iom=$iom vu=$vu rep=$rep"
      stop_pg
    done
    unset IOM_THREADED
  done
...
# ---------------- thread count + perf on the xtc fast path -----------------
log "thread-count + perf on mt (io_method=xtc fast path)"
export IOM_THREADED=xtc
start_pg threaded $CARRIERS
PM=$(cat $OUT/pg.pid)
say "THREADS postmaster=$(ls /proc/$PM/task 2>/dev/null | wc -l) iouwrk=$(ps -eLf | grep -c iou-wrk)"
# select@384 (read-heavy) -- cached-read fast path should keep futex ~0
...
wait
sudo perf report -i $OUT/perf_sel384.data --stdio --no-children --percent-limit 0.5 2>/dev/null | grep -vE '^\s*\||--|^$' | head -40 > $OUT/perf_sel384_top.txt
say "PGB_SEL384 $(grep -E 'tps =' $OUT/pgb_sel384.log | tail -1)"
say "FUTEX_sel $(grep -m1 -iE 'sys_futex|futex_wait' $OUT/perf_sel384_top.txt)"
say "SGLB $(grep -m1 update_sg_lb_stats $OUT/perf_sel384_top.txt)"
say "LWLOCK $(grep -m3 -iE 'LWLock' $OUT/perf_sel384_top.txt | tr '\n' ' ')"
# tpcb-like (read+WRITE) @192 under xtc -- the futex-storm repro; measure futex%
( sudo perf record -F 199 -a -g -o $OUT/perf_tpcb192.data -- sleep 40 ) &
"$PGBIN/pgbench" -h 127.0.0.1 -p $PORT -U postgres -c 192 -j 32 -T 45 postgres >$OUT/pgb_tpcb192.log 2>&1
wait
sudo perf report -i $OUT/perf_tpcb192.data --stdio --no-children --percent-limit 0.5 2>/dev/null | grep -vE '^\s*\||--|^$' | head -40 > $OUT/perf_tpcb192_top.txt
say "PGB_TPCB192 $(grep -E 'tps =' $OUT/pgb_tpcb192.log | tail -1)"
say "FUTEX_tpcb $(grep -m1 -iE 'sys_futex|futex_wait' $OUT/perf_tpcb192_top.txt)"
stop_pg
unset IOM_THREADED
